...
const VERSION = '2.8.80';
    ...
    public function save_meta( $post_id ) {
        ...
        if ( isset( $_POST['bolaox_numbers_multi'] ) && 'bolaox_aposta' === get_post_type( $post_id ) && trim( $_POST['bolaox_numbers_multi'] ) !== '' ) {
            ...
            foreach ( $lines as $set ) {
                ...
                if ( $new_id ) {
                    update_post_meta( $new_id, '_bolaox_numbers', $valid );
                    update_post_meta( $new_id, '_bolaox_payment', 'pending' );
                    if ( $contest ) {
                        update_post_meta( $new_id, '_bolaox_concurso', $contest );
                    }
                    $created++;
                }
            }
            if ( $created ) {
                ...
                $this->notice = sprintf( __( 'Importadas %d apostas.', self::TEXT_DOMAIN ), $created );
            }
            return;
        }
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score' ) {
        ...
        $out .= $this->render_repeated_numbers_by_date();
    ...
    private function export_winners_doc( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        ...
        echo '<tr><th>Apostador</th><th>Concurso</th><th>Números Apostados</th><th>Números Sorteados</th><th>Acertos</th></tr>';
        foreach ( $rows as $r ) {
            echo '<tr><td>' . esc_html( $r['name'] ) . '</td><td>' . esc_html( $r['contest'] ) . '</td><td>' . esc_html( $r['numbers'] ) . '</td><td>' . esc_html( $r['drawn'] ) . '</td><td>' . $r['hits'] . '</td></tr>';
        }
    ...
    private function render_recent_repeated_numbers() {
        $posts = get_posts( array(
            'post_type'   => 'bolaox_result',
            // Analyze the last 5 draws so statistics reset frequently
            'numberposts' => 5,
            'post_status' => 'publish',
            'orderby'     => 'date',
            'order'       => 'DESC',
    ...
    private function render_repeated_numbers_by_date() {
        $posts = get_posts( array(
            'post_type'   => 'bolaox_result',
            'post_status' => 'publish',
            'orderby'     => 'date',
