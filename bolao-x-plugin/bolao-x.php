...
const VERSION = '2.8.80';
    ...
    public function numbers_meta_box( $post ) {
        ...
        $placeholder = ( 'bolaox_result' === $post->post_type )
            ? __( 'Ex: 05,12,23,34,45 (até 50 dezenas)', self::TEXT_DOMAIN )
    ...
    public function save_meta( $post_id ) {
        ...
        if ( isset( $_POST['bolaox_numbers'] ) ) {
            ...
            if ( 'bolaox_result' === get_post_type( $post_id ) ) {
                ...
                if ( false === $valid ) {
                    $this->notice = __( 'Dezenas do resultado inválidas. Use até 50 números de 00 a 99 separados por vírgula.', self::TEXT_DOMAIN );
                    return;
                }
    ...
    public function contemplados_page() {
        ...
        echo '<p><a href="' . esc_url( $export_link ) . '" class="button">DOCX</a></p>';
    ...
    private function render_repeated_numbers_by_date() {
        ...
        foreach ( $posts as $index => $p ) {
            ...
            $repeats = array_intersect( $nums, $prev );
            $prev    = $nums;
            $list = '';
