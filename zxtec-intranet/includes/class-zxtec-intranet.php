...
JS;
    ...
    private function get_colaborador_contas_html() {
        ...
        foreach ( $orders as $order ) {
            ...
            $order_date = get_post_meta( $order->ID, '_zxtec_date', true );
            if ( $order_date ) {
                $activity_items[] = array(
                    'time' => strtotime( $order_date ),
                    'label' => sprintf( __( 'Ordem #%s - %s', 'zxtec' ), $order->ID, $order->post_title ),
                    'status' => $status,
                );
            }
        }

        foreach ( $accountability_entries as $entry ) {
            $status = get_post_meta( $entry->ID, '_zxtec_account_status', true );
            if ( $status === 'pendente' ) {
                $pending_accountability++;
            }
            if ( $status === 'aprovado' ) {
                $approved_accountability++;
            }

            $account_date = get_post_meta( $entry->ID, '_zxtec_account_date', true );
            if ( $account_date ) {
                $activity_items[] = array(
                    'time' => strtotime( $account_date ),
                    'label' => sprintf( __( 'Prestacao: %s', 'zxtec' ), get_the_title( $entry->ID ) ),
                    'status' => $status,
                );
            }
        }
            ...
            <aside class="zxtec-saas-sidebar" aria-label="<?php esc_attr_e( 'Menu do colaborador', 'zxtec' ); ?>">
                ...
                <nav class="zxtec-saas-nav">
                    ...
                    <button class="zxtec-saas-nav-button" type="button" data-section="orders" title="<?php esc_attr_e( 'Minhas Ordens', 'zxtec' ); ?>">
                        ...
                        <em class="zxtec-saas-nav-pill"><?php echo esc_html( $pending_orders_count ); ?></em>
                    </button>
                    ...
                    <button class="zxtec-saas-nav-button" type="button" data-section="accountability" title="<?php esc_attr_e( 'Prestacao de Contas', 'zxtec' ); ?>">
                        ...
                        <em class="zxtec-saas-nav-pill"><?php echo esc_html( $pending_accountability ); ?></em>
                    </button>
            ...
            <main class="zxtec-saas-main">
                <header class="zxtec-saas-topbar">
                    ...
                    <div class="zxtec-saas-status">
                        ...
                        <div class="zxtec-saas-status-actions">
                            <button class="zxtec-saas-chip" type="button" data-section-target="accountability"><?php esc_html_e( 'Enviar comprovante', 'zxtec' ); ?></button>
                            <button class="zxtec-saas-chip" type="button" data-section-target="orders"><?php esc_html_e( 'Ver ordens', 'zxtec' ); ?></button>
                        </div>
                    </div>
                </header>
                ...
                <section class="zxtec-saas-section is-active" data-section="overview">
                    <div class="zxtec-saas-insight">
                        ...
                        <p><?php echo esc_html( sprintf( __( '%d prestacoes aguardando validacao.', 'zxtec' ), $pending_accountability ) ); ?></p>
                    </div>
                    <div class="zxtec-saas-kpis">
                        <div>
                            <span><?php esc_html_e( 'Faturado', 'zxtec' ); ?></span>
                            <strong><?php echo esc_html( number_format_i18n( $totals['billed'], 2 ) ); ?></strong>
                        </div>
                        <div>
                            <span><?php esc_html_e( 'Recebido', 'zxtec' ); ?></span>
                            <strong><?php echo esc_html( number_format_i18n( $totals['received'], 2 ) ); ?></strong>
                        </div>
                        <div>
                            <span><?php esc_html_e( 'Pendente', 'zxtec' ); ?></span>
                            <strong><?php echo esc_html( number_format_i18n( $totals['pending'], 2 ) ); ?></strong>
                        </div>
                        <div>
                            <span><?php esc_html_e( 'Aprovado', 'zxtec' ); ?></span>
                            <strong><?php echo esc_html( number_format_i18n( $accountability_totals['saldo_aprovado'], 2 ) ); ?></strong>
                        </div>
                    </div>
                    <div class="zxtec-saas-actions">
                        <button class="zxtec-saas-action-card" type="button" data-section-target="orders">
                            <span><?php esc_html_e( 'Revisar ordens', 'zxtec' ); ?></span>
                            <small><?php esc_html_e( 'Confirme status e pendencias', 'zxtec' ); ?></small>
                        </button>
                        <button class="zxtec-saas-action-card" type="button" data-section-target="accountability">
                            <span><?php esc_html_e( 'Enviar comprovantes', 'zxtec' ); ?></span>
                            <small><?php esc_html_e( 'Agilize a validacao', 'zxtec' ); ?></small>
                        </button>
                        <button class="zxtec-saas-action-card" type="button" data-section-target="notifications">
                            <span><?php esc_html_e( 'Ver notificacoes', 'zxtec' ); ?></span>
                            <small><?php esc_html_e( 'Ultimas mensagens do admin', 'zxtec' ); ?></small>
                        </button>
                    </div>
                    ...
                    <div class="zxtec-saas-grid">
                        ...
                        <div class="zxtec-saas-panel">
                            <h2><?php esc_html_e( 'Prestacoes aprovadas', 'zxtec' ); ?></h2>
                            <p class="zxtec-saas-muted"><?php esc_html_e( 'Validacoes do admin ao longo do periodo.', 'zxtec' ); ?></p>
                            <p class="zxtec-saas-highlight"><?php echo esc_html( $approved_accountability ); ?></p>
                        </div>
                        <div class="zxtec-saas-panel">
                            <h2><?php esc_html_e( 'Linha do tempo', 'zxtec' ); ?></h2>
                            <p class="zxtec-saas-muted"><?php esc_html_e( 'Eventos recentes das suas entregas.', 'zxtec' ); ?></p>
                            <?php if ( empty( $activity_items ) ) : ?>
                                <p class="zxtec-saas-muted"><?php esc_html_e( 'Sem atividades registradas.', 'zxtec' ); ?></p>
                            <?php else : ?>
                                <ul class="zxtec-saas-timeline">
                                    <?php foreach ( $activity_items as $activity ) : ?>
                                        <li>
                                            <span class="zxtec-saas-timeline-dot status-<?php echo esc_attr( $activity['status'] ); ?>"></span>
                                            <div>
                                                <strong><?php echo esc_html( $activity['label'] ); ?></strong>
                                                <small><?php echo esc_html( date_i18n( get_option( 'date_format' ), $activity['time'] ) ); ?></small>
                                            </div>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                ...
                <section class="zxtec-saas-section" data-section="goals">
                    ...
                    <div class="zxtec-saas-goals">
                        <div class="zxtec-saas-goal">
                            <div>
                                <strong><?php esc_html_e( 'Satisfacao do cliente', 'zxtec' ); ?></strong>
                                <span><?php esc_html_e( 'Meta trimestral', 'zxtec' ); ?></span>
                            </div>
                            <div class="zxtec-saas-progress">
                                <span style="width: 72%"></span>
                            </div>
                            <small><?php esc_html_e( '72% concluido', 'zxtec' ); ?></small>
                        </div>
                        <div class="zxtec-saas-goal">
                            <div>
                                <strong><?php esc_html_e( 'Ordens no prazo', 'zxtec' ); ?></strong>
                                <span><?php esc_html_e( 'Meta mensal', 'zxtec' ); ?></span>
                            </div>
                            <div class="zxtec-saas-progress">
                                <span style="width: 84%"></span>
                            </div>
                            <small><?php esc_html_e( '84% concluido', 'zxtec' ); ?></small>
                        </div>
                        <div class="zxtec-saas-goal">
                            <div>
                                <strong><?php esc_html_e( 'Prestacoes aprovadas', 'zxtec' ); ?></strong>
                                <span><?php esc_html_e( 'Meta mensal', 'zxtec' ); ?></span>
                            </div>
                            <div class="zxtec-saas-progress">
                                <span style="width: 64%"></span>
                            </div>
                            <small><?php esc_html_e( '64% concluido', 'zxtec' ); ?></small>
                        </div>
                    </div>
                </section>
