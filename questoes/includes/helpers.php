...
function questoes_format_views( $views ) {
    $views = max( 0, (int) $views );

    return sprintf(
        _n( '%s acesso', '%s acessos', $views, 'questoes' ),
...
function questoes_render_course_card( $course, $args = array() ) {
    ...
    $cta          = isset( $course['cta'] ) && ! empty( $course['cta'] ) ? sanitize_text_field( $course['cta'] ) : __( 'Ver cursos disponíveis', 'questoes' );
    ...
    $category     = isset( $course['category'] ) ? sanitize_text_field( $course['category'] ) : '';
    $board        = isset( $course['board'] ) ? sanitize_text_field( $course['board'] ) : '';
    ...
    <article
        ...
        data-priority="<?php echo esc_attr( isset( $course['priority'] ) ? (int) $course['priority'] : 0 ); ?>"
        data-course-id="<?php echo esc_attr( isset( $course['id'] ) ? $course['id'] : '' ); ?>"
        data-course-title="<?php echo esc_attr( $title ); ?>"
        data-course-salary="<?php echo esc_attr( $salary ); ?>"
        data-course-opps="<?php echo esc_attr( $opps ); ?>"
        data-course-badge="<?php echo esc_attr( $badge ); ?>"
    >
        <div class="questoes-course-card__inner">
            <header class="questoes-course-card__header">
                <div class="questoes-course-card__identity<?php echo $icon ? ' has-icon' : ''; ?>">
                    <?php if ( $icon ) : ?>
                    ...
                    <div class="questoes-course-card__titles">
                        ...
                        <?php if ( ! empty( $meta_line ) ) : ?>
                            <span class="questoes-course-card__meta"><?php echo esc_html( implode( ' • ', $meta_line ) ); ?></span>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <?php if ( $highlight ) : ?>
                <div class="questoes-course-card__highlight"><?php echo $highlight; ?></div>
            <?php endif; ?>

            <div class="questoes-course-card__metrics">
                <?php if ( $salary ) : ?>
                    <div class="questoes-course-card__metric">
                        <span class="questoes-course-card__metric-label"><?php esc_html_e( 'Salários iniciais', 'questoes' ); ?></span>
                        <strong class="questoes-course-card__metric-value"><?php echo esc_html( $salary ); ?></strong>
                    </div>
                <?php endif; ?>

                <?php if ( $opps ) : ?>
                    <div class="questoes-course-card__metric">
                        <span class="questoes-course-card__metric-label"><?php esc_html_e( 'Oportunidades', 'questoes' ); ?></span>
                        <strong class="questoes-course-card__metric-value"><?php echo esc_html( $opps ); ?></strong>
                    </div>
                <?php endif; ?>

                <div class="questoes-course-card__metric">
                    <span class="questoes-course-card__metric-label"><?php esc_html_e( 'Questões no banco', 'questoes' ); ?></span>
                    <strong class="questoes-course-card__metric-value"><?php echo esc_html( number_format_i18n( $count ) ); ?></strong>
                </div>

                <?php if ( $comments > 0 ) : ?>
                    <div class="questoes-course-card__metric">
                        <span class="questoes-course-card__metric-label"><?php esc_html_e( 'Comentários', 'questoes' ); ?></span>
                        <strong class="questoes-course-card__metric-value"><?php echo esc_html( number_format_i18n( $comments ) ); ?></strong>
                    </div>
                <?php endif; ?>
            </div>

            <?php if ( $link ) : ?>
                <footer class="questoes-course-card__footer">
