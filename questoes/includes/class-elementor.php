...
class Questoes_Elementor_Courses extends Widget_Base {
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'regions_title',
            array(
                'label'       => __( 'Título por região', 'questoes' ),
                'type'        => Controls_Manager::TEXT,
                'default'     => __( 'Cursos por região', 'questoes' ),
                'placeholder' => __( 'Cursos por região', 'questoes' ),
            )
        );
        ...
        $this->add_control(
            'regions_description',
            array(
                'label'       => __( 'Descrição por região', 'questoes' ),
                'type'        => Controls_Manager::TEXTAREA,
                'placeholder' => __( 'Explore oportunidades conforme sua localização.', 'questoes' ),
            )
        );
    ...
    protected function build_shortcode( $settings ) {
        ...
        if ( ! empty( $settings['regions_title'] ) ) {
            $attributes[] = sprintf( 'titulo_regioes="%s"', esc_attr( $settings['regions_title'] ) );
        }

        if ( ! empty( $settings['regions_description'] ) ) {
            $attributes[] = sprintf( 'descricao_regioes="%s"', esc_attr( $settings['regions_description'] ) );
        }
