...
class Questoes_Courses {
    ...
    protected function get_course_meta( $post_id ) {
        return array(
            ...
            'views'       => get_post_meta( $post_id, 'questoes_course_views', true ),
        );
    }
    ...
    /**
     * Increment course view counters.
     *
     * @param array|int $course_ids Course IDs.
     */
    public function record_course_views( $course_ids ) {
        if ( empty( $course_ids ) ) {
            return;
        }

        $ids = array_unique( array_map( 'absint', (array) $course_ids ) );
        ...
        foreach ( $ids as $course_id ) {
            ...
            $current = (int) get_post_meta( $course_id, 'questoes_course_views', true );
            update_post_meta( $course_id, 'questoes_course_views', max( 0, $current ) + 1 );
        }
    }
    ...
    public function get_trending_courses( $args = array() ) {
        $defaults = array(
            'limit' => 6,
        );
        ...
        $items = isset( $catalog['items'] ) ? $catalog['items'] : array();
