    ...
    public function render_courses_shortcode( $atts, $content = '' ) {
        $atts = shortcode_atts(
            array(
                ...
                'titulo_regioes'      => __( 'Cursos por região', 'questoes' ),
                'descricao_regioes'   => __( 'Explore oportunidades conforme sua localização.', 'questoes' ),
                'limite_destaques'    => 6,
            ),
            $atts,
            'questoes_cursos'
        ...
        $catalog = $this->questions->get_course_catalog_data(
            array(
        ...
        $regions  = isset( $catalog['regions'] ) ? $catalog['regions'] : array();
        ...
        $regions_title        = sanitize_text_field( $atts['titulo_regioes'] );
        $regions_description  = ! empty( $atts['descricao_regioes'] ) ? wp_kses_post( $atts['descricao_regioes'] ) : '';
        $additional_content   = '';
