...
// Don't duplicate me!
if ( ! class_exists( 'RedukFramework_image_select' ) ) {

    /**
     * Main RedukFramework_image_select class
     *
     * @since       1.0.0
     */
    class RedukFramework_image_select {
        protected $parent;
        protected $field;
        protected $value;
        ...
        public function render() {
            ...
            if ( ! empty( $this->field['options'] ) ) {
                echo '<div class="reduk-table-container">';
                echo '<ul class="reduk-image-select">';
                ...
                foreach ( $this->field['options'] as $k => $v ) {
                    ...
                    $selected = ( checked( $this->value, $theValue, false ) != '' ) ? ' reduk-image-select-selected' : '';
                    ...
                    if ( isset( $this->field['presets'] ) && $this->field['presets'] !== false ) {
                        ...
                        $v['presets']['reduk-backup'] = 1;

                        $presets   = ' data-presets="' . htmlspecialchars( json_encode( $v['presets'] ), ENT_QUOTES, 'UTF-8' ) . '"';
                        $is_preset = true;

                        $this->field['class'] = trim( $this->field['class'] ) . ' reduk-presets';
                    }
                    ...
                    echo '<li class="reduk-image-select">';
                    echo '<label class="' . $selected . ' reduk-image-select' . $is_preset_class . $this->field['id'] . '_' . $x . '" for="' . $this->field['id'] . '_' . ( array_search( $k, array_keys( $this->field['options'] ) ) + 1 ) . '">';
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                'reduk-field-image-select-js',
                RedukFramework::$_url . 'inc/fields/image_select/field_image_select' . Reduk_Functions::isMin() . '.js',
                array( 'jquery', 'reduk-js' ),
                time(),
                true
            );
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'reduk-field-image-select-css',
                    RedukFramework::$_url . 'inc/fields/image_select/field_image_select.css',
                    array(),
                    time(),
                    'all'
                );
