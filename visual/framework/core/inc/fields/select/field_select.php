...
if ( ! class_exists( 'RedukFramework_select' ) ) {
    class RedukFramework_select {
        protected $parent;
        protected $field;
        protected $value;
        ...
        public function render() {
            ...
            if ( ! empty( $this->field['data'] ) && empty( $this->field['options'] ) ) {
                ...
                if ( $this->field['data'] == "elusive-icons" || $this->field['data'] == "elusive-icon" || $this->field['data'] == "elusive" ) {
                    $icons_file = RedukFramework::$_dir . 'inc/fields/select/elusive-icons.php';
                    /**
                     * filter 'reduk-font-icons-file}'
                     *
                     * @param  array $icon_file File for the icons
                     */
                    $icons_file = apply_filters( 'reduk-font-icons-file', $icons_file );

                    /**
                     * filter 'reduk/{opt_name}/field/font/icons/file'
                     *
                     * @param  array $icon_file File for the icons
                     */
                    $icons_file = apply_filters( "reduk/{$this->parent->args['opt_name']}/field/font/icons/file", $icons_file );
            ...
            if ( ! empty( $this->field['options'] ) ) {
                ...
                $placeholder = ( isset( $this->field['placeholder'] ) ) ? esc_attr( $this->field['placeholder'] ) : __( 'Select an item', 'mtrl_framework' );
                ...
                echo '<select ' . $multi . ' id="' . $this->field['id'] . '-select" data-placeholder="' . $placeholder . '" name="' . $this->field['name'] . $this->field['name_suffix'] . $nameBrackets . '" class="reduk-select-item ' . $this->field['class'] . $sortable . '"' . $width . ' rows="6">';
                echo '<option></option>';
                ...
                echo '<strong>' . __( 'No items of this type were found.', 'mtrl_framework' ) . '</strong>';
            }
        } //function
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                'reduk-field-select-js',
                RedukFramework::$_url . 'inc/fields/select/field_select' . Reduk_Functions::isMin() . '.js',
                array( 'jquery', 'select2-js', 'reduk-js' ),
                time(),
                true
            );
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'reduk-field-select-css',
                    RedukFramework::$_url . 'inc/fields/select/field_select.css',
                    array(),
                    time(),
                    'all'
                );
