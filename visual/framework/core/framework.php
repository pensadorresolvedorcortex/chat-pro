<?php
    ...
    if ( ! class_exists( 'RedukFrameworkInstances' ) ) {
        // Instance Container
        require_once dirname( __FILE__ ) . '/inc/class.reduk_instances.php';
        require_once dirname( __FILE__ ) . '/inc/lib.reduk_instances.php';
    }

    if ( class_exists( 'RedukFrameworkInstances' ) ) {
        add_action( 'reduk/init', 'RedukFrameworkInstances::get_instance' );
    }
    ...
    if ( ! class_exists( 'RedukFramework' ) ) {
        ...
        // Reduk CDN class
        require_once dirname( __FILE__ ) . '/inc/class.reduk_cdn.php';

        // Reduk API class  :)
        require_once dirname( __FILE__ ) . '/inc/class.reduk_api.php';

        // General helper functions
        require_once dirname( __FILE__ ) . '/inc/class.reduk_helpers.php';

        // General functions
        require_once dirname( __FILE__ ) . '/inc/class.reduk_functions.php';
        require_once dirname( __FILE__ ) . '/inc/class.p.php';

        require_once dirname( __FILE__ ) . '/inc/class.thirdparty.fixes.php';

        require_once dirname( __FILE__ ) . '/inc/class.reduk_filesystem.php';

        require_once dirname( __FILE__ ) . '/inc/class.reduk_admin_notices.php';

        // ThemeCheck checks
        require_once dirname( __FILE__ ) . '/inc/themecheck/class.reduk_themecheck.php';

        // Welcome
        require_once dirname( __FILE__ ) . '/inc/welcome/welcome.php';

        /**
         * Main RedukFramework class
         *
         * @since       1.0.0
         */
        class RedukFramework {
            ...
            protected $googleArray = array();

            public static function init() {
                $dir = Reduk_Helpers::cleanFilePath( dirname( __FILE__ ) );

                // Windows-proof constants: replace backward by forward slashes. Thanks to: @peterbouwmeester
                self::$_dir           = trailingslashit( $dir );
                self::$wp_content_url = trailingslashit( Reduk_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) ) );

                // See if Reduk is a plugin or not
                if ( strpos( Reduk_Helpers::cleanFilePath( __FILE__ ), Reduk_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Reduk_Helpers::cleanFilePath( __FILE__ ), Reduk_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Reduk_Helpers::cleanFilePath( __FILE__ ), Reduk_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        foreach ( get_plugins() as $key => $value ) {
                            if ( is_plugin_active( $key ) && strpos( $key, 'reduk-framework.php' ) !== false ) {
                                self::$_dir = trailingslashit( Reduk_Helpers::cleanFilePath( WP_CONTENT_DIR . '/plugins/' . plugin_dir_path( $key ) . 'RedukCore/' ) );
                                $is_plugin  = true;
                            }
                        }
                        if ( ! $is_plugin ) {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( Reduk_Helpers::cleanFilePath( __FILE__ ), Reduk_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                        $relative_url = str_replace( Reduk_Helpers::cleanFilePath( get_template_directory() ), '', self::$_dir );
                        self::$_url   = trailingslashit( get_template_directory_uri() . $relative_url );
                    } else if ( strpos( Reduk_Helpers::cleanFilePath( __FILE__ ), Reduk_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false ) {
                        $relative_url = str_replace( Reduk_Helpers::cleanFilePath( get_stylesheet_directory() ), '', self::$_dir );
                        self::$_url   = trailingslashit( get_stylesheet_directory_uri() . $relative_url );
                    } else {
                        $wp_content_dir = trailingslashit( Reduk_Helpers::cleanFilePath( WP_CONTENT_DIR ) );
                ...
                self::$_url       = apply_filters( "reduk/_url", self::$_url );
                self::$_dir       = apply_filters( "reduk/_dir", self::$_dir );
                self::$_is_plugin = apply_filters( "reduk/_is_plugin", self::$_is_plugin );
            }

            // ::init()

            public $framework_url = 'http://www.redukframework.com/';
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( isset ( $_POST ) && isset ( $_POST['action'] ) && $_POST['action'] == 'heartbeat' ) {
                    ...
                    // Hook, for purists.
                    if ( ! has_action( 'reduk/ajax/heartbeat' ) ) {
                        do_action( 'reduk/ajax/heartbeat', $this );
                    }
                ...
                Reduk_Functions::$_parent     = $this;
                Reduk_CDN::$_parent           = $this;
                Reduk_Admin_Notices::$_parent = $this;
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'mtrl_framework' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'mtrl_framework' );
                }

                $this->old_opt_name = $this->args['opt_name'];

                /**
                 * filter 'reduk/args/{opt_name}'
                 *
                 * @param  array $args RedukFramework configuration
                 */
                $this->args = apply_filters( "reduk/args/{$this->args['opt_name']}", $this->args );

                /**
                 * filter 'reduk/options/{opt_name}/args'
                 *
                 * @param  array $args RedukFramework configuration
                 */
                $this->args = apply_filters( "reduk/options/{$this->args['opt_name']}/args", $this->args );
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    /**
                     * filter 'reduk-sections'
                     *
                     * @deprecated
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( 'reduk-sections', $sections ); // REMOVE LATER
                    /**
                     * filter 'reduk-sections-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( "reduk-sections-{$this->args['opt_name']}", $this->sections ); // REMOVE LATER
                    /**
                     * filter 'reduk/options/{opt_name}/sections'
                     *
                     * @param  array $sections field option sections
                     */
                    $this->sections = apply_filters( "reduk/options/{$this->args['opt_name']}/sections", $this->sections );

                    /**
                     * Construct hook
                     * action 'reduk/construct'
                     *
                     * @param object $this RedukFramework
                     */
                    do_action( 'reduk/construct', $this );
                    ...
                    $this->filesystem = Reduk_Filesystem::get_instance( $this );

                    //set reduk upload folder
                    $this->set_reduk_content();
                    ...
                    // Tracking
                    if ( isset( $this->args['allow_tracking'] ) && $this->args['allow_tracking'] && Reduk_Helpers::isTheme( __FILE__ ) ) {
                        $this->_tracking();
                    }
                    ...
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        add_action( 'network_admin_edit_reduk_' . $this->args['opt_name'], array(
                            $this,
                            'save_network_page'
                        ), 10, 0 );
                    ...
                    if ( $this->args['dev_mode'] == true || Reduk_Helpers::isLocalHost() == true ) {
                        require_once 'core/dashboard.php';
                        new redukDashboardWidget( $this );

                        if ( ! isset ( $GLOBALS['reduk_notice_check'] ) ) {
                            require_once 'core/newsflash.php';

                            $params = array(
                                'dir_name'    => 'notice',
                                'server_file' => 'http://redukframework.com/wp-content/uploads/reduk/reduk_notice.json',
                                'interval'    => 3,
                                'cookie_id'   => 'reduk_blast',
                            );

                            new redukNewsflash( $this, $params );
                            $GLOBALS['reduk_notice_check'] = 1;
                        }
                    }
                }
                 ...
                 * @param  object $this RedukFramework
                 */
                do_action( 'reduk/loaded', $this );
            }

            // __construct()

            private function set_reduk_content() {
                $upload_dir        = wp_upload_dir();
                self::$_upload_dir = $upload_dir['basedir'] . '/reduk/';
                self::$_upload_url = str_replace( array(
                    'https://',
                    'http://'
                ), '//', $upload_dir['baseurl'] . '/reduk/' );
            }
            ...
            private function set_default_args() {
                $this->args = array(
                    ...
                    'class'                     => '',
                    // Class that gets appended to all reduk-containers
                    'admin_bar'                 => true,
                    ...
                    'dev_mode'                  => false,
                    'templates_path'            => '',
                    // Path to the templates file for various Reduk elements
                    'ajax_save'                 => false,
            ...
            public function vc_fixes() {
                if ( reduk_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
            ...
            public function network_admin_bar( $wp_admin_bar ) {
                ...
                $args = array(
                    ...
                    'meta'   => array( 'class' => 'reduk-network-admin' )
                );
            ...
            public function _update_check() {
                ...
                if ( ! isset ( $GLOBALS['reduk_update_check'] ) ) {
                    Reduk_Functions::updateCheck( self::$_version );
                    $GLOBALS['reduk_update_check'] = 1;
                }
            }

            public function _admin_notices() {
                Reduk_Admin_Notices::adminNotices( $this->admin_notices );
            }

            public function _dismiss_admin_notice() {
                Reduk_Admin_Notices::dismissAdminNotice();
            }
            ...
            private function _internationalization() {
                 ...
                 * filter 'reduk/textdomain/{opt_name}'
                 *
                 * @param string     The locale of the blog or from the 'locale' hook
                 * @param string     'mtrl_framework'  text domain
                 */
                //                $locale = apply_filters( "reduk/textdomain/{$this->args['opt_name']}", get_locale(), 'mtrl_framework' );
                //
                ...
                $basepath = apply_filters( "reduk/textdomain/basepath/{$this->args['opt_name']}", $basepath );

                load_plugin_textdomain( 'mtrl_framework', false, $basepath . 'languages' );
            }
            ...
            private function _tracking() {
                if ( file_exists( dirname( __FILE__ ) . '/inc/tracking.php' ) ) {
                    ...
                    $tracking = Reduk_Tracking::get_instance();
                    $tracking->load( $this );
                }
            }
            // _tracking()
            ...
            private function set_global_variable() {
                if ( $this->args['global_variable'] ) {
                    ...
                    /**
                     * filter 'reduk/options/{opt_name}/global_variable'
                     *
                     * @param array $value option value to set global_variable with
                     */
                    $GLOBALS[ $this->args['global_variable'] ] = apply_filters( "reduk/options/{$this->args['opt_name']}/global_variable", $this->options );
                    if ( isset ( $this->transients['last_save'] ) ) {
                        // Deprecated
                        $GLOBALS[ $this->args['global_variable'] ]['REDUK_last_saved'] = $this->transients['last_save'];
                        // Last save key
                        $GLOBALS[ $this->args['global_variable'] ]['REDUK_LAST_SAVE'] = $this->transients['last_save'];
                    }
                    if ( isset ( $this->transients['last_compiler'] ) ) {
                        // Deprecated
                        $GLOBALS[ $this->args['global_variable'] ]['REDUK_COMPILER'] = $this->transients['last_compiler'];
                        // Last compiler hook key
                        $GLOBALS[ $this->args['global_variable'] ]['REDUK_LAST_COMPILER'] = $this->transients['last_compiler'];
                    }
            ...
            public function set_options( $value = '' ) {
                ...
                if ( ! empty ( $value ) ) {
                    ...
                    //do_action( "reduk-saved-{$this->args['opt_name']}", $value ); // REMOVE
                    //do_action( "reduk/options/{$this->args['opt_name']}/saved", $value, $this->transients['changed_values'] );
                }
            }
            // set_options()
            ...
            public function get_options() {
                ...
                /**
                 * action 'reduk/options/{opt_name}/options'
                 *
                 * @param mixed $value option values
                 */
                $this->options = apply_filters( "reduk/options/{$this->args['opt_name']}/options", $this->options );
            ...
            public function get_wordpress_data( $type = false, $args = array() ) {
                ...
                //return $data;
                /**
                 * filter 'reduk/options/{opt_name}/wordpress_data/{type}/'
                 *
                 * @deprecated
                 *
                 * @param string $data
                 */
                $data = apply_filters( "reduk/options/{$this->args['opt_name']}/wordpress_data/$type/", $data ); // REMOVE LATER

                /**
                 * filter 'reduk/options/{opt_name}/data/{type}'
                 *
                 * @param string $data
                 */
                $data = apply_filters( "reduk/options/{$this->args['opt_name']}/data/$type", $data );
                ...
                if ( empty ( $data ) && ! empty ( $type ) ) {
                    ...
                    if ( ! empty ( $type ) && empty ( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            /**
                             * filter 'reduk-font-icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'reduk-font-icons', array() ); // REMOVE LATER
                             ...
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( 'reduk/font-icons', $font_icons );

                            /**
                             * filter 'reduk/{opt_name}/field/font/icons'
                             *
                             * @deprecated
                             *
                             * @param array $font_icons array of elusive icon classes
                             */
                            $font_icons = apply_filters( "reduk/{$this->args['opt_name']}/field/font/icons", $font_icons );
            ...
            public function field_default_values( $field ) {
                ...
                if ( isset ( $field['default'] ) ) {
                    $this->options_defaults[ $field['id'] ] = apply_filters( "reduk/{$this->args['opt_name']}/field/{$field['type']}/defaults", $field['default'], $field );
            ...
            public function _default_values() {
                        ...
                        if ( isset ( $section['fields'] ) ) {
                            foreach ( $section['fields'] as $k => $field ) {
                                ...
                                if ( $field['type'] == "section" && isset ( $field['indent'] ) && $field['indent'] == "true" ) {
                                    ...
                                    $field['class'] .= " reduk-section-indent-start";
                ...
                /**
                 * filter 'reduk/options/{opt_name}/defaults'
                 *
                ...
                $this->options_defaults             = apply_filters( "reduk/options/{$this->args['opt_name']}/defaults", $this->options_defaults, $this->transients['changed_values'] );
            ...
            private function _default_cleanup() {
                ...
                if ( Reduk_Helpers::isLocalHost() || ( Reduk_Helpers::isWpDebug() ) ) {
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    if ( isset ( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
                        foreach ( $submenu[ $this->args["page_slug"] ] as $index => $reduk_options_submenu ) {
                            $subnodeargs = array(
                                'id'     => $this->args["page_slug"] . '_' . $index,
                                'title'  => $reduk_options_submenu[0],
                                'parent' => $this->args["page_slug"],
                                'href'   => admin_url( 'admin.php?page=' . $reduk_options_submenu[2] ),
                            );
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . __( 'Options', 'mtrl_framework' ),
            ...
            public function _enqueue_output() {
                    ...
                    if ( isset ( $section['fields'] ) ) {
                        ...
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
                                $field_class = "RedukFramework_{$field['type']}";
                                if ( ! class_exists( $field_class ) ) {
                                    ...
                                    /**
                                     * Field class file
                                     * filter 'reduk/{opt_name}/field/class/{field.type}
                                     *
                                     * @param       string        field class file
                                     * @param array $field        field config data
                                     */
                                    $class_file = apply_filters( "reduk/{$this->args['opt_name']}/field/class/{$field['type']}", self::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
                                ...
                                if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->_can_output_css( $field ) ) {
                                    $field = apply_filters( "reduk/field/{$this->args['opt_name']}/output_css", $field );
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    $typography = new RedukFramework_typography ( null, null, $this );
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        wp_register_style( 'reduk-google-fonts-' . $this->args['opt_name'], $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        wp_enqueue_style( 'reduk-google-fonts-' . $this->args['opt_name'] );
                    }
                }
            }
            // _enqueue_output()
            ...
            public function _enqueue() {
                ...
                $enqueue = new redukCoreEnqueue ( $this );
                $enqueue->init();
            }
            // _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'mtrl_framework' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'mtrl_framework' );
                    }

                    // Make URL
                    $url = '<a class="reduk_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'mtrl_framework' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'mtrl_framework' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'mtrl_framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'reduk-hint-tab',
                        'title'   => __( 'Hints', 'mtrl_framework' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
                ...
                if ( $this->args['help_sidebar'] != '' ) {
                    ...
                    // about hints.
                    if ( true == $this->show_hints ) {
                        $screen->set_help_sidebar( '<p><strong>Reduk Framework</strong><br/><br/>Hint Tooltip Preferences</p>' );
                    }
                }

                /**
                 * action 'reduk-load-page-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "reduk-load-page-{$this->args['opt_name']}", $screen ); // REMOVE

                /**
                 * action 'reduk/page/{opt_name}/load'
                 *
                 * @param object $screen WP_Screen
                 */
                do_action( "reduk/page/{$this->args['opt_name']}/load", $screen );
            }
            // _load_page()

            /**
             * Do action reduk-admin-head for options page
             *
             * @since       1.0.0
             * @access      public
             * @return      void
            ...
            public function admin_head() {
                /**
                 * action 'reduk-admin-head-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  object $this RedukFramework
                 */
                do_action( "reduk-admin-head-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'reduk/page/{opt_name}/header'
                 *
                 * @param  object $this RedukFramework
                 */
                do_action( "reduk/page/{$this->args['opt_name']}/header", $this );
            }
            // admin_head()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'mtrl_framework' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function get_header_html( $field ) {
                ...
                if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
                    ...
                    if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
                        ...
                        $hint = apply_filters( 'reduk/hints/html', $hint, $field, $this->args );
                        ...
                        if ( 'true' == $metaVal || empty ( $metaVal ) && empty( $hint ) ) {
                            ...
                            $hint = '<div class="reduk-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' . $titleParam . '" qtip-content="' . $contentParam . '">&nbsp;<i class="' . ( isset( $this->args['hints']['icon'] ) ? $this->args['hints']['icon'] : '' ) . '"></i></div>';
                        }
                    }
                ...
                if ( ! empty ( $th ) ) {
                    $th = '<div class="reduk_field_th">' . $th . '</div>';
                }
            ...
            public function _register_settings() {
                ...
                foreach ( $this->sections as $k => $section ) {
                    ...
                    /**
                     * filter 'reduk-section-{index}-modifier-{opt_name}'
                     *
                     * @param array $section section configuration
                     */
                    $section = apply_filters( "reduk-section-{$k}-modifier-{$this->args['opt_name']}", $section );

                    /**
                     * filter 'reduk/options/{opt_name}/section/{section.id}'
                     *
                     * @param array $section section configuration
                     */
                    if ( isset ( $section['id'] ) ) {
                        $section = apply_filters( "reduk/options/{$this->args['opt_name']}/section/{$section['id']}", $section );
                    }
                    ...
                    if ( isset ( $section['fields'] ) ) {
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            ...
                            /**
                             * filter 'reduk/options/{opt_name}/field/{field.id}'
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduk/options/{$this->args['opt_name']}/field/{$field['id']}/register", $field );
                            ...
                            /**
                             * filter 'reduk-field-{field.id}modifier-{opt_name}'
                             *
                             * @deprecated
                             *
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduk-field-{$field['id']}modifier-{$this->args['opt_name']}", $field ); // REMOVE LATER
                             ...
                             * @param array $field field config
                             */
                            $field = apply_filters( "reduk/options/{$this->args['opt_name']}/field/{$field['id']}", $field );
                            ...
                            /**
                             * action 'reduk/options/{opt_name}/field/field.type}/register'
                             */
                            do_action( "reduk/options/{$this->args['opt_name']}/field/{$field['type']}/register", $field );
                            ...
                            if ( ! $display || isset ( $this->no_panel_section[ $k ] ) ) {
                                ...
                                if ( isset ( $field['hidden'] ) && $field['hidden'] ) {
                                    $field['label_for'] = 'reduk_hide_field';
                                }
                                if ( $this->args['options_api'] == true ) {
                                    add_settings_field(
                ...
                /**
                 * action 'reduk-register-settings-{opt_name}'
                 *
                 * @deprecated
                 */
                do_action( "reduk-register-settings-{$this->args['opt_name']}" ); // REMOVE

                /**
                 * action 'reduk/options/{opt_name}/register'
                 *
                 * @param array option sections
                 */
                do_action( "reduk/options/{$this->args['opt_name']}/register", $this->sections );
                ...
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                    ...
                    /**
                     * action 'reduk-compiler-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param array  options
                     ...
                     */
                    do_action( "reduk-compiler-{$this->args['opt_name']}", $this->options, $this->compilerCSS, $this->transients['changed_values'] ); // REMOVE

                    /**
                     * action 'reduk/options/{opt_name}a'
                     *
                     * @param array  options
                     * @param string CSS that get sent to the compiler hook
                     */
                    do_action( "reduk/options/{$this->args['opt_name']}/compiler", $this->options, $this->compilerCSS, $this->transients['changed_values'] );

                    /**
                     * action 'reduk/options/{opt_name}/compiler/advanced'
                     *
                     * @param array  options
                     * @param string CSS that get sent to the compiler hook, which sends the full Reduk object
                     */
                    do_action( "reduk/options/{$this->args['opt_name']}/compiler/advanced", $this );
            ...
            private function _register_extensions() {
                ...
                /**
                 * action 'reduk/extensions/before'
                 *
                 * @param object $this RedukFramework
                 */
                do_action( "reduk/extensions/before", $this );

                /**
                 * action 'reduk/extensions/{opt_name}/before'
                 *
                 * @param object $this RedukFramework
                 */
                do_action( "reduk/extensions/{$this->args['opt_name']}/before", $this );

                if ( isset( $this->old_opt_name ) ) {
                    do_action( "reduk/extensions/{$this->old_opt_name}/before", $this );
                }

                foreach ( $folders as $folder ) { if($folder != "customizer"){
                    if ( $folder === '.' || $folder === '..' || ! is_dir( $path . $folder ) || substr( $folder, 0, 1 ) === '.' || substr( $folder, 0, 1 ) === '@' || substr( $folder, 0, 4 ) === '_vti' ) {
                        continue;
                    }

                    $extension_class = 'RedukFramework_Extension_' . $folder;

                    /**
                     * filter 'reduk-extensionclass-load'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    $class_file = apply_filters( "reduk-extensionclass-load", "$path/$folder/extension_{$folder}.php", $extension_class ); // REMOVE LATER

                    /**
                     * filter 'reduk/extension/{opt_name}/{folder}'
                     *
                     * @param        string                    extension class file path
                     * @param string $extension_class          extension class name
                     */
                    $class_file = apply_filters( "reduk/extension/{$this->args['opt_name']}/$folder", "$path/$folder/extension_{$folder}.php", $class_file );
                ...
                /**
                 * action 'reduk-register-extensions-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param object $this RedukFramework
                 */
                do_action( "reduk-register-extensions-{$this->args['opt_name']}", $this ); // REMOVE

                /**
                 * action 'reduk/extensions/{opt_name}'
                 *
                 * @param object $this RedukFramework
                 */
                do_action( "reduk/extensions/{$this->args['opt_name']}", $this );

                if ( isset( $this->old_opt_name ) && ! empty( $this->old_opt_name ) ) {
                    do_action( "reduk/extensions/{$this->old_opt_name}", $this );
                }
            }
            ...
            public function _validate_options( $plugin_options ) {
                ...
                if ( isset ( $plugin_options['reduk-no_panel'] ) ) {
                    $keys = explode( '|', $plugin_options['reduk-no_panel'] );
                    foreach ( $keys as $key ) {
                        $plugin_options[ $key ] = $this->options[ $key ];
                    }
                    if ( isset ( $plugin_options['reduk-no_panel'] ) ) {
                        unset ( $plugin_options['reduk-no_panel'] );
                    }
                }
                ...
                if ( ( isset( $plugin_options['import_code'] ) && ! empty( $plugin_options['import_code'] ) ) || ( isset( $plugin_options['import_link'] ) && ! empty( $plugin_options['import_link'] ) ) ) {
                    ...
                    if ( ! empty ( $imported_options ) && is_array( $imported_options ) && isset ( $imported_options['reduk-backup'] ) && $imported_options['reduk-backup'] == '1' ) {
                        ...
                        /**
                         * action 'reduk/options/{opt_name}/import'
                         *
                         * @param  &array [&$plugin_options, reduk_options]
                         */
                        do_action_ref_array( "reduk/options/{$this->args['opt_name']}/import", array(
                            &$plugin_options,
                            $imported_options,
                            $this->transients['changed_values']
                        ) );

                        setcookie( 'reduk_current_tab', '', 1, '/', $time + 1000, "/" );
                ...
                if ( ! empty ( $plugin_options['defaults'] ) ) {
                    ...
                    /**
                     * apply_filters 'reduk/validate/{opt_name}/defaults'
                     *
                     * @param  &array [ $this->options_defaults, $plugin_options]
                     */
                    $plugin_options = apply_filters( "reduk/validate/{$this->args['opt_name']}/defaults", $this->options_defaults );
                    ...
                    //setcookie('reduk-compiler-' . $this->args['opt_name'], 1, time() + 1000, "/");
                    //setcookie("reduk-saved-{$this->args['opt_name']}", 'defaults', time() + 1000, "/");
                ...
                if ( ! empty ( $plugin_options['defaults-section'] ) ) {
                    if ( isset ( $plugin_options['reduk-section'] ) && isset ( $this->sections[ $plugin_options['reduk-section'] ]['fields'] ) ) {
                        /**
                         * apply_filters 'reduk/validate/{opt_name}/defaults_section'
                         *
                         * @param  &array [ $this->options_defaults, $plugin_options]
                         */
                        foreach ( $this->sections[ $plugin_options['reduk-section'] ]['fields'] as $field ) {
                        ...
                        $plugin_options = apply_filters( "reduk/validate/{$this->args['opt_name']}/defaults_section", $plugin_options );
                    }
                    ...
                    if ( isset ( $compiler ) ) {
                        ...
                        //setcookie('reduk-compiler-' . $this->args['opt_name'], 1, time()+1000, '/');
                        //$plugin_options['REDUK_COMPILER'] = time();
                        $this->transients['last_compiler'] = $time;
                        $this->transients['run_compiler']  = 1;
                    }

                    $this->transients['last_save_mode'] = "defaults_section"; // Last save mode
                    //setcookie("reduk-saved-{$this->args['opt_name']}", 'defaults_section', time() + 1000, "/");
                    unset ( $plugin_options['defaults'], $plugin_options['defaults_section'], $plugin_options['import'], $plugin_options['import_code'], $plugin_options['import_link'], $plugin_options['compiler'], $plugin_options['reduk-section'] );
                ...
                /**
                 * apply_filters 'reduk/validate/{opt_name}/before_validation'
                 *
                 * @param  &array [&$plugin_options, reduk_options]
                 */
                $plugin_options = apply_filters( "reduk/validate/{$this->args['opt_name']}/before_validation", $plugin_options, $this->options );
                ...
                /**
                 * action 'reduk-validate-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  &array [&$plugin_options, reduk_options]
                 */
                do_action_ref_array( "reduk-validate-{$this->args['opt_name']}", array(
                    &$plugin_options,
                    $this->options
                ) ); // REMOVE
                ...
                /**
                 * action 'reduk/options/{opt_name}/validate'
                 *
                 * @param  &array [&$plugin_options, reduk_options]
                 */
                do_action_ref_array( "reduk/options/{$this->args['opt_name']}/validate", array(
                    &$plugin_options,
                    $this->options,
                    $this->transients['changed_values']
                ) );
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "reduk_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'mtrl_framework' ),
                        'action' => ''
                    ) );
                ...
                if ( ! current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'mtrl_framework' ),
                        'action' => ''
                    ) );
                ...
                $reduk = RedukFrameworkInstances::get_instance( $_POST['opt_name'] );

                if ( ! empty ( $_POST['data'] ) && ! empty ( $reduk->args['opt_name'] ) ) {
                    ...
                    $values = $this->reduk_parse_str( $_POST['data'] );

                    $values = $values[ $reduk->args['opt_name'] ];
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        try {
                            if ( isset ( $reduk->validation_ran ) ) {
                                unset ( $reduk->validation_ran );
                            }
                            $reduk->set_options( $reduk->_validate_options( $values ) );
                            ...
                            require_once 'core/enqueue.php';
                            $enqueue = new redukCoreEnqueue ( $reduk );
                            $enqueue->get_warnings_and_errors_array();

                            $return_array = array(
                                'status'   => 'success',
                                'options'  => $reduk->options,
                                'errors'   => isset ( $reduk->localize_data['errors'] ) ? $reduk->localize_data['errors'] : null,
                                'warnings' => isset ( $reduk->localize_data['warnings'] ) ? $reduk->localize_data['warnings'] : null,
                            );
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'mtrl_framework' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                    ...
                    try {
                        ...
                        /**
                         * action 'reduk/options/{opt_name}/compiler'
                         *
                         * @param array  options
                         * @param string CSS that get sent to the compiler hook
                         */
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
                        ...
                        $panel = new redukCorePanel ( $reduk );
                        ob_start();
                    ...
                    echo json_encode( apply_filters( "reduk/options/{$this->args['opt_name']}/ajax_save/response", $return_array ) );
                }
...
//                            }
                            ...
                            if ( isset ( $field['validate'] ) ) {
                                $validate = 'Reduk_Validation_' . $field['validate'];

                                if ( ! class_exists( $validate ) ) {
                                    /**
                                     * filter 'reduk-validateclass-load'
                                     *
                                     * @deprecated
                                     *
                                     ...
                                     */
                                    $class_file = apply_filters( "reduk-validateclass-load", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $validate ); // REMOVE LATER

                                    /**
                                     * filter 'reduk/validate/{opt_name}/class/{field.validate}'
                                     *
                                     * @param        string                validation class file path
                                     * @param string $class_file           validation class file path
                                     */
                                    $class_file = apply_filters( "reduk/validate/{$this->args['opt_name']}/class/{$field['validate']}", self::$_dir . "inc/validation/{$field['validate']}/validation_{$field['validate']}.php", $class_file );
            ...
            public function section_menu( $k, $section, $suffix = "", $sections = array() ) {
                ...
                if ( isset ( $section['type'] ) && $section['type'] == "divide" ) {
                    ...
                    $string .= '<li id="' . esc_attr( $k . $suffix ) . '_section_group_li" class="reduk-group-tab-link-li' . esc_attr( $hide_section ) . esc_attr( $section['class'] ) . esc_attr( $subsectionsClass ) . '">';
                    $string .= '<a href="javascript:void(0);" id="' . esc_attr( $k . $suffix ) . '_section_group_li_a" class="reduk-group-tab-link-a" data-key="' . esc_attr( $k ) . '" data-rel="' . esc_attr( $k . $suffix ) . '">' . $extra_icon . $icon . '<span class="group_title">' . wp_kses_post( $section['title'] ) . '</span></a>';
                    ...
                    if ( $subsections ) {
                        ...
                        while ( $doLoop ) {
                            ...
                            if ( count( $sections ) < $nextK || ! isset ( $sections[ $nextK ] ) || ! isset ( $sections[ $nextK ]['subsection'] ) || $sections[ $nextK ]['subsection'] != true ) {
                                ...
                                $string .= '<li id="' . esc_attr( $nextK . $suffix ) . '_section_group_li" class="reduk-group-tab-link-li ' . esc_attr( $hide_sub ) . esc_attr( $section[ $nextK ]['class'] ) . ( $icon ? ' hasIcon' : '' ) . '">';
                                $string .= '<a href="javascript:void(0);" id="' . esc_attr( $nextK . $suffix ) . '_section_group_li_a" class="reduk-group-tab-link-a" data-key="' . esc_attr( $nextK ) . '" data-rel="' . esc_attr( $nextK . $suffix ) . '">' . $icon . '<span class="group_title">' . wp_kses_post( $sections[ $nextK ]['title'] ) . '</span></a>';
                                $string .= '</li>';
                            }
                        }
            ...
            public function _section_desc( $section ) {
                ...
                if ( isset ( $this->sections[ $id ]['desc'] ) && ! empty ( $this->sections[ $id ]['desc'] ) ) {
                    echo '<div class="reduk-section-desc">' . $this->sections[ $id ]['desc'] . '</div>';
                }
            }
            ...
            public function _field_input( $field, $v = null ) {
                ...
                if ( isset ( $field['callback'] ) && ( is_callable( $field['callback'] ) || ( is_string( $field['callback'] ) && function_exists( $field['callback'] ) ) ) ) {
                    ...
                    /**
                     * action 'reduk-before-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     ...
                     */
                    do_action( "reduk-before-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'reduk/field/{opt_name}/{field.type}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduk/field/{$this->args['opt_name']}/{$field['type']}/callback/before", array(
                        &$field,
                        &$value
                    ) );

                    /**
                     * action 'reduk/field/{opt_name}/callback/before'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduk/field/{$this->args['opt_name']}/callback/before", array(
                        &$field,
                        &$value
                    ) );
                    ...
                    /**
                     * action 'reduk-after-field-{opt_name}'
                     *
                     * @deprecated
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action( "reduk-after-field-{$this->args['opt_name']}", $field, $value ); // REMOVE

                    /**
                     * action 'reduk/field/{opt_name}/{field.type}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduk/field/{$this->args['opt_name']}/{$field['type']}/callback/after", array(
                        &$field,
                        &$value
                    ) );

                    /**
                     * action 'reduk/field/{opt_name}/callback/after'
                     *
                     * @param array  $field field data
                     * @param string $value field.id
                     */
                    do_action_ref_array( "reduk/field/{$this->args['opt_name']}/callback/after", array(
                        &$field,
                        &$value
                    ) );
                ...
                if ( isset ( $field['type'] ) ) {
                    ...
                    if ( ! class_exists( $field_class ) ) {
                        //                    $class_file = apply_filters( 'reduk/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
                        /**
                         * filter 'reduk/{opt_name}/field/class/{field.type}'
                         *
                    ...
                    if ( class_exists( $field_class ) ) {
                        ...
                        /**
                         * action 'reduk/field/{opt_name}/{field.type}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/{$field['type']}/render/before", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduk/field/{$this->args['opt_name']}/render/before'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/render/before", array(
                            &$field,
                            &$value
                        ) );
                     ...
                     */
                         ...
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduk-field-{$this->args['opt_name']}", ob_get_contents(), $field ); // REMOVE

                        /**
                         * filter 'reduk/field/{opt_name}/{field.type}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduk/field/{$this->args['opt_name']}/{$field['type']}/render/after", $_render, $field );

                        /**
                         * filter 'reduk/field/{opt_name}/render/after'
                         *
                         * @param       string        rendered field markup
                         * @param array $field        field data
                         */
                        $_render = apply_filters( "reduk/field/{$this->args['opt_name']}/render/after", $_render, $field );
                        ...
                        /**
                         * action 'reduk/field/{opt_name}/{field.type}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/{$field['type']}/fieldset/before/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduk/field/{opt_name}/fieldset/before/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/fieldset/before/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );
                        ...
                        echo '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="' . $hidden . 'reduk-field-container reduk-field reduk-field-init reduk-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
                        //}
                        ...
                        /**
                         * action 'reduk/field/{opt_name}/{field.type}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/{$field['type']}/fieldset/after/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );

                        /**
                         * action 'reduk/field/{opt_name}/fieldset/after/{opt_name}'
                         *
                         * @param array  $field field data
                         * @param string $value field id
                         */
                        do_action_ref_array( "reduk/field/{$this->args['opt_name']}/fieldset/after/{$this->args['opt_name']}", array(
                            &$field,
                            &$value
                        ) );
                    }
            ...
            public function _can_output_css( $field ) {
                ...
                $field = apply_filters( "reduk/field/{$this->args['opt_name']}/_can_output_css", $field );
             ...
             * @return  array $result
             */
            function reduk_parse_str( $string ) {
                if ( '' == $string ) {
                    return false;
                }
                ...
                foreach ( $pairs as $key => $pair ) {
                    ...
                    if ( ! isset( $result[ $k ] ) ) {
                        ...
                        $result[ $k ] = $this->reduk_array_merge_recursive_distinct( $result[ $k ], $params[ $k ] );
                    }
                }
             ...
             * @return  array $merged
             */
            function reduk_array_merge_recursive_distinct( array $array1, array $array2 ) {
                $merged = $array1;

                foreach ( $array2 as $key => $value ) {
                    if ( is_array( $value ) && isset( $merged[ $key ] ) && is_array( $merged[ $key ] ) ) {
                        $merged[ $key ] = $this->reduk_array_merge_recursive_distinct( $merged[ $key ], $value );
                    ...
                    if ( ! empty( $this->args['admin_bar_links'] ) ) {
                        foreach ( $this->args['admin_bar_links'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'reduk' ) !== false ) {
                                        $msg = __( '<strong>Reduk Framework Notice: </strong>There are references to the Reduk Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'mtrl_framework' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'reduk' ) !== false ) {
                                        $msg = __( '<strong>Reduk Framework Notice: </strong>There are references to the Reduk Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'mtrl_framework' );
         ...
         * @param null
         */
        do_action( 'reduk/init', RedukFramework::init() );
    } // class_exists('RedukFramework')
