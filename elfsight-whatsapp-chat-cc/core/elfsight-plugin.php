...
if (!class_exists('ElfsightWhatsappChatPlugin')) {
    class ElfsightWhatsappChatPlugin {
        private $name;
        private $slug;
        private $version;
        private $textDomain;
        ...
        public function __construct($config) {
            ...
            $this->update = new ElfsightWhatsappChatPluginUpdate($this->updateUrl, $this->version, $this->pluginSlug, $this->purchaseCode);
            $this->widgetsApi = new ElfsightWhatsappChatWidgetsApi($this->slug, $this->pluginFile, $this->textDomain);
            $this->admin = new ElfsightWhatsappChatPluginAdmin($config, $this->widgetsApi);
            $this->widget = new ElfsightWhatsappChatWidget($config, $this->widgetsApi);
            $this->vcElement = new ElfsightWhatsappChatVCElement($config, $this->widgetsApi);
        ...
        public function printAssets() {
            ...
            if ($this->isShortcodePresent || $force_script_add === 'on') {
                ...
                // Custom floating button style (Agencia Bot with pulse)
                wp_enqueue_style(
                    $this->slug . '-privilege-bot',
                    plugins_url('assets/privilege-bot.css', $this->pluginFile),
                    array(),
                    $this->version
                );
                wp_print_styles($this->slug . '-privilege-bot');
