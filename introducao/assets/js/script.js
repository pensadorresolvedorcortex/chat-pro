(function () {
    ...
    var AUTH_TAB_STORAGE_KEY = 'lae.auth.tab';
    ...
    var storage = (function () {
        ...
        try {
            var testKey = '__lae_storage_test__';
    ...
    var initOnboardingSlider = function (slider) {
        ...
        var sliderId = slider.getAttribute('data-lae-slider-id') || '';
        var sliderStorageKey = sliderId ? 'lae.onboarding.' + sliderId : null;
        ...
        var perfilSwitch = popup ? popup.querySelector('[data-perfil-switch]') : null;
        ...
        var applyPanelLock = function (mode) {
            ...
            currentLockMode = normalized;

            if (normalized) {
                perfilContainer.setAttribute('data-locked-panel', normalized);

                if (popup) {
                    popup.setAttribute('data-locked-panel', normalized);
                }

                if (perfilSwitch) {
                    perfilSwitch.setAttribute('hidden', 'hidden');
                    perfilSwitch.setAttribute('aria-hidden', 'true');
                }
            } else {
                perfilContainer.removeAttribute('data-locked-panel');

                if (popup) {
                    popup.removeAttribute('data-locked-panel');
                }

                if (perfilSwitch) {
                    perfilSwitch.removeAttribute('hidden');
                    perfilSwitch.setAttribute('aria-hidden', 'false');
                }
            }
        };
        ...
        var setActiveSlide = function (index) {
            ...
            if (sliderStorageKey) {
                storageSet(sliderStorageKey, activeIndex);
            }
            updateProgressLabel();
        };
        ...
        var setActiveAuthPanel = function (target) {
            ...
            if (currentLockMode && normalized !== currentLockMode) {
                normalized = currentLockMode;
            }
            ...
            if (normalized && !currentLockMode) {
                storageSet(AUTH_TAB_STORAGE_KEY, normalized);
            }
        };
        ...
        var openPopup = function (trigger, desiredTab, lockMode) {
            if (!popup) {
                return;
            }
            ...
            var storedTab = storageGet(AUTH_TAB_STORAGE_KEY);
            var lockTarget = lockMode;

            applyPanelLock(lockTarget);

            var preferredTab = lockTarget || desiredTab || storedTab;
            setActiveAuthPanel(preferredTab);
        ...
        state = {
            ...
            openPopup: function (trigger, desiredTab, lockMode) {
                openPopup(trigger, desiredTab, lockMode);
            },
        ...
        var storedIndex = sliderStorageKey ? toInt(storageGet(sliderStorageKey)) : 0;

        if (storedIndex < 0 || storedIndex >= slides.length) {
            storedIndex = 0;

            if (sliderStorageKey) {
                storageSet(sliderStorageKey, storedIndex);
            }
        }

        setActiveSlide(storedIndex);
    };
    ...
    var initPerfilSwitchers = function () {
        ...
        containers.forEach(function (container) {
            ...
            var setActive = function (target) {
                ...
                storageSet(AUTH_TAB_STORAGE_KEY, target);
            ...
            var initialTarget = container.getAttribute('data-active-panel');

            if (!initialTarget) {
                initialTarget = storageGet(AUTH_TAB_STORAGE_KEY);
            }

            if (!initialTarget && buttons.length) {
                initialTarget = buttons[0].getAttribute('data-perfil-toggle');
            }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (loginTrigger) {
            ...
            if (loginState && typeof loginState.openPopup === 'function') {
                ...
                var lockMode = loginTrigger.getAttribute('data-lae-login-mode');

                if (!desiredTab && lockMode) {
                    desiredTab = lockMode;
                }

                loginState.openPopup(loginTrigger, desiredTab, lockMode);
            }
        }
    }, true);
})();
