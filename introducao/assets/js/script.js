(function () {
    ...
    document.addEventListener('click', function (event) {
        ...
        var nextTarget = target.closest('[data-lae-next]');

        if (nextTarget) {
            var nextSlider = nextTarget.closest('[data-lae-slider]');
            var nextState = sliderStates.get(nextSlider);

            if (nextState) {
                event.preventDefault();
                nextState.setActiveSlide(nextState.getActiveIndex() + 1);

                if (
                    nextState.getSlidesLength &&
                    nextState.getActiveIndex() === nextState.getSlidesLength() - 1 &&
                    typeof nextState.focusFinishCta === 'function'
                ) {
                    nextState.focusFinishCta();
                }
            }
        ...
        var prevTarget = target.closest('[data-lae-prev]');

        if (prevTarget) {
            var prevSlider = prevTarget.closest('[data-lae-slider]');
            var prevState = sliderStates.get(prevSlider);

            if (prevState) {
                event.preventDefault();
                prevState.setActiveSlide(prevState.getActiveIndex() - 1);
            }

            return;
        }

        var stepTarget = target.closest('[data-lae-step]');

        if (stepTarget) {
            var stepSlider = stepTarget.closest('[data-lae-slider]');
            var stepState = sliderStates.get(stepSlider);

            if (stepState) {
                var targetIndex = toInt(stepTarget.getAttribute('data-lae-target'));

                if (!isNaN(targetIndex)) {
                    event.preventDefault();
                    stepState.setActiveSlide(targetIndex);

                    if (stepState.steps && stepState.steps[targetIndex]) {
                        stepState.steps[targetIndex].focus();
                    } else {
                        stepTarget.focus();
                    }
                }
        ...
        if (loginTrigger) {
            ...
            if (loginState && typeof loginState.openPopup === 'function') {
                event.preventDefault();
                loginState.openPopup(loginTrigger);
            }
        }
    }, true);
})();
