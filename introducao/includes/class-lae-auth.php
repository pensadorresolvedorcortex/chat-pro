...
class Introducao_Auth {
    ...
    /**
     * Nome do cookie utilizado para manter a impressão digital do cliente.
     */
    const CLIENT_COOKIE_NAME = 'introducao_client';

    /**
     * Tempo de vida padrão do cookie de identidade compartilhada.
     */
    const CLIENT_COOKIE_LIFETIME = YEAR_IN_SECONDS;
    ...
    public static function maybe_sync_logged_identity() {
        ...
        $challenge = self::get_client_challenge_context();

        if ( empty( $challenge ) || empty( $challenge['challenge'] ) ) {
            self::forget_client_identity();
        }
    }
    ...
    private static function get_client_fingerprint() {
        $token = self::ensure_client_token();
        ...
        $seed    = implode( '|', array( $token, $address, $agent, $accept ) );

        return hash_hmac( 'sha256', $seed, wp_salt( 'auth' ) );
    }

    /**
     * Garante que o visitante atual possua um token persistente para identificação.
     *
     * @return string
     */
    private static function ensure_client_token() {
        $token = isset( $_COOKIE[ self::CLIENT_COOKIE_NAME ] )
            ? sanitize_text_field( wp_unslash( $_COOKIE[ self::CLIENT_COOKIE_NAME ] ) )
            : '';

        if ( $token && preg_match( '/^[A-Fa-f0-9]{32}$/', $token ) ) {
            return $token;
        }

        $token = wp_generate_password( 32, false, false );

        if ( ! headers_sent() && ! wp_doing_cron() && PHP_SAPI !== 'cli' ) {
            $expires = time() + self::CLIENT_COOKIE_LIFETIME;
            $path    = defined( 'COOKIEPATH' ) ? COOKIEPATH : '/';
            $domain  = defined( 'COOKIE_DOMAIN' ) ? COOKIE_DOMAIN : ''; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound
            $secure  = is_ssl();
            ...
            if ( version_compare( PHP_VERSION, '7.3.0', '>=' ) ) {
                ...
                setcookie( self::CLIENT_COOKIE_NAME, $token, $expires, $path, $domain, $secure, true );
            }
        }

        $_COOKIE[ self::CLIENT_COOKIE_NAME ] = $token;

        return $token;
    }

    /**
     * Remove o token persistente associado ao visitante atual.
     */
    private static function clear_client_token() {
        ...
        if ( version_compare( PHP_VERSION, '7.3.0', '>=' ) ) {
            ...
            setcookie( self::CLIENT_COOKIE_NAME, '', $expiry, $path, $domain, $secure, true );
        }

        unset( $_COOKIE[ self::CLIENT_COOKIE_NAME ] );
    }
