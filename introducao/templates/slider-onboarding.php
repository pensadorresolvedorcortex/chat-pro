...
$default_slides  = array(
    array(
        ...
        'description' => __( 'Organize metas, rotinas e notificações em um painel que acompanha o seu ritmo de estudo.', 'introducao' ),
        'art'         => 'journey',
    ),
    array(
        'title'       => __( 'Treine com apoio personalizado', 'introducao' ),
        'description' => __( 'Acesse trilhas comentadas, feedback imediato e especialistas prontos para guiar seu desempenho.', 'introducao' ),
        'art'         => 'focus',
    ),
    array(
        'title'       => __( 'Conquiste resultados com a comunidade', 'introducao' ),
        'description' => __( 'Entre ou crie sua conta para desbloquear simulados, eventos ao vivo e o painel completo da Academia.', 'introducao' ),
        'art'         => 'community',
    ),
);
...
<section
    ...
    aria-label="<?php esc_attr_e( 'Passos principais da Academia da Comunicação', 'introducao' ); ?>"
>
    <div class="lae-onboarding__surface">
        <header class="lae-onboarding__header">
            ...
            <?php if ( $total_slides > 1 ) : ?>
                <button type="button" class="lae-onboarding__skip" data-lae-skip data-lae-target="<?php echo esc_attr( $total_slides - 1 ); ?>">
                    <?php esc_html_e( 'Pular', 'introducao' ); ?>
                </button>
            <?php endif; ?>
        </header>
                    ...
                    <div class="lae-onboarding__body">
                        ...
                        <?php if ( $index === $total_slides - 1 ) : ?>
                            <div class="lae-onboarding__finish" data-lae-finish hidden aria-hidden="true">
                                ...
                                <p class="lae-onboarding__hint"><?php esc_html_e( 'Escolha como continuar para acessar todos os recursos imediatamente.', 'introducao' ); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </article>
        ...
        <footer class="lae-onboarding__footer">
            <button
                ...
                data-lae-prev
                aria-label="<?php esc_attr_e( 'Voltar passo', 'introducao' ); ?>"
                disabled
                aria-disabled="true"
            >
            ...
            <div class="lae-onboarding__dots" role="tablist" aria-label="<?php esc_attr_e( 'Etapas da introdução', 'introducao' ); ?>">
                <?php foreach ( $slides as $index => $slide ) :
                    $control_id = $slider_id . '-step-' . $index;
                    $panel_id   = $slider_id . '-step-' . $index . '-panel';
                    $is_active  = 0 === $index;
            ...
            <button
                ...
                data-lae-next
                aria-label="<?php esc_attr_e( 'Avançar passo', 'introducao' ); ?>"
            >
