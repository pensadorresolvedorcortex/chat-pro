...
function juntaplay_chat_threads_for_user(int $user_id): array
{
    ...
    $unread_counts = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT admin_id, subscriber_id, group_id, COUNT(*) AS unread_total FROM {$table} WHERE recipient_id = %d AND read_at IS NULL GROUP BY admin_id, subscriber_id, group_id",
            $user_id
        ),
        ARRAY_A
    );
