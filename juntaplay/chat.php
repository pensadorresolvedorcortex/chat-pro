...
{
    return (int) apply_filters('juntaplay_chat_group_owner', 0, $group_id);
}
...
{
    $subscribers = apply_filters('juntaplay_chat_group_subscribers', [], $group_id, $current_user_id);
    ...
    foreach ($subscribers as $subscriber) {
        ...
        $normalized[] = [
            ...
            'group'  => isset($subscriber['group']) ? (string) $subscriber['group'] : '',
            'avatar' => isset($subscriber['avatar']) ? (string) $subscriber['avatar'] : get_avatar_url($id, ['size' => 96]),
        ];
    }
...
function juntaplay_chat_rest_context(WP_REST_Request $request)
{
    ...
    $groups = apply_filters('juntaplay_chat_groups_for_user', [], $current_user);
...
function juntaplay_chat_center_shortcode(): string
{
    ...
    <div class="juntaplay-chat-placeholder" data-juntaplay-chat-placeholder>
        ...
        <a class="button" href="<?php echo esc_url($messages_base_url); ?>"><?php esc_html_e('Ir para o chat', 'juntaplay'); ?></a>
    </div>
    <?php
