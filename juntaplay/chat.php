...
function juntaplay_chat_threads_for_user(int $user_id): array
{
    ...
    $unread_counts = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT admin_id, subscriber_id, group_id, COUNT(*) AS unread_total FROM {$table} WHERE recipient_id = %d AND (read_at IS NULL OR read_at = '0000-00-00 00:00:00') GROUP BY admin_id, subscriber_id, group_id",
            $user_id
        ),
        ARRAY_A
    );
    ...
    $meta_unread = get_user_meta($user_id, 'juntaplay_mensagens_nao_lidas', true);
    if (is_array($meta_unread)) {
        foreach ($meta_unread as $entry) {
            $admin_id      = isset($entry['admin_id']) ? (int) $entry['admin_id'] : 0;
            $subscriber_id = isset($entry['subscriber_id']) ? (int) $entry['subscriber_id'] : 0;
            $group_id      = isset($entry['group_id']) ? (int) $entry['group_id'] : 0;
            ...
            $unread_map[$key] = max(1, (int) ($unread_map[$key] ?? 0) + 1);
        }
    }
