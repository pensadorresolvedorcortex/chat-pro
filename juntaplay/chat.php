...
function juntaplay_chat_threads_for_user(int $user_id): array
{
    ...
    $unread_meta = get_user_meta($user_id, 'juntaplay_mensagens_nao_lidas', true);
    $unread_meta = is_array($unread_meta) ? $unread_meta : [];
    ...
    foreach ($rows as $row) {
        ...
        if (!isset($threads[$key])) {
            ...
            $admin_avatar    = $admin_id > 0 ? (string) get_avatar_url($admin_id, ['size' => 96]) : '';
            $subscriber_name = $user_cache[$subscriber_id] ? (string) $user_cache[$subscriber_id]->display_name : '';
            $subscriber_avatar = $subscriber_id > 0 ? (string) get_avatar_url($subscriber_id, ['size' => 96]) : '';

            $group_entry = $group_cache[$group_id] ?? null;
            $group_name  = is_array($group_entry) ? (string) ($group_entry['title'] ?? '') : '';

            $counterpart_id   = $user_id === $admin_id ? $subscriber_id : $admin_id;
            ...
            $counterpart_avatar = $counterpart_id > 0 ? (string) get_avatar_url($counterpart_id, ['size' => 96]) : '';
    ...
    foreach ($unread_meta as $unread_entry) {
        $admin_id_unread      = isset($unread_entry['admin_id']) ? (int) $unread_entry['admin_id'] : 0;
        $subscriber_id_unread = isset($unread_entry['subscriber_id']) ? (int) $unread_entry['subscriber_id'] : 0;
        $group_id_unread      = isset($unread_entry['group_id']) ? (int) $unread_entry['group_id'] : 0;

        if ($admin_id_unread === 0 || $subscriber_id_unread === 0) {
            continue;
        }

        $key = $admin_id_unread . ':' . $subscriber_id_unread . ':' . $group_id_unread;
        ...
        $fallback_key = $admin_id_unread . ':' . $subscriber_id_unread . ':0';

        if (isset($threads[$fallback_key])) {
            $threads[$fallback_key]['unread_count']++;
        }
    }
...
if (!shortcode_exists('juntaplay_online')) {
    ...
    function juntaplay_chat_online_shortcode(): string
    {
        ...
        ob_start();
        ?>
        <div id="<?php echo esc_attr($uid); ?>" class="juntaplay-online" data-rest-context="<?php echo esc_attr($context_url); ?>" data-rest-nonce="<?php echo esc_attr($nonce); ?>" data-messages-base="<?php echo esc_url($messages_base_url); ?>">
            ...
            (() => {
                ...
                const render = (threads = []) => {
                    ...
                    normalized.forEach((thread) => {
                        ...
                        const avatar = thread.counterpart_avatar || thread.sender_avatar || thread.admin_avatar || thread.owner_avatar || thread.subscriber_avatar || '';
                        ...
                        li.className = 'juntaplay-online__item';
                        ...
                        li.addEventListener('click', () => {
                            ...
                            if (thread.thread_id) search.set('thread_id', thread.thread_id);
                            if (thread.group_id) search.set('group_id', thread.group_id);
                            if (thread.admin_id || thread.owner_id) search.set('admin_id', thread.admin_id || thread.owner_id);
                            if (thread.subscriber_id || thread.member_id) search.set('subscriber_id', thread.subscriber_id || thread.member_id);

                            window.location.href = `${messagesBase}?${search.toString()}`;
                        });
