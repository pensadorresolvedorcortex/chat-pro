(function ($) {
  ...
  if (!$) {
    console.warn("JuntaPlay access script requires jQuery to be loaded before this file.");
    return;
  }
  ...
  function __(text) {
    return i18n && typeof i18n.__ === "function" ? i18n.__(text, "juntaplay") : text;
  }

  function ajaxUrl() {
    return window.JuntaPlay && window.JuntaPlay.ajax
      ? window.JuntaPlay.ajax
  ...
  function ajaxNonce() {
    return window.JuntaPlay && window.JuntaPlay.nonce ? window.JuntaPlay.nonce : "";
  }
  ...
  function renderCredentials($card, payload) {
    ...
    var fields = payload && payload.credentials && Array.isArray(payload.credentials)
      ? payload.credentials
      : [];
    ...
    if ($details.length) {
      ...
      fields.forEach(function (field) {
        ...
        if (field.type === "url" && value !== "") {
          $dd.append($("<a>", { href: value, target: "_blank", rel: "noopener", text: value }));
        } else if (htmlValue !== "") {
          $dd.html(htmlValue);
        } else {
          $dd.text(value);
  ...
  function renderError($card, message) {
    ...
    if ($hint.length) {
      $hint
        .text(message || __("Não foi possível recuperar os dados de acesso agora. Tente novamente em instantes."))
        .removeAttr("hidden");
    }
  ...
  $(document).on("click", "[data-group-access]", function (event) {
    var $button = $(this);
    if ($button.closest("#juntaplay-group-modal").length) return;
    ...
    $.ajax({
      ...
      method: "POST",
      dataType: "json",
      data: { action: "juntaplay_group_credentials", nonce: nonce, group_id: groupId }
    })
