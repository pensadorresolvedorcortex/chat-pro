(function ($) {
    ...
    function initGroupWizard($form) {
        ...
        function updatePlanNextState() {
            if (!$planStep.length || !$planNext.length) {
                return;
            }

            var planValue = $.trim(($form.find('[data-group-plan-input]').val() || '').toString());
            $planNext.prop('disabled', planValue === '');
        }
        ...
        updatePlanNextState();
        setPlanStepEnabled(initialPlanRequired || initialPlanValue);

        $form.on('jp:groupPlanRequired', function (_event, isRequired) {
            setPlanStepEnabled(!!isRequired);
            updatePlanNextState();
        });
    ...
    $(document).on('click', '[data-group-pool-apply]', function (event) {
        ...
        if (plans.length && $viewRoot.length) {
            ...
            if ($form.length) {
                $form.trigger('jp:groupWizardStep', ['plan']);
            }
            return;
        }
