(function ($) {
    ...
    function initGroupWizard($form) {
        ...
        var $progress = $form.find('[data-step-progress]');
        var currentIndex = 0;
        ...
        function updateWizardProgress() {
            ...
            var $availableSteps = $steps.filter(':not(.is-disabled)');
            var total = $availableSteps.length || $steps.length;
            var activeIndex = $availableSteps.index($steps.eq(currentIndex));

            if (activeIndex < 0) {
                activeIndex = currentIndex;
            }

            var percent = Math.round(((activeIndex + 1) / total) * 100);
