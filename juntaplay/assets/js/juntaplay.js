(function ($) {
    ...
    function initGroupWizard($form) {
        ...
        $form.on('jp:groupStep', function (_event, index) {
            if (!Number.isFinite(parseInt(index, 10))) {
                return;
            }

            var targetIndex = parseInt(index, 10);
            var dir = targetIndex > currentIndex ? 1 : -1;
            showStep(targetIndex, dir);
        });
        ...
        $form.on('click', '[data-step-prev]', function (event) {
            ...
            var $root = $form.closest('[data-group-view-root]');
            var hasPlans = $root.length && $root.data('groupHasPlans');
            if (currentIndex === 1 && hasPlans) {
                setGroupCreateView($root, 'plan');
                return;
            }
    ...
    function getPoolDataFromOption($option, selectedValue) {
        if (!$option || !$option.length) {
            return null;
        }

        return {
            id: ($option.data('poolId') || selectedValue || '').toString(),
            name: ($option.data('poolName') || $option.text() || '').toString(),
            price: ($option.data('poolPrice') || '').toString(),
            category: ($option.data('poolCategory') || '').toString(),
            excerpt: ($option.data('poolExcerpt') || '').toString(),
            total: ($option.data('poolTotal') || '').toString(),
            start: ($option.data('poolStart') || '').toString(),
            end: ($option.data('poolEnd') || '').toString(),
            iconId: ($option.data('poolIconId') || '').toString(),
            cover: ($option.data('poolCover') || $option.data('poolIcon') || '').toString(),
            url: ($option.data('poolUrl') || '').toString()
        };
    }

    function showPlanSelection($viewRoot, $form, poolData, plans) {
        if (!$viewRoot || !$viewRoot.length || !plans.length) {
            return false;
        }
        ...
        setGroupCreateView($viewRoot, 'plan');
    ...
    function renderPlanOptions($root, poolData, plans) {
        ...
        var $list = $root.find('[data-plan-list]').first();
        var $empty = $root.find('[data-plan-empty]').first();
        var $service = $root.find('[data-plan-service]').first();
    ...
    function syncPlanSelectionState($root, selectedPlanId) {
        if (!$root || !$root.length) {
            return;
        }
        ...
        var normalizedId = (selectedPlanId || '').toString();
        $list.find('[data-group-plan-select]').each(function () {
            var $item = $(this);
            var isSelected = normalizedId !== '' && ($item.data('planId') || '').toString() === normalizedId;
            $item.toggleClass('is-selected', isSelected);
            $item.attr('aria-pressed', isSelected ? 'true' : 'false');
        });

        var $next = $root.find('[data-group-plan-next]').first();
        if ($next.length) {
            $next.prop('disabled', normalizedId === '');
        }
    }
    ...
    function showPlanSelection($viewRoot, $form, poolData, plans) {
        ...
        $viewRoot.data('poolData', poolData);
        $viewRoot.data('groupHasPlans', true);
        $viewRoot.data('selectedPlan', null);
        renderPlanOptions($viewRoot, poolData, plans);

        var existingPlanId = '';
        if ($form && $form.length) {
            existingPlanId = ($form.find('[data-group-plan-input]').val() || '').toString();
        }
        if (existingPlanId) {
            var matchedPlan = plans.find(function (plan) {
                return (plan.id || '').toString() === existingPlanId;
            });
            if (matchedPlan) {
                $viewRoot.data('selectedPlan', matchedPlan);
            }
        }
        $viewRoot.attr('data-group-view-allowed', '0');
        $viewRoot.data('groupViewAllowed', '0');
        syncPlanSelectionState($viewRoot, $viewRoot.data('selectedPlan') ? $viewRoot.data('selectedPlan').id : '');
        setGroupCreateView($viewRoot, 'plan');
    ...
    $(document).on('click', '[data-group-pool-apply]', function (event) {
        ...
        if (!$viewRoot.length) {
            $viewRoot = $container.find('[data-group-view-root]').first();
        }
        if (plans.length && $viewRoot.length) {
            if (showPlanSelection($viewRoot, $form, poolData, plans)) {
                return;
            }
        }
        ...
        if ($viewRoot.length) {
            $viewRoot.data('groupHasPlans', false);
            allowWizardView($viewRoot);
            setGroupCreateView($viewRoot, 'wizard');
        }
    });

    $(document).on('click', '[data-group-plan-select]', function (event) {
        event.preventDefault();
        ...
        if (!$viewRoot.length || !planData) {
            return;
        }

        $viewRoot.data('selectedPlan', planData);
        syncPlanSelectionState($viewRoot, planData.id || '');
    });

    $(document).on('click', '[data-group-plan-next]', function (event) {
        event.preventDefault();
        ...
        var planData = $viewRoot.data('selectedPlan') || null;
        ...
        allowWizardView($viewRoot);
        setGroupCreateView($viewRoot, 'wizard');
        $form.trigger('jp:groupStep', [1]);
    });

    $(document).on('click', '[data-group-plan-back]', function (event) {
        event.preventDefault();

        var $button = $(this);
        var $viewRoot = $button.closest('[data-group-view-root]');
        if ($viewRoot.length) {
            setGroupCreateView($viewRoot, 'selector');
        }
    });
    ...
    $(document).on('change', '[name="jp_profile_group_pool"]', function () {
        ...
        var plans = normalizePoolPlans($option.data('poolPlans'));
        var $form = $select.closest('form');
        var $root = $select.closest('[data-group-view-root]');
        if (!$root.length) {
            $root = $('[data-group-view-root]').first();
        }
        ...
        if (!plans.length) {
            ...
            if ($root.length) {
                $root.data('groupHasPlans', false);
                $root.data('selectedPlan', null);
            }
            return;
        }

        if (!$root.length || !$form.length || selectedValue === '') {
            return;
        }

        var poolData = getPoolDataFromOption($option, selectedValue);
        if (!poolData) {
            return;
        }

        showPlanSelection($root, $form, poolData, plans);
    });
