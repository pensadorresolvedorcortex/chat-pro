    ...
    function initPoolCatalog($catalog) {
        ...
        function togglePoolsLoading(isLoading) {
            ...
            if (isLoading) {
                ...
                $load.prop('disabled', false).text(state.hasMore ? __('Carregar mais grupos', 'juntaplay') : __('Tudo carregado', 'juntaplay'));
            }
        }
        ...
        function fetchPools(reset) {
            ...
            $.ajax({
                ...
                if (!state.hasMore) {
                    ...
                    $load.attr('disabled', false).text(__('Carregar mais grupos', 'juntaplay'));
                }
    ...
    function renderGroupCard(item, variant) {
        ...
        if (mode === 'compact') {
            ...
            return '<article class="' + classes.join(' ') + '" data-group-card>'
                + '<a class="juntaplay-group-card__link" href="' + link + '">' 
        ...
        function ensureFrame() {
            ...
            frame.on('upload:complete', function (file) {
                window.setTimeout(function () {
                    ...
                    var toolbar = frame.toolbar && frame.toolbar.get ? frame.toolbar.get('select') : null;
                    if (toolbar && toolbar.$el) {
                        toolbar.$el.find('.media-button-select').prop('disabled', false);
                    }
                }, 120);
            });
        ...
        function updateGroupCard(group) {
            ...
            if ($details.length) {
                ...
                if (group.support_channel) {
                    detailHtml.push('<li><strong>' + escapeHtml(__('Suporte aos membros', 'juntaplay')) + ':</strong> ' + escapeHtml(group.support_channel) + '</li>');
                }
