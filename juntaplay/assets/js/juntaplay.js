(function ($) {
    ...
    function initNotifications() {
        ...
        $bells.each(function () {
            ...
            function renderChatNotification(thread) {
                if (!thread) {
                    return false;
                }

                var unreadCount = parseInt(thread.unread_count || thread.unread || 0, 10) || 0;
                if (unreadCount <= 0) {
                    return false;
                }

                var senderId = parseInt(thread.sender_id || thread.senderId || 0, 10) || 0;
                var senderName = '';

                if (thread.sender && typeof thread.sender === 'object') {
                    senderName = (thread.sender.display_name || thread.sender.name || '').toString().trim();
                }

                if (!senderName && thread.sender_display_name) {
                    senderName = (thread.sender_display_name || '').toString().trim();
                }

                if (!senderName && senderId && Array.isArray(thread.users)) {
                ...
                if (!senderName) {
                    senderName = (thread.counterpart_name || thread.admin_name || thread.subscriber_name || '').toString().trim();
                }

                if (!senderName && thread.sender_name) {
                    senderName = (thread.sender_name || '').toString();
                }
                ...
                $list.find('[data-jp-notifications-empty], .juntaplay-notifications__empty').remove();

                var itemHtml = '<li class="juntaplay-notifications__item" data-jp-chat-notification>' +
                    '<a href="' + chatMessagesUrl + '">' +
                    '<strong>' + escapeHtml(senderName) + '</strong> ' + __('te enviou uma mensagem.', 'juntaplay') +
                    '</a>' +
                    '</li>';

                var $existingChat = $list.children('[data-jp-chat-notification]').first();
                if ($existingChat.length) {
                    $existingChat.replaceWith(itemHtml);
                } else {
            ...
            function fetchChatNotifications(onComplete) {
                if (!chatContextUrl) {
                ...
                $.ajax({
                    ...
                    if (targetThread) {
                        renderChatNotification(targetThread);
