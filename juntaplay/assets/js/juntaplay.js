(function ($) {
    ...
    if (!coverPlaceholder) {
        var placeholderNode = document.querySelector('[data-cover-placeholder]');
        if (placeholderNode) {
            var datasetPlaceholder = placeholderNode.getAttribute('data-cover-placeholder') || '';
            if (datasetPlaceholder) {
                coverPlaceholder = datasetPlaceholder.toString();
            }
        }
    }

    function getSupportCountryMeta($select) {
        var meta = { code: '', mask: '' };
        ...
        if (typeof code !== 'undefined' && code !== null) {
            meta.code = code.toString();
        }

        if (typeof mask !== 'undefined' && mask !== null) {
            meta.mask = mask.toString();
        }

        return meta;
    }

    function formatSupportDigits(digits, mask) {
        ...
        if (index < digits.length) {
            ...
            formatted += digits.substr(index);
        }

        return formatted.trim();
    }

    function buildSupportSummary($container) {
        var type = ($container.find('[data-support-type]').val() || '').toString();

        if (type === 'whatsapp') {
            var meta = getSupportCountryMeta($container.find('[data-support-country]'));
            var digits = ($container.find('[data-support-phone-input]').val() || '').toString().replace(/\D+/g, '');

            if (!digits) {
                return '';
            }

            var formatted = formatSupportDigits(digits, meta.mask);
            var parts = [];
            ...
            return display ? 'WhatsApp ' + display : '';
        }

        if (type === 'email') {
            var email = ($container.find('[data-support-email-input]').val() || '').toString().trim();
    ...
    function syncSupportHidden($container) {
        var $hidden = $container.find('[data-support-hidden]');

        if ($hidden.length) {
            $hidden.val(buildSupportSummary($container));
        }
    }

    function updateSupportPhone($container) {
        var meta = getSupportCountryMeta($container.find('[data-support-country]'));
        var $code = $container.find('[data-support-code-display]');
        if ($code.length) {
            $code.text(meta.code || '+');
        }

        var $input = $container.find('[data-support-phone-input]');
        if ($input.length) {
            var digits = ($input.val() || '').toString().replace(/\D+/g, '');
            var formatted = formatSupportDigits(digits, meta.mask);
            $input.val(formatted);

            var placeholderPieces = [];
            if (meta.code) {
                placeholderPieces.push(meta.code);
            }

            if (meta.mask) {
                placeholderPieces.push(meta.mask);
            }

            var placeholder = placeholderPieces.join(' ').trim();
            if (!placeholder) {
                placeholder = __('Número com DDI', 'juntaplay');
            }
    ...
    function updateSupportVisibility($container) {
        var type = ($container.find('[data-support-type]').val() || '').toString();
        var $whatsapp = $container.find('[data-support-field="whatsapp"]');
        var $email = $container.find('[data-support-field="email"]');
        var $phoneInput = $container.find('[data-support-phone-input]');
        var $emailInput = $container.find('[data-support-email-input]');
        var $country = $container.find('[data-support-country]');

        if (type === 'whatsapp') {
            if ($whatsapp.length) {
                $whatsapp.removeAttr('hidden');
            }
            if ($email.length) {
                $email.attr('hidden', 'hidden');
            }
            $phoneInput.prop('disabled', false).prop('required', true);
            $country.prop('disabled', false).prop('required', true);
            $emailInput.prop('disabled', true).prop('required', false);
            updateSupportPhone($container);
        } else if (type === 'email') {
            if ($whatsapp.length) {
                $whatsapp.attr('hidden', 'hidden');
            }
            if ($email.length) {
                $email.removeAttr('hidden');
            }
            $phoneInput.prop('disabled', true).prop('required', false);
        ...
        syncSupportHidden($container);
    }

    function initSupportChannelFields($context) {
        var $scope = ($context && $context.length) ? $context : $(document);

        $scope.find('[data-support-channel]').each(function () {
            var $container = $(this);
            updateSupportVisibility($container);
        });
    }

    function prepareSupportChannelPayload($context) {
        var $scope = ($context && $context.length) ? $context : $(document);

        $scope.find('[data-support-channel]').each(function () {
            syncSupportHidden($(this));
        });
    }

    $(document).on('change', '[data-support-type]', function () {
        var $container = $(this).closest('[data-support-channel]');
        if ($container.length) {
            updateSupportVisibility($container);
        }
    });

    $(document).on('change', '[data-support-country]', function () {
        var $container = $(this).closest('[data-support-channel]');
        if ($container.length) {
            updateSupportPhone($container);
            syncSupportHidden($container);
    ...
    $(document).on('input', '[data-support-phone-input]', function () {
        var $container = $(this).closest('[data-support-channel]');
        if ($container.length) {
            updateSupportPhone($container);
            syncSupportHidden($container);
    ...
    $(document).on('input', '[data-support-email-input]', function () {
        var $container = $(this).closest('[data-support-channel]');
        if ($container.length) {
            syncSupportHidden($container);
        }
    ...
    function renderGroupCard(item, variant) {
        ...
        if (mode === 'compact') {
            ...
            return '<article class="' + classes.join(' ') + '"' + articleAttributes + '>'
                + '<a class="juntaplay-group-card__link" href="' + link + '"' + detailAttributes + '>'
                + '<figure class="juntaplay-group-card__cover">'
                + '<img src="' + escapeHtml(cover) + '" alt="' + alt + '" loading="lazy" width="320" height="220" />'
                + '</figure>'
        ...
        return '<article class="' + classes.join(' ') + '"' + articleAttributes + '>'
            + '<figure class="juntaplay-group-card__cover">'
            + '<a class="juntaplay-group-card__media" href="' + link + '"' + detailAttributes + '>'
    ...
    var jpGroupModal = (function () {
        ...
        function openGroupModal(html) {
            ...
            initSupportChannelFields($modal);
        ...
        function save(form) {
            ...
            prepareSupportChannelPayload($form);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (relationshipForms.length) {
            relationshipForms.forEach(function (form) {
                ...
                var relationshipRadios = form.querySelectorAll('input[name="jp_relationship_choice"]');

                if (relationshipRadios.length) {
                    var refreshRelationshipCards = function () {
                        relationshipRadios.forEach(function (radio) {
                            var card = radio.closest('.juntaplay-relationship-card');
                    ...
                    relationshipRadios.forEach(function (radio) {
                        radio.addEventListener('change', refreshRelationshipCards);
                    });

                    refreshRelationshipCards();
                }
            });
        }
    ...
    $(function () {
        initSupportChannelFields($(document));
    });
})(jQuery);
