(function ($) {
    ...
    $(function () {
        ...
        var $successTemplate = $('#jp-group-create-success-template');
        if ($successTemplate.length && parseInt($successTemplate.data('autoOpen'), 10)) {
            openGroupCreateSuccess();
        }
    });
    ...
    $(document).on('submit', '.juntaplay-groups__form', function () {
    ...
    function closeModal($modal) {
        ...
        var redirectUrl = '';
        if ($modal.is('[data-group-create-success-modal]')) {
            redirectUrl = ($modal.data('successRedirect') || '').toString();
        }
    ...
    function clearGroupCreatedFlag() {
        ...
        if (href.indexOf('jp_group_created=') === -1) {
            return;
        }
        ...
        if (typeof URL === 'function') {
            try {
                ...
                if (url.searchParams.has('jp_group_created')) {
                    url.searchParams.delete('jp_group_created');
        ...
        if (updated === href) {
            var pattern = /([?&])jp_group_created=[^&]*(&)?/;
            if (!pattern.test(href)) {
                return;
            }

            updated = href.replace(pattern, function (match, leading, trailing) {
                if (leading === '?' && typeof trailing === 'string') {
                    return '?';
                }
            ...
            if (updated.slice(-1) === '?' || updated.slice(-1) === '&') {
                updated = updated.slice(0, -1);
            }
        }

        if (updated !== href) {
            window.history.replaceState(window.history.state || {}, document.title, updated);
        }
    }
    ...
    function openGroupCreateSuccess() {
        var templateSelector = '#jp-group-create-success-template';
        var $template = $(templateSelector);
        if (!$template.length) {
            return;
        }

        var $modal = $('[data-group-create-success-modal]');
        if (!$modal.length) {
            return;
        }
