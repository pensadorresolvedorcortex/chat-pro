(function ($) {
    ...
    function createGroupSuccessContent(state) {
        ...
        var fallbackUrl = 'https://www.agenciadigitalsaopaulo.com.br/juntaplay/grupos';
    ...
    function initNotifications() {
        ...
        $bells.each(function () {
            ...
            function renderEmptyState() {
                $list.find('.juntaplay-notifications__empty--error').remove();
            ...
            function renderErrorState(message) {
                var text = message || __('Não foi possível carregar as notificações. Tente novamente.', 'juntaplay');
                var errorMarkup = '<li class="juntaplay-notifications__empty juntaplay-notifications__empty--error">' + text + '</li>';

                if ($list.children('[data-notification-id]').length) {
                    var $existingError = $list.find('.juntaplay-notifications__empty--error');

                    if ($existingError.length) {
                        $existingError.replaceWith(errorMarkup);
                    } else {
                        $list.append(errorMarkup);
                    }
                } else {
                    $list.html(errorMarkup);
                }
            }
            ...
            function fetchNotifications(force) {
                ...
                var hasItems = $list.children('[data-notification-id]').length > 0;
