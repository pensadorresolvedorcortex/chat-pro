(function ($) {
    ...
    function renderGroupCard(item) {
        ...
        var description = truncate(item.description || '', 160);
        var price = item.memberPriceLabel || item.priceLabel || '';
        ...
        var category = escapeHtml(item.categoryLabel || '');
        var badge = item.instantAccess ? '<span class="juntaplay-group-card__badge">' + __('Acesso imediato', 'juntaplay') + '</span>' : '';
        var owner = escapeHtml(item.ownerName || '');
        var avatar = item.ownerAvatar ? '<span class="juntaplay-group-card__avatar" style="background-image:url(' + escapeHtml(item.ownerAvatar) + ');"></span>' : '<span class="juntaplay-group-card__avatar" aria-hidden="true">' + (owner ? escapeHtml(owner.charAt(0).toUpperCase()) : 'J') + '</span>';
        var link = item.permalink ? escapeHtml(item.permalink) : '#';
        var updated = item.updated ? '<span class="juntaplay-group-card__updated">' + escapeHtml(item.updated) + '</span>' : '';

        return '<article class="juntaplay-group-card">'
            + '<header class="juntaplay-group-card__header">'
            + (category ? '<span class="juntaplay-group-card__category">' + category + '</span>' : '')
            + badge
            + '</header>'
            + '<h3 class="juntaplay-group-card__title"><a href="' + link + '">' + title + '</a></h3>'
            + (service ? '<p class="juntaplay-group-card__service">' + service + '</p>' : '')
            + (description ? '<p class="juntaplay-group-card__description">' + escapeHtml(description) + '</p>' : '')
            + '<dl class="juntaplay-group-card__meta">'
            + '<div><dt>' + __('Valor por membro', 'juntaplay') + '</dt><dd>' + escapeHtml(price || __('Sob consulta', 'juntaplay')) + '</dd></div>'
            + '<div><dt>' + __('Participantes', 'juntaplay') + '</dt><dd>' + members + '</dd></div>'
            + '<div><dt>' + __('Vagas livres', 'juntaplay') + '</dt><dd>' + available + (total ? ' / ' + total : '') + '</dd></div>'
            + '</dl>'
            + '<footer class="juntaplay-group-card__footer">'
            + '<div class="juntaplay-group-card__owner">' + avatar + (owner ? '<span>' + owner + '</span>' : '') + updated + '</div>'
            + '<a class="juntaplay-button juntaplay-button--primary" href="' + link + '">' + __('Ver detalhes', 'juntaplay') + '</a>'
            + '</footer>'
            + '</article>';
    }
    ...
    function initGroupsDirectory($root) {
        ...
        function render(items, reset) {
            ...
            if (items && items.length) {
                var html = items.map(renderGroupCard).join('');
                $list.append(html);
