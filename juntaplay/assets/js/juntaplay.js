(function ($) {
    ...
    function updateGroupPricePreview($form) {
        ...
        var available = Math.max(1, total);
    ...
    $(document).on('input change', '[data-group-price-input], [data-group-slot-input]', function () {
        ...
        if ($input.attr('name') === 'jp_profile_group_slots_reserved') {
            updateGroupSharePreview($form);
            return;
        }
    ...
    function initGroupWizard($form) {
        ...
        var $planStep = $form.find('[data-group-step][data-step="plan"]');
        var $planStepper = $form.find('[data-group-stepper] .juntaplay-steps__item[data-step="plan"]');
        var $planNext = $planStep.find('[data-step-next]').first();
        var $inlinePlan = $form.find('[data-group-plan-inline]').first();
        var $serviceNext = $form.find('[data-group-step][data-step-index="0"]').find('[data-step-next]').first();
        var $accessStep = $form.find('[data-group-step][data-step-index="5"]');
        var $accessStepper = $form.find('[data-group-stepper] .juntaplay-steps__item[data-step-index="5"]');
        var $progress = $();
        var currentIndex = 0;
        ...
        function getPlanStepIndex() {
            if (!$planStep.length) {
                return -1;
            }

            var index = parseInt($planStep.data('stepIndex'), 10);
            return Number.isFinite(index) ? index : -1;
        }

        function updatePlanNextState() {
            if ((!$planStep.length || !$planNext.length) && (!$serviceNext.length || !$inlinePlan.length)) {
                return;
            }

            var planValue = $.trim(($form.find('[data-group-plan-input]').val() || '').toString());
            var disableNext = planValue === '' && !!$form.data('planRequired');
            if ($planNext.length) {
                $planNext.prop('disabled', disableNext);
            }
            if ($serviceNext.length) {
                $serviceNext.prop('disabled', disableNext);
            }
        }

        function setPlanStepEnabled(enabled) {
            if (!$planStep.length || !$planStepper.length) {
                return;
            }

            $planStep.toggleClass('is-disabled is-hidden', !enabled);
            $planStepper.toggleClass('is-disabled', !enabled);

            if (!enabled) {
                updatePlanNextState();
                if (currentIndex === getPlanStepIndex()) {
                    showStep(Math.max(0, currentIndex - 1), -1);
                }
            }
        }

        function setInlinePlanVisible(visible) {
            if (!$inlinePlan.length) {
                return;
            }

            $inlinePlan.toggleClass('is-hidden', !visible);
        }
        ...
        $form.on('input change', 'input, select, textarea', function () {
            ...
            if ($field.is('[data-group-plan-input]')) {
                updatePlanNextState();
            }
        });
        ...
        var initialPlanRequired = !!$form.data('planRequired');
        var initialPlanValue = $.trim(($form.find('[data-group-plan-input]').val() || '').toString()) !== '';
        updatePlanNextState();
        setPlanStepEnabled(false);
        setInlinePlanVisible(initialPlanRequired || initialPlanValue);

        $form.on('jp:groupPlanRequired', function (_event, isRequired) {
            setPlanStepEnabled(false);
            setInlinePlanVisible(!!isRequired);
            updatePlanNextState();
        });

        $form.on('jp:groupWizardStep', function (_event, stepName) {
            var step = (stepName || '').toString();
            if (step !== 'plan') {
                return;
            }

            setPlanStepEnabled(true);
            setInlinePlanVisible(false);
            updatePlanNextState();
            var targetIndex = getPlanStepIndex();
            if (targetIndex >= 0) {
    ...
    function setGroupCreateView($root, view) {
        ...
        if (requestedStep) {
            ...
            if ($form.length) {
                $form.data('planRequired', true);
                $form.trigger('jp:groupPlanRequired', [true]);
                if (requestedStep !== 'inline-plan') {
                    $form.trigger('jp:groupWizardStep', [requestedStep]);
                }
            }
        }
    }
    ...
    function normalizePoolPlans(raw) {
        if (!raw) {
            return [];
        }
    ...
    function applyPoolData($form, poolData, planData) {
        if (!$form || !$form.length || !poolData) {
            return;
        }

        var poolId = (poolData.id || '').toString();
        var poolName = (poolData.name || '').toString();
        var poolPrice = parseFloat((poolData.price || '').toString().replace(',', '.'));
        var poolCategory = (poolData.category || '').toString();
        var poolExcerpt = (poolData.excerpt || '').toString();
        var poolTotal = parseInt((poolData.total || '').toString(), 10);
        var poolStart = parseInt((poolData.start || '').toString(), 10);
        var poolIconId = parseInt((poolData.iconId || '').toString(), 10) || 0;
        var poolCover = (poolData.cover || '').toString();
        ...
        var $planInput = $form.find('[data-group-plan-input]');
        if ($planInput.length) {
        ...
        if (Number.isFinite(planPrice) && planPrice > 0) {
            ...
            if ($price.length) {
                $price.val(toLocaleDecimal(planPrice)).trigger('input');
            }
        }
        ...
        if (Number.isFinite(planMax) && planMax > 0) {
            var $total = $form.find('[name="jp_profile_group_slots_total"]');
            if ($total.length) {
                $total.val(planMax).trigger('input');
            }
        }
    ...
    function renderPlanOptions($root, poolData, plans) {
        if (!$root || !$root.length) {
            return;
        }
        ...
        var $list = $scope.find('[data-plan-list]').first();
        var $empty = $scope.find('[data-plan-empty]').first();
        var $service = $scope.find('[data-plan-service]').first();
        var selectedPlanId = $.trim(($form.find('[data-group-plan-input]').val() || '').toString());
        ...
        if ($service.length) {
            var serviceName = (poolData && poolData.name) ? poolData.name : '';
            $service.text(serviceName ? 'Plano de ' + serviceName : '');
        }
        ...
        plans.forEach(function (plan) {
            var name = (plan.name || '').toString();
            var description = (plan.description || '').toString();
            var maxUsers = parseInt((plan.max_users || '').toString(), 10) || 0;
            var price = parseFloat((plan.price || '').toString().replace(',', '.'));
            var displayMax = maxUsers > 0 ? maxUsers + ' usuários máx.' : '';
            var displayPrice = Number.isFinite(price) && price >= 0
            ...
            if (description) {
                $content.append($('<span />', { class: 'juntaplay-service-list__description', text: description }));
            }
            var $meta = $('<span />', { class: 'juntaplay-service-list__meta' });
            if (displayMax) {
                $meta.append($('<span />', { class: 'juntaplay-service-list__pill juntaplay-service-list__pill--muted', text: displayMax }));
            }
            if (displayPrice) {
                $meta.append($('<span />', { class: 'juntaplay-service-list__price', text: displayPrice }));
            }
            $content.append($meta);
            $button.append($content);
            $button.append($('<span />', { class: 'juntaplay-service-list__chevron', 'aria-hidden': 'true' }));
    ...
    $(document).on('click', '[data-group-pool-apply]', function (event) {
        ...
        var $button = $(this);
        if ($button.data('poolBlocked')) {
            window.alert(($button.data('poolBlockedMessage') || __('Você já possui um grupo ativo para este serviço pré-aprovado.', 'juntaplay')).toString());
            return;
        }
        ...
        var poolData = {
            id: ($button.data('poolId') || '').toString(),
            name: ($button.data('poolName') || '').toString(),
            price: ($button.data('poolPrice') || '').toString(),
            category: ($button.data('poolCategory') || '').toString(),
            excerpt: ($button.data('poolExcerpt') || '').toString(),
            total: ($button.data('poolTotal') || '').toString(),
            start: ($button.data('poolStart') || '').toString(),
            end: ($button.data('poolEnd') || '').toString(),
            iconId: ($button.data('poolIconId') || '').toString(),
            cover: ($button.data('poolCover') || $button.data('poolIcon') || '').toString(),
            url: ($button.data('poolUrl') || '').toString()
        };

        var plans = normalizePoolPlans($button.data('poolPlans'));
        var hasPlansData = $button.data('hasPlans');
        var hasPlans = hasPlansData === 1 || hasPlansData === '1' || hasPlansData === true;

        var $viewRoot = $button.closest('[data-group-view-root]');
        if ((hasPlans || plans.length) && $viewRoot.length) {
            if ($form.length) {
                $form.data('planRequired', true);
                $form.trigger('jp:groupPlanRequired', [true]);
                $form.find('[data-group-plan-input]').val('').trigger('change');
            }
            $viewRoot.data('poolData', poolData);
            renderPlanOptions($viewRoot, poolData, plans);
        ...
        if ($form.length) {
            $form.data('planRequired', false);
            $form.trigger('jp:groupPlanRequired', [false]);
        }
        applyPoolData($form, poolData, null);
    ...
    $(document).on('click', '[data-group-plan-apply]', function (event) {
        event.preventDefault();

        var $button = $(this);
        var $list = $button.closest('[data-plan-list]');
        var $viewRoot = $button.closest('[data-group-view-root]');
        var $form = $viewRoot.find('form').first();
        var poolData = $viewRoot.data('poolData') || null;
        var planData = $button.data('planData') || null;

        if (!$form.length || !poolData || !planData) {
            return;
        }

        if ($list.length) {
            $list.find('[data-group-plan-apply]').removeClass('is-selected').attr('aria-pressed', 'false');
        ...
        applyPoolData($form, poolData, planData);
        allowWizardView($viewRoot);
    });
    ...
    function renderGroupCard(item, variant) {
        ...
        if (mode === 'service') {
            var serviceClasses = ['juntaplay-service-card', 'juntaplay-group-card--service'];
            var serviceSubtitle = item.service || '';
            if (serviceSubtitle && serviceSubtitle === titleText) {
                serviceSubtitle = '';
            }

            var serviceMeta = [];
            var memberCount = parseInt(item.slotsTotal, 10) || 0;
            var categoryLabel = item.categoryLabel || '';
            if (categoryLabel) {
                serviceMeta.push('<span class="juntaplay-service-card__pill">' + escapeHtml(categoryLabel) + '</span>');
            }

            if (memberCount > 0) {
                var countLabel = memberCount === 1
                    ? __('1 usuário', 'juntaplay')
                    : memberCount + ' ' + __('usuários', 'juntaplay');
                serviceMeta.push('<span class="juntaplay-service-card__pill juntaplay-service-card__pill--muted">' + escapeHtml(countLabel) + '</span>');
            }

            if (priceLabel) {
                serviceMeta.push('<span class="juntaplay-service-card__price">' + escapeHtml(priceLabel) + '</span>');
            }
            ...
            return '<article class="' + serviceClasses.join(' ') + '"' + articleIdAttr + articleAttributes + '>'
                ...
                + avatar
                + '<span class="juntaplay-service-card__title"' + titleAttr + '>' + escapeHtml(displayTitle) + '</span>'
    ...
    function initGroupCoverPicker($wrapper) {
        ...
        var $error = $wrapper.find('[data-group-cover-error]');
        ...
        function applyUploadContext(targetFrame) {
            ...
            if (typeof wp !== 'undefined'
                ...
                if (typeof wp.media.model.settings.postId !== 'undefined') {
                    wp.media.model.settings.postId = 0;
                }
        ...
        function notifyCoverUploadError(error) {
            var message = '';
            ...
            if (!message) {
                message = __('Não foi possível enviar o ícone agora. Use PNG, JPG ou WEBP com até 5MB e tente novamente.', 'juntaplay');
            }
        ...
        function ensureFrame() {
            ...
            frame.on('upload:error', function (file, response) {
                notifyCoverUploadError(response || file || null);
            });
            ...
            if (frame.uploader && frame.uploader.uploader && typeof frame.uploader.uploader.on === 'function') {
                frame.uploader.uploader.on('error', function (uploader, error) {
                    notifyCoverUploadError(error || uploader || null);
                });
            }
    ...
    function initGroupsDirectory($root) {
        ...
        var allowedVariants = ['spotlight', 'compact', 'service'];
        if (allowedVariants.indexOf(cardVariant) === -1) {
    ...
    function initGroupRotator($root) {
        ...
        function render(items) {
            ...
            var html = items.map(function (item) {
                return renderGroupCard(item, 'service');
            }).join('');
    ...
    function initNotifications() {
        ...
        var chatContextUrl = (window.JuntaPlayChat && window.JuntaPlayChat.restBase)
            ? window.JuntaPlayChat.restBase.replace(/\/$/, '') + '/chat/context'
            : '';
        var chatContextNonce = (window.JuntaPlayChat && window.JuntaPlayChat.nonce)
            ? window.JuntaPlayChat.nonce
            : '';
        var chatMessagesUrl = '/perfil/?section=juntaplay-chat';
        ...
        $bells.each(function () {
            ...
            var isLoaded = false;
            var isLoading = false;
            var lastChatUnread = 0;
            var emptyMarkup = '<li class="juntaplay-notifications__empty" data-jp-notifications-empty>' + __('Nenhuma notificação por enquanto.', 'juntaplay') + '</li>';
            var isChatPage = typeof window !== 'undefined' && window.location && window.location.search.indexOf('section=juntaplay-chat') !== -1;
            ...
            function renderChatNotification(senderName) {
                if (!senderName) {
                    return false;
                }

                $list.html(''
                    + '<li class="juntaplay-notifications__item" data-jp-chat-notification>'
                    + '<a href="' + chatMessagesUrl + '">'
                    + '<strong>' + escapeHtml(senderName) + '</strong> ' + __('te enviou uma mensagem.', 'juntaplay')
                    + '</a>'
                    + '</li>');
            ...
            function fetchChatNotifications(options) {
                var onComplete = null;
                var silent = false;
                ...
                $.ajax({
                    ...
                    method: 'GET',
                    dataType: 'json',
                    headers: chatContextNonce ? { 'X-WP-Nonce': chatContextNonce } : {}
                }).done(function (response) {
                    var threads = [];
                    ...
                    threads.forEach(function (thread) {
                        var count = parseInt(thread.unread_count || thread.unread || 0, 10) || 0;
                        totalUnread += count;
                        if (!targetThread && count > 0) {
                            targetThread = thread;
                        }
                    });
                    ...
                    if (targetThread) {
                        ...
                        if (targetThread.last_message && typeof targetThread.last_message === 'object') {
                            senderId = parseInt(targetThread.last_message.sender_id || targetThread.last_message.senderId || 0, 10) || 0;

                            if (targetThread.last_message.sender && typeof targetThread.last_message.sender === 'object') {
                                senderName = (targetThread.last_message.sender.display_name || targetThread.last_message.sender.name || '').toString().trim();
                            }
                        }

                        if (!senderId) {
                            senderId = parseInt(targetThread.sender_id || targetThread.senderId || 0, 10) || 0;
                        }

                        if (!senderName && targetThread.sender && typeof targetThread.sender === 'object') {
                            senderName = (targetThread.sender.display_name || targetThread.sender.name || '').toString().trim();
                        }

                        if (!senderName && senderId && Array.isArray(targetThread.users)) {
                            targetThread.users.forEach(function (user) {
                                if (senderName || !user || typeof user !== 'object') {
                                    return;
                                }

                                var userId = parseInt(user.id || user.user_id || 0, 10) || 0;
                                if (userId !== senderId) {
                                    return;
                                }

                                senderName = (user.display_name || user.name || '').toString().trim();
                            });
                        }

                        if (!senderName && targetThread.last_message && targetThread.last_message.sender_name) {
                            senderName = (targetThread.last_message.sender_name || '').toString().trim();
                        }

                        if (!senderName && targetThread.sender_name) {
                            senderName = (targetThread.sender_name || '').toString().trim();
                        }

                        if (!senderName && targetThread.counterpart_name) {
                            senderName = (targetThread.counterpart_name || '').toString().trim();
                        }

                        if (!senderName) {
                            senderName = __('Contato', 'juntaplay');
                        }

                        lastChatUnread = totalUnread;

                        renderChatNotification(senderName);
                        syncCount(totalUnread);
                        if (typeof onComplete === 'function') {
                            onComplete(true);
                        }
                        return;
            ...
            if ($bell.length && typeof MutationObserver !== 'undefined') {
                var bellNode = $bell.get(0);
                ...
                if (bellNode) {
                    new MutationObserver(function (mutations) {
                        mutations.forEach(function (mutation) {
                            ...
                            var currentCount = parseInt($bell.attr('data-count'), 10) || 0;

                            if (currentCount === lastChatUnread) {
                                return;
                            }

                            lastChatUnread = currentCount;

                            if (currentCount > 0) {
                                fetchChatNotifications({ silent: true });
                            } else {
                                renderEmptyState();
                            }
                        });
                    }).observe(bellNode, { attributes: true, attributeFilter: ['data-count'] });
                }
            }

            var initialBellCount = parseInt($bell.attr('data-count'), 10) || 0;
            if (initialBellCount > 0 && !isChatPage) {
                lastChatUnread = initialBellCount;
                fetchChatNotifications({ silent: true });
            }
            ...
            function fetchNotifications() {
                ...
                $.getJSON(window.JuntaPlay.ajax, {
                    ...
                    fetchChatNotifications(function (rendered) {
                        if (!hasItems && !rendered && !$list.children().length) {
                            renderEmptyState();
                        }
                    });
                }).always(function () {
            ...
            $panel.on('click', '[data-jp-notifications-clear]', function (event) {
                ...
                $.post(window.JuntaPlay.ajax, {
                    action: 'juntaplay_notifications_clear',
                    nonce: window.JuntaPlay.nonce
                }, null, 'json').done(function (response) {
                    syncCount(0);
                    renderEmptyState();
    ...
    var jpGroupModal = (function () {
        ...
        function setModalState(view, groupId) {
            ...
            if (view) {
                $modal.attr('data-group-view', view);
            }
        }
        ...
        function load(action, groupId, view) {
        ...
        return {
            loadDetail: function (groupId) {
                load('juntaplay_group_detail', groupId, 'detail');
            },
            loadEdit: function (groupId) {
                load('juntaplay_group_edit_form', groupId, 'edit');
            },
            loadAccess: function (groupId) {
                load('juntaplay_group_access', groupId, 'access');
            },
            loadAccessForm: function (groupId) {
                load('juntaplay_group_access_form', groupId, 'credentials');
            },
            close: closeModal,
            save: save,
    ...
    jQuery(document).on('click', '#juntaplay-group-modal [data-group-access]', function (event) {
        event.preventDefault();
        event.stopPropagation();
        ...
        if (!groupId) {
            groupId = parseInt(jQuery('#juntaplay-group-modal').attr('data-group-id'), 10) || 0;
        }
        ...
        jpGroupModal.loadAccess(groupId);
    });

    jQuery(document).on('click', '#juntaplay-group-modal [data-group-modal-back]', function (event) {
        event.preventDefault();
        event.stopPropagation();

        var $button = jQuery(this);
        var groupId = parseInt($button.data('groupId'), 10);
    ...
    jQuery(document).on('click', '#juntaplay-group-modal [data-group-access-register]', function (event) {
        event.preventDefault();
        event.stopPropagation();

        var groupId = parseInt(jQuery(this).data('groupId'), 10);
        ...
        jpGroupModal.loadAccessForm(groupId);
    });

    jQuery(document).on('click', '#juntaplay-group-modal [data-group-access-cancel]', function (event) {
        event.preventDefault();
        event.stopPropagation();

        var groupId = parseInt(jQuery('#juntaplay-group-modal').attr('data-group-id'), 10) || 0;
    ...
    jQuery(document).on('submit', '#juntaplay-group-modal [data-group-credentials-form]', function (event) {
        event.preventDefault();
        event.stopPropagation();

        var $form = jQuery(this);
        var groupId = parseInt($form.find('[name="group_id"]').val(), 10);
        var $submit = $form.find('button[type="submit"]').first();
        ...
        var payload = new window.FormData(this);
        payload.append('action', 'juntaplay_group_access_save');

        var nonce = jpGroupModal ? jQuery('#juntaplay-group-modal').attr('data-ajax-nonce') || '' : '';
        if (nonce) {
            payload.append('nonce', nonce);
        }

        jQuery.ajax({
            url: jQuery('#juntaplay-group-modal').attr('data-ajax-endpoint'),
            method: 'POST',
            data: payload,
            processData: false,
            contentType: false
            ...
            if (response && response.success && response.data && response.data.html) {
                jpGroupModal.render(response.data.html);
            } else {
                var message = (response && response.data && response.data.message) ? response.data.message : __('Não foi possível salvar as credenciais.', 'juntaplay');
                jpGroupModal.message('error', message);
            }
        }).fail(function (xhr) {
            var message = __('Não foi possível salvar as credenciais.', 'juntaplay');
    ...
    var toggleGroupCancelModal = function ($modal, shouldOpen) {
    ...
    jQuery(document).on('click', '[data-group-cancel-open]', function (event) {
        event.preventDefault();
        event.stopPropagation();

        var $modal = jQuery(this).closest('.juntaplay-group-modal__cta').find('[data-group-cancel-modal]').first();
        toggleGroupCancelModal($modal, true);
    });

    jQuery(document).on('click', '[data-group-cancel-close]', function (event) {
        event.preventDefault();
        var $modal = jQuery(this).closest('[data-group-cancel-modal]');
        toggleGroupCancelModal($modal, false);
    });
