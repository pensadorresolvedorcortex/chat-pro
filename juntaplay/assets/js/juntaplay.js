(function ($) {
    ...
    $(function () {
        ...
        var $successState = $('[data-group-success-state]');
        if ($successState.length) {
            openGroupSuccess({
                heading: ($successState.data('successHeading') || '').toString(),
                body: ($successState.data('successBody') || '').toString(),
                image: ($successState.data('successImage') || '').toString(),
                redirect: ($successState.data('successRedirect') || '').toString()
            });
    ...
    $(document).on('click', '[data-modal-close]', function (event) {
        event.preventDefault();
        var $modal = $(this).closest('.juntaplay-modal');
        closeModal($modal);
        maybeHandleModalRedirect($modal);
    });
    ...
    function maybeHandleModalRedirect($modal) {
        if (!$modal.length) {
            return;
        }

        var redirect = ($modal.attr('data-success-redirect') || '').toString();

        if (redirect) {
            $modal.removeAttr('data-success-redirect');
    ...
    function openGroupSuccess(state) {
        var data = state || {};
        ...
        var heading = (data.heading || '').toString();
        var body = (data.body || '').toString();
        var image = (data.image || '').toString();
        var redirect = (data.redirect || '').toString();
        ...
        if ($image.length) {
            ...
            if (heading) {
                $image.attr('alt', heading);
            }
        }

        var $close = $content.find('[data-group-success-close]');
        if ($close.length) {
        ...
        var $createModal = $('[data-group-create-modal]');
        if ($createModal.length) {
            closeModal($createModal);
        }

        if (redirect) {
            $modal.attr('data-success-redirect', redirect);
        } else {
            $modal.removeAttr('data-success-redirect');
        }
