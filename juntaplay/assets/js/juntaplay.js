                    ...
                    if (response.data && response.data.items) {
                        if (!response.data.items.length) {
                            ...
                            var html = response.data.items.map(function (item) {
                                ...
                                if (type === 'chat' && item.participant && item.group) {
                                    var preview = truncate(item.preview || message || '', 80);
                                    var name = item.participant.name || '';
                                    var avatar = item.participant.avatar || '';
                                    var groupTitle = item.group.title || '';
                                    var chatAttrs = ' class="juntaplay-notifications__item' + statusClass + ' juntaplay-notifications__item--chat" data-notification-id="' + item.id + '"';
                                    var avatarMarkup = avatar
                                        ? '<span class="juntaplay-notifications__chat-avatar"><img src="' + escapeHtml(avatar) + '" alt="" loading="lazy"></span>'
                                        : '<span class="juntaplay-notifications__chat-avatar is-placeholder" aria-hidden="true">👤</span>';

                                    var chatContent = '<div class="juntaplay-notifications__chat-body">' +
                                        '<div class="juntaplay-notifications__chat-top">' +
                                        '<span class="juntaplay-notifications__chat-name">' + escapeHtml(name) + '</span>' +
                                        '<span class="juntaplay-notifications__chat-group">' + escapeHtml(groupTitle) + '</span>' +
                                        '</div>' +
                                        '<div class="juntaplay-notifications__chat-preview">' + escapeHtml(preview) + '</div>' +
                                        '<span class="juntaplay-notifications__item-time">' + escapeHtml(time) + '</span>' +
                                        '</div>';

                                    if (href) {
                                        return '<li' + liStatus + '><a' + chatAttrs + ' href="' + href + '">' + avatarMarkup + chatContent + '</a></li>';
                                    }

                                    return '<li' + liStatus + '><span' + chatAttrs + '>' + avatarMarkup + chatContent + '</span></li>';
                                }
