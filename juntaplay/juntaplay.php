...
add_action('init', static function (): void {
    add_shortcode('juntaplay_chatonline', static function (): string {
        ...
        $resolve_avatar = static function (int $user_id): string {
            if (class_exists('\\JuntaPlay\\Front\\Profile')) {
                return \JuntaPlay\Front\Profile::resolve_avatar_url($user_id);
            }
        ...
        $header_avatar    = '';
        $fallback_avatar  = $resolve_avatar($current_id);
        ...
        if ($header_avatar === '') {
            $header_avatar = $resolve_avatar($current_id);
        }

        if ($fallback_avatar === '') {
            $fallback_avatar = $header_avatar ?: (string) get_avatar_url($current_id, ['size' => 160]);
        }
        ...
        foreach ($owned_groups as $group) {
            ...
            foreach ($member_ids as $member_id) {
                ...
                $avatar_url = $resolve_avatar($member_id);
        ...
        if ($default_member_group && ($default_member_group['owner_id'] ?? 0)) {
            ...
            $subscriber_admin = [
                ...
                'avatar' => $resolve_avatar($admin_id) ?: $fallback_avatar,
            ];
        }
        ...
        $context = [
            ...
            'nonce'          => wp_create_nonce('wp_rest'),
                ...
                'currentUser'    => [
                    ...
                    'avatar' => $header_avatar ?: $resolve_avatar($current_id) ?: $fallback_avatar,
                ],
        ...
        <style>
            ...
            .jp-chat-avatar-glass {
                ...
                border: 3px solid rgb(10 163 153)
                display: flex;
                align-items: center;
            ...
            .jp-chat-avatar-glass::after {
                content: '';
                position: absolute;
                inset: 2px;
                border-radius: 50%;
                background: linear-gradient(145deg, rgba(14, 165, 233, 0.12), rgba(236, 72, 153, 0.08));
                z-index: 0;
                filter: blur(8px);
                opacity: 0.85;
                pointer-events: none;
            ...
            .jp-chat-avatar-img {
                ...
                background: radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.75), rgba(255, 255, 255, 0.2));
                position: relative;
                z-index: 1;
            }
            ...
            .jp-chat-message-input button {
                width: 100%;
                background-color: #FF4858;  font-size: 15px; font-weight: 600; fill: #FFFFFF; color: #FFFFFF; border-style: solid; border-width: 2px 2px 2px 2px; border-color: #FF4858; border-radius: 60px 60px 60px 60px;
    padding: 12px 18px 12px 18px;
            ...
            .jp-chat-empty {
                ...
                text-align: center;
                color: #334155;
                font-weight: 600;font-size: 10px;display:none;
            }
        ...
        <br><div class="jp-chat-shell" data-jp-chat-shell>
            <?php if ($is_admin) : ?>
                <aside class="jp-chat-sidebar" data-jp-chat-sidebar>
                        <div class="jp-chat-section">
                        <p class="jp-chat-meta subtitle" style="margin: 8px 10px 4px; font-weight:600; font-size: 13px; color:#0f172a;">
