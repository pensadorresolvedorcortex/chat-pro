...
if (!defined('JuntaPlay\\JP_GROUP_COVER_PLACEHOLDER')) {
    define('JuntaPlay\\JP_GROUP_COVER_PLACEHOLDER', '');
}
...
add_action('plugins_loaded', static function (): void {
    ...
    if (!class_exists('\\JuntaPlay\\Plugin')) {
        return;
    }
...
add_action('init', static function (): void {
    add_shortcode('juntaplay_chatonline', static function (): string {
        ...
        if (class_exists('\\JuntaPlay\\Front\\Profile')) {
            ...
            $header_avatar    = isset($header_context['avatar_url']) ? esc_url_raw((string) $header_context['avatar_url']) : '';
        }

        $current_user_avatar = $header_avatar;

        if ($current_id > 0) {
            $current_user_avatar = $current_user_avatar !== ''
                ? $current_user_avatar
                : esc_url_raw(get_avatar_url($current_id, ['size' => 64]));
        }
        ...
        foreach ($owned_groups as $group) {
            ...
            $group_icon = (string) ($group['icon_url'] ?? '');
            if ($group_icon === '' && isset($group['cover_url'])) {
                $group_icon = (string) $group['cover_url'];
            }
            ...
            foreach ($member_ids as $member_id) {
                ...
                $member_avatar = esc_url_raw(get_avatar_url($member_id, ['size' => 64]));
        ...
        if ($default_member_group && ($default_member_group['owner_id'] ?? 0)) {
            ...
            if ($admin_user) {
                $subscriber_admin = [
                    'id'     => $admin_id,
                    'name'   => (string) $admin_user->display_name,
                    'avatar' => esc_url_raw(get_avatar_url($admin_id, ['size' => 64])),
                ];
            }
        }
                ...
                <header class="jp-chat-header" data-jp-chat-header>
                    <div class="jp-avatar-glass juntaplay-profile_avatar" data-jp-chat-avatar-shell>
                        <img
                          ...
                          data-jp-chat-partner-avatar
                          src="<?php echo esc_url($current_user_avatar); ?>"
                          alt="<?php echo esc_attr($current_user->display_name ?: $context['i18n']['startChat']); ?>"
                        />
                    </div>
                    <div class="titles">
        ...
        <script>
            (function () {
                ...
                const currentAvatar = (data.currentUser && data.currentUser.avatar) || '';
                ...
                const setPartner = (user) => {
                    ...
                    if (headerAvatar) {
                        if (user && user.avatar) {
                            ...
                            headerAvatar.alt = data.currentUser?.name || '';
                        }
                    }
                };
