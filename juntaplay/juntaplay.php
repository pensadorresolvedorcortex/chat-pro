        ...
        foreach ($owned_groups as $group) {
            ...
            foreach ($member_ids as $member_entry) {
                if (is_array($member_entry)) {
                    ...
                    if ($member_id <= 0 && isset($member_entry['user'])) {
                        $member_object = $member_entry['user'];
                        $member_id     = is_object($member_object) && isset($member_object->ID)
                            ? (int) $member_object->ID
                            : (int) ($member_entry['user']['ID'] ?? 0);
                    }

                    if ($member_id <= 0) {
                        $member_id = (int) ($member_entry['id'] ?? 0);
                    }
                } else {
                    $member_id = (int) $member_entry;
                }
