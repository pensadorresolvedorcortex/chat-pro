...
add_action('init', static function (): void {
    add_shortcode('juntaplay_chatonline', static function (): string {
        ...
        $resolve_user_avatar = static function (int $user_id) use ($current_id): string {
            if ($user_id <= 0) {
                return '';
            }
            ...
            if (class_exists('\\JuntaPlay\\Front\\Profile')) {
                ...
                if (method_exists($profile_instance, 'get_header_context')) {
                    ...
                    if (is_array($profile_context) && isset($profile_context['avatar_url'])) {
                        $context_avatar = esc_url_raw((string) $profile_context['avatar_url']);

                        if ($context_avatar !== '' && (!isset($profile_context['user_id'])
        ...
        foreach ($owned_groups as $group) {
            ...
            foreach ($member_ids as $member_entry) {
                $member_id = is_array($member_entry)
                    ? (int) ($member_entry['user_id'] ?? $member_entry['id'] ?? 0)
                    : (int) $member_entry;
                ...
                $members[] = [
                    ...
                    'avatar' => $resolve_user_avatar($member_id),
