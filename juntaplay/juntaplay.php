...
add_action('init', static function (): void {
    add_shortcode('juntaplay_chatonline', static function (): string {
        ...
        $resolve_user_avatar = static function (int $user_id): string {
            ...
            if ($avatar === '') {
                $avatar = (string) get_avatar_url($user_id, [
                    'size'          => 64,
                    'default'       => 'mystery',
                    'force_default' => true,
                ]);
        ...
        foreach ($owned_groups as $group) {
            ...
            foreach ($member_ids as $member_entry) {
                ...
                // Força uso do avatar do próprio participante, nunca do admin logado.
                $member_avatar  = esc_url_raw((string) get_avatar_url($member_id, [
                    'size'    => 64,
                    'default' => 'mystery',
                ]));
        ...
        <script>
            (function () {
                ...
                const renderMembers = (groupId) => {
                    ...
                    const renderableMembers = members.filter((member) => member && member.avatar_url);

                    if (!renderableMembers.length) {
                        membersEmpty.textContent = data.i18n.emptyMembers;
                        membersEmpty.style.display = 'block';
                        return;
                    }

                    renderableMembers.forEach((member) => {
                        const li = document.createElement('li');
                        li.dataset.memberId = member.id;
                        li.dataset.memberName = member.name;
                        li.dataset.memberGroup = member.group;
                        li.dataset.memberAvatar = member.avatar_url;
