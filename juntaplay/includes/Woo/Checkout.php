...
class Checkout
{
    ...
    private const SESSION_MEMBER_FEE_KEY = 'juntaplay_member_fee';
    ...
    private const MEMBER_FEE_LABEL     = 'Taxa Administrativa';
    ...
    public function apply_group_fees(WC_Cart $cart): void
    {
        ...
        foreach ($cart->get_cart() as $cart_item) {
            ...
            $fee_per_member = isset($cart_item['juntaplay_group']['fee_per_member'])
                ? (float) $cart_item['juntaplay_group']['fee_per_member']
                : Settings::get_fee_per_member();
            $member_fee_total += max(0.0, $fee_per_member) * $quantity;
        }
        ...
        $deposit_total = $group_total + $member_fee_total;
        ...
        $session->set(self::SESSION_MEMBER_FEE_KEY, $this->format_decimal($member_fee_total));
        ...
        if ($member_fee_total > 0) {
            $cart->add_fee(self::MEMBER_FEE_LABEL, $member_fee_total, false);
        }
        ...
        if ($user_id > 0) {
            ...
            if ($balance > 0 && $use_wallet) {
                $estimated_total = $cart->get_cart_contents_total() + $deposit_total + $processing_fee + $member_fee_total;
    ...
    public function persist_checkout_meta(WC_Order $order, $data): void
    {
        ...
        $member_fee = (float) $session->get(self::SESSION_MEMBER_FEE_KEY, 0.0);
        ...
        if ($member_fee > 0) {
            $order->update_meta_data('_juntaplay_member_fee', $this->format_decimal($member_fee));
        }
    ...
    private function clear_session_totals(): void
    {
        ...
        $session->__unset(self::SESSION_MEMBER_FEE_KEY);
        $session->__unset(self::SESSION_WALLET_KEY);
    }
