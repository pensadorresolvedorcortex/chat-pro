...
class Checkout
{
    ...
    public function format_fee_amount(string $html, WC_Fee $fee): string
    {
    ...
    public function format_fee_label(string $label, WC_Fee $fee): string
    {
        if ($this->is_processing_fee($fee)) {
    ...
    private function clear_session_totals(): void
    {
        ...
        WC()->session->__unset(self::SESSION_DEPOSIT_KEY);
        WC()->session->__unset(self::SESSION_PROCESSING_KEY);
        WC()->session->__unset(self::SESSION_WALLET_KEY);
    }
