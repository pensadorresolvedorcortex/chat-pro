...
class Hooks
{
    ...
    public function render_thankyou_message($order_id): void
    {
        ...
        foreach ($order->get_items() as $item) {
            ...
            $pool_meta = (string) $item->get_meta('JuntaPlay Pool', true);

            if ($pool_meta === '') {
                continue;
            }

            $quotas = (string) $item->get_meta('JuntaPlay Cotas', true);
        ...
        wc_get_template(
            ...
            [
                ...
                'help_url'          => $help_url,
                'illustration_url'  => JP_URL . 'assets/images/thankyou-placeholder.svg',
            ],
            '',
            JP_DIR . 'templates/'
        );
