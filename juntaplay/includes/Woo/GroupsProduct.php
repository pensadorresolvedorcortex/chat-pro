...
class GroupsProduct
{
    ...
    public static function get_product_id(): int
    {
        ...
        if ($product_id > 0) {
            ...
            if ($status === 'publish') {
                self::ensure_product_configuration($product_id);
    ...
    private static function create_product(): int
    {
        ...
        self::ensure_product_configuration((int) $product_id);

        return (int) $product_id;
    }

    private static function ensure_product_configuration(int $product_id): void
    {
        update_post_meta($product_id, '_sold_individually', 'no');
        update_post_meta($product_id, '_manage_stock', 'no');
        update_post_meta($product_id, '_min_purchase_quantity', '1');
