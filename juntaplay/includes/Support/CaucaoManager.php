...
class CaucaoManager
{
    ...
    private static function evaluate_cycle(array $cycle): array
    {
        ...
        $active_until_end_status = defined(__NAMESPACE__ . '\\GroupMembers::STATUS_ACTIVE_UNTIL_END_OF_CYCLE')
        ...
        if (
            in_array($membership['status'], [GroupMembers::STATUS_EXIT_SCHEDULED, $active_until_end_status], true)
            && $exit_effective_ts
        ...
        if (in_array($membership['status'], [GroupMembers::STATUS_ACTIVE, $active_until_end_status], true) && !$exit_effective_ts) {
            return [
                'approve' => false,
                'status'  => CaucaoCycles::STATUS_RETIDO,
        ...
        if (!in_array($membership['status'], [GroupMembers::STATUS_ACTIVE, GroupMembers::STATUS_EXIT_SCHEDULED, GroupMembers::STATUS_EXITED, $active_until_end_status], true)) {
            return [
                'approve' => false,
                'status'  => CaucaoCycles::STATUS_RETIDO_DEFINITIVO,
