...
class CaucaoManager
{
    ...
    private static function evaluate_cycle(array $cycle): array
    {
        ...
        $latest_cancel = GroupMembershipEvents::get_latest_for_user($user_id, [$group_id], GroupMembershipEvents::TYPE_CANCEL);
        $cancel_date   = '';

        if (isset($latest_cancel[$group_id]['created_at'])) {
            $cancel_date = (string) $latest_cancel[$group_id]['created_at'];
        }

        $cancel_ts = $cancel_date !== '' ? strtotime($cancel_date) : false;
        ...
        if ($cancel_ts && $cycle_end_ts && $cancel_ts > $cycle_end_ts) {
            return [
                'approve' => false,
                'status'  => CaucaoCycles::STATUS_RETIDO_DEFINITIVO,
                'note'    => 'Saída após o ciclo sem quitação registrada.',
            ];
        }

        if ($membership['status'] !== 'active' && !$cancel_ts) {
            return [
                'approve' => false,
                'status'  => CaucaoCycles::STATUS_RETIDO_DEFINITIVO,
