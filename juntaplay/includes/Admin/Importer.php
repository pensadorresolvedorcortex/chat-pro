...
class Importer
{
    public function init(): void
    {
        ...
        add_action('admin_post_juntaplay_seed_pools', [$this, 'handle_seed_pools']);
    ...
    public function render(): void
    {
        ...
        if ($success === 'demo') {
            ...
            $pool_result = get_transient('juntaplay_pools_seed_result');
            if ($pool_result !== false) {
                delete_transient('juntaplay_pools_seed_result');
            }
        }
        ...
        if ($error === 'demo') {
            ...
            $stored_error = get_transient('juntaplay_pools_seed_error');
            if ($stored_error !== false) {
                $pool_error = (string) $stored_error;
                delete_transient('juntaplay_pools_seed_error');
            }
        }
        ...
        <div class="wrap">
            ...
            <?php if ($pool_result && is_array($pool_result)) : ?>
                <div class="notice notice-success is-dismissible">
                    <p><strong><?php esc_html_e('Serviços pré-aprovados importados.', 'juntaplay'); ?></strong></p>
                    <p><?php echo esc_html(sprintf(
                        /* translators: 1: total pools imported, 2: pools created */
                        __('Total de serviços carregados: %1$d · Criados/atualizados: %2$d.', 'juntaplay'),
                        $pool_result['total'] ?? 0,
                        $pool_result['created'] ?? 0
                    )); ?></p>
                </div>
            <?php endif; ?>
            ...
            <?php if ($pool_error !== '') : ?>
                <div class="notice notice-error">
                    <p><strong><?php esc_html_e('Não foi possível importar os serviços pré-aprovados.', 'juntaplay'); ?></strong></p>
                    <p><?php echo esc_html($pool_error); ?></p>
                </div>
            <?php endif; ?>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
                ...
                <p class="description"><?php esc_html_e('Colunas esperadas: title, slug, price, quota_start, quota_end, category (opcional), excerpt, thumbnail_id, icon_id, cover_id, service_url, is_featured.', 'juntaplay'); ?></p>
                <?php submit_button(__('Importar CSV', 'juntaplay')); ?>
            </form>

            <hr />

            <h2><?php esc_html_e('Popular serviços pré-aprovados (ANEXO 100/101)', 'juntaplay'); ?></h2>
            <p class="description">
                <?php esc_html_e('Carrega os 6 destaques e o catálogo completo de 67 cards de serviços prontos para aprovação automática.', 'juntaplay'); ?>
            </p>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <?php wp_nonce_field('juntaplay_seed_pools'); ?>
                <input type="hidden" name="action" value="juntaplay_seed_pools" />
                <?php submit_button(__('Importar serviços aprovados', 'juntaplay'), 'secondary'); ?>
            </form>
    ...
    public function handle_import(): void
    {
        ...
        foreach ($rows as $row) {
            $pool_id = Pools::create_or_update([
                ...
                'icon_id'      => isset($row['icon_id']) ? absint($row['icon_id']) : null,
                'cover_id'     => isset($row['cover_id']) ? absint($row['cover_id']) : null,
                'service_url'  => isset($row['service_url']) ? sanitize_text_field($row['service_url']) : null,
    ...
    public function handle_seed_pools(): void
    {
        if (!current_user_can('manage_options')) {
            wp_die(__('Você não tem permissão para executar esta ação.', 'juntaplay'));
        }

        check_admin_referer('juntaplay_seed_pools');

        $seeder = new PoolSeeder();
        $result = $seeder->seed();

        set_transient('juntaplay_pools_seed_result', $result, 5 * MINUTE_IN_SECONDS);

        wp_safe_redirect(add_query_arg('jp_success', 'pool', admin_url('admin.php?page=juntaplay-import')));
        exit;
    }
}
