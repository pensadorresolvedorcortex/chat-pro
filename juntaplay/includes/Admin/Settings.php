...
class Settings
{
    ...
    public const OPTION_MEMBER_FEE      = 'juntaplay_fee_per_member';

    private const OPTION_SPLIT_PERCENT  = 'juntaplay_split_superadmin_percent';

    private const DEFAULT_SPLIT_PERCENTAGE = 25;
    
    private const DEFAULT_PROCESSING_FEE = 0.68;
    private const DEFAULT_MEMBER_FEE     = 2.50;
    ...
    public function register_settings(): void
    {
        ...
        register_setting('juntaplay_settings', self::OPTION_MEMBER_FEE, ['sanitize_callback' => [$this, 'sanitize_member_fee']]);
        register_setting(
            'juntaplay_settings',
    ...
    public function render(): void
    {
        ...
        $member_fee = get_option(self::OPTION_MEMBER_FEE, self::DEFAULT_MEMBER_FEE);
        if (!is_numeric($member_fee)) {
            $member_fee = self::DEFAULT_MEMBER_FEE;
        }
        $can_manage_fees = function_exists('is_super_admin') ? is_super_admin() : current_user_can('manage_options');
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                $split_value   = (int) get_option(self::OPTION_SPLIT_PERCENT, 0);
                                $split_display = (string) $split_value;
                                ?>
                                <input
                                    type="number"
                                    inputmode="numeric"
                ...
                <?php if ($can_manage_fees) : ?>
                    <h2 class="title"><?php esc_html_e('Taxas e repasses', 'juntaplay'); ?></h2>
                    <table class="form-table" role="presentation">
                        <tbody>
                            <tr>
                                <th scope="row"><?php esc_html_e('Taxa fixa por membro (R$)', 'juntaplay'); ?></th>
                                <td>
                                    <input type="number" min="0" step="0.01" name="<?php echo esc_attr(self::OPTION_MEMBER_FEE); ?>" value="<?php echo esc_attr(number_format((float) $member_fee, 2, '.', '')); ?>" />
                                    <p class="description"><?php esc_html_e('Valor adicional cobrado de cada participante e repassado à plataforma. Não altera o valor base do grupo.', 'juntaplay'); ?></p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    public function sanitize_member_fee($input): float
    {
        if (function_exists('is_super_admin') && !is_super_admin()) {
            $current = get_option(self::OPTION_MEMBER_FEE, self::DEFAULT_MEMBER_FEE);
            if (!is_numeric($current)) {
                return self::DEFAULT_MEMBER_FEE;
            }
    ...
    public static function get_fee_per_member(): float
    {
        $fee = get_option(self::OPTION_MEMBER_FEE, self::DEFAULT_MEMBER_FEE);
