...
class Settings
{
    ...
    public function register_settings(): void
    {
        ...
        register_setting('juntaplay_settings', self::OPTION_SPLIT_PERCENT, ['sanitize_callback' => [$this, 'sanitize_split_percentage_option']]);
    }
                ...
                <table class="form-table" role="presentation">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                $split_percentage = self::get_split_percentage();
                                $split_display    = number_format($split_percentage, 2, ',', '');
                                ?>
                                <input
                                    type="text"
                                    inputmode="decimal"
                                    pattern="^(?:100(?:[\.,]0{1,2})?|\d{1,2}(?:[\.,]\d{0,2})?)$"
                                    name="<?php echo esc_attr(self::OPTION_GENERAL); ?>[split_percentage]"
    ...
    public function sanitize_general(array $input): array
    {
        ...
        $split = $this->sanitize_split_percentage_option($input['split_percentage'] ?? self::DEFAULT_SPLIT_PERCENTAGE);
        ...
        update_option(self::OPTION_SPLIT_PERCENT, $split);
    ...
    public function sanitize_split_percentage_option($value): float
    {
        if (is_string($value)) {
            $value = str_replace(',', '.', $value);
        }

        $number = is_scalar($value) ? (float) $value : 0.0;
    ...
    public static function get_split_percentage(): float
    {
        ...
        if ($stored_percent !== null && $stored_percent !== '') {
            return self::normalize_percentage_value($stored_percent);
        }

        $general = get_option(self::OPTION_GENERAL, []);
        $split   = $general['split_percentage'] ?? self::DEFAULT_SPLIT_PERCENTAGE;

        return self::normalize_percentage_value($split);
    }

    public static function normalize_percentage_value($value): float
    {
        if (is_string($value)) {
            $value = str_replace(',', '.', $value);
        }
