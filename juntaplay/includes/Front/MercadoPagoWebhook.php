...
class MercadoPagoWebhook
{
    ...
    public function handle_paid_order(int $order_id): void
    {
        ...
        if ((!$via_callback && !$this->is_mercado_pago_order($order)) && !$split_processed) {
            return;
        }

        if (!$via_callback && !$split_processed && !$this->has_gateway_payment_reference($order)) {
            return;
        }
        ...
        if ($split_processed && !$via_callback) {
    ...
    private function process_order_payment(WC_Order $order, float $percentage, ?array $payload_log, string $cycle_start): array
    {
        ...
        foreach ($summaries as $summary) {
            ...
            $superadmin_amt = $base_value * ($percentage / 100);
            $admin_amt      = max(0.0, $base_value - $superadmin_amt);
            ...
            $split_id = PaymentSplits::create([
                ...
                'payload'           => $payload_log + ['superadmin_id' => $superadmin_id],
            ]);
