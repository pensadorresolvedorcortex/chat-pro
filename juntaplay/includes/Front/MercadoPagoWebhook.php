...
class MercadoPagoWebhook
{
    ...
    public function handle_notification(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if (!$summaries) {
            return new WP_REST_Response(['message' => 'Nenhum grupo associado ao pedido.'], 200);
        }

        $percentage      = Settings::normalize_percentage_value(Settings::get_split_percentage());
        ...
        if ($participant_id <= 0) {
            return new WP_REST_Response(['message' => 'Usuário do pedido ausente.'], 400);
        }
        ...
        if ($error_message !== '') {
            ...
            return new WP_REST_Response(['message' => $error_message], 500);
        }
