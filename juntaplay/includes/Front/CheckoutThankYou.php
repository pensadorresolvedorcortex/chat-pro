...
class CheckoutThankYou
{
    ...
    {
        /** @var bool $override */
        $override = apply_filters('juntaplay/checkout/override_thankyou', true, $order);
    ...
    private function maybe_send_custom_email(WC_Order $order): void
    {
        ...
        $profile_url = isset($context['profile_url']) ? (string) $context['profile_url'] : '';
        $groups_url  = isset($context['groups_url']) ? (string) $context['groups_url'] : '';
        $destination = $groups_url !== '' ? $groups_url : $profile_url;
        $headline    = __('Sua assinatura está confirmada!', 'juntaplay');
        $preheader   = isset($context['primary_item']) && $context['primary_item'] !== ''
            ? sprintf(__('Resumo do seu pedido de %s', 'juntaplay'), $context['primary_item'])
        ...
        $sent = EmailHelper::send(
            ...
            [
                ...
                'preheader' => $preheader,
                'footer'    => array_filter([
                    __('Esse e-mail foi enviado automaticamente pelo JuntaPlay assim que seu pagamento foi confirmado.', 'juntaplay'),
                    $destination !== ''
                        ? sprintf(__('Acompanhe o andamento da sua assinatura acessando %s.', 'juntaplay'), $destination)
                        : '',
                ]),
            ]
        );
    ...
    private function build_email_blocks(array $context): array
    {
        ...
        $blocks = [
            [
                'type'    => 'paragraph',
                'content' => $greeting . ' ' . __('Recebemos o seu pagamento e preparamos os próximos passos da sua assinatura.', 'juntaplay'),
            ],
        ];
