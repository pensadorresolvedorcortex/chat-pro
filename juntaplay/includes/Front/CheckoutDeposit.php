...
class CheckoutDeposit
{
    ...
    private function has_group_checkout_items(): bool
    {
    ...
    {
        return $this->is_deposit_checkout() || $this->has_group_checkout_items();
    }
    ...
    public function filter_checkout_fields(array $fields): array
    {
        ...
        $first_name   = $user ? (string) ($user->first_name ?: $user->display_name ?: $user->user_login) : '';
        $last_name    = $user ? (string) ($user->last_name ?: $user->display_name ?: $user->user_login) : '';
        $email        = $user ? (string) $user->user_email : '';
        $phone        = (string) get_user_meta($user_id, 'billing_phone', true);
        $tax_document = (string) get_user_meta($user_id, 'juntaplay_tax_document', true);

        $hidden_label = ['label_class' => ['screen-reader-text']];
        $hidden_field = ['type' => 'hidden', 'required' => false, 'class' => ['jp-checkout-hidden']];

        $fields['billing'] = [
            'billing_tax_document' => [
                'type'              => 'text',
                'label'             => __('CPF', 'juntaplay'),
                'placeholder'       => __('Digite seu CPF', 'juntaplay'),
                'required'          => true,
                'class'             => ['form-row-wide'],
                'input_class'       => ['juntaplay-checkout__tax-document'],
                'priority'          => 10,
                'inputmode'         => 'numeric',
                'custom_attributes' => [
                    'autocomplete' => 'off',
                ...
                'default'           => $tax_document,
            ],
            'billing_first_name' => array_merge(
                [
                    'label'    => __('Nome', 'juntaplay'),
                    'default'  => $first_name,
                    'priority' => 20,
                ],
                $hidden_field,
