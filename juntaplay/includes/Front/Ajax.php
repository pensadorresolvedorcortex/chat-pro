...
class Ajax
{
    ...
    {
        if (!$this->is_group_cover_upload_request($user)) {
            return $allcaps;
        }
    ...
    {
        if ($this->is_group_cover_upload_request()) {
            return 'upload_files';
        }
    ...
    private function is_group_cover_upload_request(?WP_User $user = null): bool
    {
        ...
        $allowed_actions = ['juntaplay_group_edit_save', 'upload-attachment', 'query-attachments'];

        if (!in_array($action, $allowed_actions, true)) {
            return false;
        }

        $requires_flag = in_array($action, ['upload-attachment', 'query-attachments'], true);
        if ($requires_flag) {
            $flag = '';

            if (isset($_REQUEST['juntaplay_group_cover'])) {
                $flag = sanitize_key(wp_unslash((string) $_REQUEST['juntaplay_group_cover']));
            } elseif (isset($_REQUEST['query']) && is_array($_REQUEST['query']) && isset($_REQUEST['query']['juntaplay_group_cover'])) {
                $flag = sanitize_key(wp_unslash((string) $_REQUEST['query']['juntaplay_group_cover']));
            }
