...
class Ajax
{
    ...
    public function group_credentials(): void
    {
        ...
        $access_notes = isset($raw_credentials['access_notes']) ? sanitize_textarea_field((string) $raw_credentials['access_notes']) : '';
        ...
        if ($access_notes !== '') {
            $fields[] = [
                'label' => __('Observações', 'juntaplay'),
                'value' => $access_notes,
                'type'  => 'note',
                'html'  => $format_html($access_notes),
            ];
        }
        ...
        $empty_credentials = $fields === [];

        do_action('juntaplay/groups/send_access', $group_id, $user_id, $raw_credentials, [
            'source' => 'profile',
        ]);

        $hint = $empty_credentials
            ? __('O administrador ainda não cadastrou os dados de acesso deste grupo.', 'juntaplay')
            : __('Também enviamos uma cópia das credenciais para o seu e-mail.', 'juntaplay');
    ...
    private function format_group(array $group, array $categories): array
    {
        ...
        return [
            ...
            'poolLink'          => $pool_link,
            'ctaUrl'            => $pool_link,
            'ctaDisabled'       => $availability_state === 'full' || $pool_link === '',
