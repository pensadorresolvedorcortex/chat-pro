...
class Ajax
{
    ...
    public function notifications_feed(): void
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (($item['type'] ?? '') === 'chat' && ($item['status'] ?? '') === NotificationsData::STATUS_UNREAD) {
                $chat_items_unread++;
            }
            ...
            if (($notification['type'] ?? '') === 'chat') {
                $participant_id = 0;
                $group_id       = 0;
                $action_url     = isset($notification['action_url']) ? (string) $notification['action_url'] : '';

                if ($action_url !== '') {
                    $parsed = wp_parse_url($action_url);
                    if (is_array($parsed) && isset($parsed['query'])) {
                        parse_str((string) $parsed['query'], $query_args);
                        $participant_id = isset($query_args['participant_id']) ? absint($query_args['participant_id']) : 0;
                        $group_id       = isset($query_args['group_id']) ? absint($query_args['group_id']) : 0;
                    }
                }

                $participant = get_user_by('id', $participant_id);
                $group       = $group_id > 0 ? Groups::get($group_id) : null;

                $item['participant'] = [
                    'id'     => $participant_id,
                    'name'   => $participant && $participant->exists() ? ($participant->display_name ?: $participant->user_login) : '',
                    'avatar' => $participant_id > 0 ? get_avatar_url($participant_id, ['size' => 96]) : '',
                ];
                ...
                if ($participant_id > 0 && $group_id > 0) {
                    global $wpdb;
                    $table = $wpdb->prefix . 'juntaplay_chat_messages';

                    $last_message = $wpdb->get_row(
                        $wpdb->prepare(
                            "SELECT message, image_url FROM {$table} WHERE group_id = %d AND user_id = %d AND admin_id = %d ORDER BY created_at DESC, id DESC LIMIT 1",
                            $group_id,
                            $participant_id,
                            $user_id
                        )
                    ...
                    if ($last_message) {
                        $item['preview'] = !empty($last_message->image_url)
                            ? __('📷 Imagem', 'juntaplay')
        ...
        $is_group_admin = !empty($groups['owned']);

        $chat_unread = $is_group_admin
            ? $chat->count_unread_for_admin($user_id)
            : $chat->count_unread_for_user($user_id);

        $chat_notifications_unread = NotificationsData::count_unread_by_type($user_id, 'chat');
        $chat_unread_missing      = max(0, $chat_unread - $chat_notifications_unread);

        if ($chat_unread_missing > 0) {
            foreach ($this->get_unread_chat_notifications($user_id, $is_group_admin, min(15, $chat_unread_missing)) as $chat_item) {
                $items[] = $chat_item;
            }
        }
        ...
        $notifications_unread = NotificationsData::count_unread($user_id) + $chat_unread_missing;
    ...
    public function notifications_mark(): void
    {
        ...
        $notification_ids = array_values(array_filter($ids, static fn (int $id): bool => $id > 0));
        $chat_ids         = array_map('absint', array_values(array_filter($ids, static fn (int $id): bool => $id < 0)));

        if ($notification_ids) {
            NotificationsData::mark_read($user_id, $notification_ids);
        }

        if ($chat_ids) {
            $this->mark_chat_notifications_read($user_id, $chat_ids);
        }

        $chat_unread   = $this->count_chat_unread($user_id);
        $chat_missing  = max(0, $chat_unread - NotificationsData::count_unread_by_type($user_id, 'chat'));
        $total_unread  = NotificationsData::count_unread($user_id) + $chat_missing;

        wp_send_json_success(['unread' => $total_unread]);
    }
    ...
    public function notifications_clear(): void
    {
        ...
        $this->mark_all_chat_notifications_read($user_id);
     ...
     */
    private function get_unread_chat_notifications(int $user_id, bool $is_group_admin, int $limit = 10): array
    {
        global $wpdb;

        $limit      = max(1, min(50, $limit));
        $chat_table = $wpdb->prefix . 'juntaplay_chat_messages';

        if ($is_group_admin) {
            $query = $wpdb->prepare(
                "SELECT id, user_id, group_id, message, image_url, created_at\n                 FROM {$chat_table}\n                 WHERE admin_id = %d AND sender = %s AND is_read_admin = 0\n                 ORDER BY created_at DESC, id DESC\n                 LIMIT %d",
                $user_id,
                'user',
                $limit
            );
            ...
            $query = $wpdb->prepare(
                "SELECT id, user_id, group_id, message, image_url, created_at\n                 FROM {$chat_table}\n                 WHERE user_id = %d AND sender = %s AND is_read_user = 0\n                 ORDER BY created_at DESC, id DESC\n                 LIMIT %d",
                $user_id,
                'admin',
                $limit
            );
        ...
        foreach ($rows as $row) {
            ...
            $participant_id = $is_group_admin ? (int) ($row['user_id'] ?? 0) : $user_id;
            $participant    = $participant_id > 0 ? get_user_by('id', $participant_id) : false;
            $group          = $group_id > 0 ? Groups::get($group_id) : null;

            $participant_name = $participant && $participant->exists()
                ? ($participant->display_name ?: $participant->user_login)
                : __('Participante', 'juntaplay');

            $group_title = $group && isset($group->title) ? (string) $group->title : '';
            ...
            $preview    = $image_url !== '' ? __('📷 Imagem', 'juntaplay') : $message;

            $items[] = [
                'id'         => -abs($message_id),
                'title'      => sprintf(__('Nova mensagem de %s', 'juntaplay'), $participant_name),
                'message'    => $preview,
                'preview'    => $preview,
                'status'     => NotificationsData::STATUS_UNREAD,
     ...
     */
    private function mark_chat_notifications_read(int $user_id, array $message_ids): void
    {
        global $wpdb;
        ...
        $table       = $wpdb->prefix . 'juntaplay_chat_messages';
        $column      = $is_group_admin ? 'admin_id' : 'user_id';
        $read_column = $is_group_admin ? 'is_read_admin' : 'is_read_user';

        $placeholders = implode(',', array_fill(0, count($message_ids), '%d'));

        $query = $wpdb->prepare(
            "UPDATE {$table} SET {$read_column} = 1 WHERE {$column} = %d AND id IN ({$placeholders})",
            ...array_merge([$user_id], $message_ids)
        );
    ...
    private function mark_all_chat_notifications_read(int $user_id): void
    {
        global $wpdb;
        ...
        $query = $wpdb->prepare(
            "UPDATE {$table} SET {$read_column} = 1 WHERE {$column} = %d",
            $user_id
        );
    ...
    private function count_chat_unread(int $user_id): int
    {
        $groups         = Groups::get_groups_for_user($user_id);
        $is_group_admin = !empty($groups['owned']);
        $chat           = new ChatMessage();
