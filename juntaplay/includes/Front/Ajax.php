...
class Ajax
{
    public function __construct(private Profile $profile)
    {
        ...
        add_action('wp_ajax_juntaplay_group_access', [$this, 'group_access']);
        add_action('wp_ajax_juntaplay_group_access_form', [$this, 'group_access_form']);
        add_action('wp_ajax_juntaplay_group_access_save', [$this, 'group_access_save']);
    }
    ...
    public function group_credentials(): void
    {
        ...
        $result   = $this->resolve_group_access_payload($group_id, $user_id);
    ...
    public function group_access(): void
    {
        check_ajax_referer('jp_nonce', 'nonce');
        ...
        $html = $this->render_template('group-modal-access.php', [
            'group_id'    => $group_id,
            'title'       => isset($payload['title']) ? (string) $payload['title'] : '',
            'credentials' => isset($payload['credentials']) && is_array($payload['credentials']) ? $payload['credentials'] : [],
            'hint'        => isset($payload['hint']) ? (string) $payload['hint'] : '',
            'hint_html'   => isset($payload['hint_html']) ? (string) $payload['hint_html'] : '',
            'is_admin'    => !empty($payload['is_admin']),
            'has_credentials' => !empty($payload['has_credentials']),
        ]);

        if ($html === '') {
            wp_send_json_error(['message' => __('Não foi possível carregar os dados de acesso.', 'juntaplay')], 500);
        }

        wp_send_json_success(['html' => $html]);
    }

    public function group_access_form(): void
    {
        check_ajax_referer('jp_nonce', 'nonce');
        ...
        $admin_check = $this->assert_group_admin($group_id, $user_id);
        if (!$admin_check['success']) {
            wp_send_json_error(['message' => $admin_check['message']], $admin_check['status']);
        }

        $group = $admin_check['group'];
        $credentials = Groups::get_credentials($group_id);

        $html = $this->render_template('group-modal-credentials.php', [
            'group_id'          => $group_id,
            'title'             => isset($group->title) ? (string) $group->title : '',
            'access_login'      => isset($credentials['access_login']) ? (string) $credentials['access_login'] : '',
            'access_password'   => isset($credentials['access_password']) ? (string) $credentials['access_password'] : '',
            'access_observations' => isset($credentials['access_observations']) ? (string) $credentials['access_observations'] : '',
        ]);

        if ($html === '') {
            wp_send_json_error(['message' => __('Não foi possível carregar o formulário de credenciais.', 'juntaplay')], 500);
        }

        wp_send_json_success(['html' => $html]);
    }

    public function group_access_save(): void
    {
        check_ajax_referer('jp_nonce', 'nonce');
        ...
        $access_login = sanitize_text_field(wp_unslash($_POST['access_login'] ?? ''));
        $access_password = sanitize_text_field(wp_unslash($_POST['access_password'] ?? ''));
        $access_observations = sanitize_textarea_field(wp_unslash($_POST['access_observations'] ?? ''));

        if ($access_login === '' || $access_password === '') {
            wp_send_json_error(['message' => __('Informe login e senha para salvar as credenciais.', 'juntaplay')], 400);
        }
        ...
        if ($updated === false) {
            wp_send_json_error(['message' => __('Não foi possível salvar as credenciais.', 'juntaplay')], 500);
        }

        $raw_credentials = Groups::get_credentials($group_id);
        foreach ($this->get_group_active_member_ids($group_id) as $member_id) {
            if ($member_id === $user_id) {
                continue;
            }
            do_action('juntaplay/groups/send_access', $group_id, $member_id, $raw_credentials, [
                'source' => 'profile',
                'context' => 'admin_save',
            ]);
        }
        ...
        if ($html === '') {
            wp_send_json_error(['message' => __('Não foi possível atualizar a visualização.', 'juntaplay')], 500);
        }

        wp_send_json_success(['html' => $html]);
    }

    private function resolve_group_access_payload(int $group_id, int $user_id): array
    {
        if ($group_id <= 0) {
            return [
                'success' => false,
        ...
        if (!is_user_logged_in()) {
            return [
                'success' => false,
                'status'  => 401,
                'message' => __('Faça login para acessar os dados deste grupo.', 'juntaplay'),
            ];
        }
        ...
        if ($user_id <= 0) {
            return [
                'success' => false,
                'status'  => 401,
                'message' => __('Sessão expirada. Entre novamente para continuar.', 'juntaplay'),
            ];
        }
        ...
        if (!$group) {
            return [
                'success' => false,
                'status'  => 404,
                'message' => __('Grupo não encontrado.', 'juntaplay'),
            ];
        }

        $owns_group = isset($group->owner_id) && (int) $group->owner_id === $user_id;
        $is_member = GroupMembers::user_has_membership($group_id, $user_id);
        $is_admin = $this->is_group_admin($group_id, $user_id, $group);

        if (!$owns_group && !$is_member) {
            return [
                'success' => false,
                'status'  => 403,
                'message' => __('Você não participa deste grupo.', 'juntaplay'),
            ];
        }
        ...
        if ($is_admin) {
            $hint = $has_credentials
                ? __('Credenciais cadastradas.', 'juntaplay')
                : __('Você ainda não cadastrou as credenciais de acesso deste grupo.', 'juntaplay');
        } else {
            $hint = $has_credentials
    ...
    private function assert_group_admin(int $group_id, int $user_id): array
    {
        if ($group_id <= 0) {
            return [
                'success' => false,
        ...
        if (!is_user_logged_in() || $user_id <= 0) {
            return [
                'success' => false,
                'status'  => 403,
                'message' => __('Faça login para continuar.', 'juntaplay'),
            ];
        }
        ...
        if (!$this->is_group_admin($group_id, $user_id, $group)) {
            return [
                'success' => false,
                'status'  => 403,
                'message' => __('Você não pode cadastrar credenciais neste grupo.', 'juntaplay'),
            ];
        }
    ...
    private function is_group_admin(int $group_id, int $user_id, $group = null): bool
    {
        if ($group && isset($group->owner_id) && (int) $group->owner_id === $user_id) {
            return true;
        }
        ...
        $role = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT role FROM $table WHERE group_id = %d AND user_id = %d",
                $group_id,
                $user_id
            )
        );
     ...
     * @return int[]
     */
    private function get_group_active_member_ids(int $group_id): array
    {
        global $wpdb;

        $table = "{$wpdb->prefix}jp_group_members";
        $active = GroupMembers::STATUS_ACTIVE;
        $active_until_end = GroupMembers::STATUS_ACTIVE_UNTIL_END_OF_CYCLE;

        $rows = $wpdb->get_col(
            $wpdb->prepare(
                "SELECT user_id FROM $table WHERE group_id = %d AND status IN (%s, %s)",
                $group_id,
                $active,
                $active_until_end
            )
        ...
        $ids = array_map('absint', is_array($rows) ? $rows : []);
