...
class Auth
{
    ...
    private function complete_social_login(string $provider, array $profile, string $redirect, bool $remember, bool $popup = false, string $context = ''): void
    {
        ...
        if (!empty($profile['avatar'])) {
            ...
            $custom_avatar_url = (string) get_user_meta($user->ID, 'juntaplay_avatar_url', true);
            $custom_avatar_id  = (int) get_user_meta($user->ID, 'juntaplay_avatar_id', true);
            $avatar_source     = (string) get_user_meta($user->ID, 'juntaplay_avatar_source', true);

            $can_sync_header_avatar = ($custom_avatar_id <= 0);

            if (!$can_sync_header_avatar && $avatar_source === 'social:' . $provider) {
                // If a previous sync was marked as social, allow updates even if an attachment ID exists.
                $can_sync_header_avatar = true;
            }

            if ($can_sync_header_avatar) {
                if ($custom_avatar_url === '' || $custom_avatar_url === $avatar_url || $avatar_source === 'social:' . $provider) {
