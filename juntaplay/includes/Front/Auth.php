...
class Auth
{
    ...
    public function maybe_handle_social_login(): void
    {
        ...
        $this->start_social_flow($provider, $redirect, false, $popup);
    }
    ...
    private function start_social_flow(string $provider, string $redirect, bool $remember, bool $popup = false): void
    {
    ...
    private function handle_social_callback(string $provider): void
    {
        ...
        $this->complete_social_login($provider, $profile, $redirect, $remember, $popup);
    }

    /**
     * @param array<string, mixed> $profile
     */
    private function complete_social_login(string $provider, array $profile, string $redirect, bool $remember, bool $popup = false): void
    {
        ...
        if ($popup) {
            $this->render_social_popup_success($redirect);
        }
    ...
    private function redirect_with_error(string $message): void
    {
        if ($this->is_popup_request()) {
            $this->render_social_popup_error($message);
        }
    ...
    private function render_social_popup_success(string $redirect): void
    {
        nocache_headers();
    ...
    private function render_social_popup_error(string $message): void
    {
        nocache_headers();
        header('Content-Type: text/html; charset=utf-8');
