...
class Auth
{
    public const SOCIAL_LOG_OPTION = 'juntaplay_social_log';
    ...
    /** @var string[] */
    private array $register_error_codes = [];
    ...
    /**
     * Track the active social provider for logging purposes.
     */
    private string $current_social_provider = '';

    /**
     * Prevent duplicate shutdown guards.
     */
    private bool $social_error_guarded = false;

    private const TWO_FACTOR_TRANSIENT_PREFIX = 'juntaplay_2fa_';
    private const SOCIAL_TRANSIENT_PREFIX     = 'juntaplay_social_';
    private const SOCIAL_STATE_TTL            = MINUTE_IN_SECONDS * 10;
    private const ACCOUNT_STATUS_META         = 'juntaplay_account_status';
    private const EMAIL_VERIFIED_META         = 'juntaplay_email_verified';
    private const ACCOUNT_STATUS_PENDING      = 'pending_verification';
    ...
    public function init(): void
    {
        ...
        add_action('wp_enqueue_scripts', [$this, 'inject_register_error_codes']);
    ...
    public function maybe_handle_login(): void
    {
        ...
        if (!$this->is_account_verified($user)) {
            $challenge = $this->start_two_factor_challenge($user, $remember, $redirect, null, [
                ...
                'force_method' => $this->resolve_verification_method($user),
            ]);
    ...
    public function maybe_handle_register(): void
    {
        ...
        $cpf_raw = $this->get_register_cpf_value();
        $cpf     = $this->normalize_cpf($cpf_raw);

        // Normalize CPF to digits-only before validation and storage.
        if ($cpf === '' || strlen($cpf) !== 11) {
            $this->register_errors[] = __('Informe um CPF válido.', 'juntaplay');
        } elseif ($this->cpf_exists_for_other_user($cpf)) {
            // Backend duplication check: CPF must be unique across user_meta.
            $this->register_errors[]      = __('Este CPF já está vinculado a outra conta.', 'juntaplay');
            $this->register_error_codes[] = 'cpf_duplicado';
        }
        ...
        if (!($user instanceof WP_User)) {
            $this->register_errors[] = __('Não foi possível concluir seu cadastro. Tente novamente.', 'juntaplay');
            return;
        }

        if ($cpf !== '') {
            update_user_meta($user_id, 'juntaplay_cpf', $cpf);
        }

        $this->mark_account_pending($user);
        ...
        $challenge = $this->start_two_factor_challenge($user, true, $redirect, null, [
            'context'      => 'register',
            'verification' => 'account',
            'force_method' => $this->resolve_verification_method($user),
        ]);
        ...
        if (!$challenge) {
            $this->register_errors[] = __('Não foi possível gerar o código de verificação. Tente novamente.', 'juntaplay');
            return;
        }
     ...
     */
    private function normalize_cpf(string $cpf): string
    {
    ...
    /**
     * Retrieve CPF value from the registration request payload.
     */
    private function get_register_cpf_value(): string
    {
        $keys = [
            'jp_register_cpf',
            'jp_register_tax_document',
            'jp_register_document',
            'cpf',
            'juntaplay_cpf',
        ];
    ...
    /**
     * Check if a CPF already exists in user_meta for a different user.
     */
    private function cpf_exists_for_other_user(string $cpf, int $exclude_user_id = 0): bool
    {
        if ($cpf === '') {
            return false;
        }

        $meta_keys = ['juntaplay_cpf', 'cpf', 'juntaplay_tax_document'];
    ...
    /**
     * Send backend error codes to the frontend so it can open the CPF modal.
     */
    public function inject_register_error_codes(): void
    {
        if (empty($this->register_error_codes)) {
            return;
        }

        if (!wp_script_is('juntaplay', 'enqueued') && !wp_script_is('juntaplay', 'registered')) {
            return;
        }

        $payload = wp_json_encode(array_values(array_unique($this->register_error_codes)));
        $inline  = 'window.JuntaPlayRegisterErrors = window.JuntaPlayRegisterErrors || [];';
        $inline .= 'window.JuntaPlayRegisterErrors = window.JuntaPlayRegisterErrors.concat(' . $payload . ');';

        wp_add_inline_script('juntaplay', $inline, 'before');
    }
    ...
    private function get_two_factor_method(WP_User $user, string $fallback = ''): string
    {
        $fallback = sanitize_key($fallback);
        if ($fallback !== '' && in_array($fallback, ['email', 'whatsapp'], true)) {
            return $fallback;
        }
    ...
    private function is_two_factor_enabled(WP_User $user): bool
    {
        $method = (string) get_user_meta($user->ID, 'juntaplay_two_factor_method', true);
    ...
    private function resolve_verification_method(WP_User $user): string
    {
        $method = (string) get_user_meta($user->ID, 'juntaplay_two_factor_method', true);
    ...
    private function get_account_status(WP_User $user): string
    {
        $status = sanitize_key((string) get_user_meta($user->ID, self::ACCOUNT_STATUS_META, true));

        if ($status === '') {
            return self::ACCOUNT_STATUS_ACTIVE;
        }

        if (!in_array($status, [self::ACCOUNT_STATUS_PENDING, self::ACCOUNT_STATUS_ACTIVE], true)) {
            return self::ACCOUNT_STATUS_ACTIVE;
        }
    ...
    private function is_account_verified(WP_User $user): bool
    {
        if ($this->get_account_status($user) === self::ACCOUNT_STATUS_PENDING) {
            return false;
        }

        $verified = (string) get_user_meta($user->ID, self::EMAIL_VERIFIED_META, true);
    ...
    private function mark_account_pending(WP_User $user): void
    {
        update_user_meta($user->ID, self::ACCOUNT_STATUS_META, self::ACCOUNT_STATUS_PENDING);
        update_user_meta($user->ID, self::EMAIL_VERIFIED_META, 'no');
    }

    private function mark_account_verified(WP_User $user): void
    {
        update_user_meta($user->ID, self::ACCOUNT_STATUS_META, self::ACCOUNT_STATUS_ACTIVE);
        update_user_meta($user->ID, self::EMAIL_VERIFIED_META, 'yes');
    }
    ...
    {
        $method_override = '';
        if (isset($extra_state['force_method'])) {
            $method_override = sanitize_key((string) $extra_state['force_method']);
        }

        $method      = $this->get_two_factor_method($user, $method_override);
        $destination = $this->resolve_two_factor_destination($user, $method);

        if (!in_array($method, ['email', 'whatsapp'], true)) {
            $this->two_factor_errors[] = __('Selecione um método válido para receber o código de verificação.', 'juntaplay');
    ...
    private function build_two_factor_url(string $challenge): string
    {
        ...
        $base = ($page && $page->post_type === 'page' && $page->post_status === 'publish')
            ? (string) get_permalink($page)
            : home_url('/verificar-acesso');
    ...
    private function handle_two_factor_verify(string $challenge, string $code): void
    {
        ...
        if (!$this->is_account_verified($user)) {
            $this->mark_account_verified($user);
        }
    ...
    public function maybe_handle_social_login(): void
    {
        ...
        try {
            ...
            $this->log_social_event('critical', 'Failed to parse provider key', ['exception' => $error->getMessage()]);
        ...
        $this->current_social_provider = $provider;
        $popup = $this->is_popup_request();
        $this->current_social_popup = $popup;

        $this->register_social_error_guard();
        ...
        try {
            if (!$this->is_social_enabled($provider)) {
                $this->redirect_with_error(__('Login social indisponível no momento.', 'juntaplay'), ['provider' => $provider]);
            }

            if (isset($_GET['callback'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                $this->handle_social_callback_with_guard($provider);
                return;
            }
            ...
            $this->log_social_event('critical', $error->getMessage(), [
                'provider' => $provider,
                'stage'    => 'maybe_handle_social_login',
            ]);
            $this->redirect_with_error(__('Não foi possível iniciar o login social. Tente novamente.', 'juntaplay'));
        }
    }
    ...
    public function handle_social_ajax(): void
    {
        ...
        try {
            ...
            $this->log_social_event('critical', $error->getMessage(), ['stage' => 'handle_social_ajax']);
            $this->redirect_with_error(__('Não foi possível iniciar o login social. Tente novamente.', 'juntaplay'));
        }
    ...
    private function handle_social_callback_with_guard(string $provider): void
    {
        try {
            ...
            $this->log_social_event('critical', $error->getMessage(), [
                'provider' => $provider,
                'stage'    => 'callback_unhandled',
            ]);

            $this->redirect_with_error(__('Não foi possível concluir o login social. Tente novamente.', 'juntaplay'));
        }
    }
    ...
    private function start_social_flow(string $provider, string $redirect, bool $remember, bool $popup = false, string $context = ''): void
    {
        ...
        if (empty($settings['client_id']) || empty($settings['client_secret'])) {
            $this->redirect_with_error(
                __('Configuração do provedor ausente. Informe as credenciais no painel.', 'juntaplay'),
                ['provider' => $provider]
            );
        }
        ...
        if ($authorize === '') {
            $this->redirect_with_error(__('Não foi possível iniciar o login social.', 'juntaplay'), ['provider' => $provider]);
        }
    ...
    private function handle_social_callback(string $provider): void
    {
        ...
        $state = $this->retrieve_social_state($state_key);

        if (!is_array($state) || ($state['provider'] ?? '') !== $provider) {
            $this->redirect_with_error(__('Solicitação inválida. Tente novamente.', 'juntaplay'), ['provider' => $provider]);
        }
     ...
     */
    private function store_social_state(string $state_key, array $data): void
    {
        set_transient(self::SOCIAL_TRANSIENT_PREFIX . $state_key, $data, self::SOCIAL_STATE_TTL);

        $payload = $this->sanitize_log_context($data);

        if ($payload === []) {
            return;
        }

        $cookie_key = $this->get_social_cookie_key($state_key);
        setcookie(
            $cookie_key,
            wp_json_encode($payload),
            [
                'expires'  => time() + self::SOCIAL_STATE_TTL,
     ...
     * @return array<string, mixed>|null
     */
    private function retrieve_social_state(string $state_key): ?array
    {
        $state = get_transient(self::SOCIAL_TRANSIENT_PREFIX . $state_key);

        if (!is_array($state) || $state === []) {
            $cookie_key = $this->get_social_cookie_key($state_key);
            if (isset($_COOKIE[$cookie_key])) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                $decoded = $this->decode_json_string((string) wp_unslash($_COOKIE[$cookie_key]), 'restore_state_cookie'); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                if (is_array($decoded) && $decoded !== []) {
                    $state = $decoded;
                    $this->log_social_event(
                        'warning',
                        __('Recuperamos seu login pelo cookie, pois o estado expirou. Por favor, tente novamente se o erro persistir.', 'juntaplay'),
                        [
                            'provider' => isset($decoded['provider']) ? (string) $decoded['provider'] : '',
                            'stage'    => 'state_fallback',
                        ]
                    );
                }
            }
        ...
        $this->clear_social_state($state_key);

        return is_array($state) ? $state : null;
    }

    private function clear_social_state(string $state_key): void
    {
        delete_transient(self::SOCIAL_TRANSIENT_PREFIX . $state_key);

        $cookie_key = $this->get_social_cookie_key($state_key);
        if (isset($_COOKIE[$cookie_key])) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
            unset($_COOKIE[$cookie_key]); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
        }
    ...
    private function get_social_cookie_key(string $state_key): string
    {
        return 'jpsocial_' . preg_replace('/[^a-z0-9_-]/i', '', $state_key);
    }
    ...
    private function complete_social_login(string $provider, array $profile, string $redirect, bool $remember, bool $popup = false, string $context = ''): void
    {
        ...
        if (!empty($profile['avatar'])) {
            ...
            update_user_meta($user->ID, 'juntaplay_social_' . $provider . '_avatar', $avatar_url);

            // Define foto do Google como foto de perfil padrão se o usuário ainda não tiver uma.
            $this->sync_social_avatar($user, $provider, $avatar_url);
        }

        // Promove qualquer avatar social pré-existente se ainda não houver avatar salvo.
        $this->maybe_promote_social_avatar($user, $provider, $profile['avatar'] ?? '');
        ...
        if (!$this->is_account_verified($user)) {
            ...
            if (!$challenge) {
                $this->redirect_with_error(__('Não foi possível gerar o código de verificação. Tente novamente.', 'juntaplay'), [
                    'provider' => $provider,
                    'stage'    => 'verification',
                ]);
            }
    ...
    private function get_social_login_url(string $provider): string
    {
        ...
        return add_query_arg($args, home_url('/wp-admin/admin-ajax.php', 'https'));
    }

    private function get_social_callback_url(string $provider, bool $popup = false): string
    {
        $base = home_url('/wp-admin/admin-ajax.php', 'https');
    ...
    private function exchange_social_token(string $provider, array $settings, string $code, string $callback): ?array
    {
        ...
        if ($provider !== 'google' || $client_id === '' || $client_secret === '') {
            return null;
        }
        ...
        $data = $this->decode_json_string((string) wp_remote_retrieve_body($response), 'exchange_token');
        ...
        if (isset($data['error'])) {
            ...
            } elseif (is_array($data['error_description'] ?? null)) {
                $description = implode(' ', array_map('strval', (array) $data['error_description']));
            } elseif (is_string($data['error'])) {
                $description = (string) $data['error'];
            }

            $message = $description !== ''
                ? sprintf(__('Erro do provedor: %s', 'juntaplay'), $description)
                : __('Não foi possível validar o token de acesso.', 'juntaplay');

            $this->redirect_with_error($message, ['provider' => $provider]);
        }
    ...
    private function fetch_social_profile(string $provider, array $settings, array $token): ?array
    {
        ...
        $data = $this->decode_json_string((string) wp_remote_retrieve_body($response), 'fetch_profile');

        if ($data === null) {
            return null;
        }

        return $this->normalize_social_profile($provider, $data);
    }

    /**
     * Normalize provider profile data to a consistent shape.
     *
     * @param array<string, mixed> $data
     * @return array<string, mixed>
     */
    private function normalize_social_profile(string $provider, array $data): array
    {
        if ($provider === 'google') {
            $avatar = '';

            foreach (['picture', 'avatar', 'imageUrl', 'photoURL'] as $key) {
                if (!empty($data[$key]) && is_string($data[$key])) {
                    $avatar = (string) $data[$key];
                    break;
                }
            }

            $normalized = [
                'id'     => isset($data['sub']) ? (string) $data['sub'] : (string) ($data['id'] ?? ''),
                'email'  => isset($data['email']) ? (string) $data['email'] : '',
                'name'   => isset($data['name']) ? (string) $data['name'] : '',
                'avatar' => $avatar,
            ];

            return array_filter($normalized, static fn ($value): bool => $value !== '') + $data;
        }
    ...
    /**
     * Decode JSON safely and capture errors in the social log.
     *
     * @param string $value
     * @param string $stage
     * @return array<string, mixed>|null
     */
    private function decode_json_string(string $value, string $stage): ?array
    {
        if ($value === '') {
            return null;
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($decoded)) {
            $this->log_social_event('error', __('Resposta inválida do provedor.', 'juntaplay'), [
                'stage'      => $stage,
    ...
    /**
     * Reaproveita um avatar social já salvo quando o perfil ainda não possui foto.
     */
    private function maybe_promote_social_avatar(WP_User $user, string $provider, string $avatar_url): void
    {
        $current_avatar_id     = (int) get_user_meta($user->ID, 'juntaplay_avatar_id', true);
        $current_avatar_url    = (string) get_user_meta($user->ID, 'juntaplay_avatar_url', true);
        $current_avatar_source = (string) get_user_meta($user->ID, 'juntaplay_avatar_source', true);

        if ($current_avatar_source === 'manual' || $current_avatar_id > 0 || $current_avatar_url !== '') {
            return; // Já existe foto definida (manual ou sincronizada anteriormente).
        }

        $candidate = esc_url_raw($avatar_url);

        if ($candidate === '') {
            $candidate = esc_url_raw((string) get_user_meta($user->ID, 'juntaplay_social_' . $provider . '_avatar', true));
        }
        ...
        // Reutiliza o fluxo de sincronização para salvar localmente e marcar a origem.
        $this->sync_social_avatar($user, $provider, $candidate);
    }

    private function sync_social_avatar(WP_User $user, string $provider, string $avatar_url): void
    {
        ...
        if ($current_avatar_source === 'manual' || $current_avatar_id > 0 || $current_avatar_url !== '') {
            // Preserve qualquer avatar já salvo (manual ou previamente sincronizado).
            return;
        }
        ...
        $fallback = function (string $stage, string $detail = '') use ($user, $provider, $avatar_url): void {
            update_user_meta($user->ID, 'juntaplay_avatar_url', $avatar_url);
            update_user_meta($user->ID, 'juntaplay_avatar_source', 'social:' . $provider);

            if ($detail !== '') {
                $this->log_social_event('warning', $detail, [
                    'stage' => $stage,
                ]);
            }
        };

        $tmp_file = download_url($avatar_url, 20);

        if (is_wp_error($tmp_file)) {
            $fallback('avatar_download', $tmp_file->get_error_message());
        ...
        if ($file_size !== false && $file_size > $size_limit) {
            @unlink($tmp_file);
            $fallback('avatar_size', __('A imagem de perfil excede 5MB.', 'juntaplay'));

            return;
        }

        $file_name = basename((string) parse_url($avatar_url, PHP_URL_PATH));

        if ($file_name === '' || strpos($file_name, '.') === false) {
            $file_name = 'avatar-' . $provider . '.jpg';
        }
        ...
        $attachment_id = media_handle_sideload($file_array, 0, '', ['test_form' => false]);

        if (is_wp_error($attachment_id)) {
            @unlink($tmp_file);
            $fallback('avatar_import', $attachment_id->get_error_message());
        ...
        update_user_meta($user->ID, 'juntaplay_avatar_id', $attachment_id);

        if ($image_url) {
            update_user_meta($user->ID, 'juntaplay_avatar_url', esc_url_raw($image_url));
        } else {
            delete_user_meta($user->ID, 'juntaplay_avatar_url');
        }

        update_user_meta($user->ID, 'juntaplay_avatar_source', 'social:' . $provider);
    }

    private function redirect_with_error(string $message, array $context = []): void
    {
        $this->log_social_event('error', $message, $context);
    ...
    private function log_social_event(string $level, string $message, array $context = []): void
    {
        $context = $this->sanitize_log_context($context + [
            'provider'     => $this->current_social_provider,
            'request_uri'  => isset($_SERVER['REQUEST_URI']) ? (string) $_SERVER['REQUEST_URI'] : '',
            'user_agent'   => isset($_SERVER['HTTP_USER_AGENT']) ? (string) $_SERVER['HTTP_USER_AGENT'] : '',
            'is_popup'     => $this->current_social_popup ? '1' : '0',
            'flow_context' => $this->current_social_context,
        ]);
        ...
        $logs = get_option(self::SOCIAL_LOG_OPTION, []);
        if (!is_array($logs)) {
            $logs = [];
        }
        ...
        update_option(self::SOCIAL_LOG_OPTION, $logs, false);
    }

    private function register_social_error_guard(): void
    {
        if ($this->social_error_guarded) {
            return;
        }
        ...
        $this->social_error_guarded = true;
        ...
        register_shutdown_function(function (): void {
            ...
            $this->log_social_event('critical', 'Fatal error during social login', [
                'file'  => $last_error['file'] ?? '',
                'line'  => isset($last_error['line']) ? (string) $last_error['line'] : '',
                'stage' => 'shutdown',
            ]);

            if (!headers_sent()) {
                $this->render_social_popup_error(__('Não foi possível concluir o login social. Tente novamente.', 'juntaplay'), $this->current_social_context);
            }
        });
    }
    ...
    private function sanitize_log_context(array $context): array
    {
        ...
        foreach ($context as $key => $value) {
            ...
            if (is_scalar($value)) {
                ...
                $clean[$key] = sanitize_textarea_field(wp_json_encode($value));
            }
        }
    ...
    private function get_profile_redirect(): string
    {
        ...
        return trailingslashit($url ?: home_url('/grupos'));
    }
