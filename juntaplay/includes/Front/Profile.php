...
class Profile
{
    ...
    private const GROUP_CREATE_SUCCESS_QUERY = 'jp_group_created';
    ...
    private function build_group_hub_context(array $data): array
    {
        ...
        if ($this->group_create_success === null && isset($_GET[self::GROUP_CREATE_SUCCESS_QUERY])) {
            $flag_raw = wp_unslash((string) $_GET[self::GROUP_CREATE_SUCCESS_QUERY]);
            $flag     = sanitize_text_field($flag_raw);

            if ($flag !== '' && $flag !== '0') {
                $this->group_create_success = $this->build_group_create_success_copy();
            }
        }
    ...
    private function create_group(int $user_id): void
    {
        ...
        $success_copy = $this->build_group_create_success_copy();

        $this->group_create_success = $success_copy;
        $this->add_notice($success_copy['message']);
        $this->active_section = null;
        ...
        $redirect_url = $this->resolve_group_create_redirect_url();
    ...
    /**
     * @return array{message: string, heading: string, body: string}
     */
    private function build_group_create_success_copy(): array
    {
        $success_heading = __('Parabéns você Criou um Grupo.', 'juntaplay');
        $success_body    = __('Nossa equipe irá analizar e você será notificado.', 'juntaplay');
    ...
    private function resolve_group_create_redirect_url(): string
    {
        $request_uri = isset($_SERVER['REQUEST_URI']) ? (string) wp_unslash($_SERVER['REQUEST_URI']) : '';
        $base_url    = $request_uri !== '' ? home_url($request_uri) : $this->get_profile_base_url();
        ...
        $base_url = remove_query_arg(
            ['jp_profile_action', 'jp_profile_section', 'jp_profile_nonce', self::GROUP_CREATE_SUCCESS_QUERY],
            $base_url
        );
        ...
        return add_query_arg(self::GROUP_CREATE_SUCCESS_QUERY, '1', $base_url);
    }
