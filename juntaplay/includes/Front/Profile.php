...
class Profile
{
    ...
    /**
     * @return array{groups: array<string, mixed[]>, services: array<int, array<string, mixed>>}
     */
    public function get_meus_grupos_context(int $user_id): array
    {
        global $wpdb;
        ...
        $services     = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT q.pool_id, q.created_at AS paid_at, p.title, p.slug, p.icon_id, p.cover_id
                 FROM $quotas_table q
                 INNER JOIN $pools_table p ON p.id = q.pool_id
                 WHERE q.user_id = %d AND q.status = %s
     ...
     */
    public function merge_groups_and_services(array $groups, array $services): array
    {
        $items = [];
        $pool_ids = [];

        $group_items = array_merge(
            isset($groups['owned']) && is_array($groups['owned']) ? $groups['owned'] : [],
            isset($groups['member']) && is_array($groups['member']) ? $groups['member'] : []
        );
        ...
        foreach ($group_items as $group) {
            ...
            $group_pool_id = isset($group['pool_id']) ? (int) $group['pool_id'] : 0;
            if ($group_pool_id > 0) {
                $pool_ids[$group_pool_id] = true;
            }
        ...
        foreach ($services as $service) {
            ...
            $pool_id = isset($service['pool_id']) ? (int) $service['pool_id'] : 0;
            if ($pool_id > 0 && isset($pool_ids[$pool_id])) {
                continue;
            }

            $cover = isset($service['cover_id']) ? (int) $service['cover_id'] : 0;
            $items[] = [
                'type'       => 'service',
                'pool_id'    => $pool_id,
                'status'     => 'paid',
                ...
                'cover'      => $cover,
                'created_at' => (string) ($service['paid_at'] ?? ''),
            ];
        }

        usort($items, static function (array $left, array $right): int {
            $left_time = isset($left['created_at']) ? strtotime((string) $left['created_at']) : 0;
            $right_time = isset($right['created_at']) ? strtotime((string) $right['created_at']) : 0;
    ...
    private function cancel_group_membership(int $user_id): void
    {
        ...
        $group_id = absint($group_raw);
        if ($group_id <= 0) {
            $this->add_error('group_cancel', __('Selecione um grupo válido para agendar sua saída.', 'juntaplay'));
        ...
        if ($status === GroupMembers::STATUS_EXITED) {
            $this->add_notice(__('Sua participação neste grupo já foi encerrada.', 'juntaplay'));
        ...
        if ($reason === '' || strlen($reason) < 10) {
            $this->add_error('group_cancel_' . $group_id, __('Informe um motivo com pelo menos 10 caracteres para solicitar sua saída.', 'juntaplay'));
        ...
        if (!$schedule) {
            $this->add_error('group_cancel_' . $group_id, __('Não foi possível agendar sua saída agora. Tente novamente em instantes.', 'juntaplay'));
