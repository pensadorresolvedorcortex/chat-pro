...
class Profile
{
    ...
    private function create_group(int $user_id): void
    {
        ...
        $success_heading  = __('Grupo cadastrado com sucesso!', 'juntaplay');
        $success_body     = __('Aguarde que nossa equipe vai validar e você será notificado.', 'juntaplay');
        $success_redirect = 'https://www.agenciadigitalsaopaulo.com.br/juntaplay/grupos';
        $success_image    = '';

        if (defined('JP_URL') && JP_URL !== '') {
            $success_image = trailingslashit(JP_URL) . 'assets/image/grupo.svg';
        }

        if ($success_image === '') {
            $success_image = plugins_url('assets/image/grupo.svg', JP_FILE);
        }
