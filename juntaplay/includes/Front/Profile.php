...
class Profile
{
    ...
    public function get_sections(): array
    {
        ...
        $complaints_items = [];
        if ($this->active_section !== 'juntaplay-cancelamento') {
            $complaints_items[self::SECTION_COMPLAINTS] = [
                'label'    => __('Reclamações', 'juntaplay'),
                'type'     => 'custom',
                'editable' => false,
        ...
        $complaints_items['juntaplay-cancelamento'] = [
            'label'    => __('Cancelamento agendado', 'juntaplay'),
            'type'     => 'custom',
            'editable' => false,
            'html'     => do_shortcode('[juntaplay_cancelamento]'),
        ];
    ...
    private function cancel_group_membership(int $user_id): void
    {
        ...
        $store_cancel_payload = function (int $group_id, string $status, string $exit_at, string $reason = '') use ($user_id): void {
            $group_name = '';
            $group = Groups::get($group_id);
            if (is_array($group)) {
            ...
            if ($group_name === '') {
                $group_name = (string) get_the_title($group_id);
            }

            update_user_meta($user_id, 'juntaplay_last_cancel', [
                'group_id'   => $group_id,
                'exit_at'    => $exit_at,
        ...
        if ($status === $exit_scheduled_status) {
            ...
            $store_cancel_payload($group_id, $exit_scheduled_status, $exit_date, $reason);
            $respond_success([
                'group_id'          => $group_id,
        ...
        if ($status === $exited_status) {
            ...
            $this->add_notice($notice);
            $store_cancel_payload($group_id, $exited_status, '', $reason);
            $respond_success([
                'group_id' => $group_id,
        ...
        $store_cancel_payload($group_id, $exit_scheduled_status, $exit_effective_at, $reason);
        $respond_success([
            'group_id'          => $group_id,
            'status'            => $exit_scheduled_status,
            'exit_effective_at' => $exit_effective_at,
