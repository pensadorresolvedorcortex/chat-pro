...
class Profile
{
    ...
    /** @var array<int, array<string, mixed>> */
    private array $group_exit_events = [];
    ...
    /**
     * Resolve the user avatar using the same order as the header/profile display.
     */
    public static function resolve_avatar_url(int $user_id): string
    {
        if ($user_id <= 0) {
            return '';
        }
    ...
    public function get_sections(): array
    {
        ...
        if ($this->active_section === null && isset($_GET['section'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            if ($requested_section !== '') {
                $this->active_section = $requested_section;
            }
        }
        ...
        $messages_base_url = add_query_arg('section', 'juntaplay-chat', $this->get_profile_base_url());
        $current_user_id   = get_current_user_id();
        $owned_for_messages = array_values(array_filter($groups_owned_full, static function (array $group) use ($current_user_id): bool {
            return isset($group['owner_id']) && (int) $group['owner_id'] === $current_user_id;
        }));
        $member_for_messages = array_values(array_filter($groups_member_full, function (array $group): bool {
            return isset($group['membership_status']) && $this->is_active_membership_status((string) $group['membership_status']);
        }));

        $selected_group_id = 0;
        if (isset($_GET['group_id'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $selected_group_id = absint(wp_unslash((string) $_GET['group_id'])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        }

        $selected_group = null;
        foreach ($owned_for_messages as $group_entry) {
            if ((int) ($group_entry['id'] ?? 0) === $selected_group_id) {
                $selected_group = $group_entry;
                break;
            }
        }

        if ($selected_group === null) {
            foreach ($member_for_messages as $group_entry) {
                if ((int) ($group_entry['id'] ?? 0) === $selected_group_id) {
                    $selected_group = $group_entry;
                    break;
                }
        ...
        $is_admin_view = $selected_group && isset($selected_group['owner_id'])
            ? (int) $selected_group['owner_id'] === $current_user_id
            : false;

        if ($selected_group && $is_admin_view) {
            $active_total = GroupMembers::count_active($selected_group_id);
            $limit        = max(1, $active_total);
            $participants = GroupMembers::get_details($selected_group_id, $limit, 'active');

            $participants = array_values(array_filter($participants, static function (array $member) use ($current_user_id): bool {
                $member_id = isset($member['user_id']) ? (int) $member['user_id'] : 0;

                return $member_id > 0 && $member_id !== $current_user_id;
            }));

            if (isset($_GET['participant_id'])) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
                $participant_id = absint(wp_unslash((string) $_GET['participant_id'])); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            }

            foreach ($participants as $participant) {
                if ((int) ($participant['user_id'] ?? 0) === $participant_id) {
            ...
            if ($selected_participant) {
                $chat_id = GroupChats::ensure_chat($selected_group_id, (int) $selected_participant['user_id'], $current_user_id);
            }
        } elseif ($selected_group) {
            $chat_admin_id      = isset($selected_group['owner_id']) ? (int) $selected_group['owner_id'] : 0;
            $membership_status  = isset($selected_group['membership_status']) ? (string) $selected_group['membership_status'] : '';
            $should_create_chat = $chat_admin_id > 0 && $chat_admin_id !== $current_user_id
                && $this->is_active_membership_status($membership_status);

            if ($should_create_chat) {
                $admin_user  = get_userdata($chat_admin_id);
                $admin_title = $admin_user && isset($admin_user->display_name)
                    ? (string) $admin_user->display_name
                    : __('Administrador', 'juntaplay');

                $selected_participant = [
                    'user_id'       => $chat_admin_id,
                    'display_name'  => $admin_title,
                ];

                $chat_id = GroupChats::ensure_chat($selected_group_id, $current_user_id, $chat_admin_id);
            }
        }

        $messages_context = [
            'base_url'             => $messages_base_url,
            'groups'               => $is_admin_view ? $owned_for_messages : [],
        ...
        $sections = [
            ...
            'messages' => [
                ...
                'description' => __('Área destinada à comunicação entre Administrador e Assinantes.', 'juntaplay'),
                'items'       => [
                    'juntaplay-chat' => [
                        'label'    => __('Central de Mensagens', 'juntaplay'),
                        'type'     => 'custom',
                        'editable' => false,
                        'template' => 'profile-messages.php',
    ...
    {
        $base_price = $this->resolve_group_base_price($group);
     ...
     * @param array<string, mixed> $group
     */
    private function resolve_group_base_price(array $group): float
    {
    ...
    private function apply_member_fee(float $base_price): float
    {
        if ($base_price <= 0) {
            return 0.0;
        }

        return $base_price + Settings::get_fee_per_member();
    }
    ...
    private function prepare_group_entry(array $group, bool $is_owner): array
    {
        ...
        $member_price      = isset($group['member_price']) ? (float) $group['member_price'] : 0.0;
        $fee_per_member    = Settings::get_fee_per_member();
        $price_regular_total     = $price_regular > 0 ? $this->apply_member_fee($price_regular) : 0.0;
        $price_promotional_total = $price_promotional > 0 ? $this->apply_member_fee($price_promotional) : 0.0;
        $member_price_total      = $member_price > 0 ? $this->apply_member_fee($member_price) : 0.0;
        ...
        $complaint_url = $this->get_support_center_url(['jp_group' => $group_id]);
        ...
        $group['membership_joined_at'] = $membership_joined_at;
        $group['membership_exit_requested_at'] = $membership_exit_requested_at;
        $group['membership_exit_effective_at'] = $membership_exit_effective_at;
        $group['exit_effective_display'] = $membership_exit_effective_at !== ''
            ? date_i18n(get_option('date_format'), strtotime($membership_exit_effective_at))
            : '';

        $exit_preview_ts = $membership_joined_at !== '' ? strtotime($membership_joined_at . ' +30 days') : false;
        $group['exit_effective_preview'] = $exit_preview_ts
            ? date_i18n(get_option('date_format'), $exit_preview_ts)
            : '';
        ...
        $group['can_edit']               = $availability['can_edit'];
        ...
        $group['price_regular_display'] = $price_regular_total > 0 ? $this->format_currency($price_regular_total) : '';
        $group['price_promotional'] = $price_promotional > 0 ? $price_promotional : 0.0;
        $group['price_promotional_display'] = $price_promotional_total > 0 ? $this->format_currency($price_promotional_total) : '';
        $group['member_price']      = $member_price;
        $group['member_price_display'] = $member_price_total > 0 ? $this->format_currency($member_price_total) : '';
        ...
        $group['fee_per_member']    = $fee_per_member;
        ...
        $exit_meta = $group_id > 0 && isset($this->group_exit_events[$group_id])
            ? $this->group_exit_events[$group_id]
            : null;

        if (is_array($exit_meta)) {
            $reason_text = '';
            if (isset($exit_meta['metadata']) && is_array($exit_meta['metadata'])) {
                $reason_text = (string) ($exit_meta['metadata']['reason'] ?? '');
            }

            $group['exit_request'] = [
                'message'    => isset($exit_meta['message']) ? (string) $exit_meta['message'] : '',
                'reason'     => $reason_text,
                'created_at' => isset($exit_meta['created_at']) ? (string) $exit_meta['created_at'] : '',
            ];
        } else {
        $group['exit_request'] = null;
        }
    ...
    /**
     * @param array<string, mixed> $group
     * @param array<int, array<string, mixed>> $caucao_cycles
     * @return array<string, mixed>
     */
    private function attach_caucao_cycle(array $group, array $caucao_cycles): array
    {
        $group_id = isset($group['id']) ? (int) $group['id'] : 0;
        $cycle_end = '';

        if ($group_id > 0 && isset($caucao_cycles[$group_id]) && is_array($caucao_cycles[$group_id])) {
            $cycle_end = (string) ($caucao_cycles[$group_id]['cycle_end'] ?? '');
        }

        $group['caucao_cycle_end'] = $cycle_end;
        $group['caucao_cycle_end_display'] = $cycle_end !== ''
            ? date_i18n(get_option('date_format'), strtotime($cycle_end))
            : '';
    ...
    private function tally_group_counts(array &$counts, array $group): void
    {
        ...
        if ($status === Groups::STATUS_APPROVED && $this->is_active_membership_status($membership_status)) {
            ++$counts['approved'];
        }
    ...
    private function describe_group_status(string $status, string $membership_status, bool $is_owner): array
    {
        ...
        if (!$is_owner) {
            ...
            } elseif (!$this->is_active_membership_status($membership_status)) {
    ...
    private function build_credit_history_context(array $data): array
    {
        ...
        $context = [
            ...
            'pix_setup_url' => add_query_arg(
                [
                    'jp_category' => 'finance',
                    'jp_tab'      => 'finance_preferences',
                ],
                $this->get_profile_base_url()
            ),
        ];
    ...
    private function update_tax_document(int $user_id): void
    {
        ...
        // Normalize CPF/CNPJ to digits-only before validation.
        if ($document === '') {
        ...
        if ($length === 11) {
            $current_document = (string) get_user_meta($user_id, 'juntaplay_tax_document', true);
            $current_document = preg_replace('/\D+/', '', $current_document);

            if ($current_document !== $document && $this->cpf_exists_for_other_user($document, $user_id)) {
                // Duplicate CPF check (backend): block if another user already has this CPF.
                $this->add_error('tax_document', __('Este CPF já está vinculado a outra conta.', 'juntaplay'));
        ...
        if ($length === 11) {
            update_user_meta($user_id, 'juntaplay_cpf', $document);
        }
    ...
    /**
     * Check if a CPF already exists for a different user (digits-only value).
     */
    private function cpf_exists_for_other_user(string $cpf, int $exclude_user_id): bool
    {
        if ($cpf === '') {
            return false;
        }
    ...
    private function create_group(int $user_id): void
    {
        ...
        $plan_raw        = isset($_POST['jp_profile_group_plan_id']) ? wp_unslash($_POST['jp_profile_group_plan_id']) : '';
        ...
        $icon_id      = Groups::ensure_icon_min_size($icon_id);
        $plan_id      = sanitize_text_field(is_string($plan_raw) ? $plan_raw : '');
        ...
        $available_slots = max(1, $slots_total);
        ...
        if ($pool_id > 0) {
            ...
            if ($icon_id === 0 && isset($pool->icon_id)) {
                $pool_icon_id = Groups::ensure_icon_min_size((int) $pool->icon_id);
                if ($pool_icon_id > 0) {
                    $icon_id = $pool_icon_id;
                }
            }
        }

        $this->group_draft['icon'] = $icon_id > 0 ? (string) $icon_id : '';
        ...
        if ($success_image === '') {
            $success_image = plugins_url('assets/images/grupo.gif', JP_FILE);
        }
    ...
    private function cancel_group_membership(int $user_id): void
    {
        ...
        $resolve_group_title = static function (int $group_id): string {
            if ($group_id <= 0) {
                return '';
            }
        ...
        $store_cancel_meta = static function (int $group_id, array $data) use ($user_id, $resolve_group_title): void {
            $payload = array_merge([
                'group_id'   => $group_id,
                'group_name' => $resolve_group_title($group_id),
            ], $data);

            update_user_meta($user_id, 'jp_last_cancel', $payload);
        };

        $respond_error = function (string $message, int $group_id = 0, int $status = 400) use ($is_ajax): void {
            if ($group_id > 0) {
                $this->add_error('group_cancel_' . $group_id, $message);
            } else {
                $this->add_error('group_cancel', $message);
            }
        ...
        $respond_success = function (array $payload) use ($is_ajax): void {
            if ($is_ajax) {
        ...
        if (!wp_verify_nonce($nonce, 'jp_profile_group_cancel')) {
            $respond_error(__('Sua sessão expirou. Atualize a página e tente novamente.', 'juntaplay'));
        ...
        $group_id = absint($group_raw);
        if ($group_id <= 0) {
            $respond_error(__('Selecione um grupo válido para agendar sua saída.', 'juntaplay'));
        ...
        if (!$membership) {
            $respond_error(__('Você não participa deste grupo ou a participação já foi removida.', 'juntaplay'), $group_id, 403);
        ...
        if ($status === $exit_scheduled_status) {
            ...
            $notice = $display_date !== ''
                ? sprintf(__('Sua saída já está agendada para %s.', 'juntaplay'), $display_date)
                : __('Sua saída já está agendada.', 'juntaplay');
            $this->add_notice($notice);
            $store_cancel_meta($group_id, [
                'exit_at' => $exit_date,
        ...
        if ($role === 'owner') {
            $respond_error(__('Você é o administrador deste grupo. Use as ferramentas de gerenciamento para encerrar ou transferir a administração.', 'juntaplay'), $group_id, 403);
        ...
        if ($reason === '' || strlen($reason) < 10) {
            $respond_error(__('Informe um motivo com pelo menos 10 caracteres para solicitar sua saída.', 'juntaplay'), $group_id);
        ...
        $exit_effective_at = (string) ($schedule['exit_effective_at'] ?? '');
        $exit_display = $exit_effective_at !== '' ? date_i18n(get_option('date_format'), strtotime($exit_effective_at)) : '';
        ...
        GroupMembershipEvents::log($group_id, $user_id, GroupMembershipEvents::TYPE_EXIT_SCHEDULED, '', [
            'reason' => $reason,
            'exit_effective_at' => $exit_effective_at,
        ]);
        $this->group_exit_events[$group_id] = $event;

        $this->invalidate_cache();
        $this->invalidate_group_cache($user_id, $group_id);
        $notice = $exit_display !== ''
            ? sprintf(__('Saída agendada para %s.', 'juntaplay'), $exit_display)
            : __('Saída agendada com sucesso.', 'juntaplay');
        $this->add_notice($notice);

        $store_cancel_meta($group_id, [
            'exit_at' => $exit_effective_at,
            'status'  => $exit_scheduled_status,
            'reason'  => $reason,
        ]);
    ...
    private function cancel_group_by_admin(int $user_id): void
    {
        ...
        $role = isset($membership['role']) ? (string) $membership['role'] : 'member';
        if (!$is_owner && !in_array($role, ['owner', 'manager', 'admin', 'administrator'], true)) {
    ...
    public function handle_group_update(int $group_id, array $data, array $files = []): array
    {
        ...
        $cover_id        = Groups::ensure_icon_min_size($cover_id);
    ...
    private function invalidate_group_cache(int $user_id, int $group_id = 0): void
    {
        if ($user_id <= 0) {
            return;
        }

        wp_cache_delete(sprintf('jp_groups_for_user_%d', $user_id), 'juntaplay');
        wp_cache_delete(sprintf('jp_user_groups_%d', $user_id), 'juntaplay');

        if ($group_id > 0) {
            wp_cache_delete(sprintf('jp_group_membership_%d_%d', $group_id, $user_id), 'juntaplay');
        }
    }
