...
class Shortcodes
{
    ...
    public function cancelamento(): string
    {
        if (!is_user_logged_in()) {
            ...
            return '<p class="juntaplay-notice">' . esc_html__('Faça login para visualizar o cancelamento.', 'juntaplay')
                . ($login_url ? ' <a href="' . esc_url($login_url) . '">' . esc_html__('Entrar', 'juntaplay') . '</a>' : '')
                . '</p>';
        }

        $user_id = get_current_user_id();
        $payload = get_user_meta($user_id, 'juntaplay_last_cancel', true);
        if (is_array($payload)) {
            delete_user_meta($user_id, 'juntaplay_last_cancel');
        } else {
            $payload = [];
        }

        $group_name = isset($payload['group_name']) ? (string) $payload['group_name'] : '';
        $exit_at    = isset($payload['exit_at']) ? (string) $payload['exit_at'] : '';
        $status_raw = isset($payload['status']) ? (string) $payload['status'] : '';
        $reason     = isset($payload['reason']) ? (string) $payload['reason'] : '';

        $exit_display = $exit_at !== '' ? date_i18n(get_option('date_format'), strtotime($exit_at)) : '';
        $status_label = '';

        if ($status_raw !== '') {
            $status_label = $status_raw === 'exit_scheduled' ? 'scheduled' : $status_raw;
        }

        $my_groups_id  = (int) get_option('juntaplay_page_meus-grupos');
        $my_groups_url = $my_groups_id ? get_permalink($my_groups_id) : home_url('/meus-grupos');

        ob_start();
        ?>
        <div class="juntaplay-card juntaplay-card--highlight">
            <header class="juntaplay-card__header">
                <h3><?php echo esc_html__('Cancelamento agendado', 'juntaplay'); ?></h3>
            </header>
            <div class="juntaplay-card__body">
                <?php if (!$payload) : ?>
                    <p><?php echo esc_html__('Não encontramos um cancelamento recente para exibir agora.', 'juntaplay'); ?></p>
                <?php else : ?>
                    <p>
                        <?php
                        if ($exit_display !== '') {
                    ...
                    <p><?php echo esc_html__('Até essa data, você continuará com acesso normal ao grupo.', 'juntaplay'); ?></p>
                    <p><?php echo esc_html__('Saída agendada (não imediata).', 'juntaplay'); ?></p>
                    <?php if ($group_name !== '') : ?>
                        <p><strong><?php echo esc_html__('Grupo:', 'juntaplay'); ?></strong> <?php echo esc_html($group_name); ?></p>
                    <?php endif; ?>
                    <?php if ($reason !== '') : ?>
                        <p><strong><?php echo esc_html__('Motivo:', 'juntaplay'); ?></strong> <?php echo esc_html($reason); ?></p>
                    <?php endif; ?>
                    <?php if ($status_label !== '') : ?>
                        <p><strong><?php echo esc_html__('Status:', 'juntaplay'); ?></strong> <?php echo esc_html($status_label); ?></p>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
