...
class Shortcodes
{
    ...
    public function init(): void
    {
        ...
        add_shortcode('juntaplay_meus_grupos', [$this, 'meus_grupos']);
    }
    ...
    public function meus_grupos(): string
    {
        if (!is_user_logged_in()) {
            $login_url = wp_login_url();
        ...
        $groups_for_merge = [
            'owned'  => isset($group_context['groups_owned_full']) && is_array($group_context['groups_owned_full'])
                ? $group_context['groups_owned_full']
                : (isset($group_context['groups_owned']) && is_array($group_context['groups_owned']) ? $group_context['groups_owned'] : []),
            'member' => isset($group_context['groups_member_full']) && is_array($group_context['groups_member_full'])
                ? $group_context['groups_member_full']
                : (isset($group_context['groups_member']) && is_array($group_context['groups_member']) ? $group_context['groups_member'] : []),
        ];

        $user_id = get_current_user_id();
        $ownership = $this->profile->get_meus_grupos_context($user_id);
        if (
            empty($groups_for_merge['owned'])
            && empty($groups_for_merge['member'])
            && isset($ownership['groups'])
            && is_array($ownership['groups'])
        ) {
            $groups_for_merge = [
                'owned'  => isset($ownership['groups']['owned']) && is_array($ownership['groups']['owned'])
                    ? $ownership['groups']['owned']
                    : [],
                'member' => isset($ownership['groups']['member']) && is_array($ownership['groups']['member'])
                    ? $ownership['groups']['member']
                    : [],
            ];
        }
        $items = $this->profile->merge_groups_and_services(
            $groups_for_merge,
            $ownership['services'] ?? []
        );

        ob_start();
        $owned_count  = count($groups_for_merge['owned']);
        $member_count = count($groups_for_merge['member']);
        $total_count  = $owned_count + $member_count;
        if (!isset($context['group_context']['group_counts']) || !is_array($context['group_context']['group_counts'])) {
            $context['group_context']['group_counts'] = [];
        }
        $context['group_context']['group_counts']['owned']  = $owned_count;
        $context['group_context']['group_counts']['member'] = $member_count;
        $context['group_context']['group_counts']['total']  = $total_count;
        if (!isset($context['counts']) || !is_array($context['counts'])) {
            $context['counts'] = [];
        }
