...
class Shortcodes
{
    ...
    public function cancelamento(): string
    {
        ...
        $cancel_meta = get_user_meta($user_id, 'jp_last_cancel', true);

        $group_id = is_array($cancel_meta) ? (int) ($cancel_meta['group_id'] ?? 0) : 0;
        $status   = is_array($cancel_meta) ? (string) ($cancel_meta['status'] ?? '') : '';
        $exit_at  = is_array($cancel_meta) ? (string) ($cancel_meta['exit_at'] ?? '') : '';
        $reason   = is_array($cancel_meta) ? trim((string) ($cancel_meta['reason'] ?? '')) : '';
        $group_name = is_array($cancel_meta) ? trim((string) ($cancel_meta['group_name'] ?? '')) : '';
        ...
        if ($group_id <= 0) {
            return '<p class="juntaplay-notice">' . esc_html__('Nenhum cancelamento recente foi localizado. Volte para seus grupos e tente novamente.', 'juntaplay') . '</p>';
        }
        ...
        delete_user_meta($user_id, 'jp_last_cancel');

        ob_start();
        ?>
        <section class="juntaplay-cancelamento">
            <div class="juntaplay-cancelamento__box">
                <h2 class="juntaplay-cancelamento__title"><?php echo esc_html__('Cancelamento agendado', 'juntaplay'); ?></h2>
                ...
                <p class="juntaplay-cancelamento__text"><?php echo esc_html__('Saída agendada (não imediata).', 'juntaplay'); ?></p>
                ...
                <ul class="juntaplay-cancelamento__meta">
                    ...
                    <?php endif; ?>
                    <?php if ($reason !== '') : ?>
                        <li><strong><?php echo esc_html__('Motivo:', 'juntaplay'); ?></strong> <?php echo esc_html($reason); ?></li>
                    <?php endif; ?>
                    <?php if ($status !== '') : ?>
                        <li><strong><?php echo esc_html__('Status:', 'juntaplay'); ?></strong> <?php echo esc_html($status); ?></li>
                    <?php endif; ?>
                </ul>
