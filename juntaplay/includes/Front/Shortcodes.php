        ...
        <section class="jp-groups-slider" id="<?php echo esc_attr($slider_id); ?>">
            ...
            <div class="jp-groups-slider__track" role="list">
                <?php foreach ($items as $group) : ?>
                    ...
                    $user_count = $slots_total > 0 ? $slots_total : $members_count;
                    ...
                    ?>
                    <article class="juntaplay-service-card juntaplay-group-card--service" role="listitem" <?php echo $group_id > 0 ? 'data-group-card data-group-id="' . esc_attr((string) $group_id) . '"' : 'data-group-card'; ?>>
                        <a class="juntaplay-service-card__link" href="<?php echo esc_url($link); ?>" <?php echo wp_kses_post($data_attrs); ?>>
                            <span class="juntaplay-group-card__avatar juntaplay-service-card__icon<?php echo $cover_url !== '' ? ' has-image' : ''; ?>"<?php echo $cover_url !== '' ? ' style="background-image: url(' . esc_url($cover_url) . ')"' : ''; ?> aria-hidden="true">
                                <?php if ($cover_url === '' && $icon_initial !== '') : ?>
                                    <?php echo esc_html($icon_initial); ?>
                                <?php endif; ?>
                            </span>
                            <span class="juntaplay-service-card__title"><?php echo esc_html($title !== '' ? $title : $service); ?></span>
                            <?php if ($service !== '' && $service !== $title) : ?>
                                <span class="juntaplay-service-card__description"><?php echo esc_html($service); ?></span>
                            <?php endif; ?>
                            <?php if ($category_label !== '' || $user_count > 0 || $price_label !== '') : ?>
                                <span class="juntaplay-service-card__meta">
                                    <?php if ($category_label !== '') : ?>
                                        <span class="juntaplay-service-card__pill"><?php echo esc_html($category_label); ?></span>
                                    <?php endif; ?>
                                    <?php if ($user_count > 0) : ?>
                                        <span class="juntaplay-service-card__pill juntaplay-service-card__pill--muted">
                                            <?php echo esc_html($user_count === 1 ? __('1 usuário', 'juntaplay') : sprintf(__('%d usuários', 'juntaplay'), $user_count)); ?>
                                        </span>
                                    <?php endif; ?>
                                    <?php if ($price_label !== '') : ?>
                                        <span class="juntaplay-service-card__price"><?php echo esc_html($price_label); ?></span>
                                    <?php endif; ?>
                                </span>
                            <?php endif; ?>
                        </a>
                        <button type="button" class="juntaplay-service-card__cta-button"<?php echo wp_kses_post($data_attrs); ?>>
