...
class Shortcodes
{
    ...
    public function init(): void
    {
        ...
        add_shortcode('juntaplay-variacoes', [$this, 'pool_variations']);
        add_shortcode('juntaplay_variacoes', [$this, 'pool_variations']);
    }
    ...
    public function pool_variations($atts = []): string
    {
        $atts = shortcode_atts([
            'id' => 0,
        ], $atts, 'juntaplay-variacoes');

        $pool_id = absint($atts['id']);
        if ($pool_id <= 0) {
            $catalog = Pools::query([
                'status'   => 'publish',
                'per_page' => 100,
            ...
            ob_start();
            ?>
            <div class="juntaplay-variations">
                <?php if (!$items) : ?>
                    <p><?php esc_html_e('Nenhum serviço publicado encontrado.', 'juntaplay'); ?></p>
                <?php else : ?>
                    <ul class="juntaplay-variations__services">
                        <?php foreach ($items as $item) : ?>
                            <?php
                            $item_id = isset($item['id']) ? (int) $item['id'] : 0;
                            $item_title = (string) ($item['title'] ?? '');
                            $plans_raw = $item_id > 0 ? Pools::get_plans($item_id) : [];
                            $plans_active = [];
                            ...
                            ?>
                            <li data-pool-id="<?php echo esc_attr((string) $item_id); ?>">
                                <strong><?php echo esc_html($item_title); ?></strong>
                                <span><?php echo esc_html(sprintf(__('ID: %d', 'juntaplay'), $item_id)); ?></span>
                                <div data-pool-plans="<?php echo esc_attr(wp_json_encode($plans_active)); ?>">
                                    <?php if (empty($plans_active)) : ?>
                                        <?php esc_html_e('Nenhuma variação disponível no momento.', 'juntaplay'); ?>
                                    <?php else : ?>
                                        <?php echo esc_html(sprintf(__('%d variações encontradas', 'juntaplay'), count($plans_active))); ?>
                                    <?php endif; ?>
                                </div>
                                <div>
                                    <?php echo esc_html(sprintf(__('[juntaplay_variacoes id="%d"]', 'juntaplay'), $item_id)); ?>
                                </div>
                            </li>
                        <?php endforeach; ?>
                    </ul>
        ...
        $plans_raw = Pools::get_plans($pool_id);
        $plans_active = [];
        ...
        ob_start();
        ?>
        <div class="juntaplay-variations" data-pool-id="<?php echo esc_attr((string) $pool_id); ?>">
            <pre class="juntaplay-variations__json" data-pool-plans="<?php echo esc_attr(wp_json_encode($plans_active)); ?>">
<?php echo esc_html(wp_json_encode($plans_active, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)); ?>
            </pre>
            <?php if (empty($plans_active)) : ?>
                <p><?php esc_html_e('Nenhuma variação disponível no momento.', 'juntaplay'); ?></p>
            <?php else : ?>
                <ul class="juntaplay-variations__list">
                    <?php foreach ($plans_active as $plan) : ?>
                        <li>
                            ...
                            <span>
                                ...
                                $price_display = function_exists('wc_price')
                                    ? wc_price((float) $plan['price'])
                                    : number_format((float) $plan['price'], 2, ',', '.');
                                echo wp_kses_post($price_display);
                                ?>
                            </span>
                            <?php if (!empty($plan['max_users'])) : ?>
                                <span><?php echo esc_html(sprintf(__('%d usuários máx.', 'juntaplay'), (int) $plan['max_users'])); ?></span>
                            <?php endif; ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
