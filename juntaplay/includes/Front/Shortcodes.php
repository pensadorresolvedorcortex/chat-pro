...
class Shortcodes
{
    ...
    public function profile(): string
    {
        ...
        $chat_payload = ChatShortcode::render_chat();
        $chat_html    = is_array($chat_payload) ? (string) ($chat_payload['html'] ?? '') : (string) $chat_payload;

        $chat_section = [
            'slug'        => 'juntaplay-chat',
            'label'       => __('', 'juntaplay'),
            'title'       => __('', 'juntaplay'),
            'description' => __('', 'juntaplay'),
            'icon'        => 'message',
            'items'       => [
                'juntaplay-chat' => [
                    'label'       => __('', 'juntaplay'),
                    ...
                    'editable'    => false,
                    'html'        => $chat_html,
                    'context'     => ['chat_payload' => $chat_payload],
                ],
            ],
        ];
    ...
    public function header(): string
    {
        ...
        if ($is_guest) {
            ...
            $chat_notifications_unread = Notifications::count_unread_by_type(get_current_user_id(), 'chat');
            $notifications_unread     += max(0, $chat_unread - $chat_notifications_unread);
