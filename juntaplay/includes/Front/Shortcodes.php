...
class Shortcodes
{
    ...
    public function group_relationship(): string
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['jp_relationship_action'])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
            ...
            if ($choice === '' || !isset($relationship_options[$choice])) {
                $errors[] = __('Selecione o relacionamento com o administrador para continuar.', 'juntaplay');
            } else {
        ...
        if ($selected_choice === '' && $relationship_type !== '') {
            $selected_choice = $relationship_type;
        }
        ...
        foreach ($relationship_options as $key => $label) {
            $cards[] = [
                'key'    => (string) $key,
                'label'  => (string) $label,
                'active' => $selected_choice !== '' && $selected_choice === (string) $key,
            ];
        }

        if ($selected_choice === '' && $cards) {
            $cards[0]['active'] = true;
            if (isset($cards[0]['key'])) {
                $selected_choice = (string) $cards[0]['key'];
            }
        }
