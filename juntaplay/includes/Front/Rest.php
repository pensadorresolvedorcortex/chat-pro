...
class Rest
{
    ...
    public function get_chats(WP_REST_Request $request): WP_REST_Response
    {
        ...
        foreach ($items as &$item) {
            ...
            if ((int) ($item['admin_id'] ?? 0) <= 0 && $owner_id > 0) {
                $item['admin_id'] = $owner_id;
            }

            if (empty($item['admin_name']) && $owner) {
                $item['admin_name'] = (string) $owner->display_name;
            }
            ...
            $item['display_name_other'] = $item['role'] === 'member'
                ? ($item['admin_name'] ?? __('Administrador', 'juntaplay'))
                : ($item['member_name'] ?? __('Assinante', 'juntaplay'));
        }
    ...
    public function post_chat_message(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ($target_user > 0 && $target_user !== $user_id) {
            ...
            Notifications::add($target_user, [
                'type'    => 'chat',
                'title'   => $group['title'] ?? __('Nova mensagem', 'juntaplay'),
                'message' => __('Você recebeu uma nova mensagem no chat do grupo.', 'juntaplay'),
