...
use JuntaPlay\Support\SupportChannel;
use function __;
...
class GroupCreation
{
    ...
    public function handle_form_submission(): void
    {
        ...
        $support_type  = isset($_POST['jp_group_support_type']) ? sanitize_key(wp_unslash($_POST['jp_group_support_type'])) : '';
        $support_value = isset($_POST['jp_group_support_value']) ? sanitize_text_field(wp_unslash($_POST['jp_group_support_value'])) : '';
        $support_country = isset($_POST['jp_group_support_country']) ? sanitize_key(wp_unslash($_POST['jp_group_support_country'])) : '';

        $support = SupportChannel::normalize($support_type, $support_value, $support_country);
        $support_country_normalized = $support['country'] !== '' ? $support['country'] : ($support_country !== '' ? $support_country : 'br');
        ...
        $this->old_input = [
            ...
            'support_value'      => $support['input'] !== '' ? $support['input'] : $support_value,
            'support_country'    => $support_country_normalized,
        ];
        ...
        if ($support['errors']) {
            foreach ($support['errors'] as $error_message) {
    ...
    {
        $relationship_html = $this->render_relationship_fields(
            $this->old_input['group_relationship'] ?? '',
            'jp_group_relationship',
            'jp_group_relationship'
        );
        $current_url       = $this->get_current_url();
        $support_type      = isset($this->old_input['support_type']) ? (string) $this->old_input['support_type'] : '';
        if ($support_type === '') {
            $support_type = 'whatsapp';
        }
        $support_value     = isset($this->old_input['support_value']) ? (string) $this->old_input['support_value'] : '';
        $support_country   = isset($this->old_input['support_country']) ? (string) $this->old_input['support_country'] : 'br';
        $country_options   = SupportChannel::get_country_options();
        if (!isset($country_options[$support_country])) {
            $support_country = 'br';
        }
        ...
        <div class="jp-group-create">
            ...
            <form method="post" class="jp-group-form" novalidate>
                ...
                <fieldset class="jp-field jp-fieldset" data-support-channel>
                    <legend class="jp-fieldset__legend"><?php esc_html_e('Suporte a Membros', 'juntaplay'); ?></legend>
                    <p class="jp-fieldset__description"><?php esc_html_e('Informe como os participantes entrarão em contato para tirar dúvidas. Apenas membros confirmados verão esses dados.', 'juntaplay'); ?></p>

                    <div class="jp-support-channel">
                        <div class="jp-support-channel__type">
                            <label class="jp-label" for="jp_group_support_type"><?php esc_html_e('Canal de suporte', 'juntaplay'); ?></label>
                            <select id="jp_group_support_type" name="jp_group_support_type" class="jp-input" data-support-type required>
                                <option value="" <?php selected($support_type, ''); ?>><?php esc_html_e('Selecione uma opção', 'juntaplay'); ?></option>
                                <option value="whatsapp" <?php selected($support_type, 'whatsapp'); ?>><?php esc_html_e('WhatsApp', 'juntaplay'); ?></option>
                                <option value="email" <?php selected($support_type, 'email'); ?>><?php esc_html_e('E-mail', 'juntaplay'); ?></option>
                            </select>
                        </div>

                        <div class="jp-support-channel__details" data-support-field="whatsapp" <?php echo $support_type !== 'whatsapp' ? 'hidden' : ''; ?>>
                            <div class="jp-support-channel__row">
                                <div class="jp-support-channel__country">
                                    <label class="jp-label" for="jp_group_support_country"><?php esc_html_e('País do número', 'juntaplay'); ?></label>
                                    <select id="jp_group_support_country" name="jp_group_support_country" class="jp-input" data-support-country <?php echo $support_type === 'whatsapp' ? '' : 'disabled'; ?>>
                                        <?php foreach ($country_options as $country_key => $country_meta) :
                                            $country_label = isset($country_meta['label']) ? (string) $country_meta['label'] : strtoupper($country_key);
                                            ...
                                            ?>
                                            <option value="<?php echo esc_attr($country_key); ?>" data-code="<?php echo esc_attr($country_code); ?>" data-mask="<?php echo esc_attr($country_mask); ?>" <?php selected($support_country, (string) $country_key); ?>>
                                                <?php echo esc_html(trim($country_flag . ' ' . $country_label . ' (' . $country_code . ')')); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <div class="jp-support-channel__phone">
                                    <label class="jp-label" for="jp_group_support_phone"><?php esc_html_e('Número do WhatsApp', 'juntaplay'); ?></label>
                                    <div class="jp-support-phone">
                                        <span class="jp-support-phone__code" data-support-code-display aria-hidden="true"></span>
                                        <input type="tel" id="jp_group_support_phone" name="jp_group_support_value" class="jp-input" data-support-phone-input value="<?php echo esc_attr($support_type === 'whatsapp' ? $support_value : ''); ?>" inputmode="tel" autocomplete="tel" <?php echo $support_type === 'whatsapp' ? 'required' : 'disabled'; ?>>
                                    </div>
                                </div>
                            </div>
                            <p class="jp-support-channel__hint"><?php esc_html_e('Inclua o DDI e o DDD para que possamos validar o contato.', 'juntaplay'); ?></p>
                        </div>

                        <div class="jp-support-channel__details" data-support-field="email" <?php echo $support_type === 'email' ? '' : 'hidden'; ?>>
                            <label class="jp-label" for="jp_group_support_email"><?php esc_html_e('E-mail de suporte', 'juntaplay'); ?></label>
                            <input type="email" id="jp_group_support_email" name="jp_group_support_value" class="jp-input" data-support-email-input value="<?php echo esc_attr($support_type === 'email' ? $support_value : ''); ?>" autocomplete="email" <?php echo $support_type === 'email' ? 'required' : 'disabled'; ?>>
                        </div>
                    </div>
                </fieldset>
    ...
    public function render_relationship_shortcode($atts = []): string
    {
        ...
        if (is_array($atts)) {
            ...
            if (isset($atts['name'])) {
                $maybe_name = sanitize_key((string) $atts['name']);
                if ($maybe_name !== '') {
                    $name = $maybe_name;
                }
            }
            ...
            if (isset($atts['id'])) {
                $maybe_id = sanitize_html_class((string) $atts['id']);
                if ($maybe_id !== '') {
                    $id = $maybe_id;
                }
            }
        ...
        return $this->render_relationship_fields($selected, $name, $id);
    }
    ...
    public function handle_two_factor_success(WP_User $user, array $state): void
    {
        ...
        $post_id = wp_insert_post([
            ...
            'meta_input'  => [
                ...
                '_jp_group_support'      => $payload['support_channel'],
                '_jp_group_support_type' => $payload['support_type'],
                '_jp_group_support_value'=> $payload['support_value'],
                '_jp_group_support_country' => $payload['support_country'],
    ...
    private function render_relationship_fields(string $selected, string $input_name = 'jp_group_relationship', string $input_id = ''): string
    {
        $options   = $this->get_relationship_options();
        $input_id  = $input_id !== '' ? $input_id : $input_name;

        ob_start();
        ?>
        <select name="<?php echo esc_attr($input_name); ?>" id="<?php echo esc_attr($input_id); ?>" class="jp-input jp-select" required>
            <option value="" <?php selected($selected, ''); ?>><?php esc_html_e('Selecione uma opção', 'juntaplay'); ?></option>
    ...
    {
        $options = GroupsData::get_relationship_options();
    ...
    private function send_confirmation_email(WP_User $user, array $payload): void
    {
        ...
        if ($payload['support_channel'] !== '') {
            $list_items[] = sprintf(__('Suporte aos membros: %s', 'juntaplay'), $payload['support_channel']);
        }
