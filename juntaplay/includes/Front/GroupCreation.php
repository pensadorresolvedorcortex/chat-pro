    ...
    public function render_groups_slider(): string
    {
        ...
        <div class="jp-groups-slider" id="<?php echo esc_attr($slider_id); ?>">
            <div class="jp-groups-slider__header">
                <h3 class="jp-groups-slider__title"><?php esc_html_e('Grupos em destaque', 'juntaplay'); ?></h3>
            ...
            <div class="jp-groups-slider__track" role="list">
                <?php foreach ($group_ids as $group_id) : ?>
                    ...
                    $price = (string) get_post_meta($group_id, '_jp_group_price', true);
                    if ($price === '') {
                        $price = (string) get_post_meta($group_id, '_jp_service_price', true);
                    }
                    $owner_id = (int) get_post_meta($group_id, '_jp_group_submitted_by', true);
                    $owner_avatar = $owner_id > 0 ? get_avatar_url($owner_id, ['size' => 48]) : '';
                    ?>
                    <article class="jp-groups-slider__card" role="listitem">
                        <div class="jp-groups-slider__card-body">
                            <h4 class="jp-groups-slider__card-title"><?php echo esc_html($title); ?></h4>
                            <?php if ($price !== '') : ?>
                                <p class="jp-groups-slider__price">
                            ...
                            <div class="jp-groups-slider__avatars" aria-hidden="true">
                                <?php if ($owner_avatar !== '') : ?>
                                    <img class="jp-groups-slider__avatar" src="<?php echo esc_url($owner_avatar); ?>" alt="" />
                                <?php else : ?>
                                    <span class="jp-groups-slider__avatar jp-groups-slider__avatar--placeholder"></span>
                                <?php endif; ?>
                                <?php for ($i = 0; $i < 3; $i++) : ?>
                                    <span class="jp-groups-slider__avatar jp-groups-slider__avatar--placeholder"></span>
                                <?php endfor; ?>
                        ...
                        <?php if ($link) : ?>
                            <a class="jp-groups-slider__cta" href="<?php echo esc_url($link); ?>">
                                <?php esc_html_e('Ver grupo', 'juntaplay'); ?>
                            </a>
                        <?php endif; ?>
                    </article>
                <?php endforeach; ?>
