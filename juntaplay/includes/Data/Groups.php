    ...
    public static function get_groups_for_user(int $user_id): array
    {
        ...
        $query = $wpdb->prepare(
            "SELECT g.*, gm.role AS membership_role, gm.status AS membership_status,
             ...
             WHERE g.owner_id = %d OR (gm.user_id = %d AND gm.status != 'exited')
        ...
        $quota_statuses = ['paid'];
        $approved_statuses = [self::STATUS_APPROVED, 'publish', 'published', ''];
        $extra_results = [];

        if ($quota_statuses) {
            $quota_placeholders = implode(',', array_fill(0, count($quota_statuses), '%s'));
            $approved_placeholders = implode(',', array_fill(0, count($approved_statuses), '%s'));

            $extra_query = $wpdb->prepare(
                "SELECT DISTINCT g.*, %s AS membership_role, %s AS membership_status,
                        q.status AS quota_status,
                        IFNULL(p.title, '') AS pool_title, IFNULL(p.slug, '') AS pool_slug,
                        (SELECT COUNT(*) FROM $members_table m WHERE m.group_id = g.id AND m.status IN ('active', 'exit_scheduled')) AS members_count
                 FROM $quotas_table q
                 INNER JOIN $groups_table g ON g.pool_id = q.pool_id
                 LEFT JOIN $pools_table p ON p.id = g.pool_id
                 WHERE q.user_id = %d
                   AND q.status IN ($quota_placeholders)
                   AND ((g.status IN ($approved_placeholders)) OR g.status IS NULL)
                 ORDER BY g.created_at DESC",
                ...array_merge(['member', 'active', $user_id], $quota_statuses, $approved_statuses)
            );

            $extra_results = $wpdb->get_results($extra_query, ARRAY_A) ?: [];
        }
        ...
        if ($order_statuses) {
            ...
            if ($orders_table_exists && $order_items_table_exists && $order_itemmeta_table_exists && $order_meta_table_exists) {
                ...
                $order_group_ids = $wpdb->get_col($order_query) ?: [];
                $order_group_ids = array_values(array_filter(array_map('absint', $order_group_ids)));
            }
        }

        if ($order_group_ids) {
            $order_group_placeholders = implode(',', array_fill(0, count($order_group_ids), '%d'));
            $order_groups_query = $wpdb->prepare(
                "SELECT g.*, %s AS membership_role, %s AS membership_status,
                        '' AS membership_joined_at, '' AS membership_exit_requested_at, '' AS membership_exit_effective_at,
                        IFNULL(p.title, '') AS pool_title, IFNULL(p.slug, '') AS pool_slug,
                        (SELECT COUNT(*) FROM $members_table m WHERE m.group_id = g.id AND m.status IN ('active', 'exit_scheduled')) AS members_count
                 FROM $groups_table g
                 LEFT JOIN $pools_table p ON p.id = g.pool_id
                 WHERE g.id IN ($order_group_placeholders)
                 ORDER BY g.created_at DESC",
                ...array_merge(['member', 'active'], $order_group_ids)
            );

            $order_results = $wpdb->get_results($order_groups_query, ARRAY_A) ?: [];
            if ($order_results) {
                $extra_results = array_merge($extra_results, $order_results);
            }
        }
        ...
        $build_group = static function (array $row) use ($user_id): array {
