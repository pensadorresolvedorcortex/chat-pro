...
class CaucaoCycles
{
    ...
    public const STATUS_LIBERACAO_PROGRAMADA_CANCELAMENTO_GRUPO = 'liberacao_programada_por_cancelamento_de_grupo';
    ...
    public static function due_for_validation(int $limit = 50): array
    {
        ...
        $query = $wpdb->prepare(
            "SELECT * FROM $table WHERE status IN (%s, %s) AND cycle_end <= %s ORDER BY cycle_end ASC LIMIT %d",
            self::STATUS_RETIDO,
            self::STATUS_LIBERACAO_PROGRAMADA_CANCELAMENTO_GRUPO,
            current_time('mysql'),
            $limit
        );
     ...
     * @return array<int, array<string, mixed>>
     */
    public static function get_latest_for_group(int $group_id): array
    {
        global $wpdb;
        ...
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM $table WHERE group_id = %d ORDER BY cycle_end DESC, id DESC",
                $group_id
            ),
            ARRAY_A
        ) ?: [];
        ...
        foreach ($rows as $row) {
            ...
            if ($user_id <= 0 || isset($latest[$user_id])) {
                continue;
            }

            $latest[$user_id] = self::format_row($row);
        }

        return $latest;
    }

    public static function schedule_release_for_group_cancellation(int $cycle_id, string $cycle_end, string $note = ''): bool
    {
        global $wpdb;

        $cycle_id = absint($cycle_id);
        $cycle_end = sanitize_text_field($cycle_end);

        if ($cycle_id <= 0 || $cycle_end === '') {
            return false;
        }
        ...
        $updated = $wpdb->update(
            $table,
            [
                'status'    => self::STATUS_LIBERACAO_PROGRAMADA_CANCELAMENTO_GRUPO,
                'cycle_end' => $cycle_end,
                'note'      => $note !== '' ? $note : null,
            ],
            ['id' => $cycle_id],
