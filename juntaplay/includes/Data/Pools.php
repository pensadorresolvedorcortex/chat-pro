...
class Pools
{
    ...
    public static function create_or_update(array $data): int
    {
        ...
        if ($pool_id > 0) {
            $existing = $pool_id;
            if ($slug === '') {
                $slug = (string) $wpdb->get_var($wpdb->prepare("SELECT slug FROM $table WHERE id = %d", $pool_id));
            }
        } else {
            $existing = $slug !== ''
        ...
        $status_raw = isset($data['status']) ? sanitize_key((string) $data['status']) : 'publish';
        $payload = [
            ...
            'category'     => $category,
            'excerpt'      => isset($data['excerpt']) ? sanitize_textarea_field((string) $data['excerpt']) : null,
            ...
            'status'       => self::normalize_pool_status($status_raw),
        ];
        ...
        if ($existing) {
            $updated = $wpdb->update($table, $payload, ['id' => (int) $existing]);

            $pool_id = $updated === false ? 0 : (int) $existing;

            if ($pool_id > 0) {
                self::sync_product_data($pool_id, $payload);
            }
        ...
        $pool_id  = $inserted === false ? 0 : (int) $wpdb->insert_id;
     ...
     */
    public static function get_featured_pools(int $limit = 6): array
    {
        $query = self::query([
            'status'   => 'publish',
            'featured' => 1,
        ...
        return isset($query['items']) && is_array($query['items']) ? $query['items'] : [];
    }

    /**
     * @return array<int, array<string, mixed>>
     */
    public static function get_all_active_pools(): array
    {
        global $wpdb;
        ...
        return array_map([self::class, 'map_row'], $rows ?: []);
    }
    ...
    private static function map_row(array $row): array
    {
        ...
        if ($thumbnail_id > 0) {
            ...
            if (is_array($attachment) && !empty($attachment['url'])) {
                ...
                $thumbnail = wp_get_attachment_url($thumbnail_id) ?: null;
            }
        }
        ...
        if ($icon_id > 0) {
            ...
            if (is_array($icon_attachment) && !empty($icon_attachment['url'])) {
                ...
                $icon = wp_get_attachment_url($icon_id) ?: null;
            }
        }
        ...
        if ($cover_id > 0) {
            ...
            if (is_array($cover_attachment) && !empty($cover_attachment['url'])) {
                ...
                $cover = wp_get_attachment_url($cover_id) ?: null;
            }
        }
     ...
     */
    public static function sync_product_data(int $pool_id, array $payload): void
    {
        if (!class_exists('WC_Product_Simple')) {
            return;
        }
        ...
        if ($product_id > 0 && get_post_status($product_id)) {
            ...
            if ($product instanceof WC_Product_Simple) {
                if (!empty($payload['title'])) {
                    $product->set_name((string) $payload['title']);
                }
                ...
                update_post_meta($product_id, '_juntaplay_pool_id', $pool_id);

                return;
            }
        }

        self::ensure_product($pool_id);
    }
    ...
    public static function delete(int $pool_id): bool
    {
        ...
        if ($deleted !== false) {
            delete_option(self::plans_option_name($pool_id));
        }
     ...
     * @return array<int, array<string, mixed>>
     */
    public static function get_plans(int $pool_id): array
    {
        $pool_id = absint($pool_id);
        if ($pool_id <= 0) {
            return [];
        ...
        $plans = get_option(self::plans_option_name($pool_id), []);
        ...
        if (is_array($plans) && isset($plans['plans']) && is_array($plans['plans'])) {
            $plans = $plans['plans'];
        }

        $normalized = self::normalize_plans($plans);
        if (!$normalized || !is_array($normalized)) {
            return [];
        }

        $active = array_filter($normalized, static function (array $plan): bool {
            return self::is_plan_active($plan['status'] ?? null);
        });
        ...
        foreach ($active as $index => $plan) {
            $normalized[] = [
                ...
                'max_members' => (int) ($plan['max_members'] ?? 0),
     ...
     */
    public static function update_plans(int $pool_id, array $plans): void
    {
        $pool_id = absint($pool_id);
        if ($pool_id <= 0) {
            return;
        ...
        update_option(self::plans_option_name($pool_id), self::normalize_plans($plans), false);
    }

    private static function plans_option_name(int $pool_id): string
    {
        return sprintf('juntaplay_pool_plans_%d', $pool_id);
    }

    public static function get_type(int $pool_id): string
    {
        $pool_id = absint($pool_id);
        if ($pool_id <= 0) {
            return 'simple';
        ...
        $type = get_option(self::type_option_name($pool_id), '');
        ...
        return self::get_plans($pool_id) ? 'variable' : 'simple';
    }

    public static function update_type(int $pool_id, string $type): void
    {
        $pool_id = absint($pool_id);
        if ($pool_id <= 0) {
            return;
        ...
        update_option(self::type_option_name($pool_id), $type, false);
    }

    private static function type_option_name(int $pool_id): string
    {
        return sprintf('juntaplay_pool_type_%d', $pool_id);
    }
     ...
     * @return array<int, array<string, mixed>>
     */
    private static function normalize_plans($plans): array
    {
        if (!is_array($plans)) {
            return [];
        }
        ...
        foreach ($plans as $index => $plan) {
            ...
            if ($name === '' && isset($plan['title'])) {
                $name = sanitize_text_field((string) $plan['title']);
            }
            $description = isset($plan['description']) ? sanitize_textarea_field((string) $plan['description']) : '';
            $price = isset($plan['price']) ? (float) $plan['price'] : 0.0;

            $max_members = 0;
            if (isset($plan['max_members'])) {
                $max_members = absint($plan['max_members']);
            } elseif (isset($plan['max_users'])) {
                $max_members = absint($plan['max_users']);
            }

            $order = isset($plan['order']) ? (int) $plan['order'] : 0;
            $status_raw = $plan['status'] ?? null;
            $status = $status_raw === null ? 'active' : self::normalize_status($status_raw);

            if ($name === '' && $description === '' && $price === 0.0 && $max_members === 0) {
                continue;
            }
    ...
    private static function normalize_status($status): string
    {
        ...
        $status_text = sanitize_key((string) $status);
        $active_statuses = ['active', 'ativo', 'published', 'publicado', '1', 'true'];

        return in_array($status_text, $active_statuses, true) ? 'active' : 'inactive';
    }

    private static function normalize_pool_status(string $status): string
    {
        $status = $status !== '' ? $status : 'publish';
        return match ($status) {
            'active', 'published', 'publicado' => 'publish',
            'inactive' => 'draft',
            default => $status,
        };
    }

    private static function is_plan_active($status): bool
    {
        if ($status === null) {
            return true;
        }

        return self::normalize_status($status) === 'active';
    }
}
