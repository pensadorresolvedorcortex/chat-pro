    ...
    private static function normalize_plans($plans): array
    {
        ...
        foreach ($plans as $index => $plan) {
            ...
            if (is_bool($status_raw)) {
                $status = $status_raw ? 'active' : 'inactive';
            } elseif (is_numeric($status_raw)) {
                $status = ((int) $status_raw) === 1 ? 'active' : 'inactive';
            } else {
                $status_text = sanitize_key((string) $status_raw);
                $status = in_array($status_text, ['inactive', 'inativo', 'draft', 'disabled'], true)
                    ? 'inactive'
                    : 'active';
            }
