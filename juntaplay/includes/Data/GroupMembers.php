...
class GroupMembers
{
    ...
    public const STATUS_ACTIVE_UNTIL_END_OF_CYCLE = 'active_until_end_of_cycle';
    public const STATUS_EXITED_BY_GROUP_CANCELLATION = 'exited_by_group_cancellation';
    ...
    public static function count_active(int $group_id): int
    {
        ...
        return (int) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM $table WHERE group_id = %d AND status IN (%s, %s)",
                $group_id,
                self::STATUS_ACTIVE,
                self::STATUS_ACTIVE_UNTIL_END_OF_CYCLE
            )
        );
    }
    ...
    public static function get_membership(int $group_id, int $user_id): ?array
    {
        ...
        if (self::table_has_column($table, 'exit_requested_at')) {
            $membership['exit_requested_at'] = (string) ($row['exit_requested_at'] ?? '');
        }

        if (self::table_has_column($table, 'exit_effective_at')) {
            $membership['exit_effective_at'] = (string) ($row['exit_effective_at'] ?? '');
        }
    ...
    public static function update_status(int $group_id, int $user_id, string $status): bool
    {
        ...
        if (in_array($status, ['canceled', 'exited'], true)) {
            $deleted = $wpdb->delete(
                $table,
                [
                    'group_id' => $group_id,
    ...
    /**
     * Schedule member exit with a grace period (default 15 days).
     *
    ...
    public static function schedule_exit(int $group_id, int $user_id): ?array
    {
        ...
        $grace_days = (int) apply_filters('juntaplay/group_members/exit_grace_days', 15, $group_id, $user_id);
        if ($grace_days < 1) {
            $grace_days = 15;
        }

        $requested_at = current_time('mysql');
        $effective_at = date('Y-m-d H:i:s', strtotime(sprintf('+%d days', $grace_days), current_time('timestamp')));
        ...
        if (self::table_has_column($table, 'exit_requested_at')) {
            $columns['exit_requested_at'] = $requested_at;
            $formats[] = '%s';
        }

        $columns['exit_effective_at'] = $effective_at;
        $formats[] = '%s';
        ...
        if ($updated) {
            ...
            return [
                'exit_requested_at' => $requested_at,
    ...
    /**
     * Process scheduled member exits whose grace period has elapsed.
     */
    public static function process_scheduled_exits(): void
    {
        global $wpdb;
        ...
        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT group_id, user_id, exit_effective_at
                 FROM $table
                 WHERE status = %s
                   AND exit_effective_at IS NOT NULL
                   AND exit_effective_at <= %s",
        ...
        foreach ($rows as $row) {
            ...
            $updated = self::update_status($group_id, $user_id, self::STATUS_EXITED);
            if ($updated) {
    ...
    /**
     * Schedule exit for admin-initiated group cancellation.
     */
    public static function schedule_exit_for_group_cancellation(int $group_id, int $user_id, string $exit_effective_at): bool
    {
        global $wpdb;

        $group_id = absint($group_id);
        $user_id  = absint($user_id);
        $exit_effective_at = sanitize_text_field($exit_effective_at);

        if ($group_id <= 0 || $user_id <= 0 || $exit_effective_at === '') {
            return false;
        }

        $table = "{$wpdb->prefix}jp_group_members";

        $has_exit_requested = self::table_has_column($table, 'exit_requested_at');
        $has_exit_effective = self::table_has_column($table, 'exit_effective_at');

        $columns = [
            'status' => self::STATUS_ACTIVE_UNTIL_END_OF_CYCLE,
        ];
        $formats = ['%s'];

        if ($has_exit_requested) {
            $columns['exit_requested_at'] = current_time('mysql');
            $formats[] = '%s';
        }

        if ($has_exit_effective) {
            $columns['exit_effective_at'] = $exit_effective_at;
            $formats[] = '%s';
        }
    ...
    private static function table_has_column(string $table, string $column): bool
    {
        ...
        $results = $wpdb->get_results("SHOW COLUMNS FROM `$table` LIKE '$column'", ARRAY_A);
