...
class GroupMembers
{
    ...
    /**
     * Finalize scheduled exits and admin group cancellations at the end of the paid cycle.
     */
    public static function process_scheduled_exits(): void
    {
        ...
        $exit_scheduled = self::STATUS_EXIT_SCHEDULED;
        $active_until_end = self::STATUS_ACTIVE_UNTIL_END_OF_CYCLE;

        $rows = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT group_id, user_id, status, exit_effective_at
                 FROM $table
                 WHERE status IN (%s, %s)
        ...
        foreach ($rows as $row) {
            $group_id = isset($row['group_id']) ? absint($row['group_id']) : 0;
            ...
            $next_status = $status === $active_until_end
                ? self::STATUS_EXITED_BY_GROUP_CANCELLATION
                : self::STATUS_EXITED;
            ...
            GroupMembershipEvents::log(
                ...
                [
                    ...
                    'exit_reason'     => $status === $active_until_end ? 'group_cancellation' : 'member_cancellation',
                ]
            );
        }
    }
