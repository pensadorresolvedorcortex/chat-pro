...
class ChatShortcode
{
    ...
    {
        add_shortcode('juntaplay_chat', [self::class, 'render_chat_shortcode']);
    ...
    public static function render_chat_shortcode(): string
    {
        $chat_payload = self::render_chat();

        return is_array($chat_payload) ? (string) ($chat_payload['html'] ?? '') : (string) $chat_payload;
    }

    /**
     * @return array<string,mixed>
     */
    public static function render_chat(): array
    {
        if (!is_user_logged_in()) {
            return [
                'state'    => 'unauthenticated',
                'html'     => '<p>É necessário estar logado para acessar suas mensagens.</p>',
                'group_id' => 0,
                'members'  => [],
            ];
        }
        ...
        $members        = $chat_context['admin_conversations'] ?? [];
        ...
        if ($mode === 'admin' && $status === 'admin_empty') {
            return [
                'state'    => 'admin_empty',
                'html'     => self::render_admin_empty_state(),
                'group_id' => $group_id,
                'members'  => $members,
            ];
        }
        ...
        if ($status === 'unavailable') {
            return [
                'state'    => 'unavailable',
                'html'     => self::render_unavailable_state(),
                'group_id' => $group_id,
                'members'  => $members,
            ];
        }
        ...
        if ($status === 'selection') {
            return [
                'state'    => 'selection',
                'html'     => self::render_group_selector($chat_context['eligible_groups']),
                'group_id' => $group_id,
                'members'  => $members,
            ];
        }

        if ($mode === 'admin' && $state === 'admin_grid') {
            if ($group_id <= 0) {
                foreach ($members as $conversation) {
                    if (isset($conversation['group_id']) && (int) $conversation['group_id'] > 0) {
            ...
            if ($group_id > 0 && $participant_id <= 0 && count($members) >= 2) {
                $members = array_values(array_map(
                    static function (array $conversation) use ($group_id): array {
                        $user_id = isset($conversation['user_id']) ? (int) $conversation['user_id'] : (isset($conversation['id']) ? (int) $conversation['id'] : 0);

                        return [
                            'id'       => $user_id,
                            'name'     => (string) ($conversation['user_name'] ?? ($conversation['name'] ?? __('Participante', 'juntaplay'))),
                            'avatar'   => (string) ($conversation['user_avatar'] ?? ($conversation['avatar'] ?? self::resolve_avatar($user_id))),
                            'group'    => (string) ($conversation['group_title'] ?? ($conversation['group'] ?? __('Grupo', 'juntaplay'))),
                            'group_id' => isset($conversation['group_id']) ? (int) $conversation['group_id'] : $group_id,
                        ];
                    },
                    $members
                ));

                if ($group_id <= 0 && isset($members[0]['group_id'])) {
                    $group_id = (int) $members[0]['group_id'];
                }

                if ($group_id <= 0 || count($members) === 0) {
                    return [
                        'state'    => 'admin_empty',
                        'html'     => self::render_admin_empty_state(),
                        'group_id' => $group_id,
        ...
        if (($mode === 'admin' && ($group_id <= 0 || $participant_id <= 0)) || ($mode !== 'admin' && $group_id <= 0)) {
            $fallback_state = $mode === 'admin' ? 'admin_empty' : 'unavailable';

            return [
                'state'    => $fallback_state,
                'html'     => $mode === 'admin' ? self::render_admin_empty_state() : self::render_unavailable_state(),
                'group_id' => $group_id,
                'members'  => $members,
            ];
        }
        ...
        $output_html = (string) ob_get_clean();
    ...
    public static function render_account_endpoint(): void
    {
        ...
        if (is_array($chat_payload)) {
            $state    = isset($chat_payload['state']) ? (string) $chat_payload['state'] : '';
            $group_id = isset($chat_payload['group_id']) ? (int) $chat_payload['group_id'] : 0;
            $chat_html = isset($chat_payload['html']) ? (string) $chat_payload['html'] : '';
            ...
            echo $chat_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            return;
        }

        echo (string) $chat_payload; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
    ...
    private static function resolve_admin_state(int $admin_id, array $owned_groups, int $group_id, int $participant_id): ?array
    {
        ...
        $conversations = self::get_admin_conversations($admin_id, $owned_groups);

        $is_group_entry = $group_id > 0 && $participant_id <= 0 && isset($owned_group_map[$group_id]);
        $scoped_conversations = $is_group_entry
        ...
        if ($is_group_entry) {
            $members = array_values(array_filter(
                ...
                static function (array $member) use ($admin_id): bool {
                    $user_id = isset($member['id']) ? (int) $member['id'] : (isset($member['user_id']) ? (int) $member['user_id'] : 0);
                    return $user_id > 0 && $user_id !== $admin_id;
                }
            ));
            ...
            $conversation_map = [];
            foreach ($scoped_conversations as $conversation) {
                if (isset($conversation['user_id'])) {
                    $conversation_map[(int) $conversation['user_id']] = $conversation;
                }
            }
            ...
            foreach ($members as $member) {
                ...
                if ($user_id <= 0 || isset($seen_user_ids[$user_id])) {
                    continue;
                }
                ...
                $entry     = $conversation_map[$user_id] ?? [
                    'group_id'        => $group_id,
                ...
                $entry['group_title'] = (string) ($entry['group_title'] ?? ($group_meta['title'] ?? ''));
                $entry['group_cover'] = (string) ($entry['group_cover'] ?? ($group_meta['cover_url'] ?? ''));
                $entry['user_name']   = (string) ($entry['user_name'] ?? $user_name);
                $entry['user_avatar'] = (string) ($entry['user_avatar'] ?? self::resolve_avatar($user_id));

                $admin_grid[] = $entry;
            }

            if (count($admin_grid) === 1) {
                $solo = $admin_grid[0];
                $selected_group = self::map_owned_group(
                    $owned_groups,
                    isset($solo['group_id']) ? (int) $solo['group_id'] : 0,
                    $admin_id,
                    (string) ($solo['group_title'] ?? ''),
                    (string) ($solo['group_cover'] ?? '')
                );
                ...
                return [
                    ...
                    'participant'         => [
                        'id'     => isset($solo['user_id']) ? (int) $solo['user_id'] : 0,
                        'name'   => (string) ($solo['user_name'] ?? ''),
                        'avatar' => (string) ($solo['user_avatar'] ?? ''),
                    ],
    ...
    private static function get_group_members(int $group_id): array
    {
        ...
        foreach ($members as $member) {
            ...
            if (is_object($member) && isset($member->ID)) {
                $normalized[] = ['id' => (int) $member->ID];
                continue;
            }

            if (is_numeric($member)) {
                $normalized[] = ['id' => (int) $member];
            }
        }
    ...
    private static function render_admin_empty_state(): string
    {
        ...
        <div id="jp-chat-wrapper" class="jp-chat-admin-empty">
            <div class="jp-chat-heading">
                <h3><?php esc_html_e('Nenhuma conversa ainda.', 'juntaplay'); ?></h3>
                <p class="jp-chat-subtitle"><?php esc_html_e('Nenhum participante entrou em contato com você até o momento.', 'juntaplay'); ?></p>
            </div>
        </div>
        <?php
