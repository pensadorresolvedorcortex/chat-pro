...
class SaveGroupMeta {
    ...
    public static function handle( $post_id, $post, $update ) {
        ...
        if ( isset( $_POST['jplay_group_icon'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
            $icon_id = absint( wp_unslash( $_POST['jplay_group_icon'] ) );
            $icon_id = Groups::ensure_icon_min_size( $icon_id );

            if ( $icon_id > 0 ) {
                update_post_meta( $post_id, 'jplay_cover_id', $icon_id );
            } else {
                delete_post_meta( $post_id, 'jplay_cover_id' );
            }
        }
    }
}
