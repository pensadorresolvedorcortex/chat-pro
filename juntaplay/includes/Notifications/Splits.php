...
class Splits
{
    ...
    public function on_split_completed(WC_Order $order, array $entries, array $meta): void
    {
        ...
        $percentage_raw   = isset($meta['percentage']) ? $meta['percentage'] : Settings::get_split_percentage();
        $percentage       = (float) \juntaplay_sanitize_percent($percentage_raw);
    ...
    private function notify_superadmin(string $email, WC_Order $order, array $entries, float $percentage, string $participant_name, int $participant_id, string $timestamp): void
    {
        ...
        foreach ($entries as $entry) {
            ...
            $lines[] = sprintf(
                ...
                $entry['group_title'] ?? '',
                $entry['group_id'] ?? '',
                $this->format_price((float) ($entry['deposit_amount'] ?? 0.0)),
                __('Superadmin', 'juntaplay'),
                $this->format_price((float) ($entry['superadmin_amount'] ?? 0.0)),
                __('Admin', 'juntaplay'),
                $this->format_price((float) ($entry['admin_amount'] ?? 0.0)),
                __('Admin', 'juntaplay'),
                $admin_name,
                $admin_id ?: '—',
        ...
        $blocks = [
            ...
            [
                'type'    => 'paragraph',
                'content' => sprintf(__('Percentual aplicado: %s%%', 'juntaplay'), number_format_i18n($percentage, 0)),
            ],
            [
                'type'    => 'paragraph',
        ...
        EmailHelper::send(
            ...
            [
                ...
                'preheader' => sprintf(__('Percentual aplicado: %s%%', 'juntaplay'), number_format_i18n($percentage, 0)),
            ]
        );
    }
        ...
        foreach ($groups as $admin_id => $items) {
            ...
            EmailHelper::send(
                ...
                [
                    ...
                    'preheader' => sprintf(__('Total recebido: %s', 'juntaplay'), $this->format_price($this->sum_admin_amount($items))),
                ]
            );
        }
    }
