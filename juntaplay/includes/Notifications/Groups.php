...
class Groups
{
    public function init(): void
    {
        ...
        add_action('juntaplay/group_members/exit_scheduled', [$this, 'on_member_exit_scheduled'], 10, 3);
    ...
    public function on_group_created(int $user_id, int $group_id, array $data): void
    {
        ...
        $access_timing = isset($data['access_timing']) ? (string) $data['access_timing'] : '';
        $access_url   = isset($data['access_url']) ? (string) $data['access_url'] : '';
        $access_login = isset($data['access_login']) ? (string) $data['access_login'] : '';
        $access_pass  = isset($data['access_password']) ? (string) $data['access_password'] : '';
        $access_notes = isset($data['access_notes']) ? (string) $data['access_notes'] : '';
        $access_obs   = isset($data['access_observations']) ? (string) $data['access_observations'] : '';
        $relationship = isset($data['relationship_type']) ? (string) $data['relationship_type'] : '';
        $relationship_options = GroupsData::get_relationship_options();
        $relationship_label = $relationship !== '' && isset($relationship_options[$relationship])
            ? (string) $relationship_options[$relationship]
            : __('Não informado', 'juntaplay');
        $category_labels = GroupsData::get_category_labels();

        $timing_label = $access_timing === 'immediate'
        ...
        if ($admin_email !== '') {
            ...
            if ($relationship_label !== '') {
                $admin_lines[] = sprintf(__('Relação com o administrador: %s', 'juntaplay'), $relationship_label);
            }
            if ($timing_label !== '') {
                $admin_lines[] = sprintf(__('Quando os participantes recebem o acesso: %s', 'juntaplay'), $timing_label);
            }
            if ($access_url !== '') {
                $admin_lines[] = sprintf(__('Endereço de acesso: %s', 'juntaplay'), $access_url);
            }
            if ($access_login !== '') {
                $admin_lines[] = sprintf(__('Login compartilhado: %s', 'juntaplay'), $access_login);
            }
            if ($access_pass !== '') {
                $admin_lines[] = sprintf(__('Senha compartilhada: %s', 'juntaplay'), $access_pass);
            }
            if ($access_notes !== '') {
                $admin_lines[] = sprintf(__('Observações do acesso: %s', 'juntaplay'), $access_notes);
            }
            if ($access_obs !== '') {
                $admin_lines[] = sprintf(__('Notas adicionais: %s', 'juntaplay'), $access_obs);
            }
        ...
        if ($user && !empty($user->user_email)) {
            $summary_lines = [
                ...
                sprintf(__('Relação com o administrador: %s', 'juntaplay'), $relationship_label),
                sprintf(__('Quando os participantes recebem o acesso: %s', 'juntaplay'), $timing_label),
            ...
            if ($access_url !== '') {
                $summary_lines[] = sprintf(__('Endereço de acesso: %s', 'juntaplay'), $access_url);
            }

            if ($access_login !== '') {
                $summary_lines[] = sprintf(__('Login compartilhado: %s', 'juntaplay'), $access_login);
            }

            if ($access_pass !== '') {
                $summary_lines[] = sprintf(__('Senha compartilhada: %s', 'juntaplay'), $access_pass);
            }

            if ($access_notes !== '') {
                $summary_lines[] = sprintf(__('Observações do acesso: %s', 'juntaplay'), $access_notes);
            }

            if ($access_obs !== '') {
                $summary_lines[] = sprintf(__('Notas adicionais: %s', 'juntaplay'), $access_obs);
            }
    ...
    public function on_member_added(int $group_id, int $user_id, string $role, string $status): void
    {
        ...
        $owner_contact  = $this->get_user_contact_info($owner_id);
        $contact_lines  = $this->format_contact_lines($owner_contact);
        $credentials    = $this->extract_credentials($group);
        $has_credentials = $this->credentials_available($credentials);
        $credential_lines = $this->format_credential_lines($credentials);
        ...
        if ($member && !empty($member->user_email)) {
            $welcome_blocks = [
                [
                    'type'    => 'paragraph',
                    'content' => sprintf(__('Bem-vindo ao grupo %s!', 'juntaplay'), $group_title),
                ],
                [
                    'type'    => 'paragraph',
                    'content' => __('Acesse o painel para acompanhar orientações, pagamentos e convites.', 'juntaplay'),
                ],
                [
                    'type'  => 'button',
                    'label' => __('Ir para Meus Grupos', 'juntaplay'),
                    'url'   => $url,
                ],
            ];
            ...
            EmailHelper::send(
                $member->user_email,
                sprintf(__('Boas-vindas ao grupo %1$s — %2$s', 'juntaplay'), $group_title, $site_name),
                $welcome_blocks,
                [
                    'headline'  => __('Você entrou em um novo grupo!', 'juntaplay'),
                    'preheader' => __('Confira as instruções e acompanhe seu acesso.', 'juntaplay'),
                ]
            );
            ...
            $access_blocks = [
                [
                    'type'    => 'paragraph',
                    'content' => sprintf(__('Dados de acesso para o grupo %s', 'juntaplay'), $group_title),
                ],
            ];

            if ($has_credentials && $credential_lines) {
                $access_blocks[] = [
                    'type'  => 'list',
                ...
                $access_blocks[] = [
                    'type'    => 'paragraph',
                    'content' => __('Use estas credenciais conforme as orientações do administrador.', 'juntaplay'),
                ];
            } else {
                $access_blocks[] = [
                    'type'    => 'paragraph',
                    'content' => __('O administrador ainda não cadastrou as credenciais deste grupo. Já notificamos para que ele envie os dados o quanto antes.', 'juntaplay'),
                ];
            }
            ...
            if ($contact_lines) {
                ...
                $access_blocks[] = [
                    'type'    => 'paragraph',
                    'content' => __('Se preferir, entre em contato com o administrador pelos canais acima.', 'juntaplay'),
                ];
            }

            $access_blocks[] = [
                'type'  => 'button',
                'label' => __('Abrir área Meus Grupos', 'juntaplay'),
                'url'   => $url,
            ];

            $access_preheader = $has_credentials
                ? __('As credenciais do seu novo grupo estão disponíveis.', 'juntaplay')
                : __('Aguarde o envio das credenciais pelo administrador.', 'juntaplay');

            EmailHelper::send(
                $member->user_email,
                sprintf(__('Acesso ao seu novo grupo — %s', 'juntaplay'), $site_name),
                $access_blocks,
                [
                    'headline'  => __('Detalhes do acesso', 'juntaplay'),
                    'preheader' => $access_preheader,
                ]
            );
        }

        if (!empty($owner_contact['email'])) {
            ...
            $admin_blocks = [
                [
                    'type'    => 'paragraph',
                    'content' => sprintf(__('%1$s acabou de entrar no grupo %2$s.', 'juntaplay'), $member_name, $group_title),
                ],
            ];
            ...
            if ($has_credentials && $credential_lines) {
                $admin_blocks[] = [
                    'type'    => 'paragraph',
                    'content' => __('As credenciais cadastradas já foram encaminhadas ao novo participante.', 'juntaplay'),
                ];
            } else {
                $admin_blocks[] = [
                    'type'    => 'paragraph',
                    'content' => __('Cadastre ou envie os dados de acesso para que o participante possa usar o serviço.', 'juntaplay'),
                ];
            }

            $admin_blocks[] = [
                'type'  => 'button',
                'label' => __('Enviar credenciais agora', 'juntaplay'),
                'url'   => $manage_url,
            ];

            EmailHelper::send(
                $owner_contact['email'],
                sprintf(__('Novo participante no grupo %1$s — %2$s', 'juntaplay'), $group_title, $site_name),
                $admin_blocks,
                [
                    'headline'  => __('Um membro está aguardando o acesso', 'juntaplay'),
                    'preheader' => __('Envie as credenciais ou valide os dados cadastrados.', 'juntaplay'),
                ]
            );
        }
     ...
     * @param array<string, mixed> $data
     */
    public function on_member_exit_scheduled(int $group_id, int $user_id, array $data): void
    {
        $group = GroupsData::get($group_id);
        if (!$group) {
            return;
        ...
        $member = get_userdata($user_id);
        $group_title = isset($group->title) ? (string) $group->title : __('Grupo', 'juntaplay');
        $member_name = $member ? ($member->display_name ?: $member->user_login) : __('Participante', 'juntaplay');
        $site_name = get_bloginfo('name');
        $url = $this->get_my_groups_url();

        $exit_effective_at = isset($data['exit_effective_at']) ? (string) $data['exit_effective_at'] : '';
        $exit_display = $exit_effective_at !== ''
            ? date_i18n(get_option('date_format'), strtotime($exit_effective_at))
            : '';
        ...
        if ($member && !empty($member->user_email)) {
            $member_blocks = [
                [
                    'type'    => 'paragraph',
                    'content' => sprintf(__('Seu cancelamento no grupo %s foi registrado.', 'juntaplay'), $group_title),
                ],
                [
                    'type'    => 'paragraph',
                    'content' => $exit_display !== ''
                        ? sprintf(__('Sua saída será efetivada em %s.', 'juntaplay'), $exit_display)
                        : __('Sua saída será efetivada conforme o ciclo vigente.', 'juntaplay'),
                ],
                [
                    'type'  => 'button',
                    'label' => __('Acompanhar em Meus Grupos', 'juntaplay'),
                    'url'   => $url,
                ],
            ];
        ...
        if (!empty($owner_contact['email'])) {
            $admin_blocks = [
                [
                    'type'    => 'paragraph',
                    'content' => sprintf(__('%1$s solicitou o cancelamento no grupo %2$s.', 'juntaplay'), $member_name, $group_title),
                ],
                [
                    'type'    => 'paragraph',
                    'content' => $exit_display !== ''
                        ? sprintf(__('A saída está prevista para %s.', 'juntaplay'), $exit_display)
                        : __('A saída ocorrerá conforme o ciclo vigente.', 'juntaplay'),
                ],
                [
                    'type'  => 'button',
                    'label' => __('Abrir Meus Grupos', 'juntaplay'),
                    'url'   => $url,
                ],
            ];
    ...
    /**
     * @return array{name: string, email: string, whatsapp: string, phone: string}
     */
    private function get_user_contact_info(int $user_id): array
    {
        $contact = [
            'name'      => '',
            'email'     => '',
        ...
        $user = get_userdata($user_id);
        if ($user) {
            $contact['name']  = $user->display_name ?: $user->user_login;
            $contact['email'] = (string) $user->user_email;
        }

        $contact['whatsapp'] = (string) get_user_meta($user_id, 'juntaplay_whatsapp', true);
        $contact['phone']     = (string) get_user_meta($user_id, 'juntaplay_phone', true);
     ...
     * @return string[]
     */
    private function format_contact_lines(array $contact): array
    {
        $lines = [];

        if (!empty($contact['name'])) {
            $lines[] = sprintf(__('Administrador: %s', 'juntaplay'), $contact['name']);
        }

        if (!empty($contact['email'])) {
            $lines[] = sprintf(__('E-mail do administrador: %s', 'juntaplay'), $contact['email']);
        }

        if (!empty($contact['whatsapp'])) {
            $lines[] = sprintf(__('WhatsApp do administrador: %s', 'juntaplay'), $contact['whatsapp']);
        } elseif (!empty($contact['phone'])) {
            $lines[] = sprintf(__('Telefone do administrador: %s', 'juntaplay'), $contact['phone']);
        }
     ...
     * @return string[]
     */
    private function format_credential_lines(array $credentials): array
    {
        $credential_lines = [];

        if (!empty($credentials['access_url'])) {
            $credential_lines[] = sprintf(__('Endereço: %s', 'juntaplay'), $credentials['access_url']);
        }

        if (!empty($credentials['access_login'])) {
            $credential_lines[] = sprintf(__('Login: %s', 'juntaplay'), $credentials['access_login']);
        }

        if (!empty($credentials['access_password'])) {
            $credential_lines[] = sprintf(__('Senha: %s', 'juntaplay'), $credentials['access_password']);
        }

        if (!empty($credentials['access_notes'])) {
            $credential_lines[] = sprintf(__('Observações: %s', 'juntaplay'), $credentials['access_notes']);
        }

        if (!empty($credentials['access_observations'])) {
            $credential_lines[] = sprintf(__('Instruções adicionais: %s', 'juntaplay'), $credentials['access_observations']);
        }
    ...
    public function on_send_access(int $group_id, int $user_id, array $credentials, array $context = []): void
    {
        ...
        $credential_lines = $this->format_credential_lines($credential_values);
        $owner_contact    = $this->get_user_contact_info(isset($group->owner_id) ? (int) $group->owner_id : 0);
        $contact_lines    = $this->format_contact_lines($owner_contact);
        ...
        if ($contact_lines) {
            $blocks[] = [
                'type'    => 'paragraph',
                'content' => __('Canais de contato do administrador', 'juntaplay'),
            ];
        ...
        if ($credential_values['access_url'] !== '') {
            $blocks[] = [
                'type'  => 'button',
                'label' => __('Acessar agora', 'juntaplay'),
                'url'   => $credential_values['access_url'],
            ];
        }
