...
class Groups
{
    ...
    public function on_order_purchase_email($order, array $items, array $totals): void
    {
        ...
        foreach ($items as $item) {
            ...
            $group_lines[] = sprintf(
                /* translators: 1: group title, 2: quantity, 3: unit price, 4: deposit value */
                __('%1$s — %2$d cotas (assinatura %3$s, caução %4$s)', 'juntaplay'),
                $title,
                $quantity,
                $this->format_currency($unit_price),
                $this->format_currency($deposit)
            );
        }
        ...
        $grand_total        = isset($totals['grand_total']) ? (float) $totals['grand_total'] : ($subscription_total + $deposit_total + $processing_total - $wallet_used);
        ...
        $blocks = [
            [
                ...
                'content' => sprintf(
                    ...
                    __('Pedido #%s confirmado! Veja os detalhes da sua assinatura abaixo.', 'juntaplay'),
        ...
        $blocks[] = [
            'type'    => 'paragraph',
            'content' => __('Os dados de acesso foram enviados automaticamente para o seu e-mail e também ficam disponíveis em “Meus grupos”.', 'juntaplay'),
        ];
        ...
        EmailHelper::send(
            ...
            $blocks,
            [
                'headline'  => __('Compra confirmada!', 'juntaplay'),
                'preheader' => __('Sua assinatura foi aprovada e os dados de acesso estão disponíveis.', 'juntaplay'),
            ]
        );
    }
    ...
    public function on_send_access(int $group_id, int $user_id, array $credentials, array $context = []): void
    {
        ...
        $access_url   = isset($credentials['access_url']) ? (string) $credentials['access_url'] : '';
        $access_login = isset($credentials['access_login']) ? (string) $credentials['access_login'] : '';
        $access_pass  = isset($credentials['access_password']) ? (string) $credentials['access_password'] : '';
        $access_notes = isset($credentials['access_notes']) ? (string) $credentials['access_notes'] : '';
        $access_obs   = isset($credentials['access_observations']) ? (string) $credentials['access_observations'] : '';
        ...
        if ($credential_lines) {
            ...
            $blocks[] = [
                'type'    => 'paragraph',
                'content' => __('O administrador ainda não cadastrou credenciais para este grupo. Assim que forem adicionadas, você receberá um novo aviso.', 'juntaplay'),
            ];
        }
        ...
        $preheader = $source === 'order'
            ? __('Enviamos automaticamente as credenciais do seu novo grupo.', 'juntaplay')
            : __('Reenvio solicitado pela área do assinante.', 'juntaplay');
