<?php
/**
 * Authentication modal template (login + register).
 *
...
$resolve_asset_url = static function (string $relative_path) use ($plugin_dir): string {
    ...
    $asset_file = trailingslashit($plugin_dir) . $relative_path;
    ...
    if (defined('JP_URL') && JP_URL !== '') {
        $sources[] = trailingslashit(JP_URL) . $relative_path;
    }

    if (function_exists('plugins_url')) {
        $main_file = trailingslashit($plugin_dir) . 'juntaplay.php';
        $sources[] = plugins_url($relative_path, file_exists($main_file) ? $main_file : __FILE__);
    }
    ...
    if (defined('WP_PLUGIN_DIR') && defined('WP_PLUGIN_URL')) {
        ...
        if ($normalized_plugin_dir !== '' && strpos($normalized_plugin_dir, $normalized_plugins_dir) === 0) {
            ...
            $sources[] = $base_plugin_url . $relative_path;
        }
    }

    foreach (array_filter(array_unique($sources)) as $source) {
        if ($source !== '') {
            return $source;
        }
    }
...
$google_icon = $resolve_asset_url('assets/images/google.svg');
...
$providers = apply_filters(
    ...
    [
        [
            ...
            'register_label'  => __('Criar uma Conta', 'juntaplay'),
            'href'            => '#',
            'icon'            => $google_icon,
            'popup'           => true,
        ],
...
$register_cpf = isset($_POST['jp_register_cpf']) ? sanitize_text_field(wp_unslash($_POST['jp_register_cpf'])) : '';
...
?>
<div class="juntaplay-auth juntaplay-auth--modal" data-active-view="<?php echo esc_attr($active_view); ?>">
    <div class="juntaplay-auth__card" data-has-register="<?php echo $show_register ? '1' : '0'; ?>">
        <div class="juntaplay-auth__header">
            <h2 class="juntaplay-auth__title"><?php esc_html_e('Acesse sua conta', 'juntaplay'); ?></h2>
            <?php if ($show_register) : ?>
                <div class="juntaplay-auth__switch" role="tablist">
                    ...
                    <button type="button" class="juntaplay-auth__switch-btn<?php echo $active_view === 'register' ? ' is-active' : ''; ?>" data-target="register" role="tab" aria-selected="<?php echo $active_view === 'register' ? 'true' : 'false'; ?>" <?php disabled(!$can_register); ?>>
                        <?php echo $can_register ? esc_html__('Cadastre-se', 'juntaplay') : esc_html__('Solicitar acesso', 'juntaplay'); ?>
                    </button>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="juntaplay-auth__panes">
            <div class="juntaplay-auth__pane juntaplay-auth__pane--login<?php echo $active_view === 'login' ? ' is-active' : ''; ?>" data-pane="login" role="tabpanel" aria-hidden="<?php echo $active_view === 'login' ? 'false' : 'true'; ?>">
                ...
                <form class="juntaplay-auth__form" method="post" action="<?php echo esc_url($current_url); ?>">
                    ...
                    <div class="juntaplay-auth__field">
                        <label for="jp_login_username_modal"><?php esc_html_e('E-mail ou usuário', 'juntaplay'); ?></label>
                        <input type="text" name="jp_login_username" id="jp_login_username_modal" autocomplete="username" placeholder="nome@email.com" value="<?php echo esc_attr($username); ?>" required>
                    </div>

                    <div class="juntaplay-auth__field">
                        <label for="jp_login_password_modal"><?php esc_html_e('Senha', 'juntaplay'); ?></label>
                        <input type="password" name="jp_login_password" id="jp_login_password_modal" autocomplete="current-password" placeholder="••••••••" required>
                    </div>
            ...
            <?php if ($show_register) : ?>
                <div class="juntaplay-auth__pane juntaplay-auth__pane--register<?php echo $active_view === 'register' ? ' is-active' : ''; ?>" data-pane="register" role="tabpanel" aria-hidden="<?php echo $active_view === 'register' ? 'false' : 'true'; ?>">
                    ...
                    ?>
                    <form class="juntaplay-auth__form" method="post" action="<?php echo esc_url($current_url); ?>" data-register-wizard="1" data-step="1">
                        ...
                        <div class="juntaplay-auth__steps" data-auth-register-steps>
                            <div class="juntaplay-auth__stepper" aria-hidden="true">
                                <div class="juntaplay-auth__stepper-item is-active" data-auth-step-indicator="1">
                                    <span class="juntaplay-auth__stepper-dot"></span>
                                    <span class="juntaplay-auth__stepper-label"><?php esc_html_e('Dados da conta', 'juntaplay'); ?></span>
                                </div>
                                <div class="juntaplay-auth__stepper-item" data-auth-step-indicator="2">
                                    <span class="juntaplay-auth__stepper-dot"></span>
                                    <span class="juntaplay-auth__stepper-label"><?php esc_html_e('Segurança', 'juntaplay'); ?></span>
                                </div>
                            </div>

                        <div class="juntaplay-auth__step is-active" data-auth-step="1">
                            <div class="juntaplay-auth__field">
                                <label for="jp_register_name_modal"><?php esc_html_e('Nome completo', 'juntaplay'); ?></label>
                                <input type="text" name="jp_register_name" id="jp_register_name_modal" autocomplete="name" placeholder="Maria Silva" value="<?php echo esc_attr($register_name); ?>" <?php disabled(!$can_register); ?> required>
                            </div>

                            <div class="juntaplay-auth__field">
                                <label for="jp_register_email_modal"><?php esc_html_e('E-mail', 'juntaplay'); ?></label>
                                <input type="email" name="jp_register_email" id="jp_register_email_modal" autocomplete="email" placeholder="nome@email.com" value="<?php echo esc_attr($register_mail); ?>" <?php disabled(!$can_register); ?> required>
                            </div>

                            <div class="juntaplay-auth__field">
                                <label for="jp_register_cpf_modal"><?php esc_html_e('CPF', 'juntaplay'); ?></label>
                                <input type="text" name="jp_register_cpf" id="jp_register_cpf_modal" inputmode="numeric" autocomplete="off" placeholder="000.000.000-00" value="<?php echo esc_attr($register_cpf); ?>" data-cpf-input <?php disabled(!$can_register); ?> required>
                                <?php
                                // Show CPF validation message only when the field exists in the register form.
                                $cpf_error_message = __('Informe um CPF válido.', 'juntaplay');
                                $cpf_duplicate_message = __('Este CPF já está vinculado a outra conta.', 'juntaplay');
                                $cpf_error_to_show = '';
                                if (in_array($cpf_duplicate_message, $register_errors, true)) {
                                    $cpf_error_to_show = $cpf_duplicate_message;
                                } elseif (in_array($cpf_error_message, $register_errors, true)) {
                                    $cpf_error_to_show = $cpf_error_message;
                                }
                                if ($cpf_error_to_show !== '') :
                                    ?>
                                    <p class="juntaplay-auth__error" role="alert"><?php echo esc_html($cpf_error_to_show); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>

                            <div class="juntaplay-auth__step" data-auth-step="2">
                                <div class="juntaplay-auth__field">
                                    <label for="jp_register_password_modal"><?php esc_html_e('Senha', 'juntaplay'); ?></label>
                                    <input type="password" name="jp_register_password" id="jp_register_password_modal" autocomplete="new-password" placeholder="••••••••" <?php disabled(!$can_register); ?> required>
                                </div>

                                <div class="juntaplay-auth__field">
                                    <label for="jp_register_password_confirm_modal"><?php esc_html_e('Confirmar senha', 'juntaplay'); ?></label>
                                    <input type="password" name="jp_register_password_confirm" id="jp_register_password_confirm_modal" autocomplete="new-password" placeholder="••••••••" <?php disabled(!$can_register); ?> required>
                                </div>
                        ...
                        <div class="juntaplay-auth__actions">
                            <button type="button" class="juntaplay-button juntaplay-button--ghost juntaplay-auth__nav" data-auth-step-prev <?php disabled(!$can_register); ?>>
                                <?php esc_html_e('Voltar', 'juntaplay'); ?>
                            </button>
                            <button type="button" class="juntaplay-button juntaplay-button--primary juntaplay-auth__nav" data-auth-step-next <?php disabled(!$can_register); ?>>
                                <?php esc_html_e('Continuar', 'juntaplay'); ?>
                            </button>
                            <button type="submit" class="juntaplay-button juntaplay-button--primary juntaplay-auth__submit" data-auth-step-submit <?php disabled(!$can_register); ?>>
