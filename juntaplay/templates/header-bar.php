...
foreach ($unread_messages as $message_data) {
    ...
    $admin_id    = isset($message_data['admin_id']) ? (int) $message_data['admin_id'] : 0;
    $subscriber_id = isset($message_data['subscriber_id']) ? (int) $message_data['subscriber_id'] : 0;
    $group_id    = isset($message_data['group_id']) ? (int) $message_data['group_id'] : 0;
    ...
    $notification_messages[] = [
        'message' => sprintf(__('%s enviou uma mensagem.', 'juntaplay'), $sender_name),
        'url'     => add_query_arg($search, $messages_entry_url),
    ];
}
?>
