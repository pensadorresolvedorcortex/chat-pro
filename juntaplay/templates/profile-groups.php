...
if (!function_exists('jp_is_group_admin_role')) {
    /**
     * Determine if the current user should be treated as group admin.
     */
    function jp_is_group_admin_role(int $owner_id, int $current_user_id, string $group_role, string $role_label): bool
    {
        $group_role_key = strtolower($group_role);

        return $owner_id === $current_user_id
            || in_array($group_role_key, ['owner', 'manager', 'creator', 'admin', 'administrator'], true)
            || ($role_label !== '' && stripos($role_label, 'administra') !== false);
    }
}
...
$admin_cancel_errors     = isset($group_context['admin_cancel_errors']) && is_array($group_context['admin_cancel_errors']) ? $group_context['admin_cancel_errors'] : [];
$admin_cancel_general_errors = isset($admin_cancel_errors['general']) && is_array($admin_cancel_errors['general']) ? $admin_cancel_errors['general'] : [];
$admin_cancel_group_errors   = isset($admin_cancel_errors['groups']) && is_array($admin_cancel_errors['groups']) ? $admin_cancel_errors['groups'] : [];
        ...
        foreach ($all_groups as $group) :
            if (!is_array($group)) {
                continue;
                    ...
                    $admin_cancel_messages = isset($admin_cancel_group_errors[$group_id]) && is_array($admin_cancel_group_errors[$group_id]) ? $admin_cancel_group_errors[$group_id] : [];
                    ...
                    $is_admin_role  = jp_is_group_admin_role($owner_id, $current_user_id, $group_role, $role_label);
                            ...
                            <?php endif; ?>
                                    ...
                                    <div class="juntaplay-group-card__footer">
                                        ...
                                        <div class="juntaplay-group-card__details-extended" hidden>
                                            ...
                                            <?php endif; ?>
                                            <?php if ($is_admin_role && $status === 'active') : ?>
                                                <div class="juntaplay-group-card__complaint juntaplay-group-card__cancel" data-group-admin-cancel>
                                                    <header class="juntaplay-group-card__complaint-header">
                                                        <div>
                                                            <h4><?php echo esc_html__('Cancelar Grupo', 'juntaplay'); ?></h4>
                                                            <p class="juntaplay-group-card__complaint-summary"><?php echo esc_html__('Encerrar o grupo como oferta para novos participantes.', 'juntaplay'); ?></p>
                                                        </div>
                                                    </header>
                                                    <?php if ($admin_cancel_messages) : ?>
                                                        <div class="juntaplay-alert juntaplay-alert--danger">
                                                            <ul>
                                                                <?php foreach ($admin_cancel_messages as $cancel_message) : ?>
                                                                    <li><?php echo esc_html((string) $cancel_message); ?></li>
                                                                <?php endforeach; ?>
                                                            </ul>
                                                        </div>
                                                    <?php endif; ?>
                                                    <?php $admin_cancel_modal_id = 'jp-group-admin-cancel-modal-' . $group_id; ?>
                                                    <button type="button" class="juntaplay-button juntaplay-button--danger" data-group-admin-cancel-open data-modal-id="<?php echo esc_attr($admin_cancel_modal_id); ?>">
                                                        <?php echo esc_html__('Cancelar Grupo', 'juntaplay'); ?>
                                                    </button>
                                                        ...
                                                        <div class="juntaplay-modal__dialog" role="dialog" aria-modal="true">
                                                            ...
                                                            <div class="juntaplay-modal__content">
                                                                ...
                                                                <p class="juntaplay-modal__text">
                                                                    <?php echo esc_html__('Cancelar este grupo encerrará definitivamente o grupo como oferta. Os participantes manterão acesso até o fim do período já pago e seus calções serão devolvidos conforme as regras da plataforma. Esta ação não pode ser desfeita.', 'juntaplay'); ?>
                                                                </p>
  ...
  <?php endif; ?>
    ...
    <script>
        (() => {
            ...
            const cancelConfirmUrl = '<?php echo esc_js($cancel_confirm_url); ?>';
