...
if (!empty($cancel_pages)) {
    foreach ($cancel_pages as $cancel_page) {
        if ($cancel_page instanceof WP_Post && has_shortcode($cancel_page->post_content, 'juntaplay_cancelamento')) {
            $cancel_page_id = $cancel_page->ID;
            break;
        }
    }
}

$cancel_confirm_url = $cancel_page_id ? get_permalink($cancel_page_id) : add_query_arg('section', 'juntaplay-cancelamento', $profile_base_url);
