...
$merged_items = [];
if (isset($group_context['items']) && is_array($group_context['items'])) {
    $merged_items = $group_context['items'];
} elseif (isset($context) && is_array($context) && isset($context['items']) && is_array($context['items'])) {
    $merged_items = $context['items'];
}

if ($merged_items) {
    $all_groups = array_values(array_filter($merged_items, 'is_array'));
} else {
    $all_groups_indexed = [];
        ...
        foreach ($all_groups as $group) :
            if (!is_array($group)) {
                continue;
                    ...
                    $group_type      = isset($group['type']) ? (string) $group['type'] : 'group';
                    $is_service      = $group_type === 'service';
                    ...
                    if ($is_service) {
                        $cta_label = $campaigns_url !== '' ? __('Entrar em grupo', 'juntaplay') : __('Criar grupo', 'juntaplay');
                        $cta_variant = 'primary';
                        $cta_url = $campaigns_url;
                    ...
                    $cover_id = isset($group['cover']) ? (int) $group['cover'] : (isset($group['cover_id']) ? (int) $group['cover_id'] : 0);
                    if ($icon_source === '' && $cover_id > 0) {
                        $icon_source = (string) wp_get_attachment_image_url($cover_id, 'thumbnail');
                    }
                    if ($icon_source === '') {
                                            ...
                                            <?php if (!$is_owner) : ?>
                                                <div class="juntaplay-group-card__complaint juntaplay-group-card__cancel">
                                                    <header class="juntaplay-group-card__complaint-header">
                                                        ...
                                                        ?>
                                                        <span class="juntaplay-badge juntaplay-badge--<?php echo esc_attr($exit_status_tone); ?>">
                                                            <?php echo esc_html($exit_status_label); ?>
                                                        </span>
                                                    </header>
                                                    <?php if ($membership_status === 'exit_scheduled' && $exit_effective_display !== '') : ?>
                                                        <p class="juntaplay-group-card__complaint-note"><?php echo esc_html(sprintf(__('Saída efetivada em %s.', 'juntaplay'), $exit_effective_display)); ?></p>
                                                    <?php endif; ?>
                                                    <?php if ($exit_note !== '') : ?>
                                                    ...
                                                    <?php if ($membership_status !== 'exit_scheduled') : ?>
                                                        ...
                                                        <div class="juntaplay-group-complaint__actions">
                                                            <button type="button" class="juntaplay-button juntaplay-button--ghost" data-group-exit-trigger data-exit-modal-id="<?php echo esc_attr($exit_modal_id); ?>">
                                                                <?php echo esc_html__('Sair do grupo', 'juntaplay'); ?>
                                                            </button>
                                                        </div>
                                                        <div class="juntaplay-modal juntaplay-modal--compact" id="<?php echo esc_attr($exit_modal_id); ?>" data-group-exit-modal hidden aria-hidden="true">
                                                            ...
                                                            <div class="juntaplay-modal__dialog" role="dialog" aria-modal="true">
                                                                ...
                                                                <div class="juntaplay-modal__content">
                                                                    <h3 class="juntaplay-modal__title"><?php echo esc_html__('Confirmar saída do grupo', 'juntaplay'); ?></h3>
                                                                    <p class="juntaplay-modal__text">
                                                                        <?php echo esc_html($exit_effective_copy !== '' ? sprintf(__('Sua saída será efetivada em %s, ao completar seu ciclo mínimo de 30 dias.', 'juntaplay'), $exit_effective_copy) : __('Sua saída será efetivada ao completar seu ciclo mínimo de 30 dias.', 'juntaplay')); ?>
                                                                    </p>
                                                                    <form class="juntaplay-group-cancel__form" method="post">
                                                                        ...
                                                                        <div class="juntaplay-group-complaint__actions">
                                                                            <button type="submit" class="juntaplay-button juntaplay-button--ghost"><?php echo esc_html__('Confirmar saída', 'juntaplay'); ?></button>
                                                                        </div>
                                                                    </form>
                                                                </div>
                                                            </div>
