...
if ($chat_groups) {
    ...
    if ($initial_group_id > 0) {
        ...
        if ($initial_subscriber_id === 0 && $initial_subscribers) {
...
<div
    class="juntaplay-chat-shell"<?php echo $is_chat_section ? '' : ' hidden'; ?>
    data-juntaplay-chat
    ...
    <script>
        (() => {
            ...
            const renderGroupList = () => {
                if (!groupList) return;
                const groups = state.isAdmin ? (config.groups || []) : (config.memberGroups || []);
                ...
                groupList.querySelectorAll('[data-chat-select="group"]').forEach((item) => {
                    item.addEventListener('click', () => {
                        ...
                        const groupsSource = state.isAdmin ? (config.groups || []) : (config.memberGroups || []);
                        const found = groupsSource.find((entry) => Number(entry.id) === id);
            ...
            const fetchContext = async (groupId) => {
                if (!groupId || !config.rest?.context) {
                ...
                if (state.subscribers[groupId].length && !state.subscriberId) {
                    selectSubscriber(state.subscribers[groupId][0]);
                }
            };
                                    ...
                                    <div class="juntaplay-group-card__footer">
                                        ...
                                        <div class="juntaplay-group-card__details-extended" hidden>
                                            ...
                                            <div class="juntaplay-group-card__details-actions">
                                                ...
                                                if ($membership_status === 'active' && $messages_base_url !== '') {
                                                    if ($is_admin) {
                                                        $admin_chat_link = add_query_arg('group_id', $group_id, $messages_base_url);
                                                    } elseif ($owner_id !== $current_user_id && $owner_id > 0) {
                                                        $chat_link = add_query_arg('group_id', $group_id, $messages_base_url);
                                                    }
                                                }
                                                ?>
