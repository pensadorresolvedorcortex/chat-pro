...
// Fallback: if no group context was injected, try to load from the chat filters so the
// Central de Mensagens still receives the selectable groups.
if (!$chat_groups && !$chat_member_groups) {
    $fallback_groups = apply_filters('juntaplay_chat_groups_for_user', [], $current_user_id);

    if (is_array($fallback_groups)) {
        foreach ($fallback_groups as $group) {
            ...
            $entry = [
                ...
                'avatar'   => isset($group['avatar']) ? (string) $group['avatar'] : '',
                        ...
                        <div class="juntaplay-card">
                            ...
                            >
                                <div class="juntaplay-chat-surface">
                                        ...
                                        <div>
                                            ...
                                            <ul class="juntaplay-chat-card-list" data-chat-subscriber-list>
                                                <?php if ($chat_subscribers) : ?>
                                                    ...
                                                    <?php endforeach; ?>
                                                <?php else : ?>
                                                    <li class="juntaplay-chat-card" data-chat-empty>
                                                        <span class="juntaplay-chat-card__meta">
                                                            <strong class="juntaplay-chat-card__name"><?php esc_html_e('Nenhum assinante encontrado', 'juntaplay'); ?></strong>
                                                            <span class="juntaplay-chat-card__group"><?php esc_html_e('Escolha um grupo para conversar', 'juntaplay'); ?></span>
                                                        </span>
                                                    </li>
                                                <?php endif; ?>
                                            </ul>
                                        ...
                                        <div>
                                            ...
                                            <ul class="juntaplay-chat-card-list" data-chat-group-list>
                                                ...
                                                <?php else : ?>
                                                    <li class="juntaplay-chat-card" data-chat-empty>
                                                        <span class="juntaplay-chat-card__meta">
                                                            <strong class="juntaplay-chat-card__name"><?php esc_html_e('Nenhum grupo selecionado', 'juntaplay'); ?></strong>
                                                            <span class="juntaplay-chat-card__group"><?php esc_html_e('Escolha um grupo para iniciar o chat', 'juntaplay'); ?></span>
                                                        </span>
                                                    </li>
                                                <?php endif; ?>
                                            </ul>
                                    ...
                                    <section class="juntaplay-chat-window">
                                        <header class="juntaplay-chat-window__header">
                                            <div class="juntaplay-chat-window__participants">
                                                <div class="juntaplay-chat-window__avatars" aria-hidden="true">
                                                    <span class="juntaplay-chat-window__avatar">
    ...
    <script>
        (() => {
            ...
            const state = {
                ...
                subscriberId: config.subscriberId || 0,
            ...
            const updateHeader = (adminName, recipientName, groupLabel) => {
            ...
            const selectSubscriber = (subscriber) => {
                ...
                updateHeader(config.isAdmin ? (config.userName || '') : (subscriber?.name || ''), config.isAdmin ? (subscriber?.name || '') : (config.adminName || ''), subscriber?.group || '');
                loadMessages();
            };
            ...
            const renderSubscriberList = () => {
                ...
                if (!list.length) {
                    subscriberList.innerHTML = `
                        <li class="juntaplay-chat-card" data-chat-empty>
                            <span class="juntaplay-chat-card__meta">
                                <strong class="juntaplay-chat-card__name"><?php echo esc_js(__('Nenhum assinante encontrado', 'juntaplay')); ?></strong>
                                <span class="juntaplay-chat-card__group"><?php echo esc_js(__('Escolha um grupo para conversar', 'juntaplay')); ?></span>
                            </span>
                        </li>`;
                    return;
                }
            ...
            const selectGroup = (group) => {
                ...
                if (!state.isAdmin && group) {
                    ...
                    updateHeader(config.adminName || '', config.recipientName || '', group.title || '');
                    loadMessages();
                    return;
                }
            ...
            const renderGroupList = () => {
                ...
                if (!groups.length) {
                    groupList.innerHTML = `
                        <li class="juntaplay-chat-card" data-chat-empty>
                            <span class="juntaplay-chat-card__meta">
                                <strong class="juntaplay-chat-card__name"><?php echo esc_js(__('Nenhum grupo selecionado', 'juntaplay')); ?></strong>
                                <span class="juntaplay-chat-card__group"><?php echo esc_js(__('Escolha um grupo para iniciar o chat', 'juntaplay')); ?></span>
                            </span>
                        </li>`;
                    return;
                }
                ...
                groupList.innerHTML = groups.map((group) => {
                    ...
                    return `
                        <li class="juntaplay-chat-card" data-chat-select="group" data-group-id="${group.id}">
                            <span class="juntaplay-chat-card__avatar" aria-hidden="true">${avatar}</span>
                            <span class="juntaplay-chat-card__meta">
            ...
            updateHeader(config.adminName || '', config.recipientName || '', config.initialGroupId ? '' : config.initialGroupName || '');
            renderGroupList();
...
<div
    ...
    class="juntaplay-groups juntaplay-profile__panels juntaplay-profile__panels--groups juntaplay-my-groups-grid"
    data-groups
                                    ...
                                    <div class="juntaplay-group-card__footer">
                                        ...
                                        <div class="juntaplay-group-card__details-extended" hidden>
                                            ...
                                            <div class="juntaplay-group-card__details-actions">
                                                ...
                                                $is_admin = $group_role === 'owner' || $group_role === 'manager';

                                                $chat_target_url = $messages_endpoint !== '' ? $messages_endpoint : $messages_base_url;

                                                if ($membership_status === 'active' && $chat_target_url !== '') {
                                                    if ($is_admin) {
                                                        $admin_chat_link = add_query_arg('group_id', $group_id, $chat_target_url);
                                                    } elseif ($owner_id !== $current_user_id && $owner_id > 0) {
                                                        $chat_link = add_query_arg('group_id', $group_id, $chat_target_url);
                                                    }
                                                }
                                                ?>
                                                <?php if ($admin_chat_link !== '') : ?>
                                                    <a class="juntaplay-button juntaplay-button--primary" href="<?php echo esc_url($admin_chat_link); ?>"><?php esc_html_e('Fale com Assinantes', 'juntaplay'); ?></a>
                                                <?php elseif ($chat_link !== '') : ?>
                                                    <a class="juntaplay-button juntaplay-button--primary" href="<?php echo esc_url($chat_link); ?>"><?php esc_html_e('Fale com Administrador', 'juntaplay'); ?></a>
                                                <?php endif; ?>
                                            </div>
