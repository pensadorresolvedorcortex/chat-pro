    ...
    <script>
        (() => {
            ...
            const sendMessage = async () => {
                state.adminId = state.adminId || Number(config.adminId || 0);
                state.subscriberId = state.subscriberId || Number(config.subscriberId || config.currentUserId || 0);
                ...
                const activeGroupId = state.groupId
                    || Number(config.initialGroupId || 0)
                    || (Array.isArray(state.groups) ? Number(state.groups[0]?.id || 0) : 0)
                    || (Array.isArray(config.groups) ? Number(config.groups[0]?.id || 0) : 0)
                    || (Array.isArray(config.memberGroups) ? Number(config.memberGroups[0]?.id || 0) : 0);

                if (!activeGroupId) {
                    setStatus('<?php echo esc_js(__('Selecione um grupo para enviar mensagens.', 'juntaplay')); ?>');
                    if (input) {
                        input.focus();
                    }
                    return;
                ...
                try {
                    ...
                    // Ignore and fall through to restoring focus/input handling.
                }
