...
$pool_catalog_lookup = [];
foreach ($pool_catalog as $pool_item) {
    $pool_item_id = isset($pool_item['id']) ? (int) $pool_item['id'] : 0;
    if ($pool_item_id > 0) {
        $pool_catalog_lookup[$pool_item_id] = $pool_item;
    }
}
foreach ($pool_featured as $pool_item) {
    $pool_item_id = isset($pool_item['id']) ? (int) $pool_item['id'] : 0;
    if ($pool_item_id > 0 && !isset($pool_catalog_lookup[$pool_item_id])) {
        $pool_catalog_lookup[$pool_item_id] = $pool_item;
    }
}
                    ...
                    <div class="juntaplay-create-panels">
                                    ...
                                    <?php foreach ($pool_featured as $pool) :
                                        ...
                                        if (is_array($plans_raw)) {
                                            foreach ($plans_raw as $plan_index => $plan) {
                                                ...
                                                $status = isset($plan['status']) ? (string) $plan['status'] : 'active';
                                                if ($status !== 'active') {
                                                    continue;
                                                }
                                                $plans_active[] = [
                        ...
                        >
                            <div class="juntaplay-service-list__header">
                                ...
                                <p class="juntaplay-groups__catalog-empty"><?php echo esc_html__('Selecione a variação do serviço', 'juntaplay'); ?></p>
                            ...
                            <div class="juntaplay-service-list__footer">
                                <button type="button" class="juntaplay-service-list__all" data-group-plan-back>
                                    <?php echo esc_html__('Voltar aos serviços', 'juntaplay'); ?>
                                </button>
                                <button type="button" class="juntaplay-button juntaplay-button--primary" data-group-plan-next disabled>
                                    <?php echo esc_html__('Avançar', 'juntaplay'); ?>
                                </button>
                            </div>
                        </section>
                                ...
                                <div class="juntaplay-form__group">
                                    ...
                                    <select id="jp-group-pool" name="jp_profile_group_pool" class="juntaplay-form__input" data-group-pool-select>
                                        ...
                                        <?php foreach ($pool_choices as $pool_id => $pool_name) :
                                            ...
                                            $pool_data = $pool_catalog_lookup[$pool_id_int] ?? [];
                                            $pool_title = isset($pool_data['title']) && $pool_data['title'] !== ''
                                                ? (string) $pool_data['title']
                                                : (string) $pool_name;
                                            $pool_price = isset($pool_data['price']) ? (float) $pool_data['price'] : 0.0;
                                            $pool_category = isset($pool_data['category']) ? (string) $pool_data['category'] : '';
                                            $pool_excerpt = isset($pool_data['excerpt']) ? (string) $pool_data['excerpt'] : '';
                                            $pool_total = isset($pool_data['quotas_total']) ? (int) $pool_data['quotas_total'] : 0;
                                            $pool_start = isset($pool_data['quota_start']) ? (int) $pool_data['quota_start'] : 0;
                                            $pool_end = isset($pool_data['quota_end']) ? (int) $pool_data['quota_end'] : 0;
                                            $pool_icon_id = isset($pool_data['icon_id']) ? (int) $pool_data['icon_id'] : 0;
                                            $pool_icon = isset($pool_data['icon']) ? (string) $pool_data['icon'] : '';
                                            $pool_url = isset($pool_data['service_url']) ? (string) $pool_data['service_url'] : '';

                                            $plans_raw = $pool_id_int > 0 ? PoolsData::get_plans($pool_id_int) : [];
                                            $plans_active = [];
                                            if (is_array($plans_raw)) {
                                                foreach ($plans_raw as $plan_index => $plan) {
                                                    if (!is_array($plan)) {
                                        ...
                                        ?>
                                            <option
                                                value="<?php echo esc_attr((string) $pool_id); ?>"
                                                <?php selected((string) $pool_id, $current_pool); ?>
                                                data-pool-id="<?php echo esc_attr((string) $pool_id); ?>"
                                                data-pool-name="<?php echo esc_attr($pool_title); ?>"
                                                data-pool-price="<?php echo esc_attr((string) $pool_price); ?>"
                                                data-pool-category="<?php echo esc_attr($pool_category); ?>"
                                                ...
                                                data-pool-icon-id="<?php echo esc_attr((string) $pool_icon_id); ?>"
                                                data-pool-icon="<?php echo esc_url($pool_icon); ?>"
                                                data-pool-url="<?php echo esc_url($pool_url); ?>"
                                                data-pool-plans="<?php echo esc_attr(wp_json_encode($plans_active)); ?>"
                                            >
                                                <?php echo esc_html((string) $pool_name); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
