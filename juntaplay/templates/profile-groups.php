        ...
        (() => {
            ...
            const renderGroupList = () => {
                ...
                groupList.innerHTML = groups.map((group) => {
                    ...
                    return `
                        <li class="juntaplay-chat-card${isActive ? ' is-active' : ''}" data-chat-select="group" data-group-id="${group.id}" data-owner-id="${group.owner_id || ''}">
                            <span class="juntaplay-chat-card__avatar" aria-hidden="true">${avatar}</span>
                            <span class="juntaplay-chat-card__meta">
                                    ...
                                    <div class="juntaplay-group-card__footer">
                                        ...
                                        <div class="juntaplay-group-card__details-extended" hidden>
                                            ...
                                            <div class="juntaplay-group-card__details-actions">
                                                ...
                                                $chat_target_url = $messages_base_url !== '' ? $messages_base_url : $messages_endpoint;

                                                if ($membership_status === 'active' && $chat_target_url !== '') {
                                                    $chat_args = ['group_id' => $group_id];

                                                    if ($is_admin) {
                                                        $admin_chat_link = add_query_arg($chat_args, $chat_target_url);
                                                    } elseif ($owner_id !== $current_user_id && $owner_id > 0) {
                                                        $chat_id = \JuntaPlay\Data\GroupChats::ensure_chat($group_id, $current_user_id, $owner_id);
                                                        if ($chat_id > 0) {
                                                            $chat_args['chat'] = $chat_id;
                                                        }
                                                        $chat_link = add_query_arg($chat_args, $chat_target_url);
                                                    }
                                                }
                                                ?>
                                            ...
                                            <?php if (!$is_owner) : ?>
                                                ...
                                                <?php if ($complaints_open > 0) : ?>
                                                    <p class="juntaplay-group-card__complaint-open"><?php echo esc_html(sprintf(_n('Você possui %d disputa aberta.', 'Você possui %d disputas abertas.', $complaints_open, 'juntaplay'), $complaints_open)); ?></p>
                                                <?php endif; ?>
                                                <?php if ($complaints_total > 0) : ?>
                                                    <p class="juntaplay-group-card__complaint-open"><?php echo esc_html(sprintf(_n('Você já abriu %d disputa para este grupo.', 'Você já abriu %d disputas para este grupo.', $complaints_total, 'juntaplay'), $complaints_total)); ?></p>
                                                <?php endif; ?>
                                                ...
                                                <form id="jp-group-complaint-<?php echo esc_attr($group_id); ?>" class="juntaplay-group-complaint__form<?php echo $form_open ? ' is-open' : ' is-hidden'; ?>" method="post" enctype="multipart/form-data" data-group-complaint-form>
                                                    ...
                                                    ?>
                                                    <div class="juntaplay-form__group">
                                                        <label for="jp-group-complaint-reason-<?php echo esc_attr($group_id); ?>"><?php echo esc_html__('Motivo da disputa', 'juntaplay'); ?></label>
                                                    ...
                                                    <div class="juntaplay-group-complaint__actions">
                                                        <button type="submit" class="juntaplay-button juntaplay-button--primary"><?php echo esc_html__('Enviar disputa', 'juntaplay'); ?></button>
