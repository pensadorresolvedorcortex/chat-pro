        ...
        (() => {
            ...
            const selectSubscriber = (subscriber) => {
                if (subscriber?.group_id) {
                    state.groupId = Number(subscriber.group_id);
                }
                state.subscriberId = subscriber ? Number(subscriber.id) : 0;
                state.adminId = Number(
                    (subscriber && (subscriber.admin_id || subscriber.owner_id))
                        || state.adminId
                        || config.adminId
            ...
            const selectGroup = (group) => {
                ...
                state.adminId = group ? Number(group.owner_id || state.adminId || config.adminId || 0) : state.adminId;
            ...
            const renderGroupList = () => {
                ...
                groupList.innerHTML = groups.map((group) => {
                    ...
                    return `
                        <li class="juntaplay-chat-card${isActive ? ' is-active' : ''}" data-chat-select="group" data-group-id="${group.id}">
                            <span class="juntaplay-chat-card__avatar" aria-hidden="true">${avatar}</span>
                            <span class="juntaplay-chat-card__meta">
            ...
            const sendMessage = async () => {
                ...
                const groupId = Number(state.groupId || 0);
                const adminId = Number(state.adminId || 0);
                const subscriberId = Number(state.subscriberId || 0);
                const participantId = Number(config.currentUserId || (state.isAdmin ? adminId : subscriberId) || 0);

                if (!groupId || !adminId || !subscriberId || !participantId) {
                    console.error('Tentativa de envio sem conversa ativa');
                    setStatus('<?php echo esc_js(__('Selecione um grupo e um participante para enviar mensagens.', 'juntaplay')); ?>');
                    if (input) {
                        input.focus();
                    }
                    return;
                ...
                try {
                    ...
                    if (response.ok) {
                        input.value = '';
                        const updatedMessages = payload?.messages || payload?.data?.messages;
                        if (updatedMessages) {
