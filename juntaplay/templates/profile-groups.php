...
<style>
    ...
    .jp-inbox-button {
        ...
        background: linear-gradient(135deg, rgba(255, 255, 255, 0.85), rgba(226, 232, 240, 0.9));
        color: #0f172a;
        font-weight: 700;
        cursor: pointer;
    ...
    .jp-inbox-button:focus-visible {
        outline: 2px solid #6366f1;
        outline-offset: 2px;
    }

    .jp-inbox-button:hover {
        transform: translateY(-1px);
        box-shadow: 0 14px 34px rgba(99, 102, 241, 0.16);
    }

    .jp-inbox-button__icon {
        width: 18px;
        height: 18px;
        color: #0ea5e9;
    }

    .jp-inbox-button__badge {
        min-width: 18px;
        height: 18px;
        padding: 0 5px;
        border-radius: 999px;
    ...
    .jp-inbox-drawer.is-open {
        transform: translateX(0);
    }

    .jp-inbox-drawer__header {
        display: flex;
        align-items: center;
        justify-content: space-between;
        padding: 16px;
            ...
            <ul class="juntaplay-profile__list" role="list">
                <li class="juntaplay-profile__row juntaplay-profile__row--custom" data-section="juntaplay-chat">
                    ...
                    <div class="juntaplay-profile__custom">
                        <div class="jp-inbox-trigger-row">
                            <button
                                type="button"
                                class="jp-inbox-button"
                                data-jp-inbox-button
                                aria-haspopup="dialog"
                                aria-expanded="false"
                            >
                                <svg class="jp-inbox-button__icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                                    <path d="M5 5h14a1 1 0 0 1 1 1v9.5a1 1 0 0 1-1 1H7.4L4 20.5V6a1 1 0 0 1 1-1Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" />
                                    <path d="M8 9h8m-8 4h5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" />
                                </svg>
                                <span><?php esc_html_e('Conversas', 'juntaplay'); ?></span>
                                <span class="jp-inbox-button__badge" data-jp-inbox-badge hidden></span>
                            </button>
                        </div>
                        <div class="jp-inbox-overlay" data-jp-inbox-overlay hidden role="presentation"></div>
                        <section
                            class="jp-inbox-drawer"
                            data-jp-inbox-drawer
                            role="dialog"
                            aria-modal="true"
                            aria-label="<?php echo esc_attr__('Conversas', 'juntaplay'); ?>"
                            hidden
                        >
                            <header class="jp-inbox-drawer__header">
                                <h3><?php esc_html_e('Conversas', 'juntaplay'); ?></h3>
                                <button type="button" class="jp-inbox-close" data-jp-inbox-close aria-label="<?php echo esc_attr__('Fechar', 'juntaplay'); ?>">
                                    ✕
                                </button>
                            </header>
                            <div class="jp-inbox-drawer__body" data-jp-inbox-content>
                                <div class="jp-inbox-hidden" data-jp-inbox-wrapper>
                                    <?php echo do_shortcode('[juntaplay_online]'); ?>
                                </div>
                            </div>
                        </section>
...
<?php if ($is_chat_section) : ?>
    <script>
        (() => {
            ...
            const inboxButton = document.querySelector('[data-jp-inbox-button]');
            const inboxOverlay = document.querySelector('[data-jp-inbox-overlay]');
            const inboxDrawer = document.querySelector('[data-jp-inbox-drawer]');
            const inboxClose = document.querySelector('[data-jp-inbox-close]');
            const inboxBadge = document.querySelector('[data-jp-inbox-badge]');
            ...
            const chatMessagesBase = config.messagesBase || '';
            ...
            const setBodyScroll = (locked) => {
                document.body.classList.toggle('jp-inbox-open', !!locked);
                if (locked) {
            ...
            const openInbox = () => {
                if (!inboxDrawer || !inboxOverlay) return;
                inboxDrawer.hidden = false;
                inboxOverlay.hidden = false;
                requestAnimationFrame(() => {
                    inboxDrawer.classList.add('is-open');
                    inboxOverlay.classList.add('is-open');
                });
                inboxDrawer.setAttribute('tabindex', '-1');
                inboxDrawer.focus({ preventScroll: true });
                setBodyScroll(true);
                if (inboxButton) {
            ...
            const closeInbox = () => {
                if (inboxDrawer) {
                    inboxDrawer.classList.remove('is-open');
                }
                if (inboxOverlay) {
                    inboxOverlay.classList.remove('is-open');
                }
                setBodyScroll(false);
                if (inboxButton) {
                    inboxButton.setAttribute('aria-expanded', 'false');
                    inboxButton.focus({ preventScroll: true });
                }
                setTimeout(() => {
                    if (inboxDrawer) inboxDrawer.hidden = true;
                    if (inboxOverlay) inboxOverlay.hidden = true;
                }, 200);
            };

            const updateInboxBadge = (optimisticSubtract = 0) => {
                const contextUrl = config?.rest?.context || '';
                const nonce = config?.nonce || '';
                if (!contextUrl || !inboxBadge) return;
                ...
                fetch(contextUrl, {
                    ...
                    .then((data) => {
                        ...
                        const total = threads.reduce(
                            (sum, entry) => sum + Number(entry?.unread_count || entry?.unread || 0),
                            0
                        );

                        const adjustedTotal = Math.max(0, total - Math.max(0, Number(optimisticSubtract || 0)));

                        if (adjustedTotal > 0) {
                            inboxBadge.hidden = false;
                            inboxBadge.textContent = adjustedTotal > 9 ? '9+' : String(adjustedTotal);
                        } else {
            ...
            document.addEventListener('keydown', (event) => {
                if (event.key === 'Escape' && inboxDrawer && !inboxDrawer.hidden) {
                    closeInbox();
                }
            });

            const inboxContent = document.querySelector('[data-jp-inbox-content]');
            if (inboxContent) {
                inboxContent.addEventListener('click', (event) => {
                    const anchor = event.target.closest('a[href]');
                    if (anchor && anchor.href.includes('section=juntaplay-chat')) {
                        closeInbox();
                    }
                });
            }

            const onlineRoot = inboxDrawer ? inboxDrawer.querySelector('.juntaplay-online') : null;
            if (onlineRoot) {
                onlineRoot.addEventListener(
                    'juntaplay-online-thread-selected',
                    (event) => {
                        const href = event?.detail?.href || '';
                        const unreadDelta = Number(event?.detail?.thread?.unread_count || event?.detail?.thread?.unread || 0);
                        updateInboxBadge(unreadDelta);
                        closeInbox();
                        if (href) {
            ...
            const emitNotifications = (threads = []) => {
                ...
                const items = threads.map((threadItem) => {
                    ...
                    const name = threadItem.counterpart_name
                        || (currentUserId === Number(threadItem.admin_id) ? threadItem.subscriber_name : threadItem.admin_name)
                        || threadItem.owner_name
                        || threadItem.sender_name
                        || '';
                    const avatar = threadItem.counterpart_avatar
                        || (currentUserId === Number(threadItem.admin_id) ? threadItem.subscriber_avatar : threadItem.admin_avatar)
                        || threadItem.owner_avatar
                        || threadItem.sender_avatar
                        || '';
                    const preview = threadItem.last_message || '';
                    const timestamp = threadItem.updated_at || threadItem.last_message_at || threadItem.created_at || '';
                    const label = name ? `${name} <?php echo esc_js(__('enviou uma mensagem.', 'juntaplay')); ?>` : '<?php echo esc_js(__('Nova mensagem no chat', 'juntaplay')); ?>';
                    const search = new URLSearchParams();
                    search.set('section', 'juntaplay-chat');
                    if (threadItem.group_id) search.set('group_id', threadItem.group_id);
                    if (threadItem.admin_id) search.set('admin_id', threadItem.admin_id);
