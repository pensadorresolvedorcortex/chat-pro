...
$group_admin_id = isset($group['admin_id']) ? (int) $group['admin_id'] : 0;
$membership_role_key = $membership_role !== '' ? strtolower($membership_role) : '';
$is_group_admin = $is_owner
    || ($current_user_id > 0 && $group_admin_id > 0 && $current_user_id === $group_admin_id)
    || in_array($membership_role_key, ['owner', 'manager', 'admin', 'administrator'], true);
// Admin-only group cancellation should only render for owners/admins on active groups.
$can_admin_cancel = $is_group_admin && $group_status === 'active';
...
<script>
    (() => {
        ...
        const toggleAdminCancelModal = (modal, shouldOpen) => {
            if (!modal) {
                return;
            }
        ...
        document.addEventListener('click', (event) => {
            ...
            if (adminCancelTrigger) {
                ...
                toggleAdminCancelModal(modal, true);
                return;
            }

            const adminCancelClose = event.target.closest('[data-modal-close]');
            const adminCancelModal = adminCancelClose
                ? adminCancelClose.closest('[data-group-admin-cancel-modal]')
                : null;
            if (adminCancelModal) {
                event.preventDefault();
                event.stopPropagation();
                toggleAdminCancelModal(adminCancelModal, false);
            }
        });
