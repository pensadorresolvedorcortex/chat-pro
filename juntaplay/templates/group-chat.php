...
(() => {
    ...
    function renderThreads(items) {
        ...
        items.forEach((item) => {
            ...
            const counterpartName = item.display_name_other || item.admin_name || item.member_name || '<?php echo esc_html__('Administrador', 'juntaplay'); ?>';
            li.innerHTML = `
                <div class="juntaplay-chat__thread-title">${item.group_title || '<?php echo esc_html__('Grupo', 'juntaplay'); ?>'}</div>
                <div class="juntaplay-chat__thread-meta">${counterpartName}</div>
    ...
    function loadChat(chatId, context) {
        ...
        adminEl.textContent = context.role === 'member'
            ? (context.display_name_other || context.admin_name || '<?php echo esc_html__('Administrador', 'juntaplay'); ?>')
            : (context.member_name || context.display_name_other || '');
        fetchMessages();
    }
