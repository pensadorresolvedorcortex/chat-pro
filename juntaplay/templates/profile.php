...
if ($user && $user->exists()) {
    $avatar_html = get_avatar($user->ID, 96, '', '', [
        'class' => 'juntaplay-profile__avatar-img',
    ]);
}
            ...
            <div class="juntaplay-profile__quick-nav" role="navigation" aria-label="<?php esc_attr_e('Atalhos do perfil', 'juntaplay'); ?>">
                <?php foreach ($profile_quick_actions as $quick) : ?>
                    <button type="button"
                        ...
                        data-profile-quick
                        data-profile-quick-category="<?php echo esc_attr($quick['category']); ?>"
                        data-profile-quick-tab="<?php echo esc_attr($quick['tab']); ?>"
                    >
                        <span class="juntaplay-profile__quick-icon"><?php echo juntaplay_profile_icon($quick['icon'] ?? 'user'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
                        <span class="juntaplay-profile__quick-text">
                            <span class="juntaplay-profile__quick-label"><?php echo esc_html($quick['label'] ?? ''); ?></span>
                            <?php if (!empty($quick['summary'])) : ?>
                                <span class="juntaplay-profile__quick-summary"><?php echo esc_html((string) $quick['summary']); ?></span>
                            <?php endif; ?>
                        </span>
                    </button>
                <?php endforeach; ?>
