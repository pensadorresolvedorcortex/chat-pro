...
if (!function_exists('juntaplay_profile_icon')) {
    function juntaplay_profile_icon(string $icon): string
    {
        $icons = [
            ...
            'message'   => '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M4 5h16a1 1 0 0 1 1 1v11a1 1 0 0 1-1 1H7l-4 4V6a1 1 0 0 1 1-1Z" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/><path d="M8 10h8M8 13h5" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>',
...
if (!function_exists('juntaplay_profile_render_item')) {
    ...
    function juntaplay_profile_render_item(string $section_key, array $section, ?string $active_section, array $profile_errors): void
    {
        ...
        if (isset($section['html'])) {
            $custom_html = $section['html'];
        } elseif (isset($section['content'])) {
            // Some custom items may provide their rendered output under a "content" key.
            $custom_html = $section['content'];
        }

        $chat_payload = null;
        if ($section_key === 'juntaplay-chat') {
            if (isset($section['context']['chat_payload']) && is_array($section['context']['chat_payload'])) {
                $chat_payload = $section['context']['chat_payload'];
            } elseif (is_array($custom_html)) {
                $chat_payload = $custom_html;
            }

            if (is_array($chat_payload) && isset($chat_payload['html'])) {
                $custom_html = $chat_payload['html'];
            }

            if (is_array($custom_html)) {
                $custom_html = '';
            }

            $chat_group = 0;
            if (is_array($chat_payload) && isset($chat_payload['group_id'])) {
                $chat_group = (int) $chat_payload['group_id'];
            }

            $chat_shortcode = '[juntaplay_chat';
            if ($chat_group > 0) {
                $chat_shortcode .= ' group_id="' . $chat_group . '"';
            }
            $chat_shortcode .= ']';

            $custom_html = do_shortcode($chat_shortcode);
        }

        if (!is_string($custom_html)) {
            $custom_html = (string) $custom_html;
        }
        ...
        if ($type === 'custom') {
            ...
            <li class="<?php echo esc_attr(implode(' ', $row_classes)); ?>" data-section="<?php echo esc_attr($section_key); ?>">
                ...
                <div class="juntaplay-profile__custom">
                    ...
                    if ($section_key === 'juntaplay-chat' && is_array($chat_payload)) {
                        $chat_state   = isset($chat_payload['state']) ? (string) $chat_payload['state'] : '';
                        $chat_members = $chat_payload['members'] ?? [];
                        $chat_group   = isset($chat_payload['group_id']) ? (int) $chat_payload['group_id'] : 0;
                        $chat_html    = $custom_html;

                        if ($chat_state === 'admin_grid') {
                            /** @var array<int,array<string,mixed>> $chat_members */
...
$profile_categories = [
    ...
    'support' => [
        'label'       => __('Suporte', 'juntaplay'),
        'description' => __('Centralize as reclamações e acompanhe o andamento dos tickets.', 'juntaplay'),
        'icon'        => 'ticket',
        'tabs'        => [
            'support_complaints' => [
...
// Garantia explícita de mapeamento para o chat mesmo que algum filtro altere os grupos
if (!isset($section_to_tab['juntaplay-chat']) && isset($profile_categories['juntaplay_chat']['tabs']['juntaplay_chat_main'])) {
    $section_to_tab['juntaplay-chat'] = [
        'category' => 'juntaplay_chat',
        'tab'      => 'juntaplay_chat_main',
    ];
}
...
$help_url = home_url('/central-de-ajuda/');
