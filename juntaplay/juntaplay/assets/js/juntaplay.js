(function ($) {
    ...
    function initNotifications() {
        ...
        function syncCount(unread) {
            var totalUnread = parseInt(unread, 10);
            if (isNaN(totalUnread)) {
                totalUnread = 0;
            }

            $('[data-jp-notifications]').removeAttr('data-count');

            var $avatar = $('[data-jp-account-toggle]').first().find('.juntaplay-header__avatar');
            if (!$avatar.length) {
                return;
            }

            var badgeUnread = totalUnread;
            var $root = $avatar.closest('[data-jp-header]');
            if (!$root.length) {
                $root = $(document.body);
            }

            var chatUnreadAttr = $root.find('[data-jp-notifications-root]').first().data('jpChatUnread');
            var chatUnread = parseInt(chatUnreadAttr, 10);
            if (isNaN(chatUnread)) {
                chatUnread = 0;
            }

            badgeUnread = Math.max(chatUnread, badgeUnread);

            var $badge = $avatar.find('[data-jp-avatar-badge]');
            if (badgeUnread > 0) {
                if (!$badge.length) {
                    $badge = $('<span class="juntaplay-header__avatar-badge" data-jp-avatar-badge></span>');
                    $avatar.append($badge);
                }

                $badge.text(badgeUnread > 9 ? '9+' : badgeUnread);
            } else {
                $badge.remove();
            }
        }
        ...
        $bells.each(function () {
            ...
            var isLoaded = false;
            var isLoading = false;
            var defaultEmptyMarkup = '<li class="juntaplay-notifications__empty">' + __('Nenhuma notificação por enquanto.', 'juntaplay') + '</li>';
            var chatNotifications = [];
            var chatData = $list.data('jpChatNotifications');

            if (typeof chatData === 'string' && chatData !== '') {
                try {
                    chatNotifications = JSON.parse(chatData);
                } catch (error) {
                    chatNotifications = [];
                }
            } else if ($.isArray(chatData)) {
                chatNotifications = chatData;
            }

            if (chatNotifications.length && typeof chatNotifications[0] !== 'object') {
                chatNotifications = [];
            }

            var emptyMarkup = defaultEmptyMarkup;

            function renderChatNotifications(items) {
                return items.map(function (entry) {
                    var text = (entry && entry.message) ? entry.message.toString() : '';
                    var href = (entry && entry.url) ? entry.url.toString() : '';

                    if (href) {
                        return '<li class="juntaplay-notifications__item" data-chat-notification><a href="' + href + '">' + text + '</a></li>';
                    }

                    return '<li class="juntaplay-notifications__item" data-chat-notification>' + text + '</li>';
                }).join('');
            }

            if (chatNotifications.length) {
                $list.html(renderChatNotifications(chatNotifications));
                isLoaded = true;
                var chatUnread = parseInt($panel.closest('[data-jp-notifications-root]').data('jpChatUnread'), 10);

                if (!isNaN(chatUnread) && chatUnread > 0) {
                    syncCount(chatUnread);
                }
            }
