...
if ($current_user_id && function_exists('juntaplay_chat_threads_for_user')) {
    $threads = juntaplay_chat_threads_for_user($current_user_id);

    if (is_array($threads)) {
        foreach ($threads as $thread) {
            $chat_unread_count += isset($thread['unread_count']) ? (int) $thread['unread_count'] : 0;
        }
    }
}

$chat_unread_count = max(0, (int) $chat_unread_count);
...
foreach ($unread_messages as $message_data) {
    ...
    if ($sender_id > 0) {
        ...
        if ($sender_user) {
            $sender_name = trim((string) $sender_user->display_name);
        }
    }

    if ($sender_name === '') {
        $sender_name = isset($message_data['sender']) ? (string) $message_data['sender'] : '';
    }

    if ($sender_name === '') {
        $sender_name = __('Administrador', 'juntaplay');
    }
    ...
    <div class="juntaplay-header__inner">
        ...
        <?php else : ?>
            <div class="juntaplay-header__summary">
                <div class="juntaplay-header__notifications" data-jp-notifications-root data-jp-chat-unread="<?php echo esc_attr($chat_unread_count); ?>">
                    <button type="button" class="juntaplay-notification-bell" data-jp-notifications aria-haspopup="true" aria-expanded="false">
                    ...
                    <div class="juntaplay-notifications" data-jp-notifications-panel aria-hidden="true">
                        ...
                        <ul class="juntaplay-notifications__list" data-jp-notifications-list data-jp-chat-notifications='<?php echo esc_attr(wp_json_encode($notification_messages)); ?>'>
                            <?php if ($notification_messages) : ?>
                                <?php foreach ($notification_messages as $notification_message) :
                                    $notice_text = isset($notification_message['message']) ? (string) $notification_message['message'] : '';
                                    $notice_url  = isset($notification_message['url']) ? (string) $notification_message['url'] : '';
                                    ?>
                                    <li class="juntaplay-notifications__item" data-chat-notification>
                                        <?php if ($notice_url !== '') : ?>
                                            <a href="<?php echo esc_url($notice_url); ?>"><?php echo esc_html($notice_text); ?></a>
                                        <?php else : ?>
                    ...
                    <button type="button" class="juntaplay-header__trigger" data-jp-account-toggle aria-haspopup="true" aria-expanded="false">
                        <span class="juntaplay-header__avatar">
                            ...
                            <?php if ($chat_unread_count > 0) :
                                $badge_label = $chat_unread_count > 9 ? '9+' : (string) $chat_unread_count;
                                ?>
                                <span class="juntaplay-header__avatar-badge" data-jp-avatar-badge><?php echo esc_html($badge_label); ?></span>
                            <?php endif; ?>
                        </span>
