...
class Shortcodes
{
    ...
    public function groups_slider($atts = []): string
    {
        ...
        $hero_title = $atts['title'] !== '' ? (string) $atts['title'] : __('Mais de<br>100<br>assinaturas<br>partilháveis', 'juntaplay');
        $hero_cta_label = __('Ver todas as assinaturas', 'juntaplay');

        ob_start();
        ?>
        <section class="jp-groups-slider" id="<?php echo esc_attr($slider_id); ?>">
            <div class="jp-groups-slider__inner">
                <div class="jp-groups-slider__hero">
                    <h2 class="jp-groups-slider__hero-title"><?php echo wp_kses_post($hero_title); ?></h2>
                    <a class="jp-groups-slider__hero-cta" href="<?php echo esc_url($directory_url); ?>">
                        <?php echo esc_html($hero_cta_label); ?>
                    </a>
                </div>
                <div class="jp-groups-slider__carousel">
                    ...
                    <div class="jp-groups-slider__track" role="list">
                        <?php foreach ($items as $index => $group) : ?>
                            ...
                            if ($cover_url === '') {
                                $cover_url = $pool_slug !== '' ? ServiceIcons::get($pool_slug) : '';
                            }
                            if ($cover_url === '') {
                                $cover_url = ServiceIcons::resolve($pool_slug, $service !== '' ? $service : $title, $service_url);
                            }
                            if ($cover_url === '') {
                                $cover_url = ServiceIcons::fallback();
                            }

                            $initial_source = $title !== '' ? $title : $service;
                            ...
                            $price_display = $price_label !== '' ? $price_label : '—';
                            $price_is_empty = $price_label === '';

                            $link = $this->resolve_group_link($group);
                            $link = $link !== '' ? $link : '#';

                            $variant_class = $index % 2 === 0 ? 'jp-groups-slider__card--green' : 'jp-groups-slider__card--red';
                            $display_title = $title !== '' ? $title : $service;
                            if ($display_title === '') {
                                $display_title = __('Assinatura', 'juntaplay');
                            }

                            $data_attrs = $group_id > 0 ? sprintf(' data-group-id="%d" data-jp-group-open', $group_id) : ' data-jp-group-open';
                            ?>
                            <article class="jp-groups-slider__card <?php echo esc_attr($variant_class); ?>" role="listitem" <?php echo $group_id > 0 ? 'data-group-card data-group-id="' . esc_attr((string) $group_id) . '"' : 'data-group-card'; ?>>
                                <a class="jp-groups-slider__card-top" href="<?php echo esc_url($link); ?>" <?php echo wp_kses_post($data_attrs); ?>>
                                    <span class="jp-groups-slider__card-title"><?php echo esc_html($display_title); ?></span>
                                </a>
                                <div class="jp-groups-slider__card-bottom">
                                    <div class="jp-groups-slider__avatars" aria-hidden="true">
                                        <span class="jp-groups-slider__avatar<?php echo $cover_url !== '' ? ' has-image' : ''; ?>"<?php echo $cover_url !== '' ? ' style="background-image: url(' . esc_url($cover_url) . ')"' : ''; ?>>
                                            <?php if ($cover_url === '' && $icon_initial !== '') : ?>
                                                <?php echo esc_html($icon_initial); ?>
                                            <?php endif; ?>
                                        </span>
                                        <span class="jp-groups-slider__avatar"></span>
                                        <span class="jp-groups-slider__avatar"></span>
                                    </div>
                                    <?php if ($category_label !== '') : ?>
                                        <div class="jp-groups-slider__tag"><?php echo esc_html($category_label); ?></div>
                                    <?php endif; ?>
                                    <div class="jp-groups-slider__price<?php echo $price_is_empty ? ' is-empty' : ''; ?>">
                                        <?php echo esc_html($price_display); ?>
                                        <?php if (!$price_is_empty) : ?>
                                            <span class="jp-groups-slider__price-label"><?php esc_html_e('/mês', 'juntaplay'); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <button type="button" class="jp-groups-slider__cta"<?php echo wp_kses_post($data_attrs); ?>>
