...
class Hooks
{
    private const GROUP_ITEM_PROCESSED_META = '_juntaplay_group_item_processed';
    private const QUOTAS_PAID_META = '_juntaplay_quota_paid';
    ...
    private function should_handle_paid_flow(WC_Order $order, string $new_status): bool
    {
        if (!in_array($new_status, ['processing', 'completed'], true)) {
            return false;
        }

        $split_processed = (string) $order->get_meta('_juntaplay_split_processed', true) !== ''
            || PaymentSplits::has_completed_for_order($order->get_id());

        if ($this->is_mercado_pago_order($order)) {
            return $this->order_paid_via_callback($order)
                || $split_processed
                || $this->has_gateway_payment_reference($order)
                || $order->is_paid();
        }
    ...
    public function add_meta_to_item($item, $cart_item_key, $values, $order): void
    {
        ...
        if (!empty($values['juntaplay_group'])) {
            ...
            $fee_per_member = isset($data['fee_per_member']) ? (float) $data['fee_per_member'] : 0.0;
            ...
            if ($fee_per_member > 0) {
                $item->add_meta_data('JuntaPlay Taxa do site', wc_price($fee_per_member));
                $item->add_meta_data('_juntaplay_fee_per_member', $fee_per_member, true);
            }
    ...
    public function on_order_status_changed($order_id, $from, $to, $order): void
    {
        ...
        $paid_flow = $this->should_handle_paid_flow($order, (string) $to);

        if (in_array($to, ['processing', 'completed'], true) && $paid_flow) {
        ...
        foreach ($order->get_items() as $item) {
            ...
            $group_id = (int) $item->get_meta('_juntaplay_group_id', true);
            if ($group_id > 0) {
                if ($paid_flow || in_array($to, ['failed', 'cancelled', 'refunded'], true)) {
                    $this->handle_group_item($order, $item, $to, $paid_flow);
                }
            ...
            if (in_array($to, ['processing', 'completed'], true) && $paid_flow) {
                $already_paid = (string) $item->get_meta(self::QUOTAS_PAID_META, true) === 'yes';
                if ($already_paid) {
                    continue;
                }

                Quotas::pay($pool_id, $numbers, $order_id, (int) $order->get_user_id());
                $item->update_meta_data(self::QUOTAS_PAID_META, 'yes');
                $item->save();
            }
     ...
     */
    public function on_split_completed($order, array $processed, array $context): void
    {
        if (!($order instanceof WC_Order)) {
            return;
        }
        ...
        foreach ($order->get_items() as $item) {
            ...
            $this->handle_group_item($order, $item, $effective_status, true);
        }
    }
    ...
    private function handle_group_item(WC_Order $order, WC_Order_Item_Product $item, string $new_status, bool $paid_flow): void
    {
        $group_id = (int) $item->get_meta('_juntaplay_group_id', true);
        $user_id  = (int) $order->get_user_id();
        ...
        if (in_array($new_status, ['processing', 'completed'], true)) {
            ...
            $already_processed = (string) $item->get_meta(self::GROUP_ITEM_PROCESSED_META, true) === 'yes';
            $has_membership    = GroupMembers::user_has_membership($group_id, $user_id);

            if (!$has_membership) {
                $available_slots = Groups::get_available_slots($group_id);

                if ($available_slots !== null && $available_slots <= 0) {
                    $item->update_meta_data('_juntaplay_group_capacity_blocked', 'yes');
                    $item->save();
                    $order->add_order_note(__('Assinatura não adicionada ao grupo: vagas esgotadas.', 'juntaplay'));

                    if (!$already_processed) {
                        $item->update_meta_data(self::GROUP_ITEM_PROCESSED_META, 'yes');
                        $item->save();
                    }
        ...
        if (in_array($new_status, ['failed', 'cancelled', 'refunded'], true)) {
            GroupMembers::mark_exited($group_id, $user_id, current_time('mysql'));

            GroupMembershipEvents::log(
                $group_id,
                $user_id,
                GroupMembershipEvents::TYPE_EXITED,
                '',
                [
                    'order_id' => $order->get_id(),
                    'item_id'  => $item->get_id(),
    ...
    public function render_thankyou_message($order_id): void
    {
        ...
        $paid_via_mp = $this->is_mercado_pago_order($order)
            ? ($this->order_paid_via_callback($order)
                || (string) $order->get_meta('_juntaplay_split_processed', true) !== ''
                || PaymentSplits::has_completed_for_order($order->get_id())
                || $this->has_gateway_payment_reference($order))
            : $order->is_paid();

        if (!$paid_via_mp || !in_array($status, ['processing', 'completed'], true)) {
            return;
        }
