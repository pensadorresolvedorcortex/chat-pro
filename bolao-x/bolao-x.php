...
const VERSION = '2.8.46';
    ...
    private function build_results_table( $rows ) {
        ...
        $out .= '<th class="col-drawn">' . esc_html__( 'Números sorteados', self::TEXT_DOMAIN ) . '</th>';
        $out .= '<th class="col-hits">' . esc_html__( 'Acertos', self::TEXT_DOMAIN ) . '</th></tr></thead><tbody>';
        $idx  = 1;
        foreach ( $rows as $row ) {
            $num   = str_pad( strval( $idx++ ), 2, '0', STR_PAD_LEFT );
            $out  .= '<tr' . $row['class'] . '><td class="col-index">' . $num . '</td><td class="col-name">' . $row['name'] . '</td><td class="col-contest">' . esc_html( $row['contest'] ) . '</td><td class="bolaox-numlist col-bets">' . $row['numbers'] . '</td><td class="bolaox-numlist col-drawn">' . $row['drawn'] . '</td><td class="col-hits">' . $row['hits'] . '</td></tr>';
        }
        $out .= '</tbody></table></div>';
        return $out;
    }
