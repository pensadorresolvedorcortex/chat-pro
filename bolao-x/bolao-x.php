...
const VERSION = '2.8.80';
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score', $prizes = array(), $winners = array(), $fund = '' ) {
        ...
        $out .= '<h3 class="bolaox-section-title">' . esc_html__( 'VENCEDORES COM 8 PONTOS', self::TEXT_DOMAIN ) . '</h3>';
    ...
    private function build_prize_table( $hits10, $hits9, $hits8, $winners_low, $values, $winners, $fund, $editable = true ) {
        ...
        $out .= $this->build_prize_row( __( '8 pontos', self::TEXT_DOMAIN ), '8', $hits8, $values, $winners, $editable );
    ...
    private function export_winners_pdf( $result, $contest_id, $contest_title, $order_by = 'score', $prizes = array(), $winners = array(), $fund = '' ) {
        ...
        $non_winners = array_filter( $rows, function ( $r ) { return $r['hits'] < 8; } );
        $min_hits = $non_winners ? min( wp_list_pluck( $non_winners, 'hits' ) ) : 0;

        $hits10 = array_filter( $rows, function ( $r ) { return $r['hits'] === 10; } );
        $hits9  = array_filter( $rows, function ( $r ) { return $r['hits'] === 9; } );
        $hits8  = array_filter( $rows, function ( $r ) { return $r['hits'] === 8; } );
        $winners_low  = array_filter( $non_winners, function ( $r ) use ( $min_hits ) { return $r['hits'] === $min_hits; } );

        $repeated = $this->get_repeated_numbers_by_date( $contest_id );

        $pdf->AddPage();
        $logo = 'https://www.bolaox.com.br/wp-content/uploads/2025/06/bolaox.png';
        $pdf->Image( $logo, 10, 6, 30 );
        $pdf->Ln( 20 );
        $primary = array( 30, 115, 76 );
        $pdf->SetFont( 'Arial', 'B', 14 );
        ...
        $pdf->Cell( 0, 8, __( 'DEZENAS SORTEADAS', self::TEXT_DOMAIN ), 0, 1, 'L', true );
        $pdf->SetTextColor( 0, 0, 0 );
        $pdf->SetFont( 'Arial', '', 11 );
        $this->pdf_numbers_line( $pdf, $drawn, $drawn, true );
        $pdf->Ln( 5 );

        $this->pdf_table_section( $pdf, __( 'VENCEDORES COM 10 PONTOS', self::TEXT_DOMAIN ), $hits10 );
        $this->pdf_table_section( $pdf, __( 'VENCEDORES COM 9 PONTOS', self::TEXT_DOMAIN ), $hits9 );
        $this->pdf_table_section( $pdf, __( 'VENCEDORES COM 8 PONTOS', self::TEXT_DOMAIN ), $hits8 );
        $this->pdf_table_section( $pdf, __( 'MENOS PONTOS', self::TEXT_DOMAIN ), $winners_low );
        ...
        $pdf->SetFillColor( $primary[0], $primary[1], $primary[2] );
        $pdf->SetTextColor( 255, 255, 255 );
        $pdf->Cell( 0, 8, __( 'NÚMEROS REPETIDOS', self::TEXT_DOMAIN ), 0, 1, 'L', true );
        $pdf->SetTextColor( 0, 0, 0 );
        if ( $repeated ) {
            foreach ( $repeated as $row ) {
                $pdf->SetFont( 'Arial', '', 11 );
                $pdf->Cell( 0, 8, $row['date'] . ':', 0, 1 );
                $this->pdf_numbers_line( $pdf, $row['numbers'], $row['numbers'], true );
                $pdf->Ln( 2 );
            }
        } else {
            $pdf->SetFont( 'Arial', '', 11 );
            $pdf->Cell( 0, 8, __( 'Nenhum número repetido.', self::TEXT_DOMAIN ), 0, 1 );
        }
    ...
    private function export_winners_html( $result, $contest_id, $contest_title, $order_by = 'score', $prizes = array(), $winners = array(), $fund = '' ) {
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $counts      = array_count_values( $res_numbers );
        $args        = array(
            'post_type'   => 'bolaox_aposta',
        ...
        foreach ( $posts as $p ) {
            ...
            foreach ( $nums as $n ) {
                ...
                $display_nums[] = sprintf( '<span class="%s"><span class="bx-val">%s</span><span class="bx-count">%s</span></span>', esc_attr( implode( ' ', $classes ) ), esc_html( $n ), $bet_counts[ $n ] > 1 ? 'x' . intval( $bet_counts[ $n ] ) : '' );
            }
            $rows[]  = array(
                'name'    => $p->post_title,
                'contest' => $contest_title,
        ...
        header( 'Content-Disposition: attachment; filename="bolao-relatorio.html"' );
        $css = esc_url( plugins_url( 'assets/css/bolao-x.css', __FILE__ ) );
        echo "<html><head><meta charset='utf-8'><link rel='stylesheet' href='$css'></head><body class='bolaox-report'>";
        $logo = esc_url( 'https://www.bolaox.com.br/wp-content/uploads/2025/06/bolaox.png' );
        echo "<div class='bolaox-report-logo'><img src='$logo' alt='Bolão X'></div>";
        echo '<h1 style="text-align:center">' . esc_html( sprintf( __( 'Relatório do Concurso %s', self::TEXT_DOMAIN ), $contest_title ) ) . '</h1>';

        echo '<h2>' . esc_html__( 'DEZENAS SORTEADAS', self::TEXT_DOMAIN ) . '</h2>';
        echo '<p class="bolaox-numlist">' . $drawn . '</p>';

        echo $this->build_prize_table( $hits10, $hits9, $hits8, $winners_low, $prizes, $winners, $fund, false );

        if ( $hits10 ) {
            echo '<h2>' . esc_html__( 'VENCEDORES COM 10 PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_html_table( $hits10, false );
        }
        if ( $hits9 ) {
            echo '<h2>' . esc_html__( 'VENCEDORES COM 9 PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_html_table( $hits9 );
        }
        if ( $hits8 ) {
            echo '<h2>' . esc_html__( 'VENCEDORES COM 8 PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_html_table( $hits8 );
        }
        if ( $winners_low ) {
            echo '<h2>' . esc_html__( 'MENOS PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_html_table( $winners_low, false );
        }

        echo '<h2>' . esc_html__( 'TODAS AS APOSTAS', self::TEXT_DOMAIN ) . '</h2>';
        echo $this->build_html_table( $rows, false );
        ...
        if ( $repeated ) {
            foreach ( $repeated as $row ) {
                ...
                echo '<div class="bolaox-repeat-date"><strong>' . esc_html( $row['date'] ) . '</strong>: ' . $list . '</div>';
            }
        } else {
            echo '<p>' . esc_html__( 'Nenhum número repetido.', self::TEXT_DOMAIN ) . '</p>';
        }

        echo '<div class="bolaox-print-wrapper"><button class="bolaox-print-btn" onclick="window.print()">' . esc_html__( 'Imprimir', self::TEXT_DOMAIN ) . '</button></div>';
        echo '</body></html>';
        exit;
    }

    private function build_html_table( $rows, $include_drawn = true ) {
        $out  = '<table class="bolaox-table" border="1" cellpadding="4" cellspacing="0" width="100%">';
        ...
        foreach ( $rows as $r ) {
            $out .= '<tr><td>' . esc_html( $r['name'] ) . '</td><td>' . esc_html( $r['contest'] ) . '</td><td class="bolaox-numlist">' . $r['numbers'] . '</td>';
            if ( $include_drawn ) {
                $out .= '<td class="bolaox-numlist">' . $r['drawn'] . '</td>';
            }
    ...
    private function pdf_table_section( $pdf, $title, $rows ) {
        ...
        $pdf->Cell( 50, 8, __( 'Apostador', self::TEXT_DOMAIN ), 1, 0, '', true );
        $pdf->Cell( 30, 8, __( 'Concurso', self::TEXT_DOMAIN ), 1, 0, '', true );
        $pdf->Cell( 40, 8, __( 'Números Apostados', self::TEXT_DOMAIN ), 1, 0, '', true );
        $pdf->Cell( 50, 8, __( 'Números Sorteados', self::TEXT_DOMAIN ), 1, 0, '', true );
        $pdf->Cell( 20, 8, __( 'Acertos', self::TEXT_DOMAIN ), 1, 0, 'C', true );
        $pdf->Ln();
        $pdf->SetFont( 'Arial', '', 11 );
        $pdf->SetTextColor( 0, 0, 0 );
        $fill = false;
        foreach ( $rows as $r ) {
            $fill = ! $fill;
            $bg = $fill ? $row_alt : array( 255, 255, 255 );
            $pdf->SetFillColor( $bg[0], $bg[1], $bg[2] );
            $pdf->Cell( 50, 8, $r['name'], 1, 0, '', true );
            $pdf->Cell( 30, 8, $r['contest'], 1, 0, '', true );

            // Render bet numbers with hit highlighting and duplicate counters.
            $pdf->SetFillColor( $bg[0], $bg[1], $bg[2] );
            $x = $pdf->GetX();
            $y = $pdf->GetY();
            $pdf->Cell( 40, 8, '', 1, 0, '', true );
            $pdf->SetXY( $x, $y );
            ...
            $pdf->Cell( 20, 8, $r['hits'], 1, 0, 'C', true );
            $pdf->SetTextColor( 0, 0, 0 );
            $pdf->Ln();
        }
    }
    ...
    private function pdf_numbers_line( $pdf, $numbers, $drawn, $is_drawn = false ) {
        ...
        $y            = $pdf->GetY();
        $primary      = array( 30, 115, 76 );
        $secondary    = array( 37, 159, 60 );
        $highlight    = array( 196, 255, 90 );
        $dark_primary = array( 21, 90, 57 );
        foreach ( $numbers as $n ) {
            ...
            if ( $dup ) {
                if ( $is_hit || $is_drawn ) {
                    ...
                    $fill_color   = array( 255, 205, 210 );
                    $border_color = array( 255, 59, 59 );
                    $text_color   = array( 0, 0, 0 );
                }
            } else {
                if ( $is_hit ) {
            ...
            $pdf->SetXY( $x, $y );
            $pdf->Cell( $diameter, $diameter, sprintf( '%02d', $n ), 0, 0, 'C' );

            if ( $dup ) {
                $pdf->SetTextColor( 0, 0, 0 );
                $pdf->SetFont( 'Arial', '', 8 );
                $pdf->SetXY( $x + $diameter, $y );
                $pdf->Cell( 6, $diameter, 'x' . intval( $counts[ $n ] ), 0, 0, 'L' );
                $pdf->SetFont( 'Arial', '', 11 );
                $x += $diameter + 6 + $spacing;
            } else {
                $x += $diameter + $spacing;
            }
