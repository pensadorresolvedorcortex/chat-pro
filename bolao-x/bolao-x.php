...
const VERSION = '2.8.60';
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score' ) {
        ...
        $dups = array_unique( array_diff_assoc( $res_numbers, array_unique( $res_numbers ) ) );
    ...
    public function render_form_shortcode() {
        ...
        $html .= '<div class="bolaox-selected"></div>';
        $html .= '</div></p>';
    ...
    private function generate_my_bets_table( $user_id, $contest_id = 0 ) {
        ...
        $meta = array( $or, array( 'key' => '_bolaox_payment', 'value' => 'paid' ) );
        if ( $contest_id ) {
        ...
        if ( $contest_id ) {
            ...
            if ( $res_post ) {
                $result = get_post_meta( $res_post[0]->ID, '_bolaox_result', true );
            }
        } else {
