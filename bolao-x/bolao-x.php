...
class BOLAOX_Plugin {
    ...
    public function register_settings() {
        ...
        register_setting( 'bolaox', 'bolaox_pix_webhook_token' );
    ...
    private function build_pix_payload( $key ) {
        $gui   = $this->emv( '00', 'BR.GOV.BCB.PIX' ) . $this->emv( '01', $key );
        $ma    = $this->emv( '26', $gui );
        $add   = $this->emv( '62', $this->emv( '05', '***' ) );
        ...
        $payload .= $this->emv( '59', 'Bolao X' );
        $payload .= $this->emv( '60', 'BRASILIA' );
        $payload .= $add;

        $crc_payload = $payload . '6304';
        $crc = $this->crc16( $crc_payload . '0000' );
    ...
    private function generate_pix_qr( $key ) {
        ...
        $payload = $this->build_pix_payload( $key );
        $qr = QRCode::getMinimumQRCode( $payload, QR_ERROR_CORRECT_LEVEL_L );
        $img = $qr->createImage( 8, 2 );
        ob_start();
        imagepng( $img );
        $data = ob_get_clean();
        imagedestroy( $img );
    ...
    public function settings_page() {
        ...
        if ( isset( $_POST['bolaox_nonce'] ) && wp_verify_nonce( $_POST['bolaox_nonce'], 'bolaox_settings' ) ) {
            ...
            $token_val = isset( $_POST['bolaox_pix_webhook_token'] ) ? sanitize_text_field( $_POST['bolaox_pix_webhook_token'] ) : '';
            update_option( 'bolaox_pix_key', $pix_val );
            update_option( 'bolaox_pix_webhook_token', $token_val );
        ...
        $webhook = get_option( 'bolaox_pix_webhook_token', '' );
        ...
        echo '<tr><th scope="row">' . esc_html__( 'Token do Webhook', self::TEXT_DOMAIN ) . '</th><td><input type="text" name="bolaox_pix_webhook_token" value="' . esc_attr( $webhook ) . '" class="regular-text" /></td></tr>';
        echo '</tbody></table>';
        submit_button();
        echo '</form></div>';
    }
    ...
    private function generate_report( $result ) {
        ...
        $out = '<div class="bolaox-table-wrapper"><table class="widefat bolaox-table"><thead><tr><th>Aposta</th><th>Acertos</th><th>%</th><th>Dezenas</th></tr></thead><tbody>';
        foreach ( $rows as $row ) {
        ...
        if ( $info['id'] ) {
            ...
            $out .= '<p class="bolaox-low-msg">' . sprintf( __( 'Menos Pontos: %s com %d ponto(s)', self::TEXT_DOMAIN ), esc_html( $winner_name ), intval( $info['hits'] ) ) . '</p>';
        } elseif ( $info['pool'] > 0 ) {
            $out .= '<p class="bolaox-low-msg">' . sprintf( __( 'Premiação Menos Pontos acumulada (%d)', self::TEXT_DOMAIN ), intval( $info['pool'] ) ) . '</p>';
        }
    ...
    public function handle_pix_webhook( WP_REST_Request $request ) {
        $token_option = get_option( 'bolaox_pix_webhook_token', '' );
        $token_param  = $request->get_param( 'token' );
        if ( ! $token_option || $token_option !== $token_param ) {
            return new WP_REST_Response( array( 'error' => 'invalid_token' ), 403 );
        }
        $bet_id = intval( $request->get_param( 'bet_id' ) );
        if ( ! $bet_id ) {
            return new WP_REST_Response( array( 'error' => 'missing_bet_id' ), 400 );
        }
        update_post_meta( $bet_id, '_bolaox_payment', 'paid' );
