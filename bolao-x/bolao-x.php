...
class BOLAOX_Plugin {
    ...
    public function enqueue_assets() {
        ...
        if ( is_admin() ) {
            ...
            wp_enqueue_script(
                'bolaox-chart',
                plugin_dir_url( __FILE__ ) . 'assets/js/lib/chart.js',
                array(),
                '4.4.0',
                true
            );
            wp_enqueue_script(
                'bolaox-admin-dashboard',
    ...
    public function results_page() {
        ...
        wp_localize_script( 'bolaox-admin-dashboard', 'bolaoxStats', $stats );
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( 'Visao Geral', self::TEXT_DOMAIN ) . '</h1>';
        echo '<p>' . sprintf( esc_html__( 'Exibicoes hoje: %d | Usuarios online: %d', self::TEXT_DOMAIN ), $stats['today_views'], $stats['online'] ) . '</p>';
        echo '<div class="bolaox-dashboard-analytics">';
        echo '<div class="bx-analytics-grid">';
        echo '<div class="bx-analytics-card"><h3>' . esc_html__( 'Visitas nos ultimos 15 dias', self::TEXT_DOMAIN ) . '</h3><canvas id="bx-visits" height="220"></canvas></div>';
        echo '<div class="bx-analytics-card"><h3>' . esc_html__( 'Usuarios nos ultimos 15 dias', self::TEXT_DOMAIN ) . '</h3><canvas id="bx-users" height="220"></canvas></div>';
        echo '<div class="bx-analytics-card"><h3>' . esc_html__( 'Visitas por pais', self::TEXT_DOMAIN ) . '</h3><canvas id="bx-countries" height="220"></canvas></div>';
        echo '<div class="bx-analytics-card"><h3>' . esc_html__( 'Plataformas usadas', self::TEXT_DOMAIN ) . '</h3><canvas id="bx-platforms" height="220"></canvas></div>';
        echo '<div class="bx-analytics-card"><h3>' . esc_html__( 'Navegadores usados', self::TEXT_DOMAIN ) . '</h3><canvas id="bx-browsers" height="220"></canvas></div>';
        echo '</div></div>';
        echo '</div>';
    }
