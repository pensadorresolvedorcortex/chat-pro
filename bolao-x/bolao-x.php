...
const VERSION = '2.8.80';
    ...
    public function contemplados_page() {
        ...
        if ( isset( $_GET['export_html'] ) ) {
            $this->export_winners_html( $numbers, $selected, get_the_title( $selected ), $order );
        }
    ...
    private function build_prize_table( $hits10, $hits9, $hits8, $winners_low, $values ) {
        ...
        $out .= '</tr></thead><tbody>';
        $out .= $this->build_prize_row( __( '10 pontos', self::TEXT_DOMAIN ), '10', $hits10, $values );
        $out .= $this->build_prize_row( __( '9 pontos', self::TEXT_DOMAIN ), '9', $hits9, $values );
        $out .= $this->build_prize_row( __( '8 pontos', self::TEXT_DOMAIN ), '8', $hits8, $values );
        $out .= $this->build_prize_row( __( 'Menos pontos', self::TEXT_DOMAIN ), 'low', $winners_low, $values );
        $out .= '</tbody></table></div>';
        if ( current_user_can( 'manage_options' ) ) {
    ...
    private function build_prize_row( $label, $key, $rows, $values ) {
        $value  = isset( $values[ $key ] ) ? $values[ $key ] : '';
        $names  = $rows ? implode( ', ', wp_list_pluck( $rows, 'name' ) ) : '-';
        $input  = current_user_can( 'manage_options' )
    ...
    private function export_winners_html( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $args        = array(
            'post_type'   => 'bolaox_aposta',
            'numberposts' => -1,
