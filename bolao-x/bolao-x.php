...
const VERSION = '2.8.73';
    ...
    public function numbers_meta_box( $post ) {
        ...
        $placeholder = ( 'bolaox_result' === $post->post_type )
            ? __( 'Ex: 05,12,23,34,45 (até 100 dezenas)', self::TEXT_DOMAIN )
    ...
    public function save_meta( $post_id ) {
        ...
        if ( isset( $_POST['bolaox_numbers'] ) ) {
            ...
            if ( 'bolaox_result' === get_post_type( $post_id ) ) {
                ...
                if ( false === $valid ) {
                    $this->notice = __( 'Dezenas do resultado inválidas. Use até 100 números de 00 a 99 separados por vírgula.', self::TEXT_DOMAIN );
                    return;
                }
    ...
    private function update_bet_scores( $contest_id, $numbers ) {
        ...
        foreach ( $bets as $bid ) {
            ...
            $hits = count( array_intersect( $arr, $res ) );
    ...
    private function get_recent_result_contests( $limit = 15 ) {
        global $wpdb;
        $sql = $wpdb->prepare(
            "SELECT DISTINCT pm.meta_value FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id WHERE p.post_type = 'bolaox_result' AND p.post_status = 'publish' AND pm.meta_key = '_bolaox_concurso' ORDER BY p.post_date DESC LIMIT %d",
            $limit
        );
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score' ) {
        ...
        $active = intval( get_option( 'bolaox_active_concurso', 0 ) );
        $dups   = $contest_id && $contest_id == $active ?
            array_unique( array_diff_assoc( $res_numbers, array_unique( $res_numbers ) ) ) : array();
        $out .= '<h3 class="bolaox-section-title">' . esc_html__( 'NÚMEROS REPETIDOS', self::TEXT_DOMAIN ) . '</h3>';
        $out .= $this->build_duplicates_table( $dups, get_the_title( $contest_id ) );
    ...
    private function generate_my_bets_table( $user_id, $contest_id = 0 ) {
        ...
        foreach ( $posts as $p ) {
            ...
            $numbers   = get_post_meta( $p->ID, '_bolaox_numbers', true );
            $nums      = array_map( 'trim', explode( ',', $numbers ) );
            $hits      = $res_numbers ? array_intersect( $res_numbers, $nums ) : array();
            $hit_count = count( $hits );
            $percentage = $hit_count * 10;
            $progress     = sprintf(
            ...
            foreach ( $nums as $n ) {
                ...
                if ( $has_result && in_array( $n, $res_numbers, true ) ) {
                    $classes[] = 'hit';
                }
                $display_nums[] = sprintf(
                    '<span class="%s">%s</span>',
