...
const VERSION = '2.8.80';
    ...
    public function contemplados_page() {
        ...
        if ( isset( $_GET['export_html'] ) ) {
            $this->export_winners_html( $numbers, $selected, get_the_title( $selected ), $order, $prize_values );
        }
        $export_link = add_query_arg( 'export_html', 1 );
        echo '<p><a href="' . esc_url( $export_link ) . '" class="button bolaox-export-html">' . esc_html__( 'Exportar para HTML', self::TEXT_DOMAIN ) . '</a></p>';
        echo '<h2>' . sprintf( esc_html__( 'Resultado do Concurso %s', self::TEXT_DOMAIN ), get_the_title( $selected ) ) . '</h2>';
        echo $this->generate_report( $numbers, $selected, $order, $prize_values );
        echo '</div>';
    }
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score', $prizes = array() ) {
        ...
        $out .= $this->build_prize_table( $hits10, $hits9, $hits8, $winners_low, $prizes );
    ...
    private function build_prize_table( $hits10, $hits9, $hits8, $winners_low, $values, $editable = true ) {
        $out = '';
        if ( $editable ) {
        ...
        $out .= '</tr></thead><tbody>';
        $out .= $this->build_prize_row( __( '10 pontos', self::TEXT_DOMAIN ), '10', $hits10, $values, $editable );
        $out .= $this->build_prize_row( __( '9 pontos', self::TEXT_DOMAIN ), '9', $hits9, $values, $editable );
        $out .= $this->build_prize_row( __( '8 pontos', self::TEXT_DOMAIN ), '8', $hits8, $values, $editable );
        $out .= $this->build_prize_row( __( 'Menos pontos', self::TEXT_DOMAIN ), 'low', $winners_low, $values, $editable );
        $out .= '</tbody></table></div>';
    ...
    private function build_prize_row( $label, $key, $rows, $values, $editable = true ) {
        $value = isset( $values[ $key ] ) ? $values[ $key ] : '';
        $names = $rows ? implode( ', ', wp_list_pluck( $rows, 'name' ) ) : '-';
        $input = ( $editable && current_user_can( 'manage_options' ) )
            ? '<input type="text" name="bolaox_prize_values[' . esc_attr( $key ) . ']" value="' . esc_attr( $value ) . '" class="small-text" />'
            : esc_html( $value );
        return '<tr><td>' . esc_html( $label ) . '</td><td>' . $input . '</td><td>' . esc_html( $names ) . '</td></tr>';
    }
    ...
    private function export_winners_html( $result, $contest_id, $contest_title, $order_by = 'score', $prizes = array() ) {
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $counts      = array_count_values( $res_numbers );
        $args        = array(
            'post_type'   => 'bolaox_aposta',
        ...
        $drawn_plain = implode( ' ', $res_numbers );
        foreach ( $posts as $p ) {
            $numbers = get_post_meta( $p->ID, '_bolaox_numbers', true );
            $nums    = array_filter( array_map( 'trim', explode( ',', $numbers ) ), 'strlen' );
            $hits    = count( array_intersect( $res_numbers, $nums ) );
        ...
        echo $this->build_prize_table( $hits10, $hits9, $hits8, $winners_low, $prizes, false );
