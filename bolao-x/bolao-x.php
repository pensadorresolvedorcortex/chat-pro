...
/*
Plugin Name: Bolao X
Description: Sistema de gerenciamento de bolão com conferência automática, histórico de resultados, exportação em PDF e Excel e pagamento via Pix.
Version: 2.7.1
Text Domain: bolao-x
...
class BOLAOX_Plugin {
    ...
    const VERSION = '2.7.1';
    const WEBHOOK_TOKEN = 'CwbzYUaV8TNfv*J$Dua6JiHy@';
    ...
    private function __construct() {
        ...
        $this->log_file = $dir . '/pix-error.log';
    ...
    private function log_pix_error( $msg ) {
        if ( ! $this->log_file ) {
            return;
        }
    ...
    private function pix_payload( $key, $txid = '***' ) {
        ...
        return Bolaox_Pix::payload( $key, $merchant, $city, $txid );
    }

    private function generate_pix_qr( $key, $txid = '***' ) {
        if ( ! function_exists( 'imagepng' ) || ! $key ) {
            $this->log_pix_error( 'Envio de QR impossível: dependências ou chave ausentes' );
            return '';
        }
        $payload = $this->pix_payload( $key, $txid );
        $this->log_pix_error( 'Payload gerado: ' . $payload );
        $img = Bolaox_Pix::qr_base64( $payload );
        if ( ! $img ) {
            $this->log_pix_error( 'Falha ao gerar imagem do QR' );
        }
        return $img;
    }
    ...
    public function logs_page() {
        echo '<div class="wrap"><h1>' . esc_html__( 'Logs Pix', self::TEXT_DOMAIN ) . '</h1>';
        if ( file_exists( $this->log_file ) ) {
            echo '<pre style="white-space:pre-wrap">' . esc_html( file_get_contents( $this->log_file ) ) . '</pre>';
        } else {
            echo '<p>' . esc_html__( 'Nenhum log disponível.', self::TEXT_DOMAIN ) . '</p>';
        }
        echo '</div>';
    }
    ...
    public function handle_webhook( WP_REST_Request $request ) {
        ...
        if ( ! $post_id ) {
            return new WP_Error( 'not_found', 'Aposta não encontrada', array( 'status' => 404 ) );
        }
        update_post_meta( $post_id, '_bolaox_payment', 'paid' );
        $this->log_pix_error( 'Pagamento confirmado para aposta ' . $post_id );
