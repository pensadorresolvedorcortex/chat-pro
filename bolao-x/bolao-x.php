...
class BOLAOX_Plugin {
    ...
    public function results_page() {
        ...
        $top_country  = $stats['countries']['labels'][0] ?? '-';
        $top_platform = $stats['platforms']['labels'][0] ?? '-';
        $top_browser  = $stats['browsers']['labels'][0] ?? '-';
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( 'Visao Geral', self::TEXT_DOMAIN ) . '</h1>';
        echo '<div class="bolaox-dashboard-analytics">';
        echo '<div class="bx-info-circle">';
        echo '<div class="bx-ci-visits">' . esc_html__( 'Visitas nos ultimos 15 dias', self::TEXT_DOMAIN ) . ': ' . intval( $total_visits ) . '</div>';
        echo '<div class="bx-ci-users">' . esc_html__( 'Usuarios nos ultimos 15 dias', self::TEXT_DOMAIN ) . ': ' . intval( $total_users ) . '</div>';
        echo '<div class="bx-ci-today">' . sprintf( esc_html__( 'Exibicoes hoje: %d | Usuarios online: %d', self::TEXT_DOMAIN ), $stats['today_views'], $stats['online'] ) . '</div>';
        echo '<div class="bx-ci-country">' . esc_html__( 'Pais mais popular', self::TEXT_DOMAIN ) . ': ' . esc_html( $top_country ) . '</div>';
        echo '<div class="bx-ci-platform">' . esc_html__( 'Plataforma mais usada', self::TEXT_DOMAIN ) . ': ' . esc_html( $top_platform ) . '</div>';
        echo '<div class="bx-ci-browser">' . esc_html__( 'Navegador mais usado', self::TEXT_DOMAIN ) . ': ' . esc_html( $top_browser ) . '</div>';
        echo '</div></div>';
        echo '</div>';
    }
