...
const VERSION = '2.8.80';
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score' ) {
        ...
        $out .= $this->render_repeated_numbers( $contest_id );
    ...
    private function export_winners_doc( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        ...
        $counts = array_fill( 0, 100, 0 );
        foreach ( $posts as $p ) {
            $nums = array_filter( array_map( 'trim', explode( ',', get_post_meta( $p->ID, '_bolaox_numbers', true ) ) ), 'strlen' );
            foreach ( $nums as $n ) {
                $idx = intval( $n );
                if ( $idx >= 0 && $idx < 100 ) {
                    $counts[ $idx ]++;
