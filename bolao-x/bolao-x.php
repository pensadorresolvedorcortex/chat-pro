...
class BOLAOX_Plugin {
    ...
    private function generate_my_bets_table( $user_id ) {
        ...
        $out .= '<thead><tr><th>' . esc_html__( 'Aposta', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Acertos', self::TEXT_DOMAIN ) . '</th><th>%</th><th>' . esc_html__( 'Dezenas', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Status', self::TEXT_DOMAIN ) . '</th></tr></thead><tbody>';
        foreach ( $posts as $p ) {
            $numbers   = get_post_meta( $p->ID, '_bolaox_numbers', true );
            $nums      = array_map( 'trim', explode( ',', $numbers ) );
            ...
            $out  .= '<tr><td>' . esc_html( $p->post_title ) . '</td><td>' . $hit_count . '</td><td>' . $progress . '</td><td class="bolaox-numlist">' . implode( '', $display_nums ) . '</td><td>' . esc_html( $label ) . '</td></tr>';
        }
        $out .= '</tbody></table></div>';
        return $out;
    }
    ...
    public function rest_my_bets() {
        return $this->generate_my_bets_table( get_current_user_id() );
    }
