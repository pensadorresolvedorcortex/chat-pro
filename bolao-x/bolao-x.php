...
const VERSION = '2.8.80';
    ...
    public function contemplados_page() {
        ...
        if ( isset( $_POST['bolaox_repeats'] ) && current_user_can( 'manage_options' ) && isset( $_POST['_bxrep'] ) && wp_verify_nonce( $_POST['_bxrep'], 'bolaox_repeats' ) ) {
            update_post_meta( $result_post[0]->ID, '_bolaox_repeats', sanitize_textarea_field( wp_unslash( $_POST['bolaox_repeats'] ) ) );
        }
        $manual_repeats = get_post_meta( $result_post[0]->ID, '_bolaox_repeats', true );
        if ( isset( $_GET['export_html'] ) ) {
            $this->export_winners_html( $numbers, $selected, get_the_title( $selected ), $order, $manual_repeats );
        }
        ...
        echo $this->generate_report( $numbers, $selected, $order, $manual_repeats );
        echo '</div>';
    }
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score', $manual_repeats = '' ) {
        ...
        $out .= $this->render_repeated_numbers_by_date( $contest_id, $manual_repeats );
    ...
    private function export_winners_html( $result, $contest_id, $contest_title, $order_by = 'score', $manual_repeats = '' ) {
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $args        = array(
            'post_type'   => 'bolaox_aposta',
            'numberposts' => -1,
        ...
        if ( $manual_repeats ) {
            $repeated = $this->parse_manual_repeats( $manual_repeats );
        } else {
    ...
    private function render_repeated_numbers_by_date( $contest_id = 0, $manual = '' ) {
        $entries = array();
        if ( $manual ) {
            $entries = $this->parse_manual_repeats( $manual );
        } else {
            if ( ! $contest_id ) {
                $contest_id = intval( get_option( 'bolaox_active_concurso', 0 ) );
            }
        ...
        if ( current_user_can( 'manage_options' ) ) {
            ...
            $out .= '<form method="post" class="bolaox-repeat-form">';
            $out .= wp_nonce_field( 'bolaox_repeats', '_bxrep', true, false );
            $out .= '<textarea name="bolaox_repeats" rows="5" style="width:100%">' . esc_textarea( $value ) . '</textarea>';
            $out .= '<p class="description">' . esc_html__( 'Formato: data:n1,n2', self::TEXT_DOMAIN ) . '</p>';
    ...
    private function parse_manual_repeats( $manual ) {
        $entries = array();
