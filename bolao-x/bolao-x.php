...
const VERSION = '2.8.80';
    ...
    public function handle_export_winners_pdf_flat_numbers() {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'Você não tem permissão para gerar este relatório.', self::TEXT_DOMAIN ), '', array( 'response' => 403 ) );
        }

        check_admin_referer( 'bolao_x_export_winners_flat' );
        ...
        $this->export_winners_pdf_flat_numbers(
            $numbers,
            $contest_id,
            get_the_title( $contest_id ),
            $order,
    ...
    public function contemplados_page() {
        ...
        $export_link_flat = wp_nonce_url(
            add_query_arg(
                array(
                    'action'     => 'bolao_x_export_winners_flat',
                    'contest_id' => $selected,
                    'orderby'    => $order,
                ),
                admin_url( 'admin-post.php' )
    ...
    private function export_winners_pdf_flat_numbers( $result, $contest_id, $contest_title, $order_by = 'score', $prizes = array(), $winners = array(), $fund = '' ) {
        $pdf           = new BOLAOX_PDF();
        $bottom_margin = 20;
        $res_numbers   = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $args          = array(
        ...
        // Mantém a seção "Dezenas sorteadas" igual ao relatório completo.
        $this->pdf_section_header( $pdf, __( 'DEZENAS SORTEADAS', self::TEXT_DOMAIN ), null, null, 0 );
        $pdf->SetX( $pdf->getLeftMargin() );
        $this->pdf_numbers_line( $pdf, $drawn, $drawn, true, $pdf->getContentWidth() );
        ...
        $filename = sprintf( 'bolao-relatorio-sem-bolas-concurso-%d.pdf', (int) $contest_id );
    ...
    private function pdf_numbers_line_flat( $pdf, $numbers, $width, $line_height = 4.5 ) {
        $numbers = is_array( $numbers ) ? array_values( $numbers ) : array_values( array_filter( array_map( 'trim', (array) $numbers ), 'strlen' ) );
        if ( empty( $numbers ) ) {
            return 0;
        }
        ...
        $start_y = $pdf->GetY();
        $pdf->MultiCell( $width, $line_height, $text, 0, 'L' );
    ...
    private function pdf_table_section_flat( $pdf, $title, $rows, $bottom_margin, $options = array() ) {
        ...
        $line_height   = 4.5;
        foreach ( $rows as $index => $row ) {
            $numbers_height = $this->pdf_estimate_numbers_height_flat( $pdf, $row['numbers'], $content_width - ( 2 * $padding ), $line_height );
            $drawn_height   = $this->pdf_estimate_numbers_height_flat( $pdf, $row['drawn'], $content_width - ( 2 * $padding ), $line_height );
            $card_height    = $padding + 8 + 4 + 4 + $numbers_height + $gap + 4 + $drawn_height + $padding;
            $this->pdf_check_page_break( $pdf, $card_height + 4, $bottom_margin );
            $y = $pdf->GetY();
            $bg = ( $index % 2 === 0 ) ? array( 255, 255, 255 ) : array( 245, 249, 246 );
            ...
            $pdf->SetX( $inner_x );
            $pdf->SetTextColor( 0, 0, 0 );
            $this->pdf_numbers_line_flat( $pdf, $row['numbers'], $content_width - ( 2 * $padding ), $line_height );
            ...
            $pdf->SetX( $inner_x );
            $pdf->SetTextColor( 0, 0, 0 );
            $this->pdf_numbers_line_flat( $pdf, $row['drawn'], $content_width - ( 2 * $padding ), $line_height );
