...
class BOLAOX_Plugin {
    ...
    private $import_redirect = false;
    private $import_redirect_id = 0;
    /**
...
const VERSION = '2.8.80';
    ...
    private function __construct() {
        ...
        add_filter( 'redirect_post_location', array( $this, 'redirect_after_import' ), 10, 2 );
    ...
    public function register_meta_fields() {
        ...
        register_post_meta( 'bolaox_aposta', '_bolaox_posicao', array(
            'type'         => 'integer',
            'single'       => true,
            'show_in_rest' => true,
            'auth_callback' => $caps,
    ...
    public function numbers_meta_box( $post ) {
        ...
        if ( 'bolaox_aposta' === $post->post_type && 'auto-draft' === $post->post_status ) {
            ...
            echo '<p>' . esc_html__( 'Importar apostas (XLSX)', self::TEXT_DOMAIN ) . ': <input type="file" name="bolaox_import_file" accept=".xlsx" /></p>';
            echo '<p><button type="submit" class="button" name="bolaox_import_only" value="1">' . esc_html__( 'Carregar apostas', self::TEXT_DOMAIN ) . '</button></p>';
        }
    }
    ...
    public function save_meta( $post_id ) {
        ...
        if ( isset( $_FILES['bolaox_import_file'] ) && UPLOAD_ERR_OK === $_FILES['bolaox_import_file']['error'] && 'bolaox_aposta' === get_post_type( $post_id ) ) {
            require_once plugin_dir_path( __FILE__ ) . 'lib/simplexlsx.php';
            $xlsx = \Shuchkin\SimpleXLSX::parse( $_FILES['bolaox_import_file']['tmp_name'] );
            if ( ! $xlsx ) {
                $this->notice = __( 'Falha ao ler o arquivo XLSX.', self::TEXT_DOMAIN );
                $this->saving_meta = false;
                return;
            }
            $rows = $xlsx->rows();
            if ( ! isset( $rows[0][0], $rows[0][1], $rows[0][2], $rows[0][12] ) ) {
                $this->notice = __( 'Arquivo XLSX sem colunas esperadas.', self::TEXT_DOMAIN );
                $this->saving_meta = false;
                return;
            }
            ...
            if ( isset( $_POST['bolaox_import_only'] ) ) {
                foreach ( $rows as $row ) {
                    $line_no++;
                    if ( 1 === $line_no ) {
                        continue;
                    ...
                    $position = isset( $row[0] ) ? intval( $row[0] ) : 0;
                    $name     = isset( $row[1] ) ? sanitize_text_field( $row[1] ) : '';
                    $key      = $position . ':' . strtolower( $name );
                    if ( isset( $seen[ $key ] ) ) {
                        $invalid[] = $line_no;
                        continue;
                    }
                    ...
                    if ( $existing ) {
                        $existing_title = strtolower( get_the_title( $existing[0] ) );
                        if ( $existing_title === strtolower( $name ) ) {
                            $invalid[] = $line_no;
                            continue;
                        }
                    }
                    $nums_raw = array_slice( $row, 2, 10 );
                    $filled   = array_filter( $nums_raw, static function( $n ) {
                        return null !== $n && '' !== $n;
                    } );
                    if ( count( $filled ) < 10 ) {
                        $invalid[] = $line_no;
                    ...
                    $nums = array();
                    foreach ( $nums_raw as $n ) {
                        $nums[] = sprintf( '%02d', intval( $n ) );
                    }
                    ...
                    $status_text = strtolower( trim( (string) ( $row[12] ?? '' ) ) );
                    $status      = ( 'pago' === $status_text ) ? 'paid' : 'pending';
                    $args        = array(
                        'post_type'   => 'bolaox_aposta',
                        'post_title'  => $name,
                        'post_status' => 'publish',
                ...
                if ( $created || $invalid ) {
                    $msg = '';
                    if ( $created ) {
                        $msg .= sprintf( __( 'Importadas %d apostas.', self::TEXT_DOMAIN ), $created );
                    }
                    if ( $invalid ) {
                        $msg .= ' ' . sprintf( __( 'Linhas inválidas: %s.', self::TEXT_DOMAIN ), implode( ', ', $invalid ) );
                    }
                    $this->notice = trim( $msg );
                }
                $this->import_redirect = true;
                $this->import_redirect_id = $post_id;
            ...
            foreach ( $rows as $row ) {
                ...
                if ( ! $processed_first ) {
                    ...
                    update_post_meta( $post_id, '_bolaox_posicao', $position );
                    if ( '1' === $fixed ) {
    ...
    public function running_contests_page() {
        ...
        foreach ( $contests as $c ) {
            ...
            echo '<table class="widefat"><thead><tr><th>' . esc_html__( 'Aposta', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Dezenas', self::TEXT_DOMAIN ) . '</th></tr></thead><tbody>';
            foreach ( $bets as $b ) {
                $nums = get_post_meta( $b->ID, '_bolaox_numbers', true );
                echo '<tr><td>' . esc_html( $b->post_title ) . '</td><td>' . esc_html( $nums ) . '</td></tr>';
            }
            echo '</tbody></table>';
        }
        echo '</div>';
    ...
    public function contemplados_page() {
        ...
        if ( isset( $_GET['export_doc'] ) ) {
            $this->export_winners_doc( $numbers, $selected, get_the_title( $selected ), $order );
        }
        $export_link = add_query_arg( 'export_doc', 1 );
        echo '<p><a href="' . esc_url( $export_link ) . '" class="button bolaox-export-word">' . esc_html__( 'Exportar para o Word', self::TEXT_DOMAIN ) . '</a></p>';
    ...
    private function generate_report( $result, $contest_id = 0, $order_by = 'score' ) {
        ...
        foreach ( $posts as $p ) {
            ...
            $active     = intval( get_option( 'bolaox_active_concurso', 0 ) );
            $show_dup   = $contest_id && $contest_id == $active;
            $duplicates = $show_dup ? array_unique( array_diff_assoc( $nums, array_unique( $nums ) ) ) : array();
            $display_nums = array();
            $display_res  = array();
            ...
            foreach ( $nums as $n ) {
                ...
                if ( $show_dup && in_array( $n, $duplicates, true ) ) {
                    $classes[] = 'dup';
                }
        ...
        // Continue building output after the drawn numbers section
        ...
        $out .= '<h3 class="bolaox-section-title">' . esc_html__( 'APOSTAS COM 9 PONTOS', self::TEXT_DOMAIN ) . '</h3>';
        $out .= $this->build_results_table( $hits9 );
        $out .= '<h3 class="bolaox-section-title">' . esc_html__( 'APOSTAS COM 8 PONTOS', self::TEXT_DOMAIN ) . '</h3>';
        ...
        $out .= $this->build_stats_table( $total, $avg, $rate );

        $out .= '<div class="bolaox-legend">';
        $out .= '<span class="bolaox-number legend-hit"></span>';
        $out .= '<span>' . esc_html__( 'Acertos no Concurso', self::TEXT_DOMAIN ) . '</span>';
        $out .= '<span class="bolaox-number dup" style="margin-left:20px"></span>';
        $out .= '<span>' . esc_html__( 'Número Duplicado', self::TEXT_DOMAIN ) . '</span>';
        $out .= '</div>';
    ...
    private function build_stats_table( $total, $avg, $rate ) {
        $out  = '<div class="bolaox-table-wrapper bolaox-scroll"><table class="widefat bolaox-table bolaox-stats"><thead><tr>';
        $out .= '<th>' . esc_html__( 'Participantes', self::TEXT_DOMAIN ) . '</th>';
        $out .= '<th>' . esc_html__( 'Média de Pontos', self::TEXT_DOMAIN ) . '</th>';
        $out .= '<th>' . esc_html__( 'Taxa de Acerto', self::TEXT_DOMAIN ) . '</th>';
        $out .= '</tr></thead><tbody>';
        $out .= '<tr><td>' . intval( $total ) . '</td><td>' . number_format( $avg, 2, ',', '' ) . '</td><td>' . round( $rate ) . '%</td></tr>';
        $out .= '</tbody></table></div>';
        return $out;
    }
    ...
    private function export_winners_pdf( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        require_once __DIR__ . '/lib/fpdf.php';
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $args = array(
            'post_type'   => 'bolaox_aposta',
        ...
        if ( 'date' === $order_by ) {
            ...
            usort( $rows, function ( $a, $b ) { return $b['hits'] <=> $a['hits']; } );
        }

        $max_hits = $rows ? max( wp_list_pluck( $rows, 'hits' ) ) : 0;
        $min_hits = $rows ? min( wp_list_pluck( $rows, 'hits' ) ) : 0;
        $winners_high = array_filter( $rows, function ( $r ) use ( $max_hits ) { return $r['hits'] === $max_hits; } );
        $winners_low  = array_filter( $rows, function ( $r ) use ( $min_hits ) { return $r['hits'] === $min_hits; } );
        ...
        $this->pdf_table_section( $pdf, __( 'VENCEDORES COM MAIS PONTOS', self::TEXT_DOMAIN ), $winners_high );
        $this->pdf_table_section( $pdf, __( 'VENCEDORES COM MENOS PONTOS', self::TEXT_DOMAIN ), $winners_low );
        $this->pdf_table_section( $pdf, __( 'TODAS AS APOSTAS', self::TEXT_DOMAIN ), $rows );

        // Ensure no prior output interferes with PDF headers.
        if ( ob_get_length() ) {
            ob_end_clean();
        }
        nocache_headers();
    ...
    private function export_winners_doc( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        $res_numbers = array_filter( array_map( 'trim', explode( ',', $result ) ), 'strlen' );
        $args        = array(
            'post_type'   => 'bolaox_aposta',
            'numberposts' => -1,
        ...
        $winners_high = array_filter( $rows, function ( $r ) use ( $max_hits ) { return $r['hits'] === $max_hits; } );
        $hits9        = array_filter( $rows, function ( $r ) { return $r['hits'] === 9; } );
        $hits8        = array_filter( $rows, function ( $r ) { return $r['hits'] === 8; } );
        $winners_low  = array_filter( $rows, function ( $r ) use ( $min_hits ) { return $r['hits'] === $min_hits; } );

        $repeated = $this->get_repeated_numbers_by_date( $contest_id );

        header( 'Content-Type: application/msword' );
        header( 'Content-Disposition: attachment; filename="bolao-relatorio.doc"' );
        echo "<html><meta charset='utf-8'><body style='font-family:Arial;'>";
        echo '<h1 style="text-align:center">' . esc_html( sprintf( __( 'Relatorio do Concurso %s', self::TEXT_DOMAIN ), $contest_title ) ) . '</h1>';

        echo '<h2>' . esc_html__( 'DEZENAS SORTEADAS', self::TEXT_DOMAIN ) . '</h2>';
        echo '<p>' . esc_html( $drawn ) . '</p>';

        if ( $winners_high ) {
            echo '<h2>' . esc_html__( 'VENCEDORES COM MAIS PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_doc_table( $winners_high, false );
        }
        if ( $hits9 ) {
            echo '<h2>' . esc_html__( 'APOSTAS COM 9 PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_doc_table( $hits9 );
        }
        if ( $hits8 ) {
            echo '<h2>' . esc_html__( 'APOSTAS COM 8 PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_doc_table( $hits8 );
        }
        if ( $winners_low ) {
            echo '<h2>' . esc_html__( 'MENOS PONTOS', self::TEXT_DOMAIN ) . '</h2>';
            echo $this->build_doc_table( $winners_low, false );
        }

        echo '<h2>' . esc_html__( 'NÚMEROS REPETIDOS', self::TEXT_DOMAIN ) . '</h2>';
        if ( $repeated ) {
            foreach ( $repeated as $row ) {
                echo esc_html( $row['date'] . ' - ' . implode( ' ', $row['numbers'] ) ) . '<br />';
            }
        } else {
            echo '<p>' . esc_html__( 'Nenhum número repetido.', self::TEXT_DOMAIN ) . '</p>';
        }

        echo '<h2>' . esc_html__( 'TODAS AS APOSTAS', self::TEXT_DOMAIN ) . '</h2>';
        echo $this->build_doc_table( $rows, false );

        echo '</body></html>';
        exit;
    }

    private function build_doc_table( $rows, $include_drawn = true ) {
        $out  = '<table border="1" cellpadding="4" cellspacing="0" width="100%">';
        $out .= '<tr><th>' . esc_html__( 'Apostador', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Concurso', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Números Apostados', self::TEXT_DOMAIN ) . '</th>';
        if ( $include_drawn ) {
            $out .= '<th>' . esc_html__( 'Números Sorteados', self::TEXT_DOMAIN ) . '</th>';
        }
        $out .= '<th>' . esc_html__( 'Acertos', self::TEXT_DOMAIN ) . '</th></tr>';
        foreach ( $rows as $r ) {
            $out .= '<tr><td>' . esc_html( $r['name'] ) . '</td><td>' . esc_html( $r['contest'] ) . '</td><td>' . esc_html( $r['numbers'] ) . '</td>';
            if ( $include_drawn ) {
                $out .= '<td>' . esc_html( $r['drawn'] ) . '</td>';
            }
            $out .= '<td>' . intval( $r['hits'] ) . '</td></tr>';
        }
        $out .= '</table>';
        return $out;
    }

    private function pdf_table_section( $pdf, $title, $rows ) {
        if ( ! $rows ) {
            return;
        }
        $pdf->Ln( 5 );
        ...
        $pdf->Cell( 50, 8, __( 'Apostador', self::TEXT_DOMAIN ), 1 );
        $pdf->Cell( 30, 8, __( 'Concurso', self::TEXT_DOMAIN ), 1 );
        $pdf->Cell( 40, 8, __( 'Números Apostados', self::TEXT_DOMAIN ), 1 );
        $pdf->Cell( 50, 8, __( 'Números Sorteados', self::TEXT_DOMAIN ), 1 );
        $pdf->Cell( 20, 8, __( 'Acertos', self::TEXT_DOMAIN ), 1, 0, 'C' );
        $pdf->Ln();
    ...
    private function render_repeated_numbers_by_date( $contest_id = 0 ) {
        ...
        foreach ( $posts as $p ) {
            ...
            $list = '';
            foreach ( $repeats as $n ) {
                $list .= '<span class="bolaox-number">' . esc_html( $n ) . '</span>';
            }
            $date = get_the_date( 'd/m', $p );
            $out .= '<div class="bolaox-repeat-date"><strong>' . esc_html( $date ) . '</strong> - ' . ( $list ? $list : esc_html__( 'Nenhum', self::TEXT_DOMAIN ) ) . '</div>';
        }
        return $out;
    }
    ...
    private function get_repeated_numbers_by_date( $contest_id = 0 ) {
        ...
        foreach ( $posts as $p ) {
            ...
            $nums = array_filter( array_map( 'trim', explode( ',', $nums_str ) ) );
            $repeats = array_intersect( $nums, array_keys( $seen ) );
            foreach ( $nums as $n ) {
                $seen[ $n ] = true;
            }
    ...
    public function redirect_after_import( $location, $post_id ) {
        if ( $this->import_redirect && $post_id === $this->import_redirect_id ) {
            $this->import_redirect = false;
            $this->import_redirect_id = 0;
            return admin_url( 'post-new.php?post_type=bolaox_aposta' );
        }
        return $location;
    }
