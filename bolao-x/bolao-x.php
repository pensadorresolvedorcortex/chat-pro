...
class BOLAOX_Plugin {
    ...
    public function render_form_shortcode() {
        ...
        if ( ! is_user_logged_in() ) {
            ...
            return do_shortcode( '[bolao_x_login redirect_to="' . esc_url( $current_url ) . '"]' );
        }
        $price   = floatval( get_option( 'bolaox_price', 10 ) );
        ...
        if ( isset( $_POST['bolaox_submit'] ) && isset( $_POST['bolaox_nonce'] ) && wp_verify_nonce( $_POST['bolaox_nonce'], 'bolaox_form' ) ) {
            ...
            if ( 'paid' === $status ) {
                ...
                // New users are created automatically when paying while logged out
    ...
    public function render_login_shortcode( $atts = array() ) {
        ...
        $redirect_to = $atts['redirect_to'] ? esc_url_raw( $atts['redirect_to'] ) : ( isset( $_REQUEST['redirect_to'] ) ? esc_url_raw( $_REQUEST['redirect_to'] ) : home_url( '/participe?bx_sec=mybets' ) );
        $msg         = '';
