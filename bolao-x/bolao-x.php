...
class BOLAOX_Plugin {
    ...
    public function render_form_shortcode() {
        ...
        $html .= '<p class="bolaox-field"><label>' . esc_html__( 'Nome Completo', self::TEXT_DOMAIN ) . '<br /><input type="text" name="bolaox_name" required /></label></p>';
    ...
    public function render_profile_shortcode() {
        if ( ! is_user_logged_in() ) {
            return $this->wrap_app( '<p>' . esc_html__( 'Faça login para editar seus dados.', self::TEXT_DOMAIN ) . '</p>' );
        }
        ...
        if ( isset( $_POST['bolaox_profile_nonce'] ) && wp_verify_nonce( $_POST['bolaox_profile_nonce'], 'bolaox_profile' ) ) {
            ...
            return $this->wrap_app( '<p>' . esc_html__( 'Dados atualizados com sucesso.', self::TEXT_DOMAIN ) . '</p>' );
        }
        $html  = '<form method="post" class="bolaox-profile">';
