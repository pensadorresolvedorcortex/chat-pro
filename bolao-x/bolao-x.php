...
const VERSION = '2.8.80';
    ...
    private function export_winners_pdf( $result, $contest_id, $contest_title, $order_by = 'score', $prizes = array(), $winners = array(), $fund = '' ) {
        ...
        $pdf->SetCreator( 'Bolao X' );
        $pdf->SetAuthor( 'Bolao X' );
        $pdf->SetTitle( sprintf( __( 'Relatório do Concurso %s', self::TEXT_DOMAIN ), $contest_title ) );
        $pdf->AliasNbPages();
        ...
        $this->pdf_numbers_line( $pdf, $drawn, $drawn, true, $pdf->GetPageWidth() - $pdf->lMargin - $pdf->rMargin );
        ...
        $cards = array(
            ...
            array(
                'label'       => __( 'Vencedores 8 pontos', self::TEXT_DOMAIN ),
        ...
        $this->pdf_table_section(
            $pdf,
            __( 'VENCEDORES COM 8 PONTOS', self::TEXT_DOMAIN ),
            $hits8,
            $bottom_margin,
            array(
                'subtitle' => sprintf(
    ...
    private function pdf_summary_cards( $pdf, $cards, $bottom_margin ) {
        ...
        $available = $pdf->GetPageWidth() - $pdf->lMargin - $pdf->rMargin;
        $gap       = 6;
    ...
    private function pdf_prize_panel( $pdf, $prizes, $winners, $fund, $bottom_margin ) {
        ...
        $map = array(
            ...
            '8'  => __( 'Vencedores com 8 pontos', self::TEXT_DOMAIN ),
        ...
        $content_width = $pdf->GetPageWidth() - $pdf->lMargin - $pdf->rMargin;
        $padding       = 7;
        ...
        $pdf->Rect( $pdf->lMargin, $y, $content_width, $estimate, 'F' );
        $pdf->SetXY( $pdf->lMargin + $padding, $y + $padding );
        foreach ( $rows as $row ) {
            $pdf->SetFont( 'Arial', 'B', 9 );
            $pdf->SetTextColor( 30, 115, 76 );
            $pdf->Cell( $line_width, 5, $row['label'] . ' • ' . $row['value'], 0, 1, 'L' );
            $pdf->SetFont( 'Arial', '', 8.5 );
            $pdf->SetTextColor( 92, 110, 102 );
            $pdf->MultiCell( $line_width, 4, $row['winner'], 0, 'L' );
            $pdf->Ln( 2 );
            $pdf->SetX( $pdf->lMargin + $padding );
        }
    ...
    private function pdf_repeated_section( $pdf, $title, $entries, $bottom_margin ) {
        ...
        foreach ( $entries as $entry ) {
            ...
            $pdf->SetFillColor( 248, 252, 249 );
            $pdf->SetDrawColor( 220, 235, 227 );
            $pdf->Rect( $pdf->lMargin, $y, $content_width, $box_height, 'F' );
            $pdf->SetXY( $pdf->lMargin + $padding, $y + $padding );
    ...
    private function pdf_table_section( $pdf, $title, $rows, $bottom_margin, $options = array() ) {
        ...
        foreach ( $rows as $index => $row ) {
            ...
            $pdf->SetDrawColor( 220, 235, 227 );
            $pdf->Rect( $pdf->lMargin, $y, $content_width, $card_height, 'F' );
            $accent = ( $row['hits'] >= 9 ) ? array( 142, 201, 72 ) : array( 189, 207, 197 );
            $pdf->SetFillColor( $accent[0], $accent[1], $accent[2] );
            $pdf->Rect( $pdf->lMargin, $y, 3, $card_height, 'F' );

            $inner_x = $pdf->lMargin + $padding;
            $line_y  = $y + $padding;
            ...
            $badge_width = 28;
            $badge_x     = $pdf->lMargin + $content_width - $padding - $badge_width;
    ...
    private function pdf_section_header( $pdf, $title, $start_color = null, $end_color = null, $top_spacing = 8 ) {
        ...
        $width = $pdf->GetPageWidth() - $pdf->lMargin - $pdf->rMargin;
        $start = $start_color ? $start_color : array( 30, 115, 76 );
        $end   = $end_color ? $end_color : array( 37, 159, 60 );
        $y     = $pdf->GetY();
        $pdf->LinearGradient( $pdf->lMargin, $y, $width, 10, $start, $end );
        $pdf->SetXY( $pdf->lMargin + 4, $y + 2.5 );
        $pdf->SetFont( 'Arial', 'B', 12 );
        $pdf->SetTextColor( 255, 255, 255 );
    ...
    private function pdf_numbers_line( $pdf, $numbers, $drawn, $is_drawn = false, $width = null ) {
        ...
        $max_width = $width ? $width : ( $pdf->GetPageWidth() - $pdf->lMargin - $pdf->rMargin );
