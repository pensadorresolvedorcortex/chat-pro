...
class BOLAOX_Plugin {
    ...
    private function generate_pix_qr( $key, $txid = '***' ) {
        if ( ! function_exists( 'imagepng' ) ) {
            $this->log_pix_error( 'Extensão GD não disponível' );
            return '';
        }
        if ( ! $key ) {
            $this->log_pix_error( 'Chave Pix vazia' );
            return '';
        }
        try {
            ...
            if ( ! $img ) {
                throw new Exception( 'Falha ao criar QR code' );
            }
            $size    = imagesx( $img );
            ...
            ob_start();
            if ( ! imagepng( $scaled ) ) {
                throw new Exception( 'Erro ao gerar PNG' );
            }
            $data = ob_get_clean();
            imagedestroy( $scaled );
            return 'data:image/png;base64,' . base64_encode( $data );
        } catch ( Exception $e ) {
            $this->log_pix_error( $e->getMessage() );
            return '';
        } finally {
    ...
    public function handle_pix_webhook( WP_REST_Request $request ) {
        ...
        if ( $token !== self::WEBHOOK_TOKEN ) {
            $this->log_pix_error( 'Token incorreto no webhook' );
            return new WP_REST_Response( array( 'error' => 'forbidden' ), 403 );
        }
        $txid = sanitize_text_field( $request->get_param( 'txid' ) );
        if ( ! $txid ) {
            $this->log_pix_error( 'Webhook sem TXID' );
            return new WP_REST_Response( array( 'error' => 'missing txid' ), 400 );
        }
        ...
        if ( ! $posts ) {
            $this->log_pix_error( 'Aposta nao encontrada para TXID ' . $txid );
