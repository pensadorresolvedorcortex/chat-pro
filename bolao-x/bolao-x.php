...
const VERSION = '2.8.80';
    ...
    private function export_winners_doc( $result, $contest_id, $contest_title, $order_by = 'score' ) {
        ...
        if ( $winners_high ) {
            ...
            echo $this->build_doc_table( $winners_high );
        }
        if ( $hits9 ) {
        ...
        if ( $winners_low ) {
            ...
            echo $this->build_doc_table( $winners_low );
        }
    ...
    private function build_doc_table( $rows ) {
        $out  = '<table border="1" cellpadding="4" cellspacing="0" width="100%">';
        $out .= '<tr><th>' . esc_html__( 'Apostador', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Concurso', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Números Apostados', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Números Sorteados', self::TEXT_DOMAIN ) . '</th><th>' . esc_html__( 'Acertos', self::TEXT_DOMAIN ) . '</th></tr>';
        foreach ( $rows as $r ) {
            $out .= '<tr><td>' . esc_html( $r['name'] ) . '</td><td>' . esc_html( $r['contest'] ) . '</td><td>' . esc_html( $r['numbers'] ) . '</td><td>' . esc_html( $r['drawn'] ) . '</td><td>' . intval( $r['hits'] ) . '</td></tr>';
        }
        $out .= '</table>';
        return $out;
    }
