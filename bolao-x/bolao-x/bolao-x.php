...
class BOLAOX_Plugin {
    ...
    public function render_profile_shortcode() {
        ...
        $html .= '</form>';
        if ( $reload ) {
            $target = wp_get_referer() ? wp_get_referer() : get_permalink();
            $code   = isset( $_POST['bolaox_remove_avatar'] ) ? '2' : '1';
            $redirect = add_query_arg( 'bx_msg', $code, $target );
    ...
    public function render_dashboard_shortcode() {
        ...
        foreach ( $sections as $key => $data ) {
            ...
            if ( 'profile' === $key && $avatar_id ) {
                $icon_html = wp_get_attachment_image( $avatar_id, 'thumbnail', false, array( 'class' => 'bolaox-avatar-icon' ) );
            }
