(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelectorAll('.bolaox-numbers').forEach(function(container){
      ...
      numbers.forEach(function(n){
        ...
        n.innerHTML = '<span class="bx-val">'+val+'</span><span class="bx-count"></span>';
      });

      function update(num){
        var count = counts[num] || 0;
        numbers.forEach(function(n){
          var val = n.querySelector('.bx-val').textContent;
          if(val === num){
            if(count > 0){
              n.classList.add('selected');
              var badge = n.querySelector('.bx-count');
              if(count > 1){
                n.classList.add('multiple');
                badge.textContent = 'x'+count;
              } else {
                n.classList.remove('multiple');
                badge.textContent = '';
              }
            } else {
              n.classList.remove('selected','multiple');
              n.querySelector('.bx-count').textContent = '';
            }
          }
        });
      }
      ...
      numbers.forEach(function(btn){
        btn.addEventListener('click', function(){
          var num = btn.querySelector('.bx-val').textContent;
  ...
  document.querySelectorAll('.bolaox-login-tabs').forEach(function(box){
    ...
    document.querySelectorAll('.bolaox-add-bet').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        container.querySelectorAll('.bolaox-number.selected').forEach(function(el){
          el.classList.remove('selected','multiple');
          var b = el.querySelector('.bx-count');
          if(b) b.textContent = '';
        });
        container.bxCounts = {};
        hidden.value='';
    ...
    function bindPayNow(context){
      (context || document).querySelectorAll('.bolaox-pay-now').forEach(function(btn){
        btn.addEventListener('click', function(e){
          e.preventDefault();
