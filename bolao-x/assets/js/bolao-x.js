(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelectorAll('.bolaox-numbers').forEach(function(container){
      ...
      var list = container.querySelector('.bolaox-selected');
      var numbers = container.querySelectorAll('.bolaox-number');
      var counts = {};
      container.bxCounts = counts;

      function update(num){
        var span = list.querySelector('.bolaox-number[data-num="'+num+'"]');
        var count = counts[num] || 0;
        if(count > 0){
          if(!span){
            span = document.createElement('span');
            span.className = 'bolaox-number sel';
            span.setAttribute('data-num', num);
            span.innerHTML = '<span class="bx-val">'+num+'</span><span class="bx-count"></span>';
            list.appendChild(span);
          }
          var badge = span.querySelector('.bx-count');
          if(count > 1){
            ...
            span.classList.remove('multiple');
          }
          numbers.forEach(function(n){ if(n.textContent===num){ n.classList.add('selected'); }});
        }else if(span){
          span.remove();
          numbers.forEach(function(n){ if(n.textContent===num){ n.classList.remove('selected'); }});
        }
      }

      numbers.forEach(function(btn){
        btn.addEventListener('click', function(){
          var arr = hidden.value ? hidden.value.split(',') : [];
          if(arr.length >= 10) return;
          arr.push(btn.textContent);
          hidden.value = arr.join(',');
          counts[btn.textContent] = (counts[btn.textContent]||0) + 1;
          update(btn.textContent);
        });
      });

      list.addEventListener('click', function(e){
        var span = e.target.closest('.bolaox-number');
  ...
  document.querySelectorAll('.bolaox-login-tabs').forEach(function(box){
    ...
    document.querySelectorAll('.bolaox-add-bet').forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        container.querySelector('.bolaox-selected').innerHTML = '';
        container.bxCounts = {};
        hidden.value='';
        updatePrice();
        updateAddLabel();
    ...
    document.querySelectorAll('.bolaox-pay-now').forEach(function(btn){
      btn.addEventListener('click', function(e){
        e.preventDefault();
        ...
        fetch('/wp-json/bolao-x/v1/create-payment',{
          method:'POST',
          headers:{'Content-Type':'application/json'},
          body: JSON.stringify({qty:1, ref:id})
        }).then(function(r){ return r.json(); }).then(function(data){
          var modal = document.querySelector(btn.getAttribute('data-target'));
          if(!modal) return;
          var img = modal.querySelector('img');
          var code = modal.querySelector('#bolaox-pix-code');
          if(img){
            if(data.qr_code_base64){ img.src = 'data:image/png;base64,'+data.qr_code_base64; }
            else{ img.src = 'https://chart.googleapis.com/chart?chs=500x500&cht=qr&chl='+encodeURIComponent(data.qr_code); }
          }
          if(code){ code.value = data.qr_code; }
          var pid = document.querySelector('input[name="bolaox_payment_id"]');
          if(pid) pid.value = data.id;
