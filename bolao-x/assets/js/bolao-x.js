    ...
    document.querySelectorAll('.bolaox-numbers').forEach(function(container){
      ...
      container.querySelectorAll('.bolaox-number').forEach(function(btn){
        btn.addEventListener('click', function(){
          ...
          s.className = 'bolaox-number sel';
          s.textContent = btn.textContent;
          list.appendChild(s);
        });
      });
      list.addEventListener('click', function(e){
        if(e.target.classList.contains('bolaox-number')){
          var arr = hidden.value ? hidden.value.split(',') : [];
          var idx = Array.prototype.indexOf.call(list.children, e.target);
          arr.splice(idx,1);
          hidden.value = arr.join(',');
          e.target.remove();
        }
