...
class Academia_Simulados_Shortcode {
    ...
    public static function render_shortcode( $atts ) {
        ...
        $requested_slug = get_query_var( 'simulado' );

        if ( isset( $_GET['simulado'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $requested_slug     = sanitize_title( wp_unslash( $_GET['simulado'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $requested_explicit = true;
        ...
        $simulado = self::locate_simulado( $atts, $available_simulados, $requested_slug );
        $questions = array();
        ...
        self::render_simulado_directory( $available_simulados, $simulado ? $simulado->ID : 0 );

        if ( ! $simulado ) {
            ?>
            <p class="academia-simulados-selection-hint"><?php echo $requested_explicit ? esc_html__( 'Simulado não encontrado. Tente selecionar outra opção na lista acima.', 'academia-simulados' ) : esc_html__( 'Escolha um simulado acima para iniciar o seu treino.', 'academia-simulados' ); ?></p>
            <?php
            return ob_get_clean();
        }
        ...
        if ( empty( $questions ) ) {
            ?>
            <p class="academia-simulados-selection-hint"><?php esc_html_e( 'Este simulado ainda não possui questões cadastradas.', 'academia-simulados' ); ?></p>
            <?php
            return ob_get_clean();
        }
    ...
    /**
     * Locate the requested simulado.
     *
     * @param array     $atts Shortcode attributes.
     * @param WP_Post[] $available_simulados Available simulados.
     * @param string    $requested_slug Requested simulado slug.
     *
     * @return WP_Post|null
     */
    protected static function locate_simulado( $atts, $available_simulados, $requested_slug ) {
        $simulado = null;
        ...
        if ( ! $simulado && $requested_slug ) {
            foreach ( $available_simulados as $candidate ) {
                if ( $requested_slug === $candidate->post_name ) {
                    $simulado = $candidate;
                    break;
                }
            }
        ...
        if ( $simulado && ( 'simulado' !== $simulado->post_type || 'publish' !== $simulado->post_status ) ) {
            return null;
        }
     ...
     */
    protected static function render_simulado_directory( $simulados, $current_id ) {
        $base_url  = remove_query_arg( array( 'simulado', 'paged' ) );
        $search_id = wp_unique_id( 'academia-simulados-search-' );
        $help_id   = $search_id . '-help';

        ?>
        <section class="academia-simulados-directory" aria-label="<?php esc_attr_e( 'Catálogo de simulados disponíveis', 'academia-simulados' ); ?>">
            <form class="academia-simulados-search" role="search">
                <label for="<?php echo esc_attr( $search_id ); ?>" class="academia-simulados-search-label"><?php esc_html_e( 'Palavra-chave', 'academia-simulados' ); ?></label>
                <div class="academia-simulados-search-field">
                    <input type="search" id="<?php echo esc_attr( $search_id ); ?>" class="academia-simulados-search-input" placeholder="<?php esc_attr_e( 'Busque por título ou descrição', 'academia-simulados' ); ?>" aria-describedby="<?php echo esc_attr( $help_id ); ?>" />
                    <span id="<?php echo esc_attr( $help_id ); ?>" class="academia-simulados-search-help"><?php esc_html_e( 'Digite para filtrar os simulados listados abaixo.', 'academia-simulados' ); ?></span>
                </div>
            </form>
            <div class="academia-simulados-card-grid" data-empty-message="<?php echo esc_attr__( 'Nenhum simulado corresponde ao filtro informado.', 'academia-simulados' ); ?>">
                <?php foreach ( $simulados as $candidate ) :
                    $question_count = count( (array) get_post_meta( $candidate->ID, Academia_Simulados_Meta_Box::META_KEY, true ) );
                    $summary        = wp_trim_words( wp_strip_all_tags( $candidate->post_content ), 30, '…' );
                    $url            = add_query_arg( 'simulado', $candidate->post_name, $base_url );
                    $search_text    = self::normalise_search_content( $candidate->post_title . ' ' . $candidate->post_content );
                    $is_active      = (int) $candidate->ID === (int) $current_id;
                    ?>
                    <article class="academia-simulados-card <?php echo $is_active ? 'is-active' : ''; ?>" data-search="<?php echo esc_attr( $search_text ); ?>">
                        <a class="academia-simulados-card-link" href="<?php echo esc_url( $url ); ?>" <?php echo $is_active ? 'aria-current="page"' : ''; ?>>
                            <header class="academia-simulados-card-header">
                                <h3><?php echo esc_html( get_the_title( $candidate ) ); ?></h3>
                            </header>
                            <?php if ( $summary ) : ?>
                                <p class="academia-simulados-card-summary"><?php echo esc_html( $summary ); ?></p>
                            <?php endif; ?>
                            <ul class="academia-simulados-card-meta">
                                <li>
                                    <span class="academia-simulados-card-meta-label"><?php esc_html_e( 'Questões', 'academia-simulados' ); ?></span>
                                    <strong><?php echo esc_html( $question_count ); ?></strong>
                                </li>
                                <li>
                                    <span class="academia-simulados-card-meta-label"><?php esc_html_e( 'Comentários', 'academia-simulados' ); ?></span>
                                    <strong><?php echo comments_open( $candidate->ID ) ? esc_html__( 'Habilitados', 'academia-simulados' ) : esc_html__( 'Desativados', 'academia-simulados' ); ?></strong>
                                </li>
                                <li>
                                    <span class="academia-simulados-card-meta-label"><?php esc_html_e( 'Status', 'academia-simulados' ); ?></span>
                                    <strong><?php esc_html_e( 'Disponível', 'academia-simulados' ); ?></strong>
                                </li>
                            </ul>
                        </a>
                    </article>
            ...
            <p class="academia-simulados-card-empty" hidden></p>
        </section>
        <?php
    }

    /**
     * Normalise text for search comparisons.
     *
     * @param string $text Text to normalise.
     *
     * @return string
     */
    protected static function normalise_search_content( $text ) {
