...
$support_type_prefill = isset($form_values['support_type']) ? (string) $form_values['support_type'] : '';
$support_whatsapp_prefill = isset($form_values['support_whatsapp']) ? (string) $form_values['support_whatsapp'] : '';
$support_email_prefill = isset($form_values['support_email']) ? (string) $form_values['support_email'] : '';
...
$support_whatsapp_prefill = trim((string) preg_replace('~^\s*(whats\w*|whatsapp)\s*~i', '', $support_whatsapp_prefill));
$support_email_prefill = trim((string) preg_replace('~^\s*e-?mail\s*~i', '', $support_email_prefill));

if ($support_type_prefill === '' && $current_support !== '') {
    $normalized_support = strtolower($current_support);
    if (strpos($normalized_support, 'whats') !== false) {
        $support_type_prefill = 'whatsapp';
        if ($support_whatsapp_prefill === '') {
            $support_whatsapp_prefill = trim((string) preg_replace('~^\s*(whats\w*|whatsapp)\s*~i', '', $current_support));
        }
    } elseif (strpos($current_support, '@') !== false) {
        $support_type_prefill = 'email';
        if ($support_email_prefill === '') {
            $support_email_prefill = trim((string) preg_replace('~^\s*e-?mail\s*~i', '', $current_support));
        }
    }
}

$support_whatsapp_prefill = $support_whatsapp_prefill !== '' ? $support_whatsapp_prefill : '';
$support_email_prefill = $support_email_prefill !== '' ? $support_email_prefill : '';
                ...
                <?php foreach ($all_groups as $group) :
                    ...
                    <article class="juntaplay-group-card juntaplay-group-card--profile" data-group-item data-group-id="<?php echo esc_attr((string) $group_id); ?>" data-group-role="<?php echo esc_attr($group_role); ?>" data-group-status="<?php echo esc_attr($status); ?>">
                                    ...
                                    <div class="juntaplay-group-card__body">
                                        ...
                                        <?php endif; ?>
                                        <div class="juntaplay-group-card__actions">
                                        ...
                                        <footer class="juntaplay-group-card__footer">
                                            <?php if ($pool_link !== '') : ?>
                                                <a class="juntaplay-link" href="<?php echo esc_url($pool_link); ?>" target="_blank" rel="noopener">
                                                    <?php echo esc_html__('Ver grupo', 'juntaplay'); ?>
                                                </a>
                                            <?php else : ?>
                                                <span class="juntaplay-group-card__note"><?php echo esc_html__('Grupo será definido pelo administrador.', 'juntaplay'); ?></span>
                                            <?php endif; ?>
                                            <?php if ($campaigns_url) : ?>
                                                <div class="juntaplay-group-card__extras">
                                                    <span><?php echo esc_html__('Outras opções', 'juntaplay'); ?></span>
                                                    <a class="juntaplay-link" href="<?php echo esc_url($campaigns_url); ?>"><?php echo esc_html__('Explorar grupos disponíveis', 'juntaplay'); ?></a>
                                                </div>
                                            <?php endif; ?>
                                        </footer>
                                    </div>
    ...
    <template id="jp-group-create-template" data-auto-open="<?php echo !empty($form_errors) ? '1' : '0'; ?>" data-loading-text="<?php echo esc_attr__('Carregando formulário...', 'juntaplay'); ?>">
            <div class="juntaplay-groups__create-modal">
                ...
                <div class="juntaplay-groups__create-body">
                    ...
                    <div class="juntaplay-form__grid">
                    <div class="juntaplay-form__group">
                        <label for="jp-group-support-type"><?php echo esc_html__('Suporte a Membros', 'juntaplay'); ?></label>
                        <select
                            id="jp-group-support-type"
                            name="jp_profile_group_support_type"
                            class="juntaplay-form__input"
                            data-group-share-watch
                        >
                            <option value=""><?php echo esc_html__('Selecione o canal', 'juntaplay'); ?></option>
                            <option value="whatsapp" <?php selected('whatsapp', $support_type_prefill); ?>><?php echo esc_html__('WhatsApp', 'juntaplay'); ?></option>
                            <option value="email" <?php selected('email', $support_type_prefill); ?>><?php echo esc_html__('E-mail', 'juntaplay'); ?></option>
                        </select>
                    </div>
                    <div class="juntaplay-form__group<?php echo $support_type_prefill === 'whatsapp' ? '' : ' is-hidden'; ?>" data-support-field="whatsapp">
                        <label for="jp-group-support-whatsapp"><?php echo esc_html__('Número de WhatsApp', 'juntaplay'); ?></label>
                        <input
                            type="tel"
                            id="jp-group-support-whatsapp"
                            name="jp_profile_group_support_whatsapp"
                            class="juntaplay-form__input"
                            value="<?php echo esc_attr($support_whatsapp_prefill); ?>"
                            placeholder="<?php echo esc_attr__('Ex.: +55 11 90000-0000', 'juntaplay'); ?>"
                            data-group-share-watch
                            data-support-whatsapp-input
                            inputmode="tel"
                            autocomplete="tel-national"
                        />
                    </div>
                    <div class="juntaplay-form__group<?php echo $support_type_prefill === 'email' ? '' : ' is-hidden'; ?>" data-support-field="email">
                        <label for="jp-group-support-email"><?php echo esc_html__('E-mail de suporte', 'juntaplay'); ?></label>
                        <input
                            type="email"
                            id="jp-group-support-email"
                            name="jp_profile_group_support_email"
                            class="juntaplay-form__input"
                            value="<?php echo esc_attr($support_email_prefill); ?>"
                            placeholder="<?php echo esc_attr__('suporte@example.com', 'juntaplay'); ?>"
                            data-group-share-watch
                            autocomplete="email"
                        />
                    </div>
                    <input type="hidden" name="jp_profile_group_support" value="<?php echo esc_attr($current_support); ?>" data-support-hidden />
                    <p class="juntaplay-form__help"><?php echo esc_html__('Este contato só aparece para membros aprovados do grupo.', 'juntaplay'); ?></p>
                    <div class="juntaplay-form__group">
