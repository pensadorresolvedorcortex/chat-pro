(function ($) {
    ...
    function formatWhatsappNumber(value) {
        ...
        var countryLength = Math.max(0, digits.length - 11);
        var country = countryLength > 0 ? digits.slice(0, countryLength) : '55';
        var remaining = countryLength > 0 ? digits.slice(-11) : digits;

        var area = '';
        var local = remaining;

        if (remaining.length > 9) {
            area = remaining.slice(0, 2);
            local = remaining.slice(2);
        ...
        if (local.length > 4) {
            local = local.slice(0, local.length - 4) + '-' + local.slice(-4);
        }

        var parts = ['+' + country.replace(/^0+/, '')];
        if (area) {
            parts.push(area);
        }
        parts.push(local);
    ...
    function buildSupportChannel($form) {
        var type = ($form.find('[name="jp_profile_group_support_type"]').val() || '').toString().toLowerCase().trim();
        var $hidden = $form.find('[data-support-hidden]');
        var support = '';

        if (type === 'whatsapp') {
            var whatsappRaw = ($form.find('[name="jp_profile_group_support_whatsapp"]').val() || '').toString();
            var formattedWhats = formatWhatsappNumber(whatsappRaw);
            support = formattedWhats ? 'WhatsApp ' + formattedWhats : '';
        } else if (type === 'email') {
            var emailRaw = ($form.find('[name="jp_profile_group_support_email"]').val() || '').toString().trim();
            support = emailRaw ? 'E-mail ' + emailRaw : '';
        } else if ($hidden.length) {
            support = $.trim(($hidden.val() || '').toString());
        }
    ...
    function toggleSupportFields($form, type) {
        var normalized = (type || '').toString().toLowerCase();
        $form.find('[data-support-field]').each(function () {
            var $field = $(this);
            var fieldType = ($field.data('support-field') || '').toString();
    ...
    function initializeSupportFields($context) {
        $context.find('form').each(function () {
            var $form = $(this);
            var type = ($form.find('[name="jp_profile_group_support_type"]').val() || '').toString();
            toggleSupportFields($form, type);
            buildSupportChannel($form);
    ...
    function gatherGroupShareData($form) {
        ...
        var supportChannel = buildSupportChannel($form);
    ...
    $(document).on('change', '[name="jp_profile_group_support_type"]', function () {
        var $select = $(this);
        var $form = $select.closest('form');
        toggleSupportFields($form, $select.val());
        buildSupportChannel($form);
        updateGroupSharePreview($form);
    });

    $(document).on('input change', '[name="jp_profile_group_support_whatsapp"], [name="jp_profile_group_support_email"]', function () {
        var $input = $(this);
        var $form = $input.closest('form');
        buildSupportChannel($form);
    });

    $(document).on('blur', '[data-support-whatsapp-input]', function () {
        var $input = $(this);
        var formatted = formatWhatsappNumber($input.val());
        if (formatted) {
            $input.val(formatted);
            buildSupportChannel($input.closest('form'));
        }
    });

    $(function () {
        initializeSupportFields($(document));
    });
