...
class Profile
{
    ...
    private function get_group_cover_placeholder(): string
    {
        ...
        $plugins_placeholder = plugins_url('../../assets/img/group-cover-placeholder.svg', __FILE__);
        if ($plugins_placeholder !== '') {
            return $plugins_placeholder;
        }

        $svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 6 5" preserveAspectRatio="xMidYMid slice">'
            . '<defs><linearGradient id="jpCover" x1="0" x2="1" y1="0" y2="1">'
            . '<stop offset="0%" stop-color="%23DDE3EA" />'
            . '<stop offset="100%" stop-color="%23F1F3F6" />'
            . '</linearGradient></defs>'
            . '<rect width="6" height="5" fill="url(%23jpCover)" />'
            . '<g fill="%2390A4B8" opacity="0.8">'
            . '<path d="M1 3.2 2.4 1.6l.8.9.8-.7L5 3.4v.8H1z" />'
            . '<circle cx="1.4" cy="1.3" r="0.5" />'
            . '</g>'
            . '</svg>';
    ...
    private function prepare_group_entry(array $group, bool $is_owner): array
    {
        ...
        if ($cover_url === '') {
            $cover_url = $this->get_group_cover_placeholder();
            if ($cover_url !== '') {
    ...
    private function build_group_share_snippet(array $group): array
    {
        ...
        if ($support !== '') {
            $lines[] = sprintf(__('Suporte a membros: %s', 'juntaplay'), $support);
        }
    ...
    private function create_group(int $user_id): void
    {
        ...
        $support_hidden_raw = isset($_POST['jp_profile_group_support']) ? wp_unslash($_POST['jp_profile_group_support']) : '';
        $support_type_raw   = isset($_POST['jp_profile_group_support_type']) ? wp_unslash($_POST['jp_profile_group_support_type']) : '';
        $support_whatsapp_raw = isset($_POST['jp_profile_group_support_whatsapp']) ? wp_unslash($_POST['jp_profile_group_support_whatsapp']) : '';
        $support_email_raw  = isset($_POST['jp_profile_group_support_email']) ? wp_unslash($_POST['jp_profile_group_support_email']) : '';
        ...
        $support_inputs = $this->resolve_support_channel_inputs(
            is_string($support_type_raw) ? (string) $support_type_raw : '',
            $support_whatsapp_raw,
            $support_email_raw,
            is_string($support_hidden_raw) ? (string) $support_hidden_raw : ''
        );

        $support_type     = $support_inputs['type'];
        $support          = $support_inputs['channel'];
        $support_whatsapp = $support_inputs['whatsapp'];
        $support_email    = $support_inputs['email'];
        ...
        if ($cover_preview === '') {
            $cover_preview = $this->get_group_cover_placeholder();
        }
        ...
        if ($support_inputs['error'] !== '') {
            $this->add_error('group_create', $support_inputs['error']);
    ...
    /**
     * @param mixed $whatsapp_raw
     * @param mixed $email_raw
     * @return array{channel: string, type: string, whatsapp: string, email: string, error: string}
     */
    private function resolve_support_channel_inputs(string $type, $whatsapp_raw, $email_raw, string $fallback): array
    {
        $selected_type = sanitize_key($type);
        $raw_whatsapp = is_string($whatsapp_raw) ? (string) $whatsapp_raw : '';
        $raw_email    = is_string($email_raw) ? (string) $email_raw : '';

        $formatted_whatsapp = $this->format_whatsapp_number($raw_whatsapp);
        $sanitized_email    = sanitize_email($raw_email);

        $channel = '';
        $error   = '';

        if (in_array($selected_type, ['whatsapp', 'email'], true)) {
            if ($selected_type === 'whatsapp') {
                if ($formatted_whatsapp === '') {
                    $error = __('Informe um número de WhatsApp válido com DDI e DDD.', 'juntaplay');
                } else {
                    $channel = sprintf(__('WhatsApp %s', 'juntaplay'), $formatted_whatsapp);
                }
            } elseif ($selected_type === 'email') {
                if ($sanitized_email === '' || !is_email($sanitized_email)) {
                    $error = __('Informe um e-mail válido para suporte.', 'juntaplay');
                } else {
                    $channel = sprintf(__('E-mail %s', 'juntaplay'), $sanitized_email);
                }
            }
        } else {
            $channel = trim($fallback);
            $selected_type = $channel !== '' ? $selected_type : '';
        }

        return [
            'channel'  => $channel,
            'type'     => $selected_type,
            'whatsapp' => $formatted_whatsapp !== '' ? $formatted_whatsapp : preg_replace('/\s+/', ' ', trim($raw_whatsapp)),
            'email'    => $sanitized_email !== '' ? $sanitized_email : trim($raw_email),
            'error'    => $error,
        ];
    }

    private function format_whatsapp_number(string $input): string
    {
        ...
        $country_length = max(0, strlen($digits) - 11);
        $country = $country_length > 0 ? substr($digits, 0, $country_length) : '55';
        $remaining = $country_length > 0 ? substr($digits, -11) : $digits;

        $area = '';
        $local = $remaining;

        if (strlen($remaining) > 9) {
            $area = substr($remaining, 0, 2);
            $local = substr($remaining, 2);
        } elseif (strlen($remaining) > 8) {
            $area = substr($remaining, 0, 2);
            $local = substr($remaining, 2);
        }

        if (strlen($local) > 4) {
            $local = substr($local, 0, strlen($local) - 4) . '-' . substr($local, -4);
        }

        $parts = ['+' . ltrim($country, '+0')];
        if ($area !== '') {
            $parts[] = $area;
        }
        $parts[] = $local;
    ...
    public function handle_group_update(int $group_id, array $data, array $files = []): array
    {
        ...
        $cover_id        = absint($data['cover_id'] ?? 0);

        if (!isset($categories[$category])) {
            $category = 'other';
        }

        if (isset($files['group_cover']) && is_array($files['group_cover']) && !empty($files['group_cover']['name'])) {
            require_once ABSPATH . 'wp-admin/includes/file.php';
            require_once ABSPATH . 'wp-admin/includes/media.php';
            require_once ABSPATH . 'wp-admin/includes/image.php';
            ...
            if (!is_wp_error($uploaded)) {
                ...
                $error_message = $this->format_upload_error($uploaded);
