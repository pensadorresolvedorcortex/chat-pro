...
class GroupCreation
{
    ...
    public function handle_form_submission(): void
    {
        ...
        $relationship_input = isset($_POST['jplay_relationship_admin'])
            ? sanitize_text_field(wp_unslash($_POST['jplay_relationship_admin']))
            : '';
        ...
        if ($relationship_input === '' && isset($_POST['jp_group_relationship'])) {
            $relationship_input = sanitize_text_field(wp_unslash($_POST['jp_group_relationship']));
        }

        $relationship = $this->normalize_relationship_label($relationship_input);
        ...
        if ($service_url === '') {
            $this->errors[] = __('Informe a URL do serviço compartilhado.', 'juntaplay');
        }
        ...
        $message = __('✅ Recebemos sua submissão. A equipe Juntaplay vai avaliar se o grupo cumpre todos os requisitos e você será notificado por e-mail.', 'juntaplay');
    ...
    public function render_create_form(): string
    {
        ...
        <div class="jp-group-create">
            ...
            <form method="post" class="jp-group-form" novalidate>
                ...
                <div class="jp-field">
                    <label for="jplay_relationship_admin" class="jp-label"><?php esc_html_e('Qual a relação com o administrador?', 'juntaplay'); ?></label>
    ...
    public function render_relationship_shortcode($atts = []): string
    {
        ...
        if (is_array($atts) && isset($atts['selected'])) {
            $selected = sanitize_text_field((string) $atts['selected']);
        }
    ...
    public function maybe_prepare_success_notice(): void
    {
        ...
        if (!is_array($state)) {
            $this->success_notice = __('✅ Recebemos sua submissão. A equipe Juntaplay vai avaliar se o grupo cumpre todos os requisitos e você será notificado por e-mail.', 'juntaplay');
        ...
        $this->success_notice = $message !== ''
            ? $message
            : __('✅ Recebemos sua submissão. A equipe Juntaplay vai avaliar se o grupo cumpre todos os requisitos e você será notificado por e-mail.', 'juntaplay');
    ...
    public function handle_two_factor_success(WP_User $user, array $state): void
    {
        ...
        $encrypted_password = '';
        try {
            $encrypted_password = Crypto::encrypt($payload['service_password']);
        ...
        $post_id = wp_insert_post([
            ...
            'meta_input'  => [
                'jplay_relationship_admin' => $payload['relationship'],
                'jplay_shared_service'     => $payload['service_name'],
                'jplay_shared_user'        => $payload['service_login'],
                'jplay_shared_pass_enc'    => $encrypted_password,
                'jplay_shared_url'         => $payload['service_url'],
                'jplay_shared_notes'       => $payload['notes'],
    ...
    private function render_relationship_fields(string $selected): string
    {
        ...
        $current = $this->normalize_relationship_label($selected);

        ob_start();
        ?>
        <select
            id="jplay_relationship_admin"
            name="jplay_relationship_admin"
            class="jp-input"
            required
            aria-required="true"
        >
            ...
            <?php foreach ($options as $label) : ?>
                <option value="<?php echo esc_attr($label); ?>" <?php selected($current, $label); ?>><?php echo esc_html($label); ?></option>
            <?php endforeach; ?>
        </select>
        <?php
    ...
    {
        return [
            'moramos-juntos'      => __('Moramos Juntos', 'juntaplay'),
            'familia'             => __('Família', 'juntaplay'),
            'amigos'              => __('Amigos', 'juntaplay'),
            'colegas-de-trabalho' => __('Colegas de Trabalho', 'juntaplay'),
        ];
    }

    private function normalize_relationship_label(string $value): string
    {
        $value = trim($value);
        if ($value === '') {
            return '';
        ...
        foreach ($options as $slug => $label) {
            if (strcasecmp($value, $label) === 0 || strcasecmp($value, $slug) === 0) {
                return $label;
            }
        }
    ...
    {
        $group_name = isset($payload['group_name']) ? sanitize_text_field((string) $payload['group_name']) : '';
        $relationship = isset($payload['relationship'])
            ? $this->normalize_relationship_label((string) $payload['relationship'])
            : '';
        $service_name = isset($payload['service_name']) ? sanitize_text_field((string) $payload['service_name']) : '';
        $service_login = isset($payload['service_login']) ? sanitize_text_field((string) $payload['service_login']) : '';
        $service_password = isset($payload['service_password']) ? trim((string) $payload['service_password']) : '';
        $service_url = isset($payload['service_url']) ? esc_url_raw((string) $payload['service_url']) : '';
