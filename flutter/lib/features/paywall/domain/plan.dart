...
class Plan extends Equatable {
  ...
  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{
      ...
      'tipo': _planTypeToString(type),
      'periodicidade': periodicity,
      'preco': price,
      'moeda': currency,
      'beneficios': benefits,
      'statusAprovacao': _approvalStatusToString(approvalStatus),
      'chavePix': pixKey,
      'pixCheckout': pix?.toJson(),
      'aprovadoPor': approvedBy,
      'aprovadoEm': approvedAt?.toIso8601String(),
      'atualizadoEm': updatedAt?.toIso8601String(),
      'ultimaSolicitacao': lastRequestedAt?.toIso8601String(),
      'destaque': isFeatured,
    };
...
String _planTypeToString(PlanType type) {
  switch (type) {
    case PlanType.paid:
      return 'paid';
    case PlanType.freeStudent:
      return 'free_student';
  }
}

String _approvalStatusToString(PlanApprovalStatus status) {
  switch (status) {
    case PlanApprovalStatus.approved:
      return 'approved';
    case PlanApprovalStatus.pending:
      return 'pending';
    case PlanApprovalStatus.rejected:
      return 'rejected';
  }
}
