...
> ✅ **Compatibilidade 3.35**: a camada Android utiliza AGP 8.7.3, Kotlin 2.0.21 e Gradle 8.9 — combinação alinhada ao plugin oficial que acompanha o Flutter 3.35. Execute `python ../scripts/bootstrap_gradle_wrapper.py` sempre que clonar o repositório para baixar o wrapper correspondente.
>
> ⚠️ **Erro `com/android/builder/model/BuildType`**: esse stack trace indica um classpath faltante nas instalações onde o AGP 8.x removeu as interfaces legadas. O `android/build.gradle` agora injeta automaticamente `com.android.tools.build:builder-model:7.4.2` no classpath do Gradle para que o plugin `dev.flutter.flutter-gradle-plugin` funcione em máquinas com Flutter 3.35. Caso personalize o `build.gradle` raiz, mantenha esse bloco `buildscript` para evitar o `NoClassDefFoundError`.
