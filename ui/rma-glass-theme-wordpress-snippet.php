...
'
        . '.rma-auth-title{margin:0 0 6px;font-size:1.1rem;color:#1f2937;}
'
        . '.rma-auth-subtitle{margin:0 0 14px;color:#5a6472;font-size:.92rem;}
'
        . '.rma-otp-grid{display:grid;grid-template-columns:repeat(6,minmax(0,50px));gap:10px;margin-bottom:12px;}
'
...
'
        . '.rma-resend-link{color:#6b7280;font-size:.86rem;text-decoration:none;}
'
...
'
        . '.rma-checkout-summary{margin-top:16px;padding:14px;border:1px solid #edf1f4;border-radius:14px;background:#fafcfd;}
'
        . '.rma-checkout-summary li{display:flex;justify-content:space-between;padding:4px 0;color:#4b5563;}\n'
...
'
    ...
    if ($entity_id > 0) {
        ...
        <section class="rma-glass-card rma-premium-card" style="margin:20px 0;">
            ...
            <p class="rma-glass-subtitle">Seu cadastro está quase concluído. Finalize as próximas etapas para garantir acesso integral aos recursos da RMA.</p>

            <section class="rma-auth-card" id="rma-auth-card">
                <h3 class="rma-auth-title">Verificação de segurança</h3>
                <p class="rma-auth-subtitle">Enviamos um código de 6 dígitos para seu email. Insira abaixo para continuar.</p>
            ...
            <aside class="rma-checkout-summary">
                <strong>Resumo do Checkout</strong>
                <ul style="list-style:none;margin:8px 0 0;padding:0;">
                    <li><span>Nome</span><span><?php echo esc_html(wp_get_current_user()->display_name); ?></span></li>
                    <li><span>Email</span><span><?php echo esc_html(wp_get_current_user()->user_email); ?></span></li>
                    <li><span>Plano</span><span>Filiação Institucional</span></li>
                    <li><span>Valor</span><span><?php echo esc_html(get_option('rma_annual_due_value', 'A definir')); ?></span></li>
                    <li><span>Status documentos</span><span id="rma-summary-doc-status"><?php echo esc_html($docs_status ?: 'pendente'); ?></span></li>
                </ul>
            </aside>
        ...
        (function () {
            ...
            function updateState(payload) {
                ...
                var docSummary = document.getElementById('rma-summary-doc-status');
                ...
                if (docSummary) docSummary.textContent = payload.documentos_status || 'pendente';
