...
add_shortcode('rma_conta_setup', function () {
    ...
    $required_docs = [
        'ficha_inscricao' => [
            'label' => 'Ficha de inscrição cadastral',
            'tip' => 'Ficha de Inscrição Cadastral (preenchida e assinada, assinatura digital gov.br aceita).',
        ],
        'comprovante_cnpj' => [
            'label' => 'Comprovante de inscrição e situação cadastral do CNPJ',
            'tip' => 'Comprovante de Inscrição e Situação Cadastral do CNPJ (Receita Federal).',
        ],
        'ata_fundacao' => [
            'label' => 'Ata de fundação',
            'tip' => 'Ata de Fundação (*) – cópia simples ou PDF escaneado com carimbo de registro.',
        ],
        'ata_diretoria' => [
            'label' => 'Ata de eleição da atual diretoria',
            'tip' => 'Ata de eleição da atual diretoria (*) – cópia simples ou PDF escaneado com carimbo de registro.',
        ],
        'estatuto' => [
            'label' => 'Estatuto e alterações',
            'tip' => 'Estatuto + Ata da Assembleia que aprovou o Estatuto (*) – cópias.',
        ],
        'relatorio_atividades' => [
            'label' => 'Relatório de atividades (2 anos)',
            'tip' => 'Relatório de atividades dos últimos 2 anos.',
        ],
        'cartas_recomendacao' => [
            'label' => 'Duas cartas de recomendação',
            'tip' => 'Duas cartas de recomendação de organizações filiadas à RMA da mesma região.',
        ],
    ];
                ...
                <?php foreach ($required_docs as $doc_key => $doc_meta) : ?>
                    <div class="rma-doc-item" style="border:1px solid rgba(123,173,57,.25);border-radius:12px;padding:10px;">
                        <label style="font-weight:600;display:block;margin-bottom:6px;" for="doc_<?php echo esc_attr($doc_key); ?>">
                            <?php echo esc_html($doc_meta['label']); ?>
                            <span title="<?php echo esc_attr($doc_meta['tip']); ?>" style="cursor:help;color:#7bad39;margin-left:6px;">ⓘ</span>
                        </label>
        ...
        <div style="display:grid; gap:8px; margin-top:8px;">
                <?php foreach ($required_docs as $doc_key => $doc_meta) : ?>
                    <div style="border:1px solid rgba(123,173,57,.25);border-radius:10px;padding:8px;">
                        <strong><?php echo esc_html($doc_meta['label']); ?></strong>
                        <span title="<?php echo esc_attr($doc_meta['tip']); ?>" style="cursor:help;color:#7bad39;margin-left:6px;">ⓘ</span>
                        <input type="file" id="pre_doc_<?php echo esc_attr($doc_key); ?>" accept="application/pdf" style="display:block;margin-top:6px;" />
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    (function () {
        ...
        function uploadPreSelectedDocuments(entityId) {
            var queued = requiredDocKeys.filter(function (docKey) {
                var input = document.getElementById('pre_doc_' + docKey);
            ...
            queued.forEach(function (docKey) {
                sequence = sequence.then(function () {
                    ...
                    return fetch(base + '/entities/' + entityId + '/documents', {
                        ...
                        return res.json().then(function (json) {
                            if (!res.ok) {
                                throw new Error((json && json.message) ? json.message : 'Falha no upload de documentos.');
                            }
                        });
                    });
                });
        ...
        form.addEventListener('submit', function (event) {
            ...
            .then(function (result) {
                ...
                var createdEntityId = result.json && result.json.post_id ? parseInt(result.json.post_id, 10) : 0;
                if (createdEntityId > 0) {
                    showMessage('Entidade criada. Enviando documentos selecionados...', true);
                    uploadPreSelectedDocuments(createdEntityId)
                        .then(function () {
                            showMessage('Cadastro e uploads concluídos. Carregando próximas etapas...', true);
                            setTimeout(function () {
                                window.location.replace(contaUrl + '?rma_flow=1');
                            }, 900);
                        })
                        .catch(function (err) {
                            showMessage('Entidade criada, mas houve erro no upload: ' + (err.message || 'tente novamente no passo de documentos.'), false);
                            setTimeout(function () {
                                window.location.replace(contaUrl + '?rma_flow=1');
                            }, 1500);
                        });
