...
'
        . '.rma-flow-stepper{position:relative;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:14px;margin:16px 0 18px;--rma-progress:0%;}
'
        . '.rma-flow-stepper::before{content:"";position:absolute;left:8%;right:8%;top:32px;height:4px;border-radius:999px;background:#e9eef3;z-index:0;}
'
        . '.rma-flow-stepper::after{content:"";position:absolute;left:8%;top:32px;height:4px;border-radius:999px;width:var(--rma-progress);background:linear-gradient(135deg,#7bad39,#5ddabb);z-index:1;transition:width .35s ease;}
'
        . '.rma-flow-step{position:relative;z-index:2;background:#fff;border:1px solid #e8edf2;border-radius:14px;padding:12px 10px 10px;box-shadow:0 10px 30px rgba(0,0,0,.06);display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center;transition:transform .25s ease,box-shadow .25s ease,border-color .25s ease,filter .25s ease;}
'
        . '.rma-flow-step:hover{transform:translateY(-2px);box-shadow:0 14px 32px rgba(0,0,0,.1);}
'
        . '.rma-flow-step-figure{width:58px;height:58px;border-radius:18px;background:radial-gradient(circle at 30% 25%,rgba(93,218,187,.35),rgba(123,173,57,.14));display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 0 1px rgba(123,173,57,.2);}
'
        . '.rma-flow-step-figure svg{width:48px;height:48px;display:block;}
'
        . '.rma-flow-step-label{font-size:.82rem;font-weight:700;color:#475569;line-height:1.15;}
'
