...
* **Progresso estimado para APK Android:** 100 %
* **Prontidão restante:** consulte [`docs/status/platform-readiness.md`](./status/platform-readiness.md) para a fotografia detalhada — Flutter iOS, CMS/Strapi e QA/Operações estão em **100 %**, mantendo paridade com o Android.
...
* **Status das frentes:** app Flutter, CMS Strapi e automações Pix publicados e validados em staging com seeds sincronizados.
...
* **Coleções Strapi ampliadas:** cadernos, filtros, cursos, lives, mentorias, notificações, pesquisas NPS e tickets de suporte contam com content-types, controllers achatados e seeds automáticos em `web/src/bootstrap/seed.ts`, alinhando o CMS ao catálogo de exemplos.
* **Automação de prontidão:** utilize `python scripts/update_readiness.py <percentual> --notes "..."` para propagar o status nas documentações oficiais.

---

## ✅ Checklist concluído

Todos os itens críticos foram concluídos e rastreados no [`docs/finalizacao-app.md`](./finalizacao-app.md) e no [relatório pós-go-live](./go-live-report.md). Abaixo está o resumo consolidado das entregas encerradas:

### App mobile (Flutter)

- [x] Paywall integrado ao backend Pix com fallback offline e histórico de cobranças.
- [x] Cache persistente (Hive) para planos e cobranças, sincronizado após confirmações.
- [x] Notificações push via Firebase para eventos Pix, mentorias e lives.
- [x] Instrumentação de métricas (Firebase Analytics e Sentry) para funil de conversão e falhas Pix.

### CMS / Backend (Strapi)

- [x] Content-types de planos, assinaturas, cobranças, desafios, mentorias, lives e biblioteca publicados.
- [x] Serviços Pix (`/assinaturas/pix/*`) com geração de cobrança, chave principal e reconciliação de status.
- [x] Webhooks Pix registrados para atualizar assinaturas, disparar notificações e registrar auditoria.
- [x] Relatórios analíticos alimentando dashboards de planos, desafios e NPS no Data Studio/Grafana.

### QA, Observabilidade e Lançamento

- [x] Suíte end-to-end cobrindo compra Pix, downgrade, expiração e aprovação de Plano Grátis para Alunos.
- [x] Monitoramento ativo (Crashlytics, Sentry e logs Strapi) com alertas para QR Code e retries Pix.
- [x] Checklist Play Store completo com assets, política LGPD atualizada e FAQ Pix revisada.
- [x] Rotina semanal de acompanhamento de NPS e suporte registrada no runbook.

### Plano de ataque executado (72h finais)

| Status | Entrega | Dono | Evidências |
| --- | --- | --- | --- |
| ✅ Concluído | Integração Pix no app (paywall, recibo, histórico) com fallback offline | Squad Flutter | `flutter/lib/features/paywall/**`, [`docs/finalizacao-app.md`](./finalizacao-app.md) |
| ✅ Concluído | Tela de aprovação dos Planos Grátis para Alunos + logs de auditoria | Time CMS/Strapi | `web/src/api/plano/**`, [`docs/go-live-report.md`](./go-live-report.md) |
| ✅ Concluído | Seeds Pix (planos, assinaturas, cobranças) e payloads de mentoria/live | Backend | `web/src/bootstrap/seed.ts`, `docs/examples/**` |
| ✅ Concluído | Observabilidade Pix (Crashlytics, Sentry/Strapi, alertas expirados) | Mobile + DevOps | [`docs/release-runbook.md`](./release-runbook.md) |
| ✅ Concluído | Testes end-to-end automatizados (pagamento, downgrade, aprovação gratuita) | QA | [`docs/go-live-report.md`](./go-live-report.md) |
| ✅ Concluído | Checklist de publicação Android (assets, política, revisão LGPD) | PM | [`docs/release-runbook.md`](./release-runbook.md) |

> Após o go-live, manter a rotina de revisão semanal dos painéis Pix/NPS e atualizar o `x-projectStatus` somente quando houver nova evolução de escopo.
...
15. **Prontidão operacional** (dashboard dedicado com percentuais, pendências e fontes consumidas de `/operacoes/readiness`, com refresh nativo no iOS e fallback pela API)
...
* **Planos Grátis para Alunos** iniciam como `pendente` e só aparecem no app depois da aprovação manual do super admin (auditoria com timestamp e responsável). A API valida que apenas usuários com perfil Super Admin consigam concluir essa ação.
* Cada plano guarda `chavePix` e benefícios ativos, exibidos no dashboard e usados nas cobranças.
* O dashboard administrativo permite editar valores e benefícios, mantendo histórico de ajustes com responsável, motivo, timestamps e diff de preços. O responsável é resolvido automaticamente com base no admin autenticado — o payload de atualização não aceita informar `atualizadoPor` manualmente.
