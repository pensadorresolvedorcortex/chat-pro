...
class Router {
    ...
    protected function should_force_onboarding( $current_url, $onboarding_url ) {
        ...
        $allowed = apply_filters(
            'adc_login_onboarding_allowed_urls',
            array(
                get_onboarding_skip_url(),
                get_onboarding_start_url(),
            )
        );

        if ( $this->matches_allowed_url( $current_url, $allowed ) ) {
            return false;
        }
    ...
    /**
     * Check if the current URL matches any of the allowed destinations.
     *
     * @param string $current_url Current URL.
     ...
     * @return bool
     */
    protected function matches_allowed_url( $current_url, $allowed ) {
        if ( empty( $allowed ) ) {
            return false;
        }
        ...
        foreach ( (array) $allowed as $candidate ) {
            ...
            if ( $normalized_current === $this->normalize_url( $candidate ) ) {
                return true;
            }
        }
    ...
    protected function normalize_url( $url ) {
        ...
        if ( false === $parts ) {
            $trimmed = preg_replace( '#^https?://#i', '', $url );

            return strtolower( untrailingslashit( $trimmed ) );
        }
