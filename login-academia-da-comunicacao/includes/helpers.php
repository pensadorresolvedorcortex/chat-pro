...
function get_onboarding_skip_url() {
    ...
    return esc_url_raw( $base_url . 'home' );
}

/**
 * Retrieve the onboarding CTA target URL.
 *
 * @return string
 */
function get_onboarding_start_url() {
    $start_url = get_option_value( 'onboarding_start_url', '' );

    if ( ! empty( $start_url ) ) {
        return esc_url_raw( $start_url );
    }
...
/**
 * Retrieve the onboarding login link URL.
 *
 * @return string
 */
function get_onboarding_login_url() {
    $login_url = get_option_value( 'onboarding_login_url', '' );
...
/**
 * Retrieve the onboarding signup link URL.
 *
 * @return string
 */
function get_onboarding_signup_url() {
    $signup_url = get_option_value( 'onboarding_signup_url', '' );

    if ( ! empty( $signup_url ) ) {
        return esc_url_raw( $signup_url );
    }
...
/**
 * Retrieve the onboarding login step URL.
 *
 * @return string
 */
function get_onboarding_login_step_url() {
    return esc_url_raw( add_query_arg( 'step', 'login', get_onboarding_url() ) );
}

/**
 * Retrieve the onboarding signup step URL.
 *
 * @return string
 */
function get_onboarding_signup_step_url() {
    return esc_url_raw( add_query_arg( 'step', 'signup', get_onboarding_url() ) );
}
...
/**
 * Determine the capability required to manage the plugin settings and content.
 *
 * The capability can be customized via the `adc_login_required_capability` filter. If the
 * current user does not hold the filtered capability, the function will try common fallback
 * capabilities so that trusted editorial roles (e.g., Editors) can still access the dashboard.
 *
 * @return string
 */
function get_admin_capability() {
    $default_capability = 'manage_options';
    $capability         = apply_filters( 'adc_login_required_capability', $default_capability );
    ...
    $fallbacks = apply_filters(
        'adc_login_fallback_capabilities',
        array( 'manage_options', 'edit_theme_options', 'edit_pages', 'edit_posts' )
    );

    $candidates = array_unique(
        array_filter(
            array_map( 'strval', array_merge( array( $capability ), (array) $fallbacks ) )
        )
    );
