(function () {
        ...
        Object.keys(charts).forEach(function (key) {
            ...
            if (chart && typeof chart.resize === 'function') {
                try {
                    ...
                    // Ignore resize failures to avoid breaking other interactions.
                }
            }
        });
    }
    ...
    function notifyLayoutChange(source) {
        if (typeof document === 'undefined') {
            return;
        }

        var detail = { source: source || 'unknown' };
        var eventName = 'dap:layout-change';
        var layoutEvent;

        if (typeof root.CustomEvent === 'function') {
            layoutEvent = new CustomEvent(eventName, { detail: detail });
        } else if (document.createEvent) {
            layoutEvent = document.createEvent('CustomEvent');
            layoutEvent.initCustomEvent(eventName, false, false, detail);
        }
    ...
    function observeMenuState(buttons) {
        ...
        var observer = new MutationObserver(function () {
            ...
            notifyLayoutChange('menu-mutation');

            if (typeof root.requestAnimationFrame === 'function') {
                root.requestAnimationFrame(resizeAllCharts);
            } else {
    ...
    function bindMenuToggle() {
        ...
        buttonList.forEach(function (button) {
            button.addEventListener('click', function (event) {
                ...
                notifyLayoutChange('menu-toggle');
        ...
        if (collapseToggle) {
            collapseToggle.addEventListener('click', function () {
                ...
                delay(function () {
                    syncMenuToggleState(buttonList);
                    notifyLayoutChange('collapse-toggle');
                    resizeAllCharts();
                }, 60);
            });
        }
        ...
        if (menuBackdrop) {
            menuBackdrop.addEventListener('click', function () {
                ...
                if (!document.body.classList.contains('folded')) {
                    ...
                    syncMenuToggleState(buttonList);
                    notifyLayoutChange('menu-backdrop');
                    setTimeout(resizeAllCharts, 120);
                }
            });
        }
    ...
    function bindResponsiveCharts() {
        var debouncedResize = debounce(resizeAllCharts, 140);

        if (typeof root.addEventListener === 'function') {
            root.addEventListener('resize', debouncedResize);
        }

        document.addEventListener('dap:layout-change', function () {
            debouncedResize();
