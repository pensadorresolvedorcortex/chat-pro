...
if ( ! function_exists( 'dap_get_dashboard_max_width' ) ) {
    /**
     * Returns the maximum width applied to the dashboard shell.
     *
     * @return int
     */
    function dap_get_dashboard_max_width() {
        $width = (int) dap_get_option( 'dashboard_max_width', 1500 );
...
if ( ! function_exists( 'dap_get_user_theme_mode' ) ) {
    /**
     * Retrieves the persisted dashboard theme mode for a user.
     *
     * @param int $user_id Optional user id.
     *
     * @return string
     */
    function dap_get_user_theme_mode( $user_id = 0 ) {
        ...
        $mode = get_user_meta( $user_id, 'dap_dashboard_theme_mode', true );
...
if ( ! function_exists( 'dap_set_user_theme_mode' ) ) {
    /**
     * Persists the preferred dashboard theme mode for a user.
     *
     ...
     * @return bool
     */
    function dap_set_user_theme_mode( $mode, $user_id = 0 ) {
        $user_id = $user_id ? (int) $user_id : get_current_user_id();
        ...
        update_user_meta( $user_id, 'dap_dashboard_theme_mode', $mode );

        return true;
    }
}

if ( ! function_exists( 'dap_get_theme_mode_label' ) ) {
    /**
     * Provides a localized label for the supplied theme mode.
     *
     * @param string $mode Theme mode key.
     *
     * @return string
     */
    function dap_get_theme_mode_label( $mode ) {
        switch ( $mode ) {
            case 'light':
                return esc_html__( 'Modo claro', 'dap' );
            case 'dark':
                return esc_html__( 'Modo escuro', 'dap' );
            case 'auto':
            default:
                return esc_html__( 'Modo automático', 'dap' );
        }
    }
}
...
if ( ! function_exists( 'dap_get_dashboard_palette' ) ) {
    /**
     * Returns the colour palette used across the dashboard widgets.
     *
     * @return array
     */
    function dap_get_dashboard_palette() {
        $palette = [
            'primary'   => '#7366ff',
            'secondary' => '#2bb0f8',
            'accent'    => '#45c2ff',
        ...
        /**
         * Filters the dashboard palette before it is consumed by the UI.
         *
         * @param array $palette Hex colours keyed by intent.
         */
        return apply_filters( 'dap_dashboard_palette', $palette );
    }
}
                    ...
                    <div class="card h-100 dap-default-widget">
                        <div class="card-body d-flex flex-column gap-4">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div>
                                    <h5 class="mb-0 fw-semibold"><?php echo esc_html__( 'Visão do pipeline', 'dap' ); ?></h5>
                            ...
                            <div class="row g-3">
                                <div class="col-6">
                                    ...
                                    <h3 class="fw-semibold mb-0"><?php echo esc_html__( 'Descoberta', 'dap' ); ?></h3>
                                </div>
                ...
                <div class="col-12 col-xl-3">
                    <div class="card h-100 dap-default-widget">
                        <div class="card-body d-flex flex-column gap-4">
                            ...
                            <ul class="list-unstyled mb-0 small text-muted">
                                <li class="mb-2">
                                    <span class="fw-semibold text-dark d-block"><?php echo esc_html__( 'Daily das squads', 'dap' ); ?></span>
...
if ( ! function_exists( 'dap_get_dashboard_kpis' ) ) {
    ...
    function dap_get_dashboard_kpis() {
        ...
        return [
            [
                'key'         => 'total_projects',
                'label'       => esc_html__( 'Projetos totais', 'dap' ),
            ...
            [
                'key'         => 'new_projects',
                'label'       => esc_html__( 'Novos projetos', 'dap' ),
            ...
            [
                'key'         => 'on_progress',
                'label'       => esc_html__( 'Em andamento', 'dap' ),
            ...
            [
                'key'         => 'unfinished',
                'label'       => esc_html__( 'Incompletos', 'dap' ),
    ...
    function dap_get_dashboard_sales_cards( $kpis ) {
        ...
        if ( dap_is_woocommerce_active() ) {
            ...
            return apply_filters(
                ...
                [
                    [
                        'label'       => esc_html__( 'Vendas totais', 'dap' ),
                    ...
                    [
                        'label'       => esc_html__( 'Pedidos efetuados', 'dap' ),
                    ...
                    [
                        'label'       => esc_html__( 'Clientes ativos', 'dap' ),
                    ...
                    [
                        'label'       => esc_html__( 'Faturas pendentes', 'dap' ),
...
if ( ! function_exists( 'dap_get_recent_projects_table' ) ) {
    ...
    function dap_get_recent_projects_table() {
        ...
        if ( empty( $rows ) ) {
            $rows = [
                [ 'project' => esc_html__( 'Reestruturação do Privilege Commerce', 'dap' ), 'owner' => 'Squad Growth', 'status' => esc_html__( 'Em revisão', 'dap' ), 'badge' => 'warning', 'progress' => '78%' ],
                [ 'project' => esc_html__( 'Fluxos de automação do CRM', 'dap' ), 'owner' => 'Squad CRM', 'status' => esc_html__( 'Ativo', 'dap' ), 'badge' => 'success', 'progress' => '100%' ],
                [ 'project' => esc_html__( 'Blitz de campanha com influenciadores', 'dap' ), 'owner' => 'Squad Media', 'status' => esc_html__( 'Em espera', 'dap' ), 'badge' => 'danger', 'progress' => '35%' ],
                [ 'project' => esc_html__( 'Aplicativo Privilege 3.0', 'dap' ), 'owner' => 'Squad Product', 'status' => esc_html__( 'Em desenvolvimento', 'dap' ), 'badge' => 'primary', 'progress' => '62%' ],
            ];
        }
    ...
    function dap_get_product_inventory_rows() {
        ...
        if ( dap_is_woocommerce_active() && function_exists( 'wc_get_products' ) ) {
            ...
            foreach ( $products as $product ) {
                ...
                $status_label  = esc_html__( 'Em estoque', 'dap' );
                $status_accent = 'success';

                if ( 'outofstock' === $stock_status ) {
                    $status_label  = esc_html__( 'Sem estoque', 'dap' );
                    $status_accent = 'danger';
                } elseif ( 'onbackorder' === $stock_status ) {
                    $status_label  = esc_html__( 'Sob encomenda', 'dap' );
                    $status_accent = 'warning';
                } elseif ( $stock_qty <= 5 ) {
                    $status_label  = esc_html__( 'Estoque baixo', 'dap' );
                    $status_accent = 'warning';
                }
        ...
        if ( empty( $rows ) ) {
            $rows = [
                [ 'product' => esc_html__( 'Suite de automação de marketing', 'dap' ), 'category' => esc_html__( 'SaaS', 'dap' ), 'stock' => 28, 'price' => dap_format_currency_short( 12400 ), 'status' => 'success', 'status_label' => esc_html__( 'Em estoque', 'dap' ) ],
                [ 'product' => esc_html__( 'Onboarding do Privilege CRM', 'dap' ), 'category' => esc_html__( 'Serviços', 'dap' ), 'stock' => 9, 'price' => dap_format_currency_short( 5800 ), 'status' => 'warning', 'status_label' => esc_html__( 'Estoque baixo', 'dap' ) ],
                [ 'product' => esc_html__( 'Kit inicial de inbound', 'dap' ), 'category' => esc_html__( 'Modelos', 'dap' ), 'stock' => 16, 'price' => dap_format_currency_short( 3200 ), 'status' => 'success', 'status_label' => esc_html__( 'Em estoque', 'dap' ) ],
                [ 'product' => esc_html__( 'Campanha Blitz de mídia', 'dap' ), 'category' => esc_html__( 'Campanhas', 'dap' ), 'stock' => 4, 'price' => dap_format_currency_short( 8900 ), 'status' => 'danger', 'status_label' => esc_html__( 'Sem estoque', 'dap' ) ],
                [ 'product' => esc_html__( 'Toolkit de operações de design', 'dap' ), 'category' => esc_html__( 'Design', 'dap' ), 'stock' => 22, 'price' => dap_format_currency_short( 2400 ), 'status' => 'success', 'status_label' => esc_html__( 'Em estoque', 'dap' ) ],
                [ 'product' => esc_html__( 'Impulsionador de anúncios Privilege', 'dap' ), 'category' => esc_html__( 'Anúncios', 'dap' ), 'stock' => 11, 'price' => dap_format_currency_short( 4600 ), 'status' => 'warning', 'status_label' => esc_html__( 'Estoque baixo', 'dap' ) ],
            ];
        }
    ...
    function dap_get_recent_orders_rows() {
        ...
        if ( dap_is_woocommerce_active() && function_exists( 'wc_get_orders' ) ) {
            ...
            foreach ( $orders as $order ) {
                ...
                if ( ! $customer ) {
                    $customer = $order->get_billing_email() ? $order->get_billing_email() : esc_html__( 'Visitante', 'dap' );
                }
        ...
        if ( empty( $rows ) ) {
            ...
            if ( ! empty( $comments ) && ! is_wp_error( $comments ) ) {
                foreach ( $comments as $comment ) {
                    ...
                    $status_label = ( 'warning' === $status ) ? esc_html__( 'Processando', 'dap' ) : esc_html__( 'Concluído', 'dap' );

                    $rows[] = [
                        'order'        => sprintf( '#%1$05d', (int) $comment->comment_ID ),
                        'customer'     => $comment->comment_author ? $comment->comment_author : esc_html__( 'Visitante', 'dap' ),
        ...
        if ( empty( $rows ) ) {
            $rows = [
                [ 'order' => '#2451', 'customer' => 'Gabriela Mota', 'amount' => dap_format_currency_short( 1290 ), 'status' => 'success', 'status_label' => esc_html__( 'Concluído', 'dap' ), 'date' => esc_html__( 'Hoje', 'dap' ) ],
                [ 'order' => '#2450', 'customer' => 'Leonardo Ramos', 'amount' => dap_format_currency_short( 870 ), 'status' => 'warning', 'status_label' => esc_html__( 'Processando', 'dap' ), 'date' => esc_html__( 'Ontem', 'dap' ) ],
                [ 'order' => '#2449', 'customer' => 'Ana Júlia', 'amount' => dap_format_currency_short( 1640 ), 'status' => 'success', 'status_label' => esc_html__( 'Concluído', 'dap' ), 'date' => esc_html__( 'Ontem', 'dap' ) ],
                [ 'order' => '#2448', 'customer' => 'João Vitor', 'amount' => dap_format_currency_short( 540 ), 'status' => 'success', 'status_label' => esc_html__( 'Concluído', 'dap' ), 'date' => esc_html__( '2 dias atrás', 'dap' ) ],
                ...
                [ 'order' => '#2446', 'customer' => 'Equipe CRM', 'amount' => dap_format_currency_short( 910 ), 'status' => 'success', 'status_label' => esc_html__( 'Concluído', 'dap' ), 'date' => esc_html__( '3 dias atrás', 'dap' ) ],
            ];
        }
...
if ( ! function_exists( 'dap_get_activity_feed_items' ) ) {
    ...
    function dap_get_activity_feed_items() {
        ...
        if ( empty( $items ) ) {
            $items = [
                [ 'color' => 'primary', 'title' => esc_html__( 'Tokens do design system mesclados na main', 'dap' ), 'meta' => esc_html__( '09:24 · Added by Júlia Martins', 'dap' ) ],
                [ 'color' => 'success', 'title' => esc_html__( 'Automação do CRM pausada para QA', 'dap' ), 'meta' => esc_html__( '11:03 · Workflow Bot', 'dap' ) ],
                [ 'color' => 'warning', 'title' => esc_html__( 'Equipe de mídia aguardando criativos para a Blitz', 'dap' ), 'meta' => esc_html__( '14:17 · Comment by Joana Reis', 'dap' ) ],
                [ 'color' => 'info', 'title' => esc_html__( 'Squad Inbound agendou atualização de conteúdo', 'dap' ), 'meta' => esc_html__( '16:42 · Calendar Sync', 'dap' ) ],
            ];
        }
    ...
    function dap_get_dashboard_topbar_data() {
        ...
        return apply_filters(
            ...
            [
                ...
                'languages'        => $languages,
                'theme_mode'       => dap_get_user_theme_mode(),
                'user'             => [
                    ...
                    'edit_url'    => current_user_can( 'edit_users' ) ? admin_url( 'user-edit.php?user_id=' . $current_user->ID ) : admin_url( 'profile.php' ),
                ],
...
if ( ! function_exists( 'dap_get_important_project_items' ) ) {
    ...
    function dap_get_important_project_items() {
        ...
        if ( empty( $items ) ) {
            $items = [
                [ 'name' => esc_html__( 'Hub de Experiências 2.0', 'dap' ), 'badge' => 'primary', 'status' => esc_html__( 'Marco previsto para amanhã', 'dap' ) ],
                [ 'name' => esc_html__( 'Lançamento do Privilege Rewards', 'dap' ), 'badge' => 'success', 'status' => esc_html__( 'Go-live confirmado', 'dap' ) ],
                [ 'name' => esc_html__( 'Dashboard de análise de varejo', 'dap' ), 'badge' => 'info', 'status' => esc_html__( 'Revisão com stakeholders na sexta', 'dap' ) ],
            ];
        }
...
if ( ! function_exists( 'dap_get_monthly_comment_counts' ) ) {
    /**
     * Retrieves approved comment counts grouped by month.
     *
     * @param int $months Number of months to include.
     *
     * @return array
     */
    function dap_get_monthly_comment_counts( $months = 12 ) {
        global $wpdb;

        $months  = max( 1, (int) $months );
        $now_gmt = current_time( 'timestamp', true );

        $start_reference = strtotime( gmdate( 'Y-m-01 00:00:00', $now_gmt ) );

        if ( false === $start_reference ) {
            $start_reference = $now_gmt;
        }

        $start_ts = strtotime( '-' . ( $months - 1 ) . ' months', $start_reference );
        $start    = gmdate( 'Y-m-01 00:00:00', $start_ts );

        $sql = $wpdb->prepare(
            "
            SELECT DATE_FORMAT(comment_date_gmt, '%%Y-%%m') AS period_key, COUNT(comment_ID) AS total
            FROM {$wpdb->comments}
            WHERE comment_approved = '1'
                AND ( comment_type = '' OR comment_type IS NULL )
...
if ( ! function_exists( 'dap_get_daily_comment_counts' ) ) {
    /**
     * Retrieves approved comment counts grouped by day for the last X days.
     *
     * @param int $days Number of days to include.
     *
     * @return array
     */
    function dap_get_daily_comment_counts( $days = 7 ) {
        global $wpdb;

        $days    = max( 1, (int) $days );
        $now_gmt = current_time( 'timestamp', true );
        $start   = gmdate( 'Y-m-d 00:00:00', $now_gmt - ( ( $days - 1 ) * DAY_IN_SECONDS ) );

        $sql = $wpdb->prepare(
            "
            SELECT DATE_FORMAT(comment_date_gmt, '%%Y-%%m-%%d') AS period_key, COUNT(comment_ID) AS total
            FROM {$wpdb->comments}
            WHERE comment_approved = '1'
                AND ( comment_type = '' OR comment_type IS NULL )
                AND comment_date_gmt >= %s
...
if ( ! function_exists( 'dap_get_today_comment_segments' ) ) {
    /**
     * Buckets today's approved comments into six segments.
     *
     * @return array
     */
    function dap_get_today_comment_segments() {
        global $wpdb;
        ...
        $sql = $wpdb->prepare(
            "
            SELECT DATE_FORMAT(comment_date_gmt, '%%H') AS hour_slot, COUNT(comment_ID) AS total
            FROM {$wpdb->comments}
            WHERE comment_approved = '1'
                AND ( comment_type = '' OR comment_type IS NULL )
                AND comment_date_gmt >= %s
...
if ( ! function_exists( 'dap_get_email_categories_data' ) ) {
    ...
    function dap_get_email_categories_data() {
        ...
        return [
            ...
            'series'    => $series,
            'colors'    => [
                isset( $palette['primary'] ) ? $palette['primary'] : '#7366ff',
                isset( $palette['success'] ) ? $palette['success'] : '#1ccab8',
                isset( $palette['warning'] ) ? $palette['warning'] : '#f7b84b',
                isset( $palette['info'] ) ? $palette['info'] : '#39afd1',
            ],
...
if ( ! function_exists( 'dap_get_chart_data_payload' ) ) {
    ...
    function dap_get_chart_data_payload( $kpis ) {
        ...
        $comment_monthly       = dap_get_monthly_comment_counts();
        $comment_weekly        = dap_get_daily_comment_counts();
        $comment_today         = dap_get_today_comment_segments();
        $email_data            = dap_get_email_categories_data();
        $progress_percent      = 72;

        $monthly_categories = wp_list_pluck( $monthly, 'label' );
        $weekly_categories  = wp_list_pluck( $weekly, 'label' );
        $today_labels       = isset( $today['labels'] ) ? $today['labels'] : [];

        if ( empty( $today_labels ) && isset( $comment_today['labels'] ) ) {
            $today_labels = $comment_today['labels'];
        }

        if ( empty( $today_labels ) ) {
            $today_labels = [ '08h', '10h', '12h', '14h', '16h', '18h' ];
        }

        $palette = dap_get_dashboard_palette();

        $series_labels = apply_filters(
            'dap_dashboard_chart_series_labels',
            [
                'primary'    => esc_html__( 'Projetos publicados', 'dap' ),
                'comparison' => esc_html__( 'Comentários aprovados', 'dap' ),
            ]
        );

        $series_colors = apply_filters(
            'dap_dashboard_chart_series_colors',
            [
                'primary'    => isset( $palette['primary'] ) ? $palette['primary'] : '#7366ff',
                'comparison' => isset( $palette['info'] ) ? $palette['info'] : '#2bb0f8',
            ]
        );
        ...
        return [
            'projectStatistics' => [
                'series'     => [
                    ...
                    'today'   => isset( $today['values'] ) ? $today['values'] : array_fill( 0, count( $today_labels ), 0 ),
                ],
                'comparison_series' => [
                    'monthly' => wp_list_pluck( $comment_monthly, 'value' ),
                    'weekly'  => wp_list_pluck( $comment_weekly, 'value' ),
                    'today'   => isset( $comment_today['values'] ) ? $comment_today['values'] : array_fill( 0, count( $today_labels ), 0 ),
                ],
                'categories' => [
                    'monthly' => ! empty( $monthly_categories ) ? $monthly_categories : wp_list_pluck( $comment_monthly, 'label' ),
                    'weekly'  => ! empty( $weekly_categories ) ? $weekly_categories : wp_list_pluck( $comment_weekly, 'label' ),
                    'today'   => $today_labels,
                ],
