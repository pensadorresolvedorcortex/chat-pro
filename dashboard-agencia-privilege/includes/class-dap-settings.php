...
class DAP_Settings {
    ...
    public function register_settings() {
        ...
        add_settings_field(
            'dap_dashboard_max_width',
            __( 'Largura máxima do dashboard', 'dap' ),
            [ $this, 'render_dashboard_width_field' ],
            'dap-settings',
            'dap_settings_section'
        );
    ...
    public function sanitize_settings( $input ) {
        ...
        if ( isset( $input['dashboard_max_width'] ) ) {
            $width = absint( $input['dashboard_max_width'] );
            ...
            $output['dashboard_max_width'] = $width;
        }
    ...
    /**
     * Renders the dashboard maximum width field.
     */
    public function render_dashboard_width_field() {
        $settings = dap_get_settings();
        $value    = isset( $settings['dashboard_max_width'] ) ? (int) $settings['dashboard_max_width'] : 1500;
        ?>
        <input type="number" class="small-text" name="dap_settings[dashboard_max_width]" min="960" max="1920" step="10" value="<?php echo esc_attr( $value ); ?>" />
        <p class="description"><?php echo esc_html__( 'Define a largura máxima (em pixels) aplicada à área principal do painel. O layout continua responsivo abaixo desse valor.', 'dap' ); ?></p>
        <?php
    }
