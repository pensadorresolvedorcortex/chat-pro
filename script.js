...
function ensureNameInputIsEditable() {
  if (!waNameInput) return;
  waNameInput.disabled = false;
  waNameInput.readOnly = false;
  waNameInput.removeAttribute('aria-disabled');
  waNameInput.removeAttribute('disabled');
  waNameInput.removeAttribute('readonly');
  waNameInput.tabIndex = 0;
  waNameInput.style.pointerEvents = 'auto';
  waNameInput.style.userSelect = 'auto';
}
...
function updatePreviews() {
  ...
  const savedName = waNameInput.value.trim();
  if (namePreview) {
    namePreview.textContent = savedName
      ? savedName
      : 'Inclua seu nome para personalizar o pedido.';
  }
...
function validateAddressForm({ focus = true } = {}) {
  let valid = true;
...
function updateChecklist() {
  ...
  if (vehicleChecklist) {
    vehicleChecklist.textContent = vehicleReady ? 'Bateria recomendada' : 'Veículo pendente';
...
function updateProgress() {
  ...
  const hasName = waNameInput && waNameInput.value.trim();
  const stepsReady = Number(isAddressComplete()) + Number(isVehicleComplete()) + Number(Boolean(hasName));
  let width = 30;
  if (stepsReady === 1) width = 45;
  else if (stepsReady === 2) width = 70;
  else if (stepsReady >= 3) width = 100;
...
function updateStickyBar() {
  ...
  const fullReady = ready && !!waNameInput.value.trim();
  stickyBar.classList.toggle('ready', ready);
  stickyStatus.textContent = ready
    ? fullReady
      ? 'Tudo preenchido. Peça agora sua bateria!'
      : 'Endereço e veículo confirmados. Inclua seu nome se quiser.'
    : 'Complete endereço e veículo para agilizar o pedido.';
  if (stickySub) {
    stickySub.textContent = ready
      ? 'Toque em Peça agora para confirmar o atendimento.'
      : 'Confirme o endereço e escolha o veículo para liberar.';
  }
  stickyAction.textContent = ready ? 'Peça agora sua bateria!' : 'Preencher dados';
  stickyAction.dataset.action = ready ? 'send' : 'open';
  stickyAction.classList.toggle('pulse', fullReady);
}

function updatePulse() {
  const fullReady = isAddressComplete() && isVehicleComplete() && !!waNameInput.value.trim();
  if (ctaWhatsApp) ctaWhatsApp.classList.toggle('pulse', fullReady);
  if (stickyAction && stickyBar) {
    stickyAction.classList.toggle('pulse', fullReady && stickyBar.classList.contains('ready'));
  }
}

function toggleCtaState() {
  const ready = isAddressComplete() && isVehicleComplete();
  const hasName = !!waNameInput.value.trim();
  ctaWhatsApp.disabled = !ready;
  ctaWhatsApp.classList.toggle('disabled', !ready);
  ctaStateHint.textContent = ready
    ? hasName
      ? 'Tudo revisado. Clique em Peça agora sua bateria.'
      : 'Inclua seu nome para personalizar ou siga direto para enviar.'
...
function buildWhatsAppMessage() {
  ...
  return `Nome: ${name || 'Nome não informado'}\nEndereço: ${addressText}\nVeículo: ${vehicleText}${notesText}`;
}
...
waNameInput.addEventListener('focus', ensureNameInputIsEditable);
waNameInput.addEventListener('mousedown', ensureNameInputIsEditable);
...
addressForm.addEventListener('input', () => {
  validateAddressForm({ focus: false });
  persistAddress();
...
addressForm.addEventListener('blur', (event) => {
  if (event.target && event.target.name) validateAddressForm({ focus: false });
}, true);
