...
body.folded .wps-user-avatar img {
    max-width: 100%;
}
...
$css_styles .= 'html, #wpwrap, #wp-content-editor-tools,body {';
  if( isset($this->aof_options['bg_color']) && !empty($this->aof_options['bg_color']) ) {
    $css_styles .= 'background: ' . $this->aof_options['bg_color'] . ';';
  }
  if( isset($this->aof_options['body_text_color']) && !empty($this->aof_options['body_text_color']) ) {
    $css_styles .= 'color: ' . $this->aof_options['body_text_color'] . ';';
  }

$css_styles .= '}';

$css_styles .= '#wpfooter {';
if( isset($this->aof_options['footer_text_color']) && !empty($this->aof_options['footer_text_color']) ) {
  $css_styles .= 'color: ' . $this->aof_options['footer_text_color'] . ';';
}
...
if(isset($this->aof_options['admin_menu_width']) && !empty($this->aof_options['admin_menu_width'])) {
    ...
    .auto-fold .edit-post-layout__content .components-editor-notices__snackbar, .wpo-main-header, #formcraft-builder-cover #main-options-panel, #formcraft-builder-cover #form_styling_box, #formcraft-builder-cover .option-box{
    ...
    ';
    //WPSPowerbox admin bar style
    $css_styles .= '.has-wpspb-sidebar :not(.pinned) > #wpadminbar {transition: margin 300ms ease;-webkit-transition: margin 300ms ease;}';
    $css_styles .= '.has-wpspb-sidebar :not(.pinned) > #wpadminbar {';
    $adminbar_margin = (int) $admin_menu_width + 20;
      if(is_rtl()) {
        $css_styles .= 'margin-right: '. $adminbar_margin . 'px';
      } else {
        $css_styles .= 'margin-left: '. $adminbar_margin . 'px';
      }
    $css_styles .= '}';
    $css_styles .= '.has-wpspb-sidebar .pinned > #wpadminbar {';
      if(is_rtl()) {
          $css_styles .= 'margin-right: 65px';
        } else {
          $css_styles .= 'margin-left: 65px';
        }
    $css_styles .= '}';
}
else {
...
#adminmenuwrap i.wps-power-switch:before, #adminmenuwrap i.wps-user:before {
...
if( is_rtl() ) {
  $css_styles .= '#collapse-button .collapse-button-icon, #collapse-button .collapse-button-label{right: -23px;}';
  $css_styles .= 'body.folded .wps-menu-user-actions a{margin-right:7px;margin-left:auto}';
}
...
if($this->aof_options['design_type'] == 4) {
  $close_icon_data_uri = 'data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%2212%22%20height%3D%2212%22%20viewBox%3D%220%200%2012%2012%22%20fill%3D%22none%22%3E%3Cpath%20d%3D%22M2.1%202.1a.75.75%200%200%201%201.06%200L6%204.94%208.84%202.1a.75.75%200%201%201%201.06%201.06L7.06%206l2.84%202.84a.75.75%200%201%201-1.06%201.06L6%207.06%203.16%209.9a.75.75%200%201%201-1.06-1.06L4.94%206%202.1%203.16a.75.75%200%200%201%200-1.06Z%22%20fill%3D%22%236366F1%22/%3E%3C/svg%3E';
  //admin menu width
  $admin_menu_width = $this->aof_options['admin_menu_width'];
  $admin_sub_menu_width = (int)$admin_menu_width - 20;

  $css_styles .= '@import url("https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap");';
  $css_styles .= 'body, .wrap, #wpadminbar, #adminmenu, #adminmenu * {font-family: "Inter", "DM Sans", sans-serif;}';
  $css_styles .= 'a {color: #6366f1;}
  ...
  .widefat thead td, .widefat thead th{border-bottom: 1px solid #e5e7eb;}
  .widefat tfoot td, .widefat tfoot th {border-top: 1px solid #e5e7eb;}
  .alternate, .striped>tbody>:nth-child(odd), ul.striped>:nth-child(odd) {
    background-color: #fff;
    box-shadow: 0 1px 1px rgba(0,0,0,.04);
  }
  .widefat ol, .widefat p, .widefat ul {color: #1F2937;}
  .postbox,.wp-editor-container,#wp-content-editor-container,#wp-content-editor-tools .wp-switch-editor,
  
  #menu-management .menu-edit, #menu-settings-column .accordion-container, .comment-ays, .feature-filter, .imgedit-group, .manage-menus, .menu-item-handle, .popular-tags, .stuffbox, .widget-inside, .widget-top, .widgets-holder-wrap, .wp-editor-container, p.popular-tags, table.widefat{border: 1px solid #e5e7eb;}
  
  #wp-content-editor-tools .wp-switch-editor{border-bottom-color: #f3f4f6;}
  ...
  .tagchecklist .ntdelbutton .remove-tag-icon:before, .welcome-panel .welcome-panel-close:before,.e-notice__dismiss:before {
    content:"";
    background-image: url("' . $close_icon_data_uri . '"); background-repeat: no-repeat; background-size: contain; width: 12px; height: 12px;
  }
  .welcome-panel .welcome-panel-close { color:#6366f1;padding: 5px 15px 5px 24px; background-color: #E8E8FD; border-radius: 3px; }
  .welcome-panel .welcome-panel-close:before {top:7px;left:5px;color:#6366f1}
  .welcome-panel .welcome-panel-close:hover{color:' . $this->aof_options['msgbox_link_hover_color'] . '}
  span.activate a {
    padding: 2px 6px; display: inline-block; background-color: #E8E8FD; border-radius: 2px; 
  }
  .notice-warning.notice-alt, .updated-message.notice-success, div.updated {
    background-color: #f3f4f6; border: 2px solid #e5e7eb; -webkit-border-radius: 3px; -moz-border-radius: 3px; -ms-border-radius: 3px; border-radius: 3px
  }
  a.notice-dismiss:hover{color:#ef4444}
  ...
  .theme-browser .theme .more-details{background: rgba(35,48,60,0.7);color:#E5E7EB;text-shadow:none}
  ...
  input[type=checkbox]:focus, input[type=color]:focus, input[type=date]:focus, input[type=datetime-local]:focus, input[type=datetime]:focus, input[type=email]:focus, input[type=month]:focus, input[type=number]:focus, input[type=password]:focus, input[type=radio]:focus, input[type=search]:focus, input[type=tel]:focus, input[type=text]:focus, input[type=time]:focus, input[type=url]:focus, input[type=week]:focus, select:focus, textarea:focus {
    border-color: #818cf8; box-shadow: none;
  }
  #screen-meta-links .show-settings,#screen-meta,#screen-meta-links .show-settings:focus {border-color:#d1d5db;box-shadow:none}
  ';
...
echo '<style type="text/css"><!-- wps styles -->' . wp_strip_all_tags( $this->wps_compress_css($css_styles)) . '</style>';
