 ...
 class WPS_LOGIN_PRESETS extends WPSHAPERE {
  ...
  protected $options_data;
   ...
   function __construct()
   {
      ...
      add_action('admin_init',array($this, 'wps_import_login_preset'));
      if($this->options_data['disable_styles_login'] != 1) {
   ...
   function wps_import_login_preset() {
     ...
     if(isset($_POST) && isset($_POST['wps_login_theme_field']) && wp_verify_nonce( $_POST['wps_login_theme_field'], 'wps_login_theme_nonce' )) {
       ...
       $import_data = array_merge($selected_theme, $theme_data['theme_params']);
   ...
   function wps_login_presets_page()
   {
     ...
     <div class="wrap wps-wrap">
       <?php $aofoptions->licenseValidate(); ?>
         <h1><?php echo esc_html__('Import Login Theme', 'wps'); ?></h1>
         <?php parent::wps_help_link();
         $wps_login_presets = $this->wos_login_presets();
         $login_theme = null;
         $login_theme = (isset($this->options_data['login_theme_preset'])) ? $this->options_data['login_theme_preset'] : 'plain';
         ?>
               ...
               if(is_array($wps_login_presets)) {
                 foreach ($wps_login_presets as $theme_name => $params) {
                   ...
                   if(!empty($params['thumb_url'])) {
                     echo '<option value="'. esc_attr($theme_name) .'" data-img-src="'. esc_url($params['thumb_url']) .'" '. esc_attr($selected) .'>';
                     echo esc_html($params['theme_name']);
                     echo '</option>';
                   }
                 }
               }
