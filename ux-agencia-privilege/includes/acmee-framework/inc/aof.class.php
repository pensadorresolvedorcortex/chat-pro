...
if (!class_exists('AcmeeFramework')) {
    ...
    class AcmeeFramework {
        ...
        function __construct() {
            ...
            add_action('admin_init', array($this, 'aofLoaddefault' ));
            ...
            add_action('current_screen', array($this, 'licenseValidate') );
        }
        ...
        */
        ...
        public function fieldLoop($config) {
            ...
            if(is_array($fields_array['tabs']) && !empty($fields_array['tabs'])) {
                foreach($fields_array['tabs'] as $tabkey =>$tabvalue) {
                    echo '<li class="' . esc_html($tabkey) . '">' . esc_html($tabvalue) . '</li>';
                }
            }
            echo '</ul>
        ...
        function licenseValidate() {
          return;
            ...
            if( !empty($screen) && !empty($screen->id) && !in_array($screen->id, $wps_pages_slugs))
                return;
            
            //verify purchase code
            $purchase_data = !empty($this->validatePurchase()) ? $this->validatePurchase() : '';

            $license = !empty($purchase_data['license_key']) ? explode( '-', $purchase_data['license_key'] ) : '';

            if(!empty($license) && 12 == strlen($license[4]) && $purchase_data['product_code'] == $this->product_code)
                return;
            else {
                wp_safe_redirect( esc_url(admin_url( 'admin.php?page=register-wpshapere' )) );
                exit();
            }
        ...
        function getLicense() {
          return '123456789012'; // Return 12 characters to pass validation.
            $license = $this->validatePurchase();
            if(!empty($license['license_key'])) {
                $code = explode('-', $license['license_key']);
                return $code[4];
            }
            else
                return null;
        ...
        function SaveSettings($post) {
            if(isset($post['action']) && $post['action'] == 'license_data') {
             ...
             if(isset($post) && isset($post['aof_options_save'])) {
                ...
                if($this->getLicense() !== null && strlen($this->getLicense()) == 12) {
                    if( isset($post['aof_import_settings']) && !empty($post['aof_import_settings']) ) {
                      ...
                      $config = wps_config();
                      $fields_array['fields'] = $config['wps_fields']['wps_fields'];
                      $existing_options = $this->aofgetOptions( WPSHAPERE_OPTIONS_SLUG );
                      if( is_array($fields_array['fields']) && !empty($fields_array['fields']) ) {
                        ...
                        if ( isset( $save_data['design_type'] ) ) {
                            $new_design_type      = (int) $save_data['design_type'];
                            $previous_design_type = isset( $existing_options['design_type'] ) ? (int) $existing_options['design_type'] : null;

                            if ( 4 === $new_design_type && 4 !== $previous_design_type ) {
                                $save_data['admin_theme_preset'] = 'Liquido';
                            }
                        }
        ...
        function openTab($fields, $tab_first) {
            ...
            echo wp_kses($output, $this->allowed_html_arr);
        }

        function addfieldsetWrapStart($fields) {
          echo '<div class="aof-fieldset-wrap" id="' . esc_attr($fields['id']) . '">';
        }
        ...
        function addTitle($fields) {
            echo '<h3 class="aof-fieldset-title">' . esc_html($fields['name']) . '</h3>';
            if(!empty($fields['desc']))
              echo '<p class="aof-title-desc">' . wp_kses($fields['desc'], $this->allowed_html_arr) . '</p>';
        }

        function addNote($fields) {
            echo '<p class="aof-description">' . wp_kses_post($fields['desc']) . '</p>';
        }
        ...
        function addText($fields) {
            ...
            echo wp_kses($this->fieldWrap($fields, $form_field), $this->allowed_html_arr );
        }
        ...
        function addCheckbox($fields) {
            $meta = (isset($fields['meta'])) ? $fields['meta'] : "";
            $form_field = '<label class="' . $fields['id'] . '"><input id="' . $fields['id'] . '" class="' . $fields['id'] . '" type="checkbox" name="' . $fields['id'] . '" value="1"';
        ...
        function addMultiCheckbox($fields) {
            ...
            if(isset($fields['options'])) {
                ...
                foreach($fields['options'] as $field_key => $field_value) {
                    $form_field .= '<label class="' . $fields['id'] . '"><input class="' . $fields['id'] . '" type="checkbox" name="' . $fields['id'] . '[]" value="' . $field_key . '"  ';
        ...
        function addradio($fields) {
            ...
            if(isset($fields['options'])) {
                $form_field = "";
                foreach($fields['options'] as $field_key => $field_value) {
                    $form_field .= '<label class="' . $fields['id'] . '"><input class="' . $fields['id'] . '" type="radio" name="' . $fields['id'] . '" value="' . $field_key . '" ';
        ...
        function addwpEditor($fields) {
            ...
            echo '<div class="label"><label for="' . esc_attr($fields['id']) . '">' . esc_html($fields['name']) . '</label></div>';
            ...
            if(isset($fields['desc']) && !empty($fields['desc'])) {
                echo wp_kses('<div class="field_desc">' . $fields['desc'] . '</div>',$this->allowed_html_arr );
            }
            echo '</div></div>';
        ...
        function addUpload($fields) {
            ...
            if((!empty($meta))) {
                ...
                $form_field .=  '<img class="imgpreview_' . $fields['id'] .'" src="' . $attachment_url . '" />';
            }
        ...
        function addExport($fields) {
            ...
            wp_kses($this->addTextarea($export_options),$this->allowed_html_arr );
        }
        ...
        function addImport($fields) {
            ...
            wp_kses($this->addTextarea($import_options),$this->allowed_html_arr );
        }

        function saveBtn() {
          ?>
            <div class="save_options">
              <input type="submit" value="<?php echo esc_html__('Salvar', 'wps'); ?>" class="aof_save_button" />
            </div>
          <?php
        }
        ...
        function fieldWrap($args, $field) {
            ...
            if(isset($args['desc']) && !empty($args['desc'])) {
                $output .= '<div class="field_desc">' . wp_kses_post($args['desc']) . '</div>';
            }
            $output .= '</div></div>';
            return $output;
        }
        ...
        */
        function adminNotices() {
            if( isset($_GET['status']) && $_GET['status'] == "updated") {
                $message = esc_html__( 'Settings saved.', 'wps');
                echo "<div class='updated'><p>". esc_html($message) . "</p></div>";
            }
            if( isset($_GET['status']) && $_GET['status'] == "error") {
                $message = esc_html__( 'Error: Settings not saved.', 'wps');
