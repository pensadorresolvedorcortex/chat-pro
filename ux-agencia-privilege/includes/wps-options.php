...
//get all admin users
if(is_multisite()) {
    $admin_users_array = (is_serialized(get_site_option(WPS_ADMIN_USERS_SLUG))) ? unserialize(get_site_option(WPS_ADMIN_USERS_SLUG)) : get_site_option(WPS_ADMIN_USERS_SLUG);
  }else {
  ...
  $panel_fields[] = array(
      ...
      'type' => 'radio',
      'options' => array(
          '4' => esc_html( 'Liquido').' <span class="wps-designtype-new">' . esc_html__('New', 'wps') . '</span>',
...
if($wps_options['login_form_align_type'] != 1) {
  ...
  if(defined('ELEMENTOR_PATH') && class_exists('Elementor\Widget_Base')){
    //fetch elementor saved templates
    ...
    if( !empty($elementor_templates) && is_array($elementor_templates) ) {
        //echo '<pre>'; print_r($elementor_templates); echo '</pre>';
        $templates = [];
        $templates[] = esc_html__('Select template', 'wps');
    ...
    if ( $elementor_templates ) {
      $panel_fields[] = array(
        'name' => esc_html__( 'Select Elementor template ID', 'wps' ),
        'id' => 'wps_ele_template_id',
        'options' => $templates,
        'type' => 'select',
      );
    }
      ...
      $panel_fields[] = array(
            'name' => esc_html__( 'No Elementor templates found. Please create a template first.', 'wps' ),
            'type' => 'title',
        );
    }
  }
    ...
    $panel_fields[] = array(
      'name' => esc_html__( 'Activate Elementor if you want to display the Elementor-built dashboard.', 'wps' ),
      'type' => 'title',
    );
  }
...
if(defined('POWERBOX_PATH')) {
  ...
  $panel_fields[] = array(
    'name' => esc_html__( 'Body text color', 'wps' ),
    'id' => 'body_text_color',
    'type' => 'wpcolor',
    'default' => '#3c434a',
    );
...
$panel_fields[] = array(
    'name' => esc_html__( 'Footer text color', 'wps' ),
  ...
  $panel_fields[] = array(
      ...
      'options' => array(
          ...
          // translators: %1$s is the email address and %2$s is the email from name
          '1' => sprintf( esc_html__( 'Set Email address as %1$s From name as %2$s', 'wps' ), $blog_email, $blog_from_name ),
