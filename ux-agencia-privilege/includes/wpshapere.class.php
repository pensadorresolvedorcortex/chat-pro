...
if ( !class_exists('WPSHAPERE') ) {
  ...
  class WPSHAPERE
  {
	...
	function __construct()
	{
      ...
      add_action( 'admin_head-profile.php', array($this, 'remove_color_scheme') );
  ...
  function wps_notify_liquido_theme() {
    ...
    if ( ! empty( $this->aof_options['design_type'] ) && (int) $this->aof_options['design_type'] === 4 && 'Liquido' !== $active_admin_theme ) {
      echo '<div class="notice notice-warning wps-liquido-theme-notice"><p>';
      echo esc_html__( 'The Liquido layout is active. Import the Liquido admin theme preset so that the interface matches the purchased design.', 'wps' );
      echo '</p></div>';
    }
  }

  function load_files() {
    require_once ( AOF_PATH . 'inc/wps-license.php' );
  }
  ...
  */
  ...
  public function remove_color_scheme() {
    ...
    wp_enqueue_style( 'lineIcons', 'https://cdn.lineicons.com/2.0/LineIcons.css', array(), '2.0.0' );
    if ( isset( $this->aof_options['design_type'] ) && (int) $this->aof_options['design_type'] === 4 ) {
      wp_enqueue_style( 'wps-liquido-login-vendor', WPSHAPERE_DIR_URI . 'assets/liquido-template/css/vendor.min.css', array(), WPSHAPERE_VERSION );
      wp_enqueue_style( 'wps-liquido-login-app', WPSHAPERE_DIR_URI . 'assets/liquido-template/css/app.min.css', array( 'wps-liquido-login-vendor' ), WPSHAPERE_VERSION );
    }
  }
    ...
    wp_enqueue_style( 'lineIcons', 'https://cdn.lineicons.com/2.0/LineIcons.css', array(), '2.0.0' );
    if ( isset( $this->aof_options['design_type'] ) && (int) $this->aof_options['design_type'] === 4 ) {
      wp_enqueue_style( 'wps-liquido-vendor', WPSHAPERE_DIR_URI . 'assets/liquido-template/css/vendor.min.css', array(), WPSHAPERE_VERSION );
      wp_enqueue_style( 'wps-liquido-app', WPSHAPERE_DIR_URI . 'assets/liquido-template/css/app.min.css', array( 'wps-liquido-vendor' ), WPSHAPERE_VERSION );
      wp_enqueue_script( 'wps-liquido-config', WPSHAPERE_DIR_URI . 'assets/liquido-template/js/config.js', array( 'jquery' ), WPSHAPERE_VERSION, true );
      wp_enqueue_script( 'wps-liquido-app', WPSHAPERE_DIR_URI . 'assets/liquido-template/js/app.js', array( 'jquery', 'wps-liquido-config' ), WPSHAPERE_VERSION, true );
    }
    ...
    if( $nowpage == 'wpshapere_page_register-wpshapere' ) {
      wp_enqueue_style( 'wps-lc-style', AOF_DIR_URI . 'assets/css/admin-styles.css', '', WPSHAPERE_VERSION );
      wp_enqueue_script( 'wps-lc-script', AOF_DIR_URI . 'assets/js/adminscripts.js', array( 'jquery' ), false, true );
    }
	}
  ...
  function get_admin_users() {
    ...
    if( isset($_POST) && !empty($_POST['aof_options_save']) ) {
...
?>
    ...
    jQuery(document).ready(function($) {
      "use strict";
      jQuery('<?php echo wp_kses_post($user_profile_data); ?>').insertBefore("#adminmenu");
    });
    </script>
<?php
  }
  ...
  */
    ...
    // translators: %s is the currently logged in username
    ...
    sprintf( esc_html__('Howdy %1$s', 'wps'), $current_user->display_name );
...
function wps_get_wproles_blog( $blog_id ) {
  ...
  public function wps_display_widget_content($post, $wps_widget_handle)
  {
    ...
    elseif($wps_widget_handle['args']['type'] == 3) {
      $video_url = esc_url( $wps_widget_handle['args']['video'] );

      // Display the embed code
      if ( $video_url ) {
        // Get the embed HTML from the oEmbed provider (YouTube)
          echo wp_oembed_get( $video_url );
      } else {
          echo esc_html__( 'Video could not be embedded.', 'wps' ) ;
      }
    }
  	else {
  		echo wp_kses_post($wps_widget_handle['args']['content']);
  	}
  }
...
function wps_video_frame_css()
{
  ...
  for($i = 1; $i <= $n_widgets; $i++) {
    if(isset($this->aof_options['wps_widget_' . $i . '_type']) && $this->aof_options['wps_widget_' . $i . '_type'] == 3) {
      echo '<style>#wps_dashwidget_' . esc_html($i) . ' .inside{position:relative;padding-bottom:56.25%;padding-top:25px;height:0;margin-top:0}#wps_dashwidget_' . esc_html($i) . ' .inside object,#wps_dashwidget_' . esc_html($i) . ' .inside embed,#wps_dashwidget_' . esc_html($i) . ' .inside iframe{position: absolute;top:0;left:0;width:100%;height:100%;}</style>';
    }
  }
}
