...
<select name="custom_admin_menu[menu_icon][<?php echo esc_html($value[2]);?>]" class="wps-icon-picker">
  <option value=""><?php echo esc_html__( 'Blank', 'wps' ); ?></option>
  <?php
  if( !empty( $wps_icons_array ) ) {
    foreach ( $wps_icons_array as $icon_name => $icon_values) {
      ...
      echo '<optgroup label="' . esc_attr($icon_labels[$icon_name]) . '">';
          ...
          foreach ( $icon_values as $icon_value) {
            ...
            echo '<option value="' . esc_attr($wpsicon) . '" '. esc_attr($selected) .'>' . esc_attr($wpsicon) .'</option>';
          }
