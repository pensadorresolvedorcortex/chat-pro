...
if(!class_exists('WPS_CUSTOMIZEADMINMENU')) {
    class WPS_CUSTOMIZEADMINMENU extends WPSHAPERE {
        ...
        protected $aofoptions;

        function __construct()
        {
            $this->aofoptions = parent::get_wps_option_data( WPSHAPERE_OPTIONS_SLUG );
            ...
            add_action('admin_init', array($this, 'save_menu_data'), 1);
        ...
        public function load_menu_assets($nowpage)
        {
          ...
          if($nowpage == 'wpshapere_page_admin_menu_management' || $nowpage == 'toplevel_page_wpshapere-options') {
            ...
            wp_enqueue_style( 'icon-picker-styles', 'https://cdnjs.cloudflare.com/ajax/libs/fonticonpicker/3.1.1/css/jquery.fonticonpicker.min.css', array(), '3.1.1');
            wp_enqueue_script( 'icon-pickerr', 'https://cdnjs.cloudflare.com/ajax/libs/fonticonpicker/3.1.1/js/jquery.fonticonpicker.min.js', array( 'jquery' ), '3.1.1', true );
        ...
        function order_menu_page($menu, $submenu)
        {
            $tmenu = $menu;
            $tsubmenu = $submenu;
            if (isset($this->aofoptions['custom_admin_menu']['top_menu']) && !empty($this->aofoptions['custom_admin_menu']['top_menu'])) {
                $current_user_role = parent::wps_get_user_role();
                $current_user_id = get_current_user_id();
                $wps_menu_access = $this->aofoptions['show_all_menu_to_admin'];
                $wps_privilege_users = (!empty($this->aofoptions['privilege_users'])) ? $this->aofoptions['privilege_users'] : array();
                $toporder = isset($this->aofoptions['custom_admin_menu']['top_menu']) ? $this->aofoptions['custom_admin_menu']['top_menu'] : "";
                $multiorder = isset($this->aofoptions['custom_admin_menu']['sub_menu']) ? $this->aofoptions['custom_admin_menu']['sub_menu'] : "";
                $topmenutitle = isset($this->aofoptions['custom_admin_menu']['top_menu_title']) ? $this->aofoptions['custom_admin_menu']['top_menu_title'] : "";
                $topmenuicon = isset($this->aofoptions['custom_admin_menu']['menu_icon']) ? $this->aofoptions['custom_admin_menu']['menu_icon'] : "";
                $submenutitle = isset($this->aofoptions['custom_admin_menu']['sub_menu_title']) ? $this->aofoptions['custom_admin_menu']['sub_menu_title'] : "";
                $topmenuhide = isset($this->aofoptions['custom_admin_menu']['top_menu_hide']) ? $this->aofoptions['custom_admin_menu']['top_menu_hide'] : array();
                $submenuhide = isset($this->aofoptions['custom_admin_menu']['sub_menu_hide']) ? $this->aofoptions['custom_admin_menu']['sub_menu_hide'] : array();
                ...
                foreach($menu as $key => &$value) { 
                    ...
                    * @since 6.1.3
                    */
                    if ( isset($this->aofoptions['wps_manage_menu_slug_mthd']) && $this->aofoptions['wps_manage_menu_slug_mthd'] == 2 ) {
                      $clean_url = true;
                    }
                    else
                      $clean_url = false;
                    ...
                    if(isset($value[2]) && $value[2] == "vc-welcome") { //if top menu slug is vcwelcome, definitely it's not an administrator user
                      $if_vc_general_hidden = isset($this->aofoptions['custom_admin_menu']['top_menu_hide']['vc-general']) ?
                          $this->aofoptions['custom_admin_menu']['top_menu_hide']['vc-general'] : array();
                    ...
                    if(isset($value[2]) && $value[2] == "profile.php") { //if top menu slug is profile.php, definitely it's not an administrator user
                      $if_profile_hidden = isset($this->aofoptions['custom_admin_menu']['sub_menu_hide']['users.php']['profile.php']) ?
                          $this->aofoptions['custom_admin_menu']['sub_menu_hide']['users.php']['profile.php'] : array();
                    ...
                    if(isset($value[2]) && $value[2] == "edit-tags.php?taxonomy=category") { //if top menu slug is edit-tags.php?taxonomy=category, definitely it's not an administrator user
                      $if_profile_hidden = isset($this->aofoptions['custom_admin_menu']['sub_menu_hide']['edit.php']['edittagsphptaxonomycategory']) ?
                          $this->aofoptions['custom_admin_menu']['sub_menu_hide']['edit.php']['edittagsphptaxonomycategory'] : array();
                          if(!empty($if_profile_hidden) && array_key_exists($current_user_role, $if_profile_hidden)) {
                            unset($menu[$key]);
                          }
                    }
                    ...
                    if(self::find_customize($value[2]) == true) {
                      $if_customize_hidden = isset($this->aofoptions['custom_admin_menu']['sub_menu_hide']['themes.php']['customize.php']) ?
                          $this->aofoptions['custom_admin_menu']['sub_menu_hide']['themes.php']['customize.php'] : array(); echo esc_html($if_customize_hidden);
        ...
        function wps_fa_iconStyles(){
            if( class_exists('WPSFAICONS') ) {
                $wps_icon_data = (isset($this->aofoptions['custom_admin_menu']['menu_icon']) && !empty($this->aofoptions['custom_admin_menu']['menu_icon'])) ? $this->aofoptions['custom_admin_menu']['menu_icon'] : array();
        ...
        public function wps_admin_menu_management()
        {
           global $menu, $submenu, $_parent_pages, $_registered_pages, $admin_page_hooks,$function,$aof_options;
            $topmenutitle = isset($this->aofoptions['custom_admin_menu']['top_menu_title']) ? $this->aofoptions['custom_admin_menu']['top_menu_title'] : array();
            $topmenuicon = isset($this->aofoptions['custom_admin_menu']['menu_icon']) ? $this->aofoptions['custom_admin_menu']['menu_icon'] : array();
            $submenutitle = isset($this->aofoptions['custom_admin_menu']['sub_menu_title']) ? $this->aofoptions['custom_admin_menu']['sub_menu_title'] : array();
        ...
        ?>
            <div class="wrap wps-wrap">
              ...
              $wps_menu_access = $this->aofoptions['show_all_menu_to_admin'];
              $wps_privilege_users = (!empty($this->aofoptions['privilege_users'])) ? $this->aofoptions['privilege_users'] : array();
              $wps_manage_menu_mthd = isset($this->aofoptions['wps_manage_menu_slug_mthd']) ? $this->aofoptions['wps_manage_menu_slug_mthd'] : 1;
         ...
         ?>
              ...
              <div class="wps-adminmenu-customizer">
                ...
                echo '<a href="' . esc_url(admin_url('admin.php?page=' . WPSHAPERE_MENU_SLUG . '#aof_options_tab9')) . '"><strong>';
                ...
                <div class="manage_admin_menu_sorter">
                    ...
                    ?>
                    <form name="alter_manage_admin_menu" method="post" action="<?php echo esc_url(admin_url( 'admin.php?page=admin_menu_management' )); ?>">
                    ...
                    <ol class="topmenu sortUls" id="top_menu">
                        ...
                        <li id="<?php echo "top-li-". esc_attr($tsl);?>">
                            <input type="hidden" name="custom_admin_menu[top_menu][]" id="<?php echo "input-top-li-". esc_attr($tsl);?>" value="<?php echo esc_attr($value[2]);?>"/>
                            <div class="alter-sort-list alter-top-menu-<?php echo esc_attr($menu_key); ?>">
                                ...
                                <div class="alter-menu-contents" id="s">
                                  ...
                                  <?php if(!empty($value[0])) : ?>
                                    <div class="menu_title">
                                        ...
                                        <input type="text" id="<?php echo "customtitle-top-li-". esc_attr($tsl);?>" name="custom_admin_menu[top_menu_title][<?php echo esc_attr($value[2]);?>]" value="<?php echo esc_attr($custom_menu_title);?>" />
                                    </div>
                                    <div class="menu_icon">
                                        <label for="icon_picker"><em><?php echo esc_html__('Choose Icon', 'wps'); ?></em></label>
                                        <div id="" data-target="#menu-icon-for-<?php echo esc_attr($mm_cu); ?>" class="icon-picker <?php echo esc_attr($menu_icon_class); ?>"></div>
                                        <input type="hidden" id="menu-icon-for-<?php echo esc_attr($mm_cu++); ?>" name="custom_admin_menu[menu_icon][<?php echo esc_attr($value[2]);?>]" value="<?php echo esc_attr($menu_icon_data); ?>" />
                                    </div>
                                  <?php endif; ?>

                                    <?php echo self::hide_for_menu("top_menu", $value[2], '', $inm); ?>

                                    <ol class="menu_child_<?php echo esc_attr($menu_key); ?> submenu subsortUls" id="sub_menu">
                                        ...
                                        <?php $custom_submenu_title = (isset($submenutitle[$value[2]][$submenu_value[2]]) &&!empty($submenutitle[$value[2]][$submenu_value[2]])) ? $submenutitle[$value[2]][$submenu_value[2]] : "";?>
                                            <li id="<?php echo "sub-li-". esc_attr($tsl) ."-". esc_attr($ssl);?>" class="<?php echo esc_html($disblieitem); ?>">
                                                <input type="hidden" name="custom_admin_menu[sub_menu][<?php echo esc_attr($value[2]);?>][]" id="<?php echo "input-sub-li-".esc_attr($tsl)."-".esc_attr($ssl);?>" value="<?php echo esc_attr($submenu_value[2]);?>"/>
                                                <div class="alter-sort-list submenu_contents">
                                                    ...
                                                    <div class="alter-menu-contents">
                                                        <div class="menu_title">
                                                            <label for="menu_title"><em><?php echo esc_html__('Rename Title', 'wps'); ?></em></label>
                                                            <input type="text" id="<?php echo "customtitle-sub-li-". esc_attr($tsl) ."-". esc_attr($ssl);?>" name="custom_admin_menu[sub_menu_title][<?php echo esc_attr($value[2]);?>][<?php echo esc_attr($submenu_value[2]);?>]" value="<?php echo esc_attr($custom_submenu_title); ?>" />
                                                        </div>
                                                        <a href="#" class="alter-edit-expand"><i class="fa fa-chevron-down" aria-hidden="true"></i> <span>Edit</span></a>
                    ...
                    <input type="submit"  class="button button-primary button-hero" value="<?php esc_html_e('Salvar', 'wps'); ?>" />
                    </form>
                </div>
            </div>
            ...
            <div class="powerbox-banner">
              <a href="https://codecanyon.net/item/wpspowerbox-addon-for-wpshapere-wordpress-admin-theme/22169580" target="_blank">
                <img src="<?php echo esc_url(WPSHAPERE_DIR_URI) ?>assets/images/powerbox-vertical-banner-a-450x760.jpg" alt="" />
              </a>
            </div>
        ...
        public function wps_menu_data() {
           if (isset($this->aofoptions['custom_admin_menu']) && !empty($this->aofoptions['custom_admin_menu'])) {
               return $this->aofoptions['custom_admin_menu'];
           }
           else
               return null;;
        }
