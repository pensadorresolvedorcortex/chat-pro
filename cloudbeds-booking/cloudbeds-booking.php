...
/**
 * Redireciona para o Cloudbeds quando o campo de check-in recebe foco.
 */
function cloudbeds_redirect_on_checkin() {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.addEventListener('focusin', function (e) {
            if (e.target && e.target.matches('input[name="checkin"][type="date"]')) {
                window.location.href = redirectUrl;
            }
        });
        document.addEventListener('click', function (e) {
            if (e.target.closest('.cs-form-field.cs-check-in')) {
                window.location.href = redirectUrl;
            }
        });
    });
