 ...
 * Text Domain: questoes-comentadas-ac
...
class Questoes_Comentadas_Plugin {
    ...
    const OPTION_SEEDED     = 'qca_sample_data_seeded';
    const OPTION_REWRITE    = 'qca_rewrite_rules_version';
    ...
    private function migrate_legacy_options() {
        $mappings = [
            'qc_sample_data_seeded'    => self::OPTION_SEEDED,
            'qc_rewrite_rules_version' => self::OPTION_REWRITE,
        ];

        foreach ( $mappings as $legacy_option => $new_option ) {
            $legacy_value = get_option( $legacy_option, null );

            if ( null !== $legacy_value && ! get_option( $new_option, null ) ) {
    ...
    public function register_post_type_and_taxonomies() {
        $labels = [
            'name'               => __( 'Questões Comentadas', 'questoes-comentadas-ac' ),
            'singular_name'      => __( 'Questão Comentada', 'questoes-comentadas-ac' ),
            'add_new'            => __( 'Adicionar Nova', 'questoes-comentadas-ac' ),
            'add_new_item'       => __( 'Adicionar Nova Questão', 'questoes-comentadas-ac' ),
            'edit_item'          => __( 'Editar Questão', 'questoes-comentadas-ac' ),
            'new_item'           => __( 'Nova Questão', 'questoes-comentadas-ac' ),
            'view_item'          => __( 'Ver Questão', 'questoes-comentadas-ac' ),
            'search_items'       => __( 'Buscar Questões', 'questoes-comentadas-ac' ),
            'not_found'          => __( 'Nenhuma questão encontrada', 'questoes-comentadas-ac' ),
            'not_found_in_trash' => __( 'Nenhuma questão encontrada na lixeira', 'questoes-comentadas-ac' ),
            'menu_name'          => __( 'Questões Comentadas', 'questoes-comentadas-ac' ),
        ];
        ...
        register_post_type(
            ...
            [
                ...
                'has_archive'        => 'questoes-comentadas-ac',
                'rewrite'            => [
                    'slug'       => 'questoes-comentadas-ac',
                    'with_front' => false,
                ],
        ...
        $this->register_taxonomy(
            ...
            __( 'Categorias', 'questoes-comentadas-ac' ),
            __( 'Categoria', 'questoes-comentadas-ac' ),
            'questoes-comentadas-ac/categoria'
        );

        $this->register_taxonomy(
            self::TAXONOMY_BANK,
            __( 'Bancas', 'questoes-comentadas-ac' ),
            __( 'Banca', 'questoes-comentadas-ac' ),
            'questoes-comentadas-ac/banca'
        );

        $this->register_taxonomy(
            self::TAXONOMY_SUBJECT,
            __( 'Assuntos', 'questoes-comentadas-ac' ),
            __( 'Assunto', 'questoes-comentadas-ac' ),
            'questoes-comentadas-ac/assunto'
        );
    }

    private function register_taxonomy( $taxonomy, $plural_label, $singular_label, $rewrite_slug ) {
        $labels = [
            'name'          => $plural_label,
            'singular_name' => $singular_label,
            'search_items'  => sprintf( __( 'Buscar %s', 'questoes-comentadas-ac' ), strtolower( $plural_label ) ),
            'all_items'     => sprintf( __( 'Todas as %s', 'questoes-comentadas-ac' ), strtolower( $plural_label ) ),
            'edit_item'     => sprintf( __( 'Editar %s', 'questoes-comentadas-ac' ), strtolower( $singular_label ) ),
            'update_item'   => sprintf( __( 'Atualizar %s', 'questoes-comentadas-ac' ), strtolower( $singular_label ) ),
            'add_new_item'  => sprintf( __( 'Adicionar nova %s', 'questoes-comentadas-ac' ), strtolower( $singular_label ) ),
    ...
    public function maybe_flush_rewrite_rules() {
        $stored_version = get_option( self::OPTION_REWRITE );

        if ( $stored_version && version_compare( $stored_version, self::VERSION, '>=' ) ) {
            return;
        }

        flush_rewrite_rules( false );
        update_option( self::OPTION_REWRITE, self::VERSION );
    }

    public function register_admin_menu() {
        add_menu_page(
            __( 'Questões Comentadas', 'questoes-comentadas-ac' ),
            __( 'Questões Comentadas', 'questoes-comentadas-ac' ),
            'edit_posts',
            'questoes-comentadas-ac-dashboard',
        ...
        add_submenu_page(
            'questoes-comentadas-ac-dashboard',
            __( 'Todas as Questões', 'questoes-comentadas-ac' ),
            __( 'Todas as Questões', 'questoes-comentadas-ac' ),
            'edit_posts',
        ...
        add_submenu_page(
            'questoes-comentadas-ac-dashboard',
            __( 'Adicionar Nova', 'questoes-comentadas-ac' ),
            __( 'Adicionar Nova', 'questoes-comentadas-ac' ),
            'edit_posts',
    ...
    public function render_dashboard_page() {
        if ( ! current_user_can( 'edit_posts' ) ) {
            wp_die( __( 'Você não tem permissão para acessar esta página.', 'questoes-comentadas-ac' ) );
        }
        ...
        $taxonomy_summaries = [
            [
                'title'       => __( 'Categorias em destaque', 'questoes-comentadas-ac' ),
                'description' => __( 'Acompanhe como as questões publicadas se distribuem nas categorias principais.', 'questoes-comentadas-ac' ),
                'items'       => $category_summary,
            ],
            [
                'title'       => __( 'Bancas representadas', 'questoes-comentadas-ac' ),
                'description' => __( 'Visualize quais bancas já contam com questões comentadas.', 'questoes-comentadas-ac' ),
                'items'       => $bank_summary,
            ],
            [
                'title'       => __( 'Assuntos cobertos', 'questoes-comentadas-ac' ),
                'description' => __( 'Confira os assuntos que receberam comentários detalhados.', 'questoes-comentadas-ac' ),
        ...
        <div class="wrap qc-dashboard">
            <h1><?php esc_html_e( 'Questões Comentadas', 'questoes-comentadas-ac' ); ?></h1>
            <p class="description">
                <?php esc_html_e( 'Cadastre, gerencie e publique questões comentadas alinhadas às bancas e assuntos da Academia da Comunicação.', 'questoes-comentadas-ac' ); ?>
            </p>

            <div class="qc-dashboard__cards">
                <div class="qc-dashboard__card">
                    <h2><?php esc_html_e( 'Questões Publicadas', 'questoes-comentadas-ac' ); ?></h2>
                    <p class="qc-dashboard__card-number"><?php echo esc_html( $total ); ?></p>
                    <a class="button button-primary" href="<?php echo esc_url( $base_url ); ?>">
                        <?php esc_html_e( 'Ver todas', 'questoes-comentadas-ac' ); ?>
                    </a>
                </div>
                <div class="qc-dashboard__card">
                    <h2><?php esc_html_e( 'Rascunhos', 'questoes-comentadas-ac' ); ?></h2>
                    ...
                    <a class="button" href="<?php echo esc_url( add_query_arg( 'post_status', 'draft', $base_url ) ); ?>">
                        <?php esc_html_e( 'Gerenciar rascunhos', 'questoes-comentadas-ac' ); ?>
                    </a>
                </div>
            </div>

            <section class="qc-dashboard__progress" aria-labelledby="qc-dashboard-progress-title">
                <h2 id="qc-dashboard-progress-title"><?php esc_html_e( 'Progresso de publicação', 'questoes-comentadas-ac' ); ?></h2>
                <p>
                    <?php
                    printf(
                        esc_html__( '%1$d de %2$d questões estão publicadas (%3$d%%).', 'questoes-comentadas-ac' ),
                        intval( $total ),
                        intval( $all_total ),
                        intval( $progress )
                    );
            ...
            <section class="qc-dashboard__status" aria-labelledby="qc-dashboard-status-title">
                <h2 id="qc-dashboard-status-title"><?php esc_html_e( 'Acompanhamento por status', 'questoes-comentadas-ac' ); ?></h2>
                    ...
                    <?php foreach ( $status_data as $status_key => $status_item ) : ?>
                        <li class="qc-dashboard__status-item">
                            <div class="qc-dashboard__status-header">
                                ...
                                <span class="qc-dashboard__status-value"><?php echo esc_html( sprintf( _n( '%d registro', '%d registros', $status_item['count'], 'questoes-comentadas-ac' ), $status_item['count'] ) ); ?> · <?php echo esc_html( $status_item['percentage'] ); ?>%</span>
                            </div>
                            <div class="qc-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?php echo esc_attr( $status_item['percentage'] ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Status %1$s representa %2$d%% das questões cadastradas', 'questoes-comentadas-ac' ), $status_item['label'], $status_item['percentage'] ) ); ?>">
            ...
            if ( $has_taxonomy_summary ) :
                ...
                <section class="qc-dashboard__taxonomy" aria-labelledby="qc-dashboard-taxonomy-title">
                    <h2 id="qc-dashboard-taxonomy-title"><?php esc_html_e( 'Distribuição por taxonomias', 'questoes-comentadas-ac' ); ?></h2>
                                    ...
                                    <?php foreach ( $summary['items'] as $item ) : ?>
                                        <li class="qc-dashboard__taxonomy-item">
                                            <div class="qc-dashboard__taxonomy-header">
                                                ...
                                                <span class="qc-dashboard__taxonomy-value"><?php echo esc_html( sprintf( _n( '%d questão', '%d questões', $item['count'], 'questoes-comentadas-ac' ), $item['count'] ) ); ?> · <?php echo esc_html( $item['percentage'] ); ?>%</span>
                                            </div>
                                            <div
                                                class="qc-progress"
                                                role="progressbar"
                                                ...
                                                aria-label="<?php echo esc_attr( sprintf( __( '%1$s representa %2$d%% das questões cadastradas', 'questoes-comentadas-ac' ), $item['name'], $item['percentage'] ) ); ?>"
                                            >
            ...
            <div class="qc-dashboard__quick-actions">
                <h2><?php esc_html_e( 'Ações rápidas', 'questoes-comentadas-ac' ); ?></h2>
                <ul>
                    <li><a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=' . self::CPT ) ); ?>"><?php esc_html_e( 'Adicionar nova questão comentada', 'questoes-comentadas-ac' ); ?></a></li>
                    <li><a href="<?php echo esc_url( admin_url( 'edit-tags.php?taxonomy=' . self::TAXONOMY_CATEGORY . '&post_type=' . self::CPT ) ); ?>"><?php esc_html_e( 'Organizar categorias', 'questoes-comentadas-ac' ); ?></a></li>
                    <li><a href="<?php echo esc_url( admin_url( 'edit-tags.php?taxonomy=' . self::TAXONOMY_BANK . '&post_type=' . self::CPT ) ); ?>"><?php esc_html_e( 'Organizar bancas', 'questoes-comentadas-ac' ); ?></a></li>
                    <li><a href="<?php echo esc_url( admin_url( 'edit-tags.php?taxonomy=' . self::TAXONOMY_SUBJECT . '&post_type=' . self::CPT ) ); ?>"><?php esc_html_e( 'Organizar assuntos', 'questoes-comentadas-ac' ); ?></a></li>
                </ul>
            </div>
            ...
            if ( $preview->have_posts() ) :
                ...
                <div class="qc-dashboard__preview">
                    <h2><?php esc_html_e( 'Questões destacadas', 'questoes-comentadas-ac' ); ?></h2>
                            ...
                            <article class="qc-dashboard-card" aria-labelledby="qc-dashboard-card-title-<?php the_ID(); ?>">
                                ...
                                <ul class="qc-dashboard-card__meta">
                                    <?php if ( ! empty( $category ) ) : ?>
                                        <li><strong><?php esc_html_e( 'Categoria:', 'questoes-comentadas-ac' ); ?></strong> <?php echo esc_html( $category[0]->name ); ?></li>
                                    <?php endif; ?>
                                    <?php if ( ! empty( $bank ) ) : ?>
                                        <li><strong><?php esc_html_e( 'Banca:', 'questoes-comentadas-ac' ); ?></strong> <?php echo esc_html( $bank[0]->name ); ?></li>
                                    <?php endif; ?>
                                    <?php if ( ! empty( $subject ) ) : ?>
                                        <li><strong><?php esc_html_e( 'Assunto:', 'questoes-comentadas-ac' ); ?></strong> <?php echo esc_html( $subject[0]->name ); ?></li>
                                    <?php endif; ?>
                                </ul>
                            </article>
                            <?php
    ...
    public function enqueue_admin_assets( $hook ) {
        if ( 'toplevel_page_questoes-comentadas-ac-dashboard' !== $hook ) {
            return;
        }

        wp_enqueue_style(
            'questoes-comentadas-ac-admin',
    ...
    public function enqueue_front_assets() {
        wp_enqueue_style(
            'questoes-comentadas-ac-frontend',
    ...
    public function render_shortcode( $atts ) {
        ...
        if ( ! $query->have_posts() ) {
            return '<p>' . esc_html__( 'Nenhuma questão comentada disponível no momento.', 'questoes-comentadas-ac' ) . '</p>';
        }
        ...
        $summary_blocks   = [
            'categoria' => [
                'title' => __( 'Distribuição por categoria', 'questoes-comentadas-ac' ),
                'items' => $categories,
            ],
            'banca'     => [
                'title' => __( 'Distribuição por banca', 'questoes-comentadas-ac' ),
                'items' => $banks_summary,
            ],
            'assunto'   => [
                'title' => __( 'Distribuição por assunto', 'questoes-comentadas-ac' ),
        ...
        $results_percentage_template = __( 'Você está visualizando %percentage%%% das questões desta seção.', 'questoes-comentadas-ac' );
        ...
        <section class="qc-shortcode-wrapper">
            <header class="qc-shortcode-header">
                <p class="qc-shortcode-kicker"><?php esc_html_e( 'Academia da Comunicação', 'questoes-comentadas-ac' ); ?></p>
                <h2 class="qc-shortcode-title"><?php esc_html_e( 'Questões Comentadas de Português', 'questoes-comentadas-ac' ); ?></h2>
                <p class="qc-results-count" aria-live="polite">
                    <span><?php echo esc_html( $query->post_count ); ?></span>
                    <?php esc_html_e( 'questões disponíveis nesta visualização.', 'questoes-comentadas-ac' ); ?>
                </p>
                <p
            ...
            <?php if ( $show_summary && $has_summary_blocks ) : ?>
                        ...
                        <section class="qc-shortcode-summary" data-qc-summary-card data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>">
                            ...
                            <ul class="qc-progress-list" data-qc-summary-list data-taxonomy="<?php echo esc_attr( $taxonomy_key ); ?>">
                                <?php foreach ( $summary_block['items'] as $item ) :
                                    $count_text    = sprintf( _n( '%d questão', '%d questões', $item['count'], 'questoes-comentadas-ac' ), $item['count'] );
                                    $label_template = __( '%label% representa %percentage%%% das questões visíveis', 'questoes-comentadas-ac' );
                                    ...
                                    <li
                                        ...
                                        data-count-singular="<?php echo esc_attr__( '%d questão', 'questoes-comentadas-ac' ); ?>"
                                        data-count-plural="<?php echo esc_attr__( '%d questões', 'questoes-comentadas-ac' ); ?>"
                                    >
                            ...
                            <p class="qc-shortcode-total">
                                <?php
                                printf(
                                    esc_html( _n( '%d registro relacionado nesta taxonomia.', '%d registros relacionados nesta taxonomia.', $summary_block['total'], 'questoes-comentadas-ac' ) ),
                ...
                <p class="qc-shortcode-total qc-shortcode-total--overall">
                    <?php printf( esc_html__( '%d questões publicadas no total.', 'questoes-comentadas-ac' ), intval( $total_published ) ); ?>
                </p>
            <?php endif; ?>
            ...
            <?php if ( $show_filters ) :
                ...
                if ( ! empty( $filter_options ) ) :
                    ?>
                <form class="qc-filters" aria-label="<?php esc_attr_e( 'Filtrar questões comentadas', 'questoes-comentadas-ac' ); ?>">
                    <?php foreach ( $filter_options as $taxonomy_key => $data ) : ?>
                        <label>
                            <span class="screen-reader-text"><?php echo esc_html( $data['label'] ); ?></span>
                            <select name="qca_filter_<?php echo esc_attr( $taxonomy_key ); ?>">
                    ...
                    <?php endforeach; ?>
                    <button type="button" class="qc-filters__clear"><?php esc_html_e( 'Limpar filtros', 'questoes-comentadas-ac' ); ?></button>
                </form>
                    <?php
                endif;
            endif;
            ...
            while ( $query->have_posts() ) {
                ...
                <article class="qc-card" aria-labelledby="qc-card-title-<?php the_ID(); ?>" <?php echo $data_attributes; ?>>
                    ...
                    <details class="qc-card__details">
                        <summary><?php esc_html_e( 'Ver comentário completo', 'questoes-comentadas-ac' ); ?></summary>
            ...
            <p class="qc-empty-message" hidden><?php esc_html_e( 'Nenhuma questão corresponde aos filtros selecionados.', 'questoes-comentadas-ac' ); ?></p>

            <footer class="qc-shortcode-footer">
                <section class="qc-shortcode-progress" aria-labelledby="qc-shortcode-progress-title">
                    <h3 id="qc-shortcode-progress-title"><?php esc_html_e( 'Progresso de publicação', 'questoes-comentadas-ac' ); ?></h3>
                ...
                <?php if ( ! empty( $status_for_front ) ) : ?>
                    <section class="qc-shortcode-status" aria-labelledby="qc-shortcode-status-title">
                        <h3 id="qc-shortcode-status-title"><?php esc_html_e( 'Distribuição por status', 'questoes-comentadas-ac' ); ?></h3>
                            ...
                            <?php foreach ( $status_for_front as $status_key => $status_item ) : ?>
                                <li class="qc-status-item">
                                    <div class="qc-status-item__header">
                                        ...
                                        <span class="qc-status-item__value"><?php echo esc_html( sprintf( _n( '%d registro', '%d registros', $status_item['count'], 'questoes-comentadas-ac' ), $status_item['count'] ) ); ?> · <?php echo esc_html( $status_item['percentage'] ); ?>%</span>
                                    </div>
                                    <div class="qc-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?php echo esc_attr( $status_item['percentage'] ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'Status %1$s representa %2$d%% das questões cadastradas', 'questoes-comentadas-ac' ), $status_item['label'], $status_item['percentage'] ) ); ?>">
                                        <span class="qc-progress__bar" style="--qc-progress-value: <?php echo esc_attr( $status_item['percentage'] ); ?>%;"></span>
                                    </div>
    ...
    private function get_filter_options() {
        $taxonomies = [
            'categoria' => [
                ...
                'label'       => __( 'Filtrar por categoria', 'questoes-comentadas-ac' ),
                'placeholder' => __( 'Todas as categorias', 'questoes-comentadas-ac' ),
            ],
            'banca'     => [
                'taxonomy'    => self::TAXONOMY_BANK,
                'label'       => __( 'Filtrar por banca', 'questoes-comentadas-ac' ),
                'placeholder' => __( 'Todas as bancas', 'questoes-comentadas-ac' ),
            ],
            'assunto'   => [
                'taxonomy'    => self::TAXONOMY_SUBJECT,
                'label'       => __( 'Filtrar por assunto', 'questoes-comentadas-ac' ),
                'placeholder' => __( 'Todos os assuntos', 'questoes-comentadas-ac' ),
            ],
        ];
    ...
    private function get_post_status_counts() {
        ...
        $statuses = [
            'publish' => __( 'Publicadas', 'questoes-comentadas-ac' ),
            'draft'   => __( 'Rascunhos', 'questoes-comentadas-ac' ),
            'pending' => __( 'Pendentes', 'questoes-comentadas-ac' ),
            'future'  => __( 'Agendadas', 'questoes-comentadas-ac' ),
        ];
...
register_deactivation_hook( __FILE__, [ 'Questoes_Comentadas_Plugin', 'deactivate' ] );
