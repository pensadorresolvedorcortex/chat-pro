    ...
    if ( $screen && $screen->id === 'dashboard' ) {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'groupsNonce' => wp_create_nonce( 'gdw_groups_card' ),
                'strings'  => array(
                    'noRecent'             => __( 'Nenhuma transação recente', 'gdwlang' ),
                    'noGroups'             => __( 'Nenhum grupo precisa de tratamento', 'gdwlang' ),
                    'noUsers'              => __( 'Sem cadastros recentes', 'gdwlang' ),
                    'approve'              => __( 'Aprovar ou recusar', 'gdwlang' ),
                    'monthlyTransactions'  => __( '%d transações no período', 'gdwlang' ),
                ),
            )
        );
    }
...
function gdw_load_dashboard_widgets(){
    ...
    $widgetid = "gdw_groups_add_dashboard";
    $groups_enabled = isset($gdwadmin[$element][$widgetid]) ? $gdwadmin[$element][$widgetid] == $widgetid : true;
    if( $groups_enabled ){
    add_action( 'wp_dashboard_setup', $widgetid );
    }

    // Ensure the GRUPOS widget stays no topo da página
    add_action( 'wp_dashboard_setup', 'gdw_prioritize_groups_widget', 100 );
...
function gdw_prioritize_groups_widget() {
    ...
    if ( empty( $wp_meta_boxes['dashboard']['normal']['core']['gdw_groups_wp_dashboard'] ) ) {
        return;
    }

    $group_widget = array( 'gdw_groups_wp_dashboard' => $wp_meta_boxes['dashboard']['normal']['core']['gdw_groups_wp_dashboard'] );
    $others       = $wp_meta_boxes['dashboard']['normal']['core'];
    unset( $others['gdw_groups_wp_dashboard'] );
    $wp_meta_boxes['dashboard']['normal']['core'] = $group_widget + $others;
}
add_action( 'wp_dashboard_setup', 'gdw_prioritize_groups_widget', 100 );
...
function gdw_localize_label( $key ) {
    ...
    $map = array(
        ...
        'wc-pending'          => array( 'msgid' => 'Wc-pending', 'fallback' => 'Pagamento pendente' ),
        'wc-processing'       => array( 'msgid' => 'Wc-processing', 'fallback' => 'Processando pedido' ),
        'wc-on-hold'          => array( 'msgid' => 'Wc-on-hold', 'fallback' => 'Em espera' ),
        'wc-completed'        => array( 'msgid' => 'Wc-completed', 'fallback' => 'Concluído' ),
        'wc-cancelled'        => array( 'msgid' => 'Wc-cancelled', 'fallback' => 'Cancelado' ),
        'wc-refunded'         => array( 'msgid' => 'Wc-refunded', 'fallback' => 'Reembolsado' ),
        'wc-failed'           => array( 'msgid' => 'Wc-failed', 'fallback' => 'Pagamento com falha' ),
        'wc-checkout-draft'   => array( 'msgid' => 'Wc-checkout', 'fallback' => 'Checkout' ),
        'wc-checkout'         => array( 'msgid' => 'Wc-checkout', 'fallback' => 'Checkout' ),
        ...
        'trash'               => array( 'msgid' => 'Trash', 'fallback' => 'Lixeira' ),
        'request-pending'     => array( 'msgid' => 'Request Pending', 'fallback' => 'Solicitação pendente' ),
        'request_pending'     => array( 'msgid' => 'Request Pending', 'fallback' => 'Solicitação pendente' ),
    );
...
/**
 * Ajax: refresh data for the GRUPOS card without reloading the dashboard.
 */
function gdw_groups_card_refresh() {
    check_ajax_referer( 'gdw_groups_card', 'nonce' );

    $helper = plugin_dir_path( __FILE__ ) . '../site-stats/includes/gdw-groups-data.php';
    ...
    $data = gdw_get_groups_card_snapshot();

    $data['monthly_total_label'] = isset( $data['monthly_total_label'] ) ? wp_kses_post( $data['monthly_total_label'] ) : '';

    if ( isset( $data['new_users'] ) && is_array( $data['new_users'] ) ) {
        $data['new_users'] = array_map(
            function ( $user ) {
                return array(
                    'name'  => isset( $user['name'] ) ? sanitize_text_field( $user['name'] ) : '',
                    'photo' => isset( $user['photo'] ) ? esc_url_raw( $user['photo'] ) : '',
                );
            },
            $data['new_users']
        );
    ...
    if ( isset( $data['pending_groups'] ) && is_array( $data['pending_groups'] ) ) {
        $data['pending_groups'] = array_map(
            function ( $group ) {
                return array(
                    'title' => isset( $group['title'] ) ? sanitize_text_field( $group['title'] ) : '',
                    'thumb' => isset( $group['thumb'] ) ? esc_url_raw( $group['thumb'] ) : '',
                );
            },
            $data['pending_groups']
        );
    ...
    if ( isset( $data['recent_orders'] ) && is_array( $data['recent_orders'] ) ) {
        $data['recent_orders'] = array_map(
            function ( $order ) {
                return array(
                    'date'  => isset( $order['date'] ) ? sanitize_text_field( $order['date'] ) : '',
                    'total' => isset( $order['total'] ) ? wp_kses_post( $order['total'] ) : '',
                );
            },
            $data['recent_orders']
        );
...
add_action( 'wp_ajax_gdw_groups_card_refresh', 'gdw_groups_card_refresh' );
