  ...
  <div style="height: 200px; background-color: #F8F8F8; background-image: linear-gradient(#F8F8F8, #FFF);">
    <div class="row" style="margin-top: 10px; padding: 10px">
      <div class="col-md-8 col-xs-12">
        <h3 style="color: #3e4095; font-weight: bold">DECLARAÇÃO DE ATIVIDADE NO CLUBE</h3>
        <div class="alert alert-warning"><i class="fas fa-exclamation-triangle"></i> Esta declaração serve para comprovar suas atividades nos últimos 12 meses, mas NÃO SERVE para comprovação junto ao Exército Brasileiro. Para este fim utilizes as outras declarações disponíveis.</div>
        <div class="alert alert-warning"><i class="fas fa-exclamation-triangle"></i> Não serão consideradas as provas de ar comprimido nesta declaração.</div>
        <div class="alert alert-warning" align="justify"><i class="fas fa-exclamation-triangle"></i> Ao reemitir um documento, o QRCode apresentará a data de quando o documento foi impresso, anteriormente, e não a data atualizada da reimpressão.
          Para emitir um documento atualizado clique em imprimir ou aguarde o próximo dia útil.</div>
        </br>
        <?php
        $data = date("Y-m-d");
        $contarFiliacao = $metodos->contarDeclaracao($tipo = 'A', $data);
        if($contarFiliacao > 0){
        ?>
      ...
      <?php } ?>
        <p>Confira abaixo as declarações já emitidas:</p>
         <?php
         //echo $metodos->habitualidade();
         echo $metodos->visualizarDeclaracoes($tipo = 'A');
         ?>
         ...
         $(document).on('click',".autenticacao", function(){
           ...
           $.post('<?php echo $caminhoAbsoluto; ?>/visualizar-atividades.php',{key: codigo}, function(retorno){
