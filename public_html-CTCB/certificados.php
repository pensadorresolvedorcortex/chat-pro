  ...
  <div style="height: 200px; background-color: #F8F8F8; background-image: linear-gradient(#F8F8F8, #FFF);">
    ...
    <div class="row" style="margin-top: 10px; padding: 10px">
      <div class="col-md-8 col-xs-12">
        <h3 style="color: #3e4095; font-weight: bold">CERTIFICADOS</h3>
         <form class="" action="#!" method="post">
           <?php echo $metodos->listarEventosCertificado($_SESSION["IdUsuario"]); ?>
         ...
         $(document).on('click',".autenticacao", function(){
           ...
           var evento = $("#id_categoria option:selected").val();
           var prova = $("#id_sub_categoria option:selected").val();
           $.post('<?php echo $caminhoAbsoluto; ?>/visualizar-certificado.php',{key: codigo,evento: evento,prova: prova}, function(retorno){
  ...
  <script src="<?php echo $caminhoAbsoluto; ?>/js/jquery.min.js"></script>
   ...
   <script type="text/javascript">
        ...
        if( $(this).val() ) {
					...
					$('.carregando').show();
					$.getJSON('<?php echo $caminhoAbsoluto; ?>/selecionar-subcategoria-certificado.php?search=',{id_categoria: $(this).val(), ajax: 'true'}, function(j){
						var options = '';
          ...
          $('#botaoImprimir').html('<div align="center"> <button type="button" name="button" class="btn btn-primary autenticacao" data-toggle="modal" data-target=".bd-example-modal-lg">Visualizar Certificado</button> </div>');
				} else {
