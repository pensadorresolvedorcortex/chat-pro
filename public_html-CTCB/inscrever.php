...
$tabela = "evento";
$idTabela = "idcodevento";
$idBusca = $_SERVER["QUERY_STRING"];
$visualizar = $metodos->visualizar($tabela, $idTabela, $idBusca);
if($visualizar[0] == 0){
  echo "<script>window.location.href='".$caminhoAbsoluto."/inscricao/';</script>";
  exit();
}
?>
<!DOCTYPE html>
  ...
  <div style="height: 200px; background-color: #F8F8F8; background-image: linear-gradient(#F8F8F8, #FFF);">
    <div class="row" style="margin-top: 10px; padding: 10px">
      <div class="col-md-8 col-xs-12">
        <h3 style="color: #3e4095; font-weight: bold"><i class="fas fa-pen-nib"></i> INSCRIÇÕES EM EVENTO</h3>
        <h5 style="color: #3e4095; font-weight: bold; text-transform: uppercase; margin-top: 30px; color: #000"><i class="far fa-check-square"></i> <?php echo $visualizar[1]->nome; ?></h5>
           <?php if($erro == true){ ?>
           ...
           <div style="margin-top: 20px; text-align: center">
             <button type="button" name="button" class="btn btn-primary" onclick="window.location.href='<?php echo $caminhoAbsoluto; ?>/inscricao/'"><i class="fas fa-angle-double-left"></i> Voltar</button>
           <button type="submit" name="button" class="btn btn-success" title="Prosseguir para a finalização da inscrição">Prosseguir <i class="fas fa-angle-double-right"></i></button>
         </div>
      </div>
