  ...
  <div style="height: 200px; background-color: #F8F8F8; background-image: linear-gradient(#F8F8F8, #FFF);">
    <div class="row" style="margin-top: 10px; padding: 10px">
      <div class="col-md-8 col-xs-12">
        <h3 style="color: #3e4095; font-weight: bold">DECLARAÇÃO DE FILIAÇÃO A ENTIDADE</h3>
        <?php
        $data = date("Y-m-d");
        $contarFiliacao = $metodos->contarDeclaracao($tipo = 'F',$data);
        if($contarFiliacao > 0){
        ?>
      ...
      Para emitir um documento atualizado clique em imprimir ou aguarde o próximo dia útil.</div>
        ...
        <div align="center"><a href="!#" class="btn btn-primary autenticacao" data-toggle="modal" data-target=".bd-example-modal-lg">Imprimir <i class="fas fa-print"></i></a></div>
         <br>
         <?php
         $contarFiliacao = $metodos->contarDeclaracao($tipo = 'F',$data = null);
         if($contarFiliacao > 0){ ?>
         Confira abaixo as declarações já emitidas:
           <?php
           //echo $metodos->filiacaoEntidade();
           echo $metodos->visualizarDeclaracoes($tipo = 'F');
           ?>
         <?php } ?>
         ...
         $(document).on('click',".autenticacao", function(){
           ...
           $.post('<?php echo $caminhoAbsoluto; ?>/codigo-autenticacao.php',{key: codigo}, function(retorno){
