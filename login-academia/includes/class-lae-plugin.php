...
class LAE_Plugin {
    ...
    public function enqueue_assets() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'identity'  => class_exists( 'Introducao_Auth' ) ? Introducao_Auth::get_client_identity() : array(),
                'messages' => array(
        ...
        if ( $challenge_id && $code ) {
            ...
            if ( is_wp_error( $verification ) ) {
                ...
                wp_send_json_error(
                    array(
                        'status'    => $status,
                        'message'   => $this->prepare_error_message(
                            $verification,
                            __( 'Não foi possível validar o código de verificação. Tente novamente.', 'login-academia-da-educacao' )
                        ),
        ...
        if ( is_wp_error( $user ) ) {
            wp_send_json_error(
                array(
                    'message' => $this->prepare_error_message(
                        $user,
                        __( 'Não foi possível completar o login. Verifique os dados e tente novamente.', 'login-academia-da-educacao' )
                    ),
                )
            );
        }
        ...
        if ( is_wp_error( $challenge ) ) {
            wp_send_json_error(
                array(
                    'message' => $this->prepare_error_message(
                        $challenge,
                        __( 'Não foi possível iniciar a verificação em duas etapas. Tente novamente.', 'login-academia-da-educacao' )
                    ),
                )
            );
        }
            ...
            if ( is_wp_error( $verification ) ) {
                ...
                wp_send_json_error(
                    array(
                        ...
                        'message'   => $this->prepare_error_message(
                            $verification,
                            __( 'Não foi possível validar o código enviado. Tente novamente.', 'login-academia-da-educacao' )
                        ),
                        'challenge' => $challenge_id,
                        'context'   => $context,
                    )
            ...
            if ( is_wp_error( $user_id ) ) {
                wp_send_json_error(
                    array(
                        'message' => $this->prepare_error_message(
                            $user_id,
                            __( 'Não foi possível criar a conta. Verifique os dados e tente novamente.', 'login-academia-da-educacao' )
                        ),
                    )
                );
            }
            ...
            if ( is_wp_error( $user_id ) ) {
                wp_send_json_error(
                    array(
                        'message' => $this->prepare_error_message(
                            $user_id,
                            __( 'Não foi possível concluir o cadastro. Verifique os dados e tente novamente.', 'login-academia-da-educacao' )
                        ),
                    )
                );
            }
        ...
        if ( is_wp_error( $result ) ) {
            wp_send_json_error(
                array(
                    'message' => $this->prepare_error_message(
                        $result,
                        __( 'Não foi possível reenviar o código agora. Aguarde alguns instantes e tente novamente.', 'login-academia-da-educacao' )
                    ),
                )
            );
        }
    ...
    private function build_challenge_context( $challenge_id, $challenge_state ) {
        ...
        if ( class_exists( 'Introducao_Auth' ) ) {
            $client_state = Introducao_Auth::get_client_challenge_context();

            if ( isset( $client_state['challenge'] ) && $client_state['challenge'] === $challenge_id ) {
                $merge_keys = array(
                    'identifier',
                    'pending_login',
                    'pending_email',
                ...
                foreach ( $merge_keys as $merge_key ) {
                    if ( isset( $client_state[ $merge_key ] ) ) {
                        $context[ $merge_key ] = $client_state[ $merge_key ];
                    }
                }
            }
        }
    ...
    /**
     * Normaliza mensagens de erro para exibição amigável.
     *
     * @param string|WP_Error $error   Mensagem original ou objeto de erro.
     * @param string           $default Mensagem padrão caso a original esteja vazia.
     *
     * @return string
     */
    private function prepare_error_message( $error, $default = '' ) {
        if ( class_exists( 'Introducao_Auth' ) && method_exists( 'Introducao_Auth', 'normalize_auth_error' ) ) {
            $message = Introducao_Auth::normalize_auth_error( $error );
        } else {
            if ( $error instanceof WP_Error ) {
                $error = $error->get_error_message();
            }

            $message = is_string( $error ) ? trim( wp_strip_all_tags( wp_specialchars_decode( $error ) ) ) : '';
        }

        if ( '' === $message ) {
            $message = $default ? $default : __( 'Não foi possível concluir a solicitação. Tente novamente.', 'login-academia-da-educacao' );
        }
