...
class LAE_Pages {
    ...
    /**
     * Slug alternativo utilizado para a página de dados da conta.
     *
     * @var string
     */
    private $custom_account_slug = 'dados-conta';
    ...
    public function get_page_id( $slug ) {
        ...
        if ( 'minha-conta-academia' === $slug && $this->custom_account_slug ) {
            $custom_page = get_page_by_path( $this->custom_account_slug, OBJECT, 'page' );

            if ( $custom_page instanceof WP_Post ) {
                return (int) $custom_page->ID;
            }
        }
    ...
    public function get_page_url( $slug ) {
        if ( 'minha-conta-academia' === $slug ) {
            $custom_account = $this->get_custom_account_url();
    ...
    /**
     * Retorna a URL pública da página de dados da conta.
     *
     * @return string
     */
    public function get_account_page_url() {
        return $this->get_custom_account_url();
    }
    ...
    public function get_menu_items() {
        ...
        foreach ( $this->pages as $slug => $data ) {
            ...
            if ( 'minha-conta-academia' === $slug ) {
                $url = $this->get_custom_account_url();
            }
    ...
    public function is_current_page_slug( $slug ) {
        ...
        if ( 'minha-conta-academia' === $slug && $this->custom_account_slug ) {
            if ( is_page( $this->custom_account_slug ) ) {
                return true;
            }

            $custom_page = get_page_by_path( $this->custom_account_slug, OBJECT, 'page' );

            if ( $custom_page instanceof WP_Post && is_page( $custom_page->ID ) ) {
                return true;
            }
        }
    ...
    /**
     * Retorna a URL utilizada pelo menu para a página de dados da conta.
     *
     * @return string
     */
    private function get_custom_account_url() {
        if ( ! $this->custom_account_slug ) {
            return home_url( '/' );
        }

        $page = get_page_by_path( $this->custom_account_slug, OBJECT, 'page' );

        if ( $page instanceof WP_Post ) {
            return get_permalink( $page );
        }

        return home_url( user_trailingslashit( $this->custom_account_slug ) );
    }
}
