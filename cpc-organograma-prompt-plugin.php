...
CSS;
        ...
        private function render_user_home(array $sections, array $atts = []): string
        {
            ...
            if ('' === $hero_tag) {
                $hero_tag = sanitize_text_field(__('Academia da Comunicação', 'cpc-login-academia'));
            }

            $hero_title = $this->format_home_text($atts['hero_title'], $greeting_name);
            if ('' === $hero_title) {
                $hero_title = sanitize_text_field(sprintf(__('Olá, %s!', 'cpc-login-academia'), $greeting_name));
            }
            ...
            $first_card = $cards[0] ?? null;

            $primary_label = $this->format_home_text($atts['primary_label'], $greeting_name);
            if ('' === $primary_label) {
                if ($first_card && !empty($first_card['label'])) {
                    $primary_label = sanitize_text_field(sprintf(__('Acessar %s', 'cpc-login-academia'), $first_card['label']));
                } else {
                    $primary_label = sanitize_text_field(__('Continuar módulo', 'cpc-login-academia'));
                }
            }

            $primary_url = '' !== $atts['primary_url'] ? esc_url_raw($atts['primary_url']) : '';
            if ('' === $primary_url && $first_card && !empty($first_card['url'])) {
                $primary_url = $first_card['url'];
            }

            $secondary_label = $this->format_home_text($atts['secondary_label'], $greeting_name);
            if ('' === $secondary_label) {
                $secondary_label = sanitize_text_field(__('Ver perfil completo', 'cpc-login-academia'));
            }

            $secondary_url = '' !== $atts['secondary_url'] ? esc_url_raw($atts['secondary_url']) : '';
            if ('' === $secondary_url && '' !== $profile_url) {
            ...
            $hero_filtered = apply_filters('cpc_orgprompt_home_hero', $hero_defaults, $user, $cards, $atts);

            $hero_tag = sanitize_text_field($hero_filtered['tag'] ?? $hero_defaults['tag']);
            $hero_title = sanitize_text_field($hero_filtered['title'] ?? $hero_defaults['title']);
            $hero_subtitle = sanitize_textarea_field($hero_filtered['subtitle'] ?? $hero_defaults['subtitle']);
            $primary_label = sanitize_text_field($hero_filtered['primary_label'] ?? $hero_defaults['primary_label']);
            $primary_url = isset($hero_filtered['primary_url']) && '' !== $hero_filtered['primary_url']
                ? esc_url_raw($hero_filtered['primary_url'])
                : $hero_defaults['primary_url'];
            $secondary_label = sanitize_text_field($hero_filtered['secondary_label'] ?? $hero_defaults['secondary_label']);
            $secondary_url = isset($hero_filtered['secondary_url']) && '' !== $hero_filtered['secondary_url']
                ? esc_url_raw($hero_filtered['secondary_url'])
                : $hero_defaults['secondary_url'];

            $primary_target = $first_card ? 'cpc-section-' . $first_card['slug'] : '';
            $secondary_is_profile = $secondary_url !== '' && $secondary_url === $profile_url;
                ...
                <div class="cpc-home-shell">
                    <section class="cpc-home-hero">
                        <div class="cpc-home-hero-main">
                            <span class="cpc-home-pill"><?php echo esc_html($hero_tag); ?></span>
                            <h1 class="cpc-home-title"><?php echo esc_html($hero_title); ?></h1>
                            <p class="cpc-home-subtitle"><?php echo esc_html($hero_subtitle); ?></p>
                            <div class="cpc-home-actions">
                                <?php if ('' !== $primary_url && '' !== $primary_label) : ?>
                                    <a class="cpc-home-button is-primary" href="<?php echo esc_url($primary_url); ?>"<?php echo '' !== $primary_target ? ' data-cpc-target="' . esc_attr($primary_target) . '"' : ''; ?>>
                                        <?php echo esc_html($primary_label); ?>
                                    </a>
                                <?php endif; ?>
                                <?php if ('' !== $secondary_url && '' !== $secondary_label) : ?>
                                    <a class="cpc-home-button" href="<?php echo esc_url($secondary_url); ?>"<?php echo $secondary_is_profile ? ' data-cpc-open-profile="true"' : ''; ?>>
