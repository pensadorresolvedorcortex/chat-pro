...
class Questoes_Comentadas_Plugin {
    ...
    public function render_dashboard_page() {
        ...
        $pending            = $status_data['pending']['count'];
        $future             = $status_data['future']['count'];
    ...
    public function render_shortcode( $atts ) {
        ...
        <section class="qc-shortcode-wrapper">
            ...
            <?php if ( $show_summary && ! empty( $categories ) ) : ?>
                <div class="qc-shortcode-summary" aria-labelledby="qc-shortcode-summary-title">
                    <h3 id="qc-shortcode-summary-title"><?php esc_html_e( 'Distribuição por categoria', 'questoes-comentadas' ); ?></h3>
                    <ul class="qc-progress-list">
                        <?php foreach ( $categories as $category ) : ?>
                            <li class="qc-progress-item">
                                <span class="qc-progress-item__label"><?php echo esc_html( $category['name'] ); ?></span>
                                <div class="qc-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="<?php echo esc_attr( $category['percentage'] ); ?>" aria-label="<?php echo esc_attr( sprintf( __( '%s representa %d%% das questões publicadas', 'questoes-comentadas' ), $category['name'], $category['percentage'] ) ); ?>">
                                    <span class="qc-progress__bar" style="--qc-progress-value: <?php echo esc_attr( $category['percentage'] ); ?>%;"></span>
                                </div>
                                <span class="qc-progress-item__value"><?php echo esc_html( sprintf( _n( '%d questão', '%d questões', $category['count'], 'questoes-comentadas' ), $category['count'] ) ); ?> · <?php echo esc_html( $category['percentage'] ); ?>%</span>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                    <p class="qc-shortcode-total"><?php printf( esc_html__( '%d questões publicadas no total.', 'questoes-comentadas' ), intval( $total_published ) ); ?></p>
                </div>
            <?php endif; ?>
