...
class Bot {
    ...
    public function __construct(callable $getName) {
    ...
    public function handleMessage($userId, $message) {
        ...
        if (!isset($this->states[$userId])) {
            $name = call_user_func($this->getName, $userId);
        ...
        if ($state['step'] === 'handoff') {
            return 'Aguarde enquanto um analista assume o atendimento.';
        }
